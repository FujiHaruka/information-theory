...
The endpoint right-continuity infrastructure and the operational converse headline:
...
/-- A row-stochastic kernel induces a factorizable joint. -/
...
/-- Kernel-space form of the factorizable rate: the factorizable rate equals
...
/-- Fixed-`U` right-continuity of the factorizable rate. If the rate at
...
/-- The objective image over the factorizable constraint set equals the objective image
...
/-- Entropy-mixture identity. The kernel-space Wyner–Ziv
...
@audit:ok (`hmix`/`hw0`/`hw1` place `M ∈ convexHull (range Φ)`, then
`eq_pos_convex_span_of_mem_convexHull` + `card_le_finrank_succ` + `finrank_pi` bound the
support by `finrank ℝ (D → ℝ) + 1 = card D + 1 ≤ K`, then zero-pad/reindex into `Fin K`;
no hypothesis assumes the reduced support it produces.) -/
...
/-- Carathéodory support reduction. Any feasible factorizable kernel at
...
* Entropy-mixture identity `wzKernelObjective_eq_blockSum`:
...
@audit:ok (non-circular and non-bundled: `h_pmf` is simplex regularity, `hκ` the input datum
being reduced, and the conclusion `∃ κ' : Fin (|α|+3)` feasible with objective ≤ is not a
hypothesis restated; the objective is proven equal — stronger than ≤ — and distortion
...
/-- Carathéodory fixed-`K` identification of the reshaped Wyner–Ziv rate.
...
@audit:ok (a genuine equality of two infima: `≤` from `B ⊆ A` (`csInf_le_csInf`), `≥` from
`wz_support_reduce` landing every union witness into `B` with objective ≤; K = |α|+3 is the
minimal size for `hcardK`, and a too-small K would break `≥`, so it is true as-framed, not
vacuous.) -/
...
theorem wynerZivRate_le_of_forall_pos_add_endpoint
  ...
  -- Identify with the fixed-`K` factorizable rate, then apply fixed-`K`
  -- right-continuity, transporting `hstep` through the identification at each `D + ε`.
...
set at `D`:
* `S(D) = ∅` gives `R_WZ(D) = sInf ∅ = 0 ≤ R`;
* an anchor `D₀ < D` with `S(D₀)` nonempty gives the bound by the time-sharing perturbation
  `wzRateValueSet_timeShare_mem` plus `t(ε) → 0`;
* the left-endpoint case (`h_endpoint`) is discharged by
  `wynerZivRate_le_of_forall_pos_add_endpoint`.
    ...
    · -- Case (B): an anchor `D₀ < D` exists; time-sharing perturbation.
      ...
      have hbound : ∀ ε > 0,
        ...
        calc wynerZivRate P_XY' d' D
            ≤ (1 - t) * R + t * w := hfinal
          _ = R + t * (w - R) := by ring
      -- The ε-parametrized bound tends to `R` as `ε → 0⁺`.
