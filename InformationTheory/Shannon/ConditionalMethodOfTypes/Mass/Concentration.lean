...
/-!
# Conditional method of types — entropy concentration

The entropy-concentration machinery for the conditional method of types:
the marginal-Y identification, the per-`y` Y-product mass lower bound
`productMass_columnProd_ge`, and the KL concentration helper
`conditional_KL_concentration_ge` that controls the polynomial floor error.
-/
...
lemma productMass_columnProd_ge
    (μ : Measure Ω) [IsProbabilityMeasure μ]
    (Xs : ℕ → Ω → α) (Ys : ℕ → Ω → β)
...
lemma conditional_KL_concentration_ge
    (μ : Measure Ω) [IsProbabilityMeasure μ]
    (Xs : ℕ → Ω → α) (Ys : ℕ → Ω → β)
