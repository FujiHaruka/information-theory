...
-- pulls `M_k` out of the conditional Lebesgue expectation over `(⨆ⱼℱⱼ).comap Tᵏ⁺¹`, and applies
-- the increment bound (`condKuhnTucker_infPast` transported to epoch `k+1` via
-- `condExp_comp_measurePreserving`). `hcoh` is the shift/past coherence (measurability only,
-- non-load-bearing) supplying that pull-out; `hInf_dom` is the received KT dominance of
-- `bstarInf`, not the proof core.
