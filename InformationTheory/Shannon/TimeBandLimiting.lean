...
projections are `Submodule.starProjection`s onto closed subspaces, so `A`'s
...
Compactness (`timeBandLimitingOp_isCompact`) reduces — via `A = P_W ∘ C` with
...
The decreasing eigenvalue enumeration: Mathlib's ordered `ℕ → ℝ` eigenvalue sequence is
...
* `prolateEigenvalues_hasEigenvalue` — every nonzero entry is an eigenvalue of `A`.
* `prolateEigenvalues_tendsto_zero` — the enumeration tends to `0`.

Non-vacuity of that enumeration: the headlines above are all satisfied by the constant-zero
...
The `2WT` degrees-of-freedom bound, in the Bessel form the Hilbert-space structure
...
The exact trace upgrades that Bessel *inequality* to a Parseval *equality*: along a *complete* basis
the trace is exactly `2WT`. The mechanism needs neither the spectral theorem nor trace-class
theory — the terms are nonnegative, so Tonelli exchanges `∑'` with `∫` unconditionally, and
completeness replaces Bessel by Parseval.
...
The spectral gap below `c`: on the orthogonal complement of the span of the eigenspaces
...
consumed here (that route is `tsum_prolateEigenvalues_eq`, taken separately).
...
These assemble into the two-sided eigenvalue count concentration, with `D := 2 + log(1+2WT)`
...
* `TimeBandLimiting.Operator` — the operator `A`, its subspaces, self-adjointness, positivity,
  norm bound, compactness, and boundary degeneracy.
...
* `TimeBandLimiting.TraceBound` — the reproducing kernel, the Bessel trace bound `≤ 2WT`, and
  spectral gap below `c`.
* `TimeBandLimiting.SecondMoment` — the window deficit and the second moment `tr A²`.
* `TimeBandLimiting.Count` — the two-sided eigenvalue count and achievability.
-/
