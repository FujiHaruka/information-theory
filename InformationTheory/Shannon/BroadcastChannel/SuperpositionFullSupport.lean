...
/-! ## The channel law generated by a law of the auxiliaries and the input -/
...
instance uvLawOfInput_isProbabilityMeasure (W : BCChannel α β₁ β₂) [IsMarkovKernel W]
    (ρ : Measure (U × V × α)) [IsProbabilityMeasure ρ] :
    IsProbabilityMeasure (uvLawOfInput W ρ) := by
  unfold uvLawOfInput
  exact Measure.isProbabilityMeasure_map measurable_uvUnassoc.aemeasurable

lemma uvLawOfInput_isUVChannelLaw (W : BCChannel α β₁ β₂) [IsMarkovKernel W]
    (ρ : Measure (U × V × α)) [SFinite ρ] : IsUVChannelLaw W (uvLawOfInput W ρ) := by
  ...
  have hcomp₁ : (fun q : U × V × α × β₁ × β₂ ↦ ((q.1, q.2.1, q.2.2.1), q.2.2.2)) ∘
      (fun z : (U × V × α) × (β₁ × β₂) ↦ (z.1.1, z.1.2.1, z.1.2.2, z.2.1, z.2.2)) = id := rfl
  have hcomp₂ : (fun q : U × V × α × β₁ × β₂ ↦ (q.1, q.2.1, q.2.2.1)) ∘
      (fun z : (U × V × α) × (β₁ × β₂) ↦ (z.1.1, z.1.2.1, z.1.2.2, z.2.1, z.2.2))
      = Prod.fst := rfl
  unfold IsUVChannelLaw uvLawOfInput
  rw [Measure.map_map hP measurable_uvUnassoc, Measure.map_map hπ measurable_uvUnassoc,
    hcomp₁, hcomp₂, Measure.map_id,
    show (ρ ⊗ₘ W.comap (fun r : U × V × α ↦ r.2.2) hg).map Prod.fst = ρ
      from Measure.fst_compProd _ _]

lemma uvLawOfInput_map_aux_input (W : BCChannel α β₁ β₂) [IsMarkovKernel W]
    (ρ : Measure (U × V × α)) [SFinite ρ] :
    (uvLawOfInput W ρ).map (fun q ↦ (q.1, q.2.2.1)) = ρ.map (fun r ↦ (r.1, r.2.2)) := by
  have hg : Measurable (fun r : U × V × α ↦ r.2.2) := measurable_snd.comp measurable_snd
  have h1 : Measurable (fun q : U × V × α × β₁ × β₂ ↦ (q.1, q.2.2.1)) := by fun_prop
  rw [uvLawOfInput, Measure.map_map h1 measurable_uvUnassoc]
  rw [show ((fun q : U × V × α × β₁ × β₂ ↦ (q.1, q.2.2.1)) ∘
      fun z : (U × V × α) × (β₁ × β₂) ↦ (z.1.1, z.1.2.1, z.1.2.2, z.2.1, z.2.2))
      = (fun r : U × V × α ↦ (r.1, r.2.2)) ∘ Prod.fst from rfl,
...
lemma condMutualInfo_le_condMutualInfo_of_map_forget (W : BCChannel α β₁ β₂) [IsMarkovKernel W]
...
lemma mul_condMutualInfo_le_condMutualInfo_uvPerturbLaw (W : BCChannel α β₁ β₂)
  ...
  have hforget := condMutualInfo_le_condMutualInfo_of_map_forget W
...
lemma uvInfo₂_uvPerturbLaw_ge (W : BCChannel α β₁ β₂) [IsMarkovKernel W]
    {ν : Measure (U × V × α × β₁ × β₂)} [IsProbabilityMeasure ν] (v₀ : V) {lam : ℝ≥0∞}
    (hlam : lam ≤ 1) :
...
theorem exists_fullSupport_bcInfo_ge_of_isUVChannelLaw (W : BCChannel α β₁ β₂) [IsMarkovKernel W]
  ...
  · rw [bcInfo₁_uvCloudLaw W hlaw]
    ...
    nlinarith
  · rw [bcInfo₂_uvCloudLaw W hlaw]
    have hge := uvInfo₂_uvPerturbLaw_ge W (ν := ν) v₀ hlam1
...
/-- The five-tuple law of an achievability pair, with a one-point second auxiliary. -/
