...
noncomputable def martonAuxCoeff (κ : Kernel V₁ V₂) (K : Kernel (V₁ × V₂) α)
...
private lemma martonAuxKernelSlot_nonneg (κ : Kernel V₁ V₂) (K : Kernel (V₁ × V₂) α)
...
private lemma sum_mul_martonAuxCoeff_eq_sub (q : Measure V₁) (κ : Kernel V₁ V₂)
    (K : Kernel (V₁ × V₂) α) (W : BCChannel α β₁ β₂) (μ₁ μ₃ : ℝ) :
    ∑ u : V₁, q.real {u} * martonAuxCoeff κ K W μ₁ μ₃ u
      = μ₃ * (∑ u : V₁, q.real {u} * ∑ v₂ : V₂, Real.negMulLog ((κ u).real {v₂}))
        - (μ₁ + μ₃) * (∑ u : V₁, q.real {u} *
            ∑ y₁ : β₁, Real.negMulLog (martonAuxOutput₁Row κ K W u y₁)) := by
  have h : ∀ u : V₁, q.real {u} * martonAuxCoeff κ K W μ₁ μ₃ u
      = μ₃ * (q.real {u} * ∑ v₂ : V₂, Real.negMulLog ((κ u).real {v₂}))
        - (μ₁ + μ₃) * (q.real {u} *
            ∑ y₁ : β₁, Real.negMulLog (martonAuxOutput₁Row κ K W u y₁)) := by
    intro u
    simp only [martonAuxCoeff]
    ring
  rw [Finset.sum_congr rfl fun u _ ↦ h u, Finset.sum_sub_distrib, ← Finset.mul_sum,
    ← Finset.mul_sum]

private lemma sum_negMulLog_martonAuxKernelSlot_eq_entropy
...
private lemma sum_negMulLog_martonAuxKernelSlot_mixture_eq_entropy
    (q : Measure V₁) [IsProbabilityMeasure q] (κ : Kernel V₁ V₂) [IsMarkovKernel κ]
    (K : Kernel (V₁ × V₂) α) [IsMarkovKernel K] (W : BCChannel α β₁ β₂) [IsMarkovKernel W] :
    ∑ y₂ : β₂, Real.negMulLog
...
private lemma martonOutput₁Aggregate_eq_mul_entropy
    ...
    (K : Kernel (V₁ × V₂) α) [IsMarkovKernel K] (W : BCChannel α β₁ β₂) [IsMarkovKernel W]
    (μ₁ μ₃ : ℝ) :
    martonOutput₁Aggregate W μ₁ μ₃ (fun x ↦ ∑ u : V₁, q.real {u} * martonAuxRow κ K u x)
...
theorem martonWeightedSum_eq_auxWeightObjective_add_aggregate
    ...
    μ₁ * martonInfo₁ (q ⊗ₘ κ) K W
      ...
      = auxWeightObjective (martonAuxKernelSlot κ K W) (martonAuxCoeff κ K W μ₁ μ₃) 0 μ₃
          (fun u ↦ q.real {u})
        + martonOutput₁Aggregate W μ₁ μ₃ (fun x ↦ ∑ u, q.real {u} * martonAuxRow κ K u x) := by
  simp only [martonInfo₁, martonInfo₂, martonInfoV₁V₂, auxWeightObjective]
  rw [martonOutput₁Aggregate_eq_mul_entropy q κ K W μ₁ μ₃,
    sum_mul_martonAuxCoeff_eq_sub q κ K W μ₁ μ₃,
...
theorem exists_support_card_le_martonWeightedSum
    ...
    ∃ q' : V₁ → ℝ, 0 ≤ q' ∧
      ...
      μ₁ * martonInfo₁ (q ⊗ₘ κ) K W
        + μ₃ * (martonInfo₁ (q ⊗ₘ κ) K W + martonInfo₂ (q ⊗ₘ κ) K W
                - martonInfoV₁V₂ (q ⊗ₘ κ) K W)
        ≤ auxWeightObjective (martonAuxKernelSlot κ K W) (martonAuxCoeff κ K W μ₁ μ₃) 0 μ₃ q'
  ...
  obtain ⟨q', hq'nonneg, hq'agg, hle, hcard⟩ :=
    exists_support_card_le_auxWeightObjective_add_aggregate (martonAuxRow κ K)
      ...
      (martonAuxKernelSlot_nonneg κ K W) (martonAuxCoeff κ K W μ₁ μ₃) 0 μ₃ hμ₃
      (martonOutput₁Aggregate W μ₁ μ₃) (fun u ↦ q.real {u}) fun _ ↦ measureReal_nonneg
  refine ⟨q', hq'nonneg, hq'agg, ?_, hcard⟩
  rw [martonWeightedSum_eq_auxWeightObjective_add_aggregate q κ K W μ₁ μ₃]
  exact hle

omit [Nonempty V₁] [Nonempty V₂] [Nonempty α] in
private lemma sum_eq_one_of_martonAuxRow_aggregate
    (q : Measure V₁) [IsProbabilityMeasure q] (κ : Kernel V₁ V₂) [IsMarkovKernel κ]
    (K : Kernel (V₁ × V₂) α) [IsMarkovKernel K] (q' : V₁ → ℝ)
    (hagg : ∀ x, ∑ u, q' u * martonAuxRow κ K u x = ∑ u, q.real {u} * martonAuxRow κ K u x) :
    ∑ u, q' u = 1 := by
  calc ∑ u : V₁, q' u
      = ∑ u : V₁, ∑ x : α, q' u * martonAuxRow κ K u x := by
        refine Finset.sum_congr rfl fun u _ ↦ ?_
        rw [← Finset.mul_sum, sum_martonAuxRow_eq_one κ K u, mul_one]
    _ = ∑ x : α, ∑ u : V₁, q' u * martonAuxRow κ K u x := Finset.sum_comm
    _ = ∑ x : α, ∑ u : V₁, q.real {u} * martonAuxRow κ K u x :=
        Finset.sum_congr rfl fun x _ ↦ hagg x
    _ = ∑ u : V₁, ∑ x : α, q.real {u} * martonAuxRow κ K u x := Finset.sum_comm
    _ = ∑ u : V₁, q.real {u} := by
        refine Finset.sum_congr rfl fun u _ ↦ ?_
        rw [← Finset.mul_sum, sum_martonAuxRow_eq_one κ K u, mul_one]
...
theorem exists_support_card_le_martonWeightedSum_measure
  ...
  obtain ⟨qv, hqvnonneg, hqvagg, hle, hcard⟩ :=
    exists_support_card_le_martonWeightedSum q κ K W μ₁ μ₃ hμ₃
