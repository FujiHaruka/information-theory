...
/-!
# Marton's inner bound — receiver-1 error analysis
...
Receiver 1 decodes the message alone, quantifying the covering index existentially, so a second
...
* `marton_errorProbAt₁_le_bonferroni` and `marton_averageErrorProb₁_toReal_le` — the receiver-1
  error decomposition, pointwise and averaged over message pairs.
* `marton_random_codebook_alias₁_le` — the alias estimate over the full three-tier ensemble.
-/
