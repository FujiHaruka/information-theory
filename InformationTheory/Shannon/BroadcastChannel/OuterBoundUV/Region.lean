...
instance uvLawOfInput_isProbabilityMeasure (W : BCChannel α β₁ β₂) [IsMarkovKernel W]
  ...
  unfold uvLawOfInput
  exact Measure.isProbabilityMeasure_map measurable_uvUnsplit.aemeasurable
...
lemma uvLawOfInput_isUVChannelLaw (W : BCChannel α β₁ β₂) [IsMarkovKernel W]
  ...
  rw [Measure.map_map measurable_uvSplit measurable_uvUnsplit,
    Measure.map_map measurable_uvFirstThree measurable_uvUnsplit,
...
lemma uvLawOfInput_map_aux_input (W : BCChannel α β₁ β₂) [IsMarkovKernel W]
  ...
  rw [uvLawOfInput, Measure.map_map h1 measurable_uvUnsplit]
...
noncomputable def uvConstLaw (W : BCChannel α β₁ β₂) (x₀ : α) :
    Measure (ℕ × ℕ × α × β₁ × β₂) :=
  uvLawOfInput W (Measure.dirac ((0 : ℕ), (0 : ℕ), x₀))

instance uvConstLaw_isProbabilityMeasure (W : BCChannel α β₁ β₂) [IsMarkovKernel W] (x₀ : α) :
    IsProbabilityMeasure (uvConstLaw W x₀) :=
  uvLawOfInput_isProbabilityMeasure W _
...
lemma uvConstLaw_isUVChannelLaw (W : BCChannel α β₁ β₂) [IsMarkovKernel W] (x₀ : α) :
    IsUVChannelLaw W (uvConstLaw W x₀) :=
