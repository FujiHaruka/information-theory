...
- **When a pivot reframes the residual as a named textbook object** (Markov lemma, Fano, Sanov, EPI…), diff that object's **standard hypotheses / strength** against the in-project definitions it will run on **before** recommending the build. The in-project realization is often a *weaker relative* (e.g. an entropy-only "weak" typicality where the theorem needs the strong / TV-pinned version); a build started on the weak frame is false-as-framed and overturns 1–2 legs later after the atoms are already stacked. Cheap tell: the nearest existing in-project asset's *name* carries a strength qualifier (`Strong…` / `Robust…`) your target definition lacks. (CLAUDE.md "guarding against under-estimation".)
- **Before recommending Proposal B (split / isolate a leaf), verify the consuming site's ACTUAL measure / coupling / typicality-strength verbatim.** An isolation that silently weakens any of these — an independent-product simplification, a free-parameter generalization, weak-instead-of-strong typicality — produces a leaf that is false-as-framed relative to what the consumer needs, yet passes its own local audit and only breaks at wiring time. Read the consumer body before drawing the leaf's signature.
