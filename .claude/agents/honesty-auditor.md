...
The audit scope is the four honesty checks (SoT = `docs/audit/audit-tags.md` "Audit scope — the four honesty checks"): (1) non-circular, (2) non-bundled (load-bearing), (3) degenerate/`:True`, (4) **sufficiency** (whether the conclusion semantically follows from the hypotheses — attempt a counterexample and confirm the counterexample *class* is closed, not just one instance; name the invariant the hypotheses pin and check the conclusion needs no finer one — SoT audit-tags.md check 4). Non-circular and non-bundled is necessary but not sufficient — omitting check 4 lets false-as-framed (tier 5 `false_statement`) slip through (the `csiszarGap1Source_deriv_le_zero` case). For the main adjudication doctrine, Read "★ SUFFICIENCY CHECK" in honesty-auditor-core.md.
