...
- **The dev tooling (`loogle`) is a conditional dependency** (`lakefile.lean`, guarded by `meta if (get_config? dev).isSome`), so that consumers of the library do not clone it. Its already-built binary under `.lake/packages/loogle/.lake/build/bin/` keeps working without any flag — **only re-resolving or rebuilding a tool needs `lake -R -Kdev=on update`** (`-R` is required: without it Lake reuses the cached config olean and the guard never re-fires). That command rewrites `lake-manifest.json`, so **restore the release manifest with `lake -R update` before committing** — the committed manifest must list Mathlib and its transitive deps only. API documentation is never built locally: CI generates it with litedoc4 and publishes it to GitHub Pages on every release (`.github/workflows/docs.yml`).
...
- **When declaring a wall (guarding against over-estimation)**: a loogle 0-hit is *necessary but not sufficient*. After a 0-hit, do (a) a **two-stage conclusion-shape search** (re-search by subterm / conclusion pattern such as `|- _ ≤ _`, not a bare identifier), and (b) **name one template lemma close to the expected conclusion form and estimate the self-build line count**. If you can't write that, hold off on the wall verdict. Distinguish whether the blocker is "the proposition is absent from Mathlib (a genuine gap)" or "wiring to an existing asset (plumbing, including import cycles)".
  - **Search in-project before concluding "absent" (`cause:loogle-blind`)**: loogle only sees Mathlib. A verdict of the form "Mathlib doesn't have it ⟹ self-build" is invalid until you `rg` the project itself (+ `scripts/dep_consumers.sh`). Real case (Shannon-Hartley Leg B Leaf 2, 2026-07-17): an implementer and an independent honesty-auditor both declared the L¹∩L² Fourier bridge absent and budgeted a 150–600-line self-build — the general-form lemma already existed **two files away in the same family**, sorry-free with two consumers, and the leaf closed in ~125 lines by consuming it. All three loogle 0-hits were literally true and entirely beside the point. Independence does not protect against a shared blind spot.
  - **A dismissed asset must be dismissed by the compiler, not by prose**: if your wall argument names a candidate route and rules it out ("that detour is a black box against what we need", "that lemma's conclusion is the wrong shape"), the dismissal only counts once you have written one line against it and let `lake env lean` reject it. In the case above, both agents named the tempered-distribution detour and dismissed it in prose — the in-tree lemma closes precisely through it. An **unverified negative claim about an asset you already found** is the failure mode that survives every search-based guard.
...
- **When declaring something not-a-wall / a hypothesis OK (guarding against under-estimation)**: before accepting, **search once for a counterexample in a small case** + **substitute one degenerate boundary** (`=0` / Dirac / non-integrable / `N=0`) to confirm the statement is still alive. Check that the predicate's signature hasn't dropped a constraint (Read its `*_def`). For concrete numeric / type predictions, follow the "Verbatim confirmation of concrete numeric / type predictions" section. **One counterexample is necessary but not sufficient** — three further guards close the gap that lets an under-hypothesis survive an audit (and re-surface, overturning a PASS, one or two legs later):
  - **Class, not instance**: a single counterexample (or a single degenerate boundary) can pass an under-hyp. Try **two structurally-different** degenerate cases, and after any fix that kills a specific counterexample, **characterize the most-general object still satisfying every hypothesis and re-test that** — a fix that patches the one instance routinely leaves the counterexample *class* open (e.g. adding consistency hyps kills the constant/point-mass case, but an entropy-preserving relabel still satisfies them and breaks the conclusion).
  - **Name the pinned invariant**: state which invariant the hypotheses actually pin, and confirm the conclusion needs **exactly that, not a finer one**. The recurring false-as-framed axis is hyps pinning a *coarse* scalar functional (an entropy value) while the conclusion needs the *fine* structure (the full law / total-variation). Coarser-than-needed = false-as-framed even when non-circular and non-bundled.
  - **Textbook-object strength diff (at reframe time)**: when you reframe a residual as a **named textbook result** (Markov lemma, Fano, Sanov, EPI…), diff that object's **standard hypotheses / strength** against your in-project realization **before building on it** — the in-project def is often a *weaker relative* (weak/entropy-only vs strong/robust typicality). Cheapest signal: the **nearest existing in-project asset's name** — a strength qualifier it carries (`Strong…`, `Robust…`) that your target definition lacks flags the mismatch as one diff, instead of a multi-session build on a false frame.
...
The "Formalized results" table in `README.md` is a **generated artifact, not hand-edited** (a hand-kept table silently drifts when a theorem is renamed / moved / scoped-out → the public README lies). Curation lives only in `docs/readme-theorems.txt` (chapter → headline theorem names, one per line, `NAME | statement override` / `NAME @ path-substring` to disambiguate same-named decls). Paths are **not** stored — the script resolves each name against `InformationTheory/` every run, so a moved file self-heals and only a true rename/delete fails. The table's Statement column is **not** stored either: it is summarized from each declaration's docstring (cut before `@audit:` / `sorryAx-free` / `See also` / `Proof:`, `…` when truncated), so the docstring is the single place a statement's wording is edited. The `| override` escape hatch exists for docstrings that don't read standalone; keep it rare, since a hand-written statement drifts exactly like a hand-written table.

- After adding/renaming a headline theorem: edit `docs/readme-theorems.txt`, then `deno run -A scripts/gen_readme_table.ts --write` to regenerate the table between the README `<!-- THEOREMS:START/END -->` markers. Never edit inside the markers by hand. (The `update-readme-table` skill drives this end-to-end from a plain "update the README theorem table" request.)
...
**Default**: `InformationTheory/` implementation (filling `sorry`, new decls) runs as orchestrator — dispatch to `lean-implementer` (solo, no worktree) + honesty/style gates — without an explicit request.
...
`docs/rules/` (naming / docstrings / module-structure / lean-style) is the **SoT for how code reads**; honesty is a separate gate.

**The mechanical layer runs first — `scripts/lean_doc_lint.ts`.** Everything in `docs/rules/` that a machine can decide is enforced by that linter from three points: the Claude Code `PostToolUse` hook (`.claude/hooks/lean-doc-lint.sh`, blocks at edit time), `.githooks/pre-commit` (WARN), and CI job `doc-lint` (fails on a strict violation or a ratchet regression). Two rule classes: **strict** (English-only prose, no dates / audit narration / `Phase`-`Leg` labels / `wall:` slug outside a `@residual` tag, `## Main statements` heading, no empty docstring, `fun … ↦`, every decl / file a docstring names must exist in HEAD) and **ratchet** (leading-bold count, internal-lemma docstring count, line length, process vocabulary in decl names — a single occurrence cannot be judged mechanically, so only an increase fails). Run `deno run -A scripts/lean_doc_lint.ts` for the tree report and `--fix` for the auto-fixable subset. `style-auditor` therefore owns only the judgment items (is this bold a named theorem or a topic label? does the name carry the statement? is this prose a proof recipe or a design reason?) — do not spend a dispatch on what the linter already catches. As part of the orchestrator loop, after a leg's implementation lands (and after the honesty audit, when one ran), the orchestrator launches the **`style-auditor`** subagent (`subagent_type: "style-auditor"`) on the files the leg touched, and does **not** close the leg / wire it into `InformationTheory.lean` until the file **passes**: 0 compile errors, and every convention item either fixed in place or on the flag-only list.
