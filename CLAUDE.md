...
**Pick the subagent model by task type**: **creative work runs on Fable** (`model: "fable"`) — idea generation for a new approach, breaking out of a dead end, redesigning a definition or framework, exploring an unknown line of attack (`proof-pivot-advisor`, exploratory arcs, "this route died, produce the next one"). **Everything else stays on Opus** (`model: "opus"`) — implementation, inventory, adversarial / honesty / style audits, plan drafting and compaction, carrying out a settled decision. Audits in particular stay on Opus: verification is systematic demolition from a default position of "the verdict is false", not invention. When unsure, default to Opus.
