/-
  ...
  `private` 宣言も consumer として列挙し、ソースレベル名 + `[private]` で表示する
  (ファイルスコープなので touch が要るのは同一ファイル内だけ)。target 側もソースレベル名で
  指定できる。
...
/-- 名前の最終 component が Lean elaborator の自動生成 suffix か
    ...
    `.splitter` だけは単独では判定できないので、親が auto-gen のとき
    (`foo.match_1.splitter`) に限って auto-gen 扱いにする。 -/
def hasAutoGenSuffix : Name → Bool
  | .str p s =>
    ...
    || (s == "splitter" && hasAutoGenSuffix p)
  | _ => false

/-- declaration のソースレベル名。`private` 宣言の実体名は
    `_private.<Module>.<idx>.<UserName>` で、読み手が知っているのは後半だけなので
    表示・判定はこちらを使う。`private` でなければそのまま返す。 -/
def userNameOf (n : Name) : Name :=
...
/-- declaration が auto-generated (ctor / rec / projection / 派生 helper / notation /
    ...
    判定は実体名ではなく `userNameOf` (ソースレベル名) に対して行う。`private` 宣言の
    実体名は `_private.…` で始まり `Name.isInternal` を通ってしまうため、実体名で
    判定すると人が書いた `private` 補題まで補助定義として畳まれ、consumer 逆引きから
    丸ごと消える。ソースレベル名で見れば `private` 由来の auto-gen
    (`foo._simp_1_6` 等) は従来どおり畳まれる。 -/
def isAutoGenerated (env : Environment) (n : Name) : Bool :=
  ...
  || match env.find? n with
     ...
     | some (.recInfo _)  => true
     | _                  => (env.getProjectionFnInfo? n).isSome || hasAutoGenSuffix u
...
def render (env : Environment) (cfg : Config) (st : GraphState) : String := Id.run do
  ...
  for n in nodes do
    -- id は実体名のまま (private の user name は module 間で衝突しうる)、
    -- 見出しだけソースレベル名にする。
    let id := dotEscape (toString n)
    let label := dotEscape (toString (userNameOf n) ++ if isPrivateName n then " (private)" else "")
    let attr :=
      if n == cfg.root then
    ...
    out := out.push s!"  \"{id}\" [label=\"{label}\", {attr}];"
  out := out.push ""
...
/-- consumer 1 行分 (名前 + 所属 module + 開始行)。`name` はソースレベル名
    (`private` なら demangle 済) で、`isPrivate` はファイルスコープであることを示す。 -/
...
def runConsumers (env : Environment) (cfg : Config) (target : Name)
  ...
  let toRows (names : List Name) : IO (Array ConsumerRow) := do
    ...
    for nm in names do
      let priv := isPrivateName nm
      match ← getDeclLineRange env rangeCache nm with
      | some (mod, s, _) =>
        rows := rows.push { name := userNameOf nm, module := mod, line := s, isPrivate := priv }
      | none             =>
        rows := rows.push { name := userNameOf nm, module := moduleOf env nm, line := 0,
                            isPrivate := priv }
  ...
  let printRows (rows : Array ConsumerRow) : IO Unit := do
    ...
    for r in rows do
      ...
      let tag := if r.isPrivate then "  [private]" else ""
      IO.println s!"    {padLeft 5 (toString r.line)}  {r.name}{tag}"
  IO.println "==== Consumers (逆依存) ===="
  let targetTag := if isPrivateName target then "  [private]" else ""
  let targetLabel := s!"{userNameOf target}{targetTag}"
  match ← getDeclLineRange env rangeCache target with
  | some (mod, s, _) => IO.println s!"  target : {targetLabel}"
                        IO.println s!"           {srcPathStr mod}:{s}"
  | none             => IO.println s!"  target : {targetLabel}  (定義位置不明)"
...
/-- ユーザーが打った名前を実体名に解決する。`private` 宣言はソースレベル名では
    環境に存在しない (実体は `_private.…`) ので、user name 一致で引き直す。
    同名の `private` が複数モジュールにある場合は最初の 1 件。 -/
def resolveDeclName (env : Environment) (n : Name) : Option Name :=
  if env.contains n then some n
  else env.constants.fold (init := none) fun acc m _ =>
    match acc with
    | some _ => acc
    | none   => if isPrivateName m && userNameOf m == n then some m else none
...
#eval! show MetaM Unit from do
  ...
  if !consStr.isEmpty then
    let some target := resolveDeclName env consStr.toName
      | IO.eprintln s!"[DepGraph] 未知の declaration: {consStr}"
