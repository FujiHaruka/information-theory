...
//#endregion
//#region src/data.ts
var b = null, x = null, S = null, C = (e) => fetch(r(e)).then((e) => e.ok ? e.json() : Promise.reject(Error(String(e.status)))).catch(() => null);
function w() {
	return b ??= C("modules.json"), b;
}
function T() {
...
async function k() {
	...
	if (!e) return;
	let t = await w(), i = (t?.modules?.find((e) => e.n === n)?.i ?? []).map((e) => t?.modules[e]).filter((e) => e !== void 0);
	if (i.length === 0) {
		e.remove();
		return;
	}
...
function A() {
	let e = document.querySelectorAll("[data-fill=\"instances\"], [data-fill=\"instances-for\"]");
	for (let t of e) t.addEventListener("toggle", async () => {
		let e = t.querySelector("ul");
		if (!e) return;
		let [n, r] = await Promise.all([E(), D()]), i = t.dataset.fill === "instances" ? n?.instances : n?.instancesFor, a = t.dataset.name ?? "", o = i?.[a] ?? [];
		if (e.textContent = "", o.length === 0) {
			let t = document.createElement("li");
			t.className = "search-empty", t.textContent = n ? "None" : "Index unavailable", e.append(t);
			return;
		}
		let s = r ? y(r.index, o) : /* @__PURE__ */ new Map();
		for (let t of o) e.append(j(r, t, s.get(t)));
	}, { once: !0 });
}
function j(e, t, n) {
...
function F(e, t) {
	...
	if (i && t.startsWith(i.query)) return ee(e, i, n, r, t);
...
async function L() {
	...
	let r = await D();
	if (!r) return;
	let i = F(r.index, n).slice(0, I);
	if (i.length !== 0) {
		for (let t of i) e.append(M(r, t));
...
function B() {
	...
	e.addEventListener("input", () => {
		clearTimeout(i), i = setTimeout(() => void o(), R);
...
function U() {
	...
	let i = async () => {
		...
		let a = F(i.index, r);
		for (let t of a.slice(0, H)) e.append(M(i, t));
		t && (t.textContent = a.length === 0 ? "No matching declaration." : a.length > H ? `${a.length} matches, showing the first ${H}.` : `${a.length} match${a.length === 1 ? "" : "es"}.`);
	}, a = 0;
	n.addEventListener("input", () => {
		clearTimeout(a), a = setTimeout(() => void i(), V);
...
var K = "litedoc4-theme", q = [
	"auto",
	"light",
	"dark"
], J = (e) => e !== null && q.includes(e);
function Y() {
	try {
...
function Z() {
	X(Y()), document.getElementById("theme-toggle")?.addEventListener("click", () => {
		let e = q[(q.indexOf(Y()) + 1) % q.length];
		try {
...
async function te() {
	...
	if (!e) return;
	let t = await w();
	t?.modules?.length && (e.textContent = "", e.append($(Q(t.modules), "", n)), e.querySelector("[aria-current]")?.scrollIntoView({ block: "center" }));
}
Z(), i(), U(), B(), A(), G(), W(), te(), k(), L();
//#endregion
