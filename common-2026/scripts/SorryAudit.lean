/-
  scripts/SorryAudit.lean — sorry 影響範囲監査スクリプト

  `Common2026/*.lean` で定義されたすべての thm / def を走査し、`Lean.collectAxioms`
  で推移的依存軸 (axiom 集合) を取得して `sorryAx` の有無で分類する。

  ## 出力分類

    sorry-free       : 推移的に sorry に依存していない (完全に証明された定理)
    own sorry        : 宣言自身の body / type に sorry を含む
    transitive only  : 自身は sorry 無いが、依存先に sorry を持つ宣言が存在する
  ...
  namespace prefix は project 全体で統一されていない (`InformationTheory.Shannon.*`
  / `MACCode` / `Common2026.A_Q1_1_i` 等 30+ top-level namespace) ため、Lean namespace
  ではなく **module path** (`getModuleIdxFor?` → moduleName) で filter する。

  `Common2026` umbrella は本セッション時点で `ParallelGaussianPerCoord.lean:295` の
  既存型エラーが原因で import できないため、`Common2026.lean` から当該 2 モジュールを
  除外した import 一覧を直接書き出している (CLAUDE.md「sorry を書けない箇所での対処」
  ではなく純粋な signature drift)。defect 報告は別途。
-/
import Lean
-- 以下、Common2026.lean から ParallelGaussianPerCoord{,Regularity} のみ除外したコピー
...
import Common2026.Shannon.HuffmanWalls
