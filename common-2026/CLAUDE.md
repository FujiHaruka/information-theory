...
2. **継承 `@audit:*` タグの語彙整合 inline check** (git history からの body 復元時) — `git show <commit>^:<path>` で旧 body を抽出 + sed 書換するワークフローでは、旧 docstring の `@audit:suspect("")` (空 slug placeholder) / 古い `@audit:staged(<slug>)` も literally 引き継ぐ。brief の検証 step に「貼付後 `grep -n '@audit:' <touched-file>` で empty slug / 既存に未参照の slug を列挙し orchestrator に報告」と 1 行追加。orchestrator 側で `docs/audit/audit-tags.md` 語彙と照合 → `Edit` で確定 → 追加 commit。
