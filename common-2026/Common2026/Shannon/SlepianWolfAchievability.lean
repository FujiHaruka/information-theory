...
/-- The trivial SW encoder pair + decoder have **zero** error. -/
theorem swErrorProb_trivial_zero
    (μ : Measure Ω) [IsProbabilityMeasure μ]
    {n : ℕ}
    (Xs : Ω → Fin n → α) (Ys : Ω → Fin n → β) :
    swErrorProb μ Xs Ys (swTrivialEncoderX n) (swTrivialEncoderY n)
      (swTrivialDecoder n) = 0 := by
  unfold swErrorProb swTrivialDecoder swTrivialEncoderX swTrivialEncoderY
  -- The error event is empty: the trivial decoder perfectly inverts the encoders.
  have h_empty : {ω | (((Fintype.equivFin (Fin n → α)).invFun
        ((Fintype.equivFin (Fin n → α)).toFun (Xs ω))),
        (Fintype.equivFin (Fin n → β)).invFun
        ((Fintype.equivFin (Fin n → β)).toFun (Ys ω))) ≠ (Xs ω, Ys ω)} = ∅ := by
    ext ω
    simp only [Set.mem_setOf_eq, Set.mem_empty_iff_false, iff_false, not_not]
    have hX : (Fintype.equivFin (Fin n → α)).invFun
        ((Fintype.equivFin (Fin n → α)).toFun (Xs ω)) = Xs ω := by
      exact (Fintype.equivFin (Fin n → α)).left_inv (Xs ω)
    have hY : (Fintype.equivFin (Fin n → β)).invFun
        ((Fintype.equivFin (Fin n → β)).toFun (Ys ω)) = Ys ω := by
      exact (Fintype.equivFin (Fin n → β)).left_inv (Ys ω)
    rw [hX, hY]
  rw [h_empty]
...
/-! ## Phase D — `slepian_wolf_achievability_sum`: corner-point statements

Symmetric Y-side AEP and X-side AEP corner-point achievability statements. Together
they cover the two corners `(log|α|, R_Y)` (for `R_Y > H(Y)`) and `(R_X, log|β|)`
(for `R_X > H(X)`) of the SW rate region. -/

/-- **Slepian–Wolf achievability, corner-point form (E-5 MVP, Y-AEP corner)**: for any
`R_Y > H(Y)`, there exists an SW encoder pair `(f_X^n, f_Y^n)` and joint decoder `d^n`
with rate pair `(log|α|, R_Y)` and `errorProb → 0`.

This realizes the SW achievability **at the corner `R_X = log|α|`** of the rate region.
The X-coordinate is sent uncompressed (`f_X = identity`), and the Y-coordinate uses
single-source AEP at rate `R_Y > H(Y)`.

The full 3-bound SW rate region (`R_X > H(X|Y), R_Y > H(Y|X), R_X + R_Y > H(X, Y)`)
requires `random binning + joint typicality decoder` (Cover-Thomas 15.4) and is
**scope-deferred** to a follow-up plan (E-5'). -/
theorem slepian_wolf_achievability_corner_Y
    (μ : Measure Ω) [IsProbabilityMeasure μ]
    (Xs : ℕ → Ω → α) (Ys : ℕ → Ω → β)
    (hYs : ∀ i, Measurable (Ys i))
    (hposY : ∀ y : β, 0 < (μ.map (Ys 0)).real {y})
    ...
    {R_Y : ℝ} (hR_Y : entropy μ (Ys 0) < R_Y) :
    ∃ M_X : ℕ → ℕ, ∃ M_Y : ℕ → ℕ,
      ∃ _hM_X_pos : ∀ n, 0 < M_X n, ∃ _hM_Y_pos : ∀ n, 0 < M_Y n,
    ∃ f_X : ∀ n, (Fin n → α) → Fin (M_X n),
    ∃ f_Y : ∀ n, (Fin n → β) → Fin (M_Y n),
    ∃ d : ∀ n, Fin (M_X n) × Fin (M_Y n) → (Fin n → α) × (Fin n → β),
      -- Rate of X-encoder → log|α| (uncompressed).
      (∀ n, M_X n = Fintype.card (Fin n → α)) ∧
      -- Rate of Y-encoder → R_Y.
      Tendsto (fun n => Real.log (M_Y n : ℝ) / n) atTop (𝓝 R_Y) ∧
      -- Joint error → 0.
      Tendsto
        (fun n => swErrorProb μ (jointRV Xs n) (jointRV Ys n)
                    (f_X n) (f_Y n) (d n))
        atTop (𝓝 0) := by
  obtain ⟨M_Y, hM_Y_pos, f_Y, d_Y, hRateY, hPe⟩ :=
    slepian_wolf_achievability_via_Y_aep μ Xs Ys hYs hposY hindepY_full hidentY hR_Y
  refine ⟨fun n => Fintype.card (Fin n → α), M_Y,
    fun n => Fintype.card_pos, hM_Y_pos,
    fun n => swTrivialEncoderX n, f_Y,
    fun n p => ((Fintype.equivFin (Fin n → α)).invFun p.1, d_Y n p.2),
    fun n => rfl, hRateY, hPe⟩

/-- Symmetric X-side AEP corner-point form: for any `R_X > H(X)`, there exists an SW
encoder pair with rate pair `(R_X, log|β|)` and `errorProb → 0`. The Y-coordinate is
sent uncompressed and the X-coordinate uses single-source AEP. -/
theorem slepian_wolf_achievability_corner_X
    (μ : Measure Ω) [IsProbabilityMeasure μ]
    (Xs : ℕ → Ω → α) (Ys : ℕ → Ω → β)
    ...
    {R_X : ℝ} (hR_X : entropy μ (Xs 0) < R_X) :
    ∃ M_X : ℕ → ℕ, ∃ M_Y : ℕ → ℕ,
      ∃ _hM_X_pos : ∀ n, 0 < M_X n, ∃ _hM_Y_pos : ∀ n, 0 < M_Y n,
    ∃ f_X : ∀ n, (Fin n → α) → Fin (M_X n),
    ∃ f_Y : ∀ n, (Fin n → β) → Fin (M_Y n),
    ∃ d : ∀ n, Fin (M_X n) × Fin (M_Y n) → (Fin n → α) × (Fin n → β),
      -- Rate of X-encoder → R_X.
      Tendsto (fun n => Real.log (M_X n : ℝ) / n) atTop (𝓝 R_X) ∧
      -- Rate of Y-encoder → log|β| (uncompressed).
      (∀ n, M_Y n = Fintype.card (Fin n → β)) ∧
      -- Joint error → 0.
      Tendsto
        (fun n => swErrorProb μ (jointRV Xs n) (jointRV Ys n)
                    (f_X n) (f_Y n) (d n))
        atTop (𝓝 0) := by
  -- Mirror of `slepian_wolf_achievability_via_Y_aep`: apply AEP to Xs.
  obtain ⟨M_X, hM_X_pos, c_X, d_X, hRateX, hPe⟩ :=
    source_coding_achievability μ Xs hXs hposX hindepX_full hidentX hR_X
  refine ⟨M_X, fun n => Fintype.card (Fin n → β),
    hM_X_pos, fun n => Fintype.card_pos,
    c_X, fun n => swTrivialEncoderY n,
    fun n p => (d_X n p.1, (Fintype.equivFin (Fin n → β)).invFun p.2),
    hRateX, fun n => rfl, ?_⟩
  -- The SW error equals the X-side AEP error.
  have h_eq : ∀ n,
      swErrorProb μ (jointRV Xs n) (jointRV Ys n)
          (c_X n) (swTrivialEncoderY n)
          (fun p => (d_X n p.1, (Fintype.equivFin (Fin n → β)).invFun p.2))
        = InformationTheory.MeasureFano.errorProb μ
            (jointRV Xs n) (fun ω => c_X n (jointRV Xs n ω)) (d_X n) := by
    intro n
    unfold swErrorProb InformationTheory.MeasureFano.errorProb
    congr 1
    ext ω
    simp only [Set.mem_setOf_eq]
    have hY_inv : (Fintype.equivFin (Fin n → β)).invFun
        (swTrivialEncoderY n (jointRV Ys n ω)) = jointRV Ys n ω := by
      unfold swTrivialEncoderY
      exact (Fintype.equivFin (Fin n → β)).left_inv (jointRV Ys n ω)
    rw [hY_inv]
    constructor
    · intro h hXeq
      apply h
      rw [← hXeq]
    · intro h hPair
      have hX_eq : d_X n (c_X n (jointRV Xs n ω)) = jointRV Xs n ω :=
        (Prod.mk.inj hPair).1
      exact h hX_eq.symm
  refine Tendsto.congr (fun n => (h_eq n).symm) ?_
  exact hPe
