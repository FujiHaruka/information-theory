...
* §1 Fisher info sum predicate (`IsFisherInfoSumHyp`) — `J(X+Y)` の **値** を
  hypothesis 化 (Mathlib に Fisher info conv lemma 不在、scope-out)。
...
* `IsFisherInfoSumHyp X Y P` (§1) — `J(X+Y) = something` を hypothesis 化
...
/-! ## §1 — Fisher info sum predicate

`Mathlib` に Fisher information の convolution lemma `J(X+Y) = ...` 不在
(`rg "fisherInfo.*conv" .lake/packages/mathlib → 0 hit`)。値の hypothesis 化のみ
本 file で扱う。 -/

/-- **Fisher info sum hypothesis** (L-FS1 形). Stam 推論の前段で `J(X+Y)`
の **value** が `J(X), J(Y), ...` に関する何らかの関係に従うことを述べる
hypothesis 形。本 file では具体的な等式形を取らず `Prop` placeholder として
公開。Cover-Thomas 17.7.2 で Stam 推論のために要する `J(X+Y)` の data
(value + a 整合性 + 正値性) を保持。

具体的な discharge plan は `epi-fisher-info-sum-discharge-plan.md` (未着手)。
本 file では trivial `True` で導入し、将来 plan で本格化する。 -/
def IsFisherInfoSumHyp {Ω : Type*} [MeasurableSpace Ω]
    (X Y : Ω → ℝ) (P : Measure Ω) : Prop := True

/-- The Fisher info sum hypothesis is trivially provable. -/
theorem isFisherInfoSumHyp_trivial {Ω : Type*} [MeasurableSpace Ω]
    (X Y : Ω → ℝ) (P : Measure Ω) : IsFisherInfoSumHyp X Y P := trivial
