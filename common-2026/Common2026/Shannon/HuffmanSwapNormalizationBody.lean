...
**strictly more primitive** な predicate。

`l_norm` を「任意の長さ関数」から「`ll` の permutation」に固定した点が genuine な
縮約: `swap_step_le` を least-probable order に沿って合成すれば σ が構成できる, という
Cover-Thomas argument の核がそのまま残余になる。 -/
...
/-- **strictly-more-primitive predicate `EqualizingSwapTargetHypothesis`**: 等長化を
...
`EqualizingPermHypothesis` よりさらに primitive: expected length 非増加の核
(`swap_step_le`) を要求せず, swap target の **存在のみ** を要求する。 -/
