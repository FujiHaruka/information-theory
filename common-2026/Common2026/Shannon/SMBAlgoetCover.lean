...
/-- Downward likelihood ratio: `exp(negLogQk - n · blockLogAvg)` lifted to ENNReal. -/
noncomputable def MRatioLo
    (μ : Measure Ω) [IsFiniteMeasure μ] (p : StationaryProcess μ α) (k n : ℕ) :
    Ω → ℝ≥0∞ :=
  fun ω => ENNReal.ofReal (Real.exp (
    negLogQk μ p k n ω - (n : ℝ) * blockLogAvg μ p n ω))

/-- The downward ratio integrates to at most `1`. -/
theorem integral_MRatioLo_le_one
    (μ : Measure Ω) [IsProbabilityMeasure μ] (p : StationaryProcess μ α) (k n : ℕ) :
    ∫⁻ ω, MRatioLo μ p k n ω ∂μ ≤ 1 := by
  sorry

/-- Borel–Cantelli consequence: the downward ratio is eventually bounded by `n²` a.s. -/
theorem MRatioLo_le_sq_eventually
    (μ : Measure Ω) [IsProbabilityMeasure μ] (p : StationaryProcess μ α) (k : ℕ) :
    ∀ᵐ ω ∂μ, ∀ᶠ n in Filter.atTop,
      MRatioLo μ p k n ω ≤ ENNReal.ofReal ((n : ℝ) ^ 2) := by
  sorry
...
/-! ## D.5 — liminf direction -/

/-- Logarithmic form of `MRatioLo_le_sq_eventually`: pointwise `blockLogAvg`
lower bound by the `k`-Markov approximation minus a `2 log n / n` error. -/
theorem blockLogAvg_ge_negLogQk_minus_error
    (μ : Measure Ω) [IsProbabilityMeasure μ] (p : StationaryProcess μ α) (k : ℕ) :
    ∀ᵐ ω ∂μ, ∀ᶠ n in Filter.atTop,
      negLogQk μ p k n ω / n - 2 * Real.log n / n ≤ blockLogAvg μ p n ω := by
  sorry

/-- Taking `liminf` in `blockLogAvg_ge_negLogQk_minus_error` and using
Birkhoff for the `k`-Markov approximation gives the per-`k` liminf bound. -/
theorem liminf_blockLogAvg_ge_condEntropyTail
    (μ : Measure Ω) [IsProbabilityMeasure μ] (p : ErgodicProcess μ α) (k : ℕ) :
    ∀ᵐ ω ∂μ,
      conditionalEntropyTail μ p.toStationaryProcess k
        ≤ Filter.liminf (fun n => blockLogAvg μ p.toStationaryProcess n ω) Filter.atTop := by
  sorry

/-- Letting `k → ∞` discharges the `liminf` hypothesis of
`shannon_mcmillan_breiman_of_sandwich`. -/
...
theorem blockLogAvg_bddBelow_ae
    ...
    ∀ᵐ ω ∂μ, Filter.IsBoundedUnder (· ≥ ·) Filter.atTop
      (fun n => blockLogAvg μ p.toStationaryProcess n ω) := by
  -- `blockLogAvg μ p n ω = -(1/n) * log P_n({block_n ω}) ≥ 0` for every `n`
  -- and every `ω`: `P_n.real ≤ 1` (probability measure pushforward) gives
  -- `log ≤ 0`, hence `-(1/n) * log ≥ 0` whenever `1/n ≥ 0`. So `0` is a uniform
  -- lower bound — no a.s. exception needed.
  ...
  -- blockLogAvg = -(1/n) * log P; -(1/n) ≤ 0 and log ≤ 0, so product ≥ 0.
  have h_inv_nn : (0 : ℝ) ≤ 1 / (n : ℝ) := by positivity
  have h_neg_inv_nonpos : -(1 / (n : ℝ)) ≤ 0 := neg_nonpos_of_nonneg h_inv_nn
  -- Compute: -(1/n) * log ≥ 0 since both factors are ≤ 0.
  unfold blockLogAvg
  exact mul_nonneg_of_nonpos_of_nonpos h_neg_inv_nonpos h_log_nonpos
