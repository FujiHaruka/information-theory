...
private theorem huffmanStep_initMultiset_sibling
  ...
  -- 最小性: huffmanStep の内部 Classical.choose_spec を直接取り出す
  -- (publish された step lemma だけからは取り出せないため、ここで直接 unfold で取得)
  have hmin1 : ∀ z ∈ initMultiset P, step.1.2 ≤ z.2 := by
    -- huffmanStep を unfold する: step.1 = x1 := Classical.choose (Multiset.exists_min_image ...)
    unfold huffmanStep at hstep_def
    have hne : (initMultiset P) ≠ 0 := by
      intro h0
      rw [h0, Multiset.card_zero] at h_init_card
      omega
    have hspec := Classical.choose_spec
      (Multiset.exists_min_image (α := Finset α × ℝ) (R := ℝ)
        (fun p => p.2) hne)
    -- step.1 = Classical.choose (...) by hstep_def
    have hstep1_eq : step.1 = Classical.choose
        (Multiset.exists_min_image (α := Finset α × ℝ) (R := ℝ)
          (fun p => p.2) hne) := by
      rfl
    rw [hstep1_eq]
    exact hspec.2
  have hmin2 : ∀ z ∈ (initMultiset P).erase step.1, step.2.1.2 ≤ z.2 := by
    unfold huffmanStep at hstep_def
    have hne : (initMultiset P) ≠ 0 := by
      intro h0
      rw [h0, Multiset.card_zero] at h_init_card
      omega
    -- s' := (initMultiset P).erase x1
    have hs'_ne : (initMultiset P).erase step.1 ≠ 0 := by
      have hcard_s' : ((initMultiset P).erase step.1).card =
          (initMultiset P).card - 1 := Multiset.card_erase_of_mem hx1_mem
      intro h0
      rw [h0, Multiset.card_zero] at hcard_s'
      omega
    have hspec := Classical.choose_spec
      (Multiset.exists_min_image (α := Finset α × ℝ) (R := ℝ)
        (fun p => p.2) hs'_ne)
    have hstep21_eq : step.2.1 = Classical.choose
        (Multiset.exists_min_image (α := Finset α × ℝ) (R := ℝ)
          (fun p => p.2) hs'_ne) := by
      rfl
    rw [hstep21_eq]
