...
* **L-BM1 (Brunn-Minkowski entropy hypothesis, 核心 retreat)**:
  ...
  `brunn-minkowski-from-epi-discharge-plan.md` (未着手) で EPI の
  n-dim 拡張 + Cover-Thomas Theorem 17.7.4 経路で導出する想定。
...
Discharge plan `brunn-minkowski-from-epi-discharge-plan.md` (未着手) で
EPI (T2-D) の n-dim 拡張 + Cover-Thomas Theorem 17.7.4 経路から導出する
想定。 -/
