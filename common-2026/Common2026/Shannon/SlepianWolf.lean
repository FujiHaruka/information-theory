...
`slepian_wolf_converse_X / _Y / _sum` を `And` で束ねた publish 形。3 bound の各 Pe は
それぞれ別の (conditioner, decoder) で計算されるため、独立に維持される。 -/

/-- Slepian–Wolf converse (single-shot, 3 bound 統合形).

Cover & Thomas, 15.4 の single-shot 版に相当。各 bound の `Pe` は対応する decoder で
別々に評価されることに注意 (X bound は marginal X error、Y bound は marginal Y error、
sum bound は joint error)。 -/
theorem slepian_wolf_converse_single_shot
    ...
    (dec : Fin Mx × Fin My → α × β)
    (hXs : Measurable Xs) (hYs : Measurable Ys)
    (hcardα : 2 ≤ Fintype.card α)
    (hcardβ : 2 ≤ Fintype.card β)
    (hcardαβ : 2 ≤ Fintype.card (α × β)) :
    -- X bound
    (Real.log (Mx : ℝ) ≥
      InformationTheory.MeasureFano.condEntropy μ Xs Ys
        - Real.binEntropy
  ...
  ⟨slepian_wolf_converse_X μ Xs Ys eX eY dec hXs hYs hcardα,
   slepian_wolf_converse_Y μ Xs Ys eX eY dec hXs hYs hcardβ,
   slepian_wolf_converse_sum μ Xs Ys eX eY dec hXs hYs hcardαβ⟩
