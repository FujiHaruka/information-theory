...
- **既存資産流用**:
  - `InformationTheory/Shannon/AEP.lean`: `source_coding_achievability` (single-source AEP) を
    Y 側 source `Ys` に直接適用。
  - `InformationTheory/Shannon/SlepianWolf.lean`: 既存 single-shot converse (本 file は touch せず並立)。
