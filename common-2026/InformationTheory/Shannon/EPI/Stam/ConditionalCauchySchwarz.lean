...
That skeleton left Steps 1-3 as **`Prop := True` pass-through** (`IsStamScoreConvolution`
...
* `IsStamScoreConvHyp` (§2) — Step 1 typed predicate carrying the optimal `λ`-witness.
* `IsStamCondExpCSHyp` (§3) — Step 2 typed predicate carrying the `∀λ` convex Fisher bound.
...
/-- The Step-2 typed predicate implies `IsStamCauchySchwarz`
...
/-- **Step 2 typed predicate ⇒ optimal Cauchy-Schwarz**.
...
theorem stamCauchySchwarzOptimal_of_condExpCSHyp {Ω : Type*} [MeasurableSpace Ω]
  ...
  have h_bd := h J_X J_Y J_sum fX fY fXY hJX hJY hJsum hJX_def hJY_def hJsum_def
    hregX hregY hnormX hnormY hconv hready
    (J_Y / (J_X + J_Y)) (by positivity) (by rw [div_le_one hsum]; linarith)
  -- Closed form: at λ*, the convex bound equals the harmonic mean.
  have h_min := stam_lambda_min hJX hJY
  -- h_min : (J_Y/(J_X+J_Y))² J_X + (1 - J_Y/(J_X+J_Y))² J_Y = J_X J_Y / (J_X+J_Y).
  linarith [h_bd, h_min]

/-- **Step 1 + Step 2 ⇒ optimal Cauchy-Schwarz**. The combined deliverable:
...
/-- **Step 1 + Step 2 ⇒ existential Cauchy-Schwarz** (`IsStamCauchySchwarz`),
