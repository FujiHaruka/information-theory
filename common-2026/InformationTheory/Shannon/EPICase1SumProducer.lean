...
唯一 FALSE な `Z_law` field を park** (撤退ライン L-Sum-struct-park)。`integrable_deriv` の
`t`-可測性は残1 (X/Y producer) と同型障害なので compound park。
-/
...
**唯一の honesty 上の制約 (park)**: `reg_at .Z_law` field は unit-hardcode
...
なので `Z_law` は FALSE-as-stated。これは `IsRegularDeBruijnHypV2` の structure-level unit-hardcode
が原因の型不充足であり、本 producer の前提を追加しても閉じられない (structure 改変が必要だが
blast radius / wrapper 固定で本 session scope 外)。
...
noncomputable def isDeBruijnRegularityHyp_sum_of_methodX_unitnoise
  ...
  · -- Z_law: PARK. The unit-hardcode requires `P.map (Z_X+Z_Y) = gaussianReal 0 1`, but the
    ...
    -- contradicts `hZXZY_law`. Park (unit-hardcode structural mismatch, L-Sum-struct-park).
    -- @residual(plan:epi-case1-sum-producer-plan)
    sorry
