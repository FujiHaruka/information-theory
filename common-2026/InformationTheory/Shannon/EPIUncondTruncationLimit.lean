...
**Phase 2 progress (2026-06-08)**: body は genuine 配線済 (IndepFun 保存 / fibre 同定 / 等式→
...
由来負部、両部有限 ⟹ 全エントロピー可積分)。**honest sorry 残**: sum-marginal `Q.map(W+V)` を参照する
4 件 (`hWV_ne_bot` = mixture 負部 Jensen+Tonelli / `h_ac` = 連続 disintegration a.c. / `hκ_cross_int`
= cross-entropy domination / `hκ_KL` = 上記の下流) + Mathlib 不在の `hκ_dens_meas` (joint 密度可測、
真 gap)。前者 4 件は route-T-scale (≥150 行) ゆえ別 fill / escalate へ。
...
theorem differentialEntropyExt_mono_add_truncW
  ...
  -- **`h_ac` (joint ≪ product)**: per-fibre `condDistrib (W+V) V Q z ≪ Q.map(W+V)` a.e. z
  -- (= general disintegration `condDistrib z ≪ μ.map X`). Mathlib general (non-discrete) 版は不在
  -- (in-tree `Bridge.condDistrib_ae_absolutelyContinuous_map` は `[Countable X]` 専用、`X=ℝ` 不可)。
  -- continuous 版 self-build (`compProd_map_condDistrib` + 連続 disintegration a.c.) が必要、予算超過。
  -- @residual(plan:epi-uncond-truncation-lsc-plan)
  ...
  have hκ_cross_int : ∀ᵐ z ∂(Q.map V), Integrable
    ...
    sorry
  -- **`hκ_KL` (per-fibre KL ≠ ∞)**: per finite ② docstring, derivable from `hκ_ac` + `hκ_logp_int`
  -- + `hκ_cross_int` + (κz ≪ ν from `h_ac`). Since it transitively depends on `hκ_cross_int` /
  -- `h_ac` (both sum-marginal crux), it stays sorry until those land.
  -- @residual(plan:epi-uncond-truncation-lsc-plan)
