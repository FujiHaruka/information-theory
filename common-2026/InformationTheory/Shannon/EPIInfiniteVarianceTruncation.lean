...
独立 honesty audit 2026-06-07 PASS (fresh auditor、self-audit 不可ゆえ独立判定): ok (proof done)。
(1) 非循環: 結論 `∃ C, 0≤C ∧ ∀ᵐ z, p_n∗q_n ≤ C·(p∗q)` (優関数不等式)、仮説は
`hX`/`hY`/`hXY`/`hX_ac`/`hY_ac`/`hpos` (regularity)、結論型 ≢ 仮説型。(2) 非バンドル: usc 結論や
和エントロピー可積分を仮説で受けていない。`hX_ac`/`hY_ac` は unused warning だが helper A と並行
interface を保つ意図 (defect でない)。(3) sufficiency: 各成分 bound `condTrunc_marginal_density_le`
(`@audit:ok`、genuine) を `hn=le_refl n` で fixed-n に呼び + 畳込み単調性 `integral_mono_of_nonneg`。
`C=C_X·C_Y` は `hpos` (positive mass `m≠0`) ゆえ finite-positive (退化境界悪用なし)。
(4) `#print axioms` = `[propext, Classical.choice, Quot.sound]` (sorryAx-free、body 独自 sorry 0、
機械確認)。0 sorry / 0 residual = proof done。
@audit:ok -/
...
独立 honesty audit 2026-06-07 PASS (fresh auditor、genuine fill 後の独立判定、旧 skeleton 時点
honest_residual から昇格): ok (proof done)。(1) 非循環: 結論は a.e. 各点収束 `p_n∗q_n → p∗q`、
仮説は a.c. + measurability の regularity precondition、結論型 ≢ 仮説型。(2) 非バンドル: usc 結論を
仮説で受けていない。(3) sufficiency: body は filter 版 DCT (`tendsto_integral_filter_of_dominated_convergence`)
の 4 義務を genuine 供給 — 各成分各点収束 (cond density formula `pnZ_n =ᵐ (m_n)⁻¹·1_{Sn}·pZ` ×
`(m_n)⁻¹→1` × `1_{Sn}→1`)、eventual 優関数 A (`condTrunc_marginal_density_le`)、優関数可積分
(slice integrability)、Y 成分は `measurePreserving_sub_left` transport。各点収束が follow。
(4) `#print axioms` = `[propext, Classical.choice, Quot.sound]` (sorryAx-free、body 独自 sorry 0、
機械確認)。0 sorry / 0 transitive sorry = proof done。旧 `@residual(plan:...)` は genuine fill 後の
stale residual ゆえ除去。
@audit:ok -/
...
独立 honesty audit 2026-06-07 PASS (fresh auditor、genuine fill 後の独立判定、旧 skeleton 時点
honest_residual から昇格): ok (proof done)。(1) 非循環: 結論は cross-entropy 被積分関数の可積分性
`Integrable (log ν 密度) μ_n`、仮説は a.c. + measurability + `hent_sum`、結論型 ≢ 仮説型。
(2) 非バンドル (最重点): usc 結論を仮説で受けていない。**`hent_sum` は load-bearing でなく precondition** —
`hent_sum = Integrable (negMulLog(ν 密度)) volume` は **別測度 (volume≠μ_n)・別被積分 (negMulLog≠log)**
で、結論 (per-n `Integrable (log) μ_n`) を encode しない。優関数 (fixed-n #1) 経由で支配可積分性を
供給する上流 regularity (極限測度 ν の有限微分エントロピー)。(3) sufficiency: pull-back
`integrable_rnDeriv_smul_iff` + density 同定 + 優関数 `(μn.rnDeriv).toReal ≤ C·(ν.rnDeriv).toReal`
+ 恒等式 `r·|log r| = |negMulLog r|` (r≥0、line 1536 `negMulLog_eq_neg` 経由、符号正) +
`hent_sum.abs.const_mul C` で `Integrable.mono'`。per-n 可積分性が follow。(4) `#print axioms` =
`[propext, Classical.choice, Quot.sound]` (sorryAx-free、body 独自 sorry 0、機械確認)。0 sorry /
0 transitive sorry = proof done。旧 `@residual(plan:...)` は genuine fill 後の stale residual ゆえ除去。
@audit:ok -/
...
独立 honesty audit 2026-06-07 PASS (fresh auditor、genuine fill 後の独立判定、旧 skeleton 時点
honest_residual から昇格): ok (proof done)。(1) 非循環: 結論は `RHS_n → h(ν)` (数列収束)、仮説は
a.c. + measurability + `hent_sum` (= regularity)、結論型 ≢ 仮説型。(2) 非バンドル: usc 不等式や h(ν) の値を
仮説に bundle していない — **収束先 `differentialEntropy ν` は body 内 `htarget` で導出** (`-∫(p∗q)·g =
∫ negMulLog((ν.rnDeriv).toReal) = differentialEntropy ν`、`rnDeriv_map_sum_ae` + `negMulLog_eq_neg`)、
仮説で受けていない。`hent_sum` は優関数可積分性供給の precondition (#3 と同じ理由で load-bearing でない)。
(3) sufficiency + 符号整合: `crossEntropySeq = -∫ log(ν 密度) ∂μ_n` (def `@audit:ok`)、pull-back
`crossEntropySeq n =ᶠ -∫(p_n∗q_n)·g` (g=log ν 密度)、外側 DCT `∫(p_n∗q_n)·g → ∫(p∗q)·g` (各点収束 B ×
g 定数、優関数 A+hent_sum)、`-∫(p∗q)·g = +h(ν)` (符号正、degenerate 収束先でない)。収束が follow。
(4) `#print axioms` = `[propext, Classical.choice, Quot.sound]` (sorryAx-free、body 独自 sorry 0、
機械確認)。0 sorry / 0 transitive sorry = proof done。旧 `@residual(plan:...)` は genuine fill 後の
stale residual ゆえ除去。
@audit:ok -/
