...
**残 hyp なし** (2026-06-12 cleanup): 本 wrapper が取っていた dead `h_mi_bridge`
(F-2、mutual info bridge) は body 未参照だったため signature から除去済。除去後の
signature は `awgn_theorem_F1_discharged` (= headline `awgn_achievability` +
`isAwgnChannelMeasurable`) と内容重複 (`@audit:superseded-by` 参照)。
...
Classical.choice, Quot.sound]`. The unused `h_mi_bridge` was an F-2 wiring artefact
(it had been kept for `awgn_channel_coding_theorem` signature consistency, not
load-bearing — the body never used it; 2026-06-12 cleanup で同 hyp は signature
から除去済).
...
hypothesis only weakens the signature, never strengthens it dishonestly); 2026-06-12
