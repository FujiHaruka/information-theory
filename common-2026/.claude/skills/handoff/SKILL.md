...
- やって空振りした探索 (loogle Found 0 / grep 空振り) — 再探索の無駄を防ぐ。クエリも添える
- 決定したけどまだコードに反映していない方針
- 試して却下したアプローチ + 理由 (再度トライ防止)
- 当たりをつけた仮説 (「この補題が鍵になりそう」)
- 今セッションで判明した Mathlib の落とし穴 / バージョン依存挙動

重い定性ログ (空振り一覧・設計の後戻りの経緯) は proof-log 行き。ここには次の一手に効く 1〜3 行のポインタだけ残す。なければこのセクションごと省略する。
```
