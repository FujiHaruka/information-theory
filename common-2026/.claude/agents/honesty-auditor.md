...
**audit state の単一 source of truth = コード内 `@residual(...)` / `@audit:*` タグ** (docstring 直書き)。snapshot 文書も SoT ではない。verdict 書込先 = 当該 decl の docstring に該当タグ追加 / 修正 (Edit tool 使用)。機械整合チェックは `rg "@residual|@audit:" InformationTheory/` で行う。
...
- 対象 file path (e.g., `InformationTheory/Shannon/EPIStamWalls.lean`)
