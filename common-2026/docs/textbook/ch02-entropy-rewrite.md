...
**証明の底：Mathlib に委ねる2つの事実.** 形式化は相対エントロピーについて
次の2つの性質だけをブラックボックスとして用いる。これらの証明（測度論と
対数の凹性に基づく）は本書では展開せず、Mathlib の KL ダイバージェンスの
理論に委ねる。ここが本証明の **信頼境界** である。

- **(KL1) 非負性.** $D(P\|U) \ge 0$。— Mathlib では KL ダイバージェンスが
  そもそも区間 $[0,\infty]$ に値をとる量として定義されており、非負性はこの定義に
  組み込まれている（Gibbs の不等式に相当する内容がここに吸収されている）。
- **(KL2) 退化条件.** $D(P\|U) = 0 \iff P = U$。— `InformationTheory.klDiv_eq_zero_iff`。

**本文で展開する段：代数的恒等式.** 残りは初等的な変形だけである。
$U(x) = 1/M$ を代入すると、
$$
D(P\|U)
...
**結論.** 上の恒等式と (KL1) を合わせると
$$
\log M - H(X) \;=\; D(P\|U) \;\ge\; 0
\quad\Longrightarrow\quad H(X) \le \log M.
$$
これが主定理 `entropy_le_log_card` である。等号 $H(X) = \log M$ は
$D(P\|U) = 0$ と同値で、(KL2) よりこれは $P = U$、すなわち $X$ が一様分布で
あることと同値である（`entropy_eq_log_card_iff`）。$\qquad\blacksquare$
...
> 恒等式 `klDiv_uniformOn_univ_toReal_eq` (`Common2026/Shannon/MaxEntropy.lean`)。
> 相対エントロピーの非負性 (KL1)・退化条件 (KL2) は Mathlib `InformationTheory.klDiv`
> の理論に基づく。
