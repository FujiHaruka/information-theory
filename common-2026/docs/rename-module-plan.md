...
`rg 'Common2026'` で **552 ファイル / 7387 箇所**。ただし用途は質的に 3 層に分かれ、
リネームの「必須度」が層ごとに違う。

| 層 | 対象 | 箇所 | 必須度 |
|----|------|------|--------|
| **A. モジュールパス / ライブラリ名** | `lakefile.toml` (`name`/`defaultTargets`/`[[lean_lib]]`)、root `Common2026.lean`、ディレクトリ `Common2026/` (205 `.lean`)、`import Common2026.*` 872 行 | 大半 | **必須**(壊れる) |
| **B. ハードコード Name リテラル** | `scripts/{SorryAudit,FindOrphans,EntryPointReport,SorryAuditPerModule}.lean` の `` `Common2026 `` プレフィックス判定 | ~6 | **必須**(無言で 0 件返す) |
| **C. namespace `Common2026.*`** | lib の Shannon 系 ~20 ファイル (`namespace Common2026.Shannon[.…]`) + scripts の `Common2026.{SorryAudit,…}` | ~45 (20+21+4) | **任意**(モジュールパスと独立) |
...
重要な事実:
- **Lean ではモジュールパスと namespace は独立**。ディレクトリ `Common2026/` を改名しても、
  ファイル内の `namespace Common2026.Shannon` は強制では変わらない。逆も同様。
  → A と C は**別判断**。
- lib 本体の namespace の主流は `InformationTheory.*`。`Common2026.*` namespace は
...
- `lake-manifest.json` / `.gitignore` に `Common2026` 参照なし。
...
機械置換の安全性:`Common2026` は他の語の部分文字列にならない一意なトークンなので、
単純な文字列 `Common2026` → `<NEW>` の全置換で誤爆リスクは低い(`grep -F` レベル)。
唯一の注意は B 層の Name リテラル `` `Common2026 `` がパスではなく**シンボル名前空間**を
...
1. ディレクトリ改名:`git mv Common2026 <NEW>` および `git mv Common2026.lean <NEW>.lean`。
   (`git mv` で rename を履歴に残す)
2. `lakefile.toml`:`name` / `defaultTargets` / `[[lean_lib]] name` の 3 箇所を `<NEW>` に。
3. 全 `.lean` の `import Common2026` → `import <NEW>`(lib 205 + scripts、872 行)。
4. scripts の Name リテラル `` `Common2026 `` → `` `<NEW> ``(4 ファイル)。
5. root `<NEW>.lean` 冒頭コメント `the \`Common2026\` library` を更新。
...
### Phase 2 — 確定(C):namespace `Common2026.*` → `InformationTheory.*` に統一

- 対象:lib の Shannon 系 ~20 ファイルの `namespace Common2026.Shannon[.…]` を
  `namespace InformationTheory.Shannon[.…]` に。これで lib 主流規約に揃い、揺れが解消する。
- 完全修飾参照(docstring 含む `Common2026.Shannon.foo` 等)も同時に置換。
- scripts の `Common2026.{SorryAudit,FindOrphans,EntryPointReport,SorryAuditPerModule}` namespace は
...
- docstring 内のパス文字列 `Common2026/…` / `Common2026.…`(lib `.lean` 多数)。
...
- **Phase 1 は原子的に**。中途半端な状態(import だけ直してディレクトリ未改名 等)は
  全面ビルド不能。1 コミットに収める。
- **B 層の無言失敗**:`` `Common2026 `` リテラル置換漏れは、コンパイルは通るのに
...
- **GitHub リポジトリ名 / `common-2026` ディレクトリ名**:lakefile の `name` は
  ライブラリ名(`Common2026`)であり、リポジトリのフォルダ名 `common-2026` とは別物。
