...
> `jointEntropySubset_submodular`) が `Common2026/Shannon/Polymatroid.lean`
> (288 行 / 0 sorry) で立っているのを起点に、`Polymatroid` structure を
> 導入し entropy をその term として登録する。
>
> ゴールは Mathlib 不在の `Polymatroid` structure を Common2026 に新規導入し、
...
**ゴール**: 任意の `ι : Type*` `[DecidableEq ι]` に対し `Polymatroid ι` structure を定義し、`Common2026/Shannon/Polymatroid.lean` の `jointEntropySubset` 4 主定理を充足する形で `entropyPolymatroid : Polymatroid (Fin n)` を構成する。
...
1. **(設計 a) Where** — `Common2026/Polymatroid/Basic.lean` (新規ディレクトリ)。Shannon 領域に閉じない構造体 (情報理論専用ではなく組合せ最適化や Matroid 理論との接点) なので、`Common2026/Shannon/` から外に出して再利用しやすくする。`Common2026/Shannon/Polymatroid.lean` に `entropyPolymatroid` だけ追記。
...
Common2026/
  ...
  Shannon/
    Polymatroid.lean        ← 既存 (4 主定理) + entropyPolymatroid def 末尾追記
                              import Common2026.Polymatroid.Basic を追加
```

`Common2026.lean` (library root) に `import Common2026.Polymatroid.Basic` を追記 (Shannon.Polymatroid の前)。
...
ターゲット: `Common2026/Polymatroid/Basic.lean` 新規作成、`structure Polymatroid` を Matroid mirror 流儀で定義。
...
- [ ] **ディレクトリ作成**: `Common2026/Polymatroid/` 新規作成 (`mkdir`)
...
- [ ] **`Common2026.lean` に `import Common2026.Polymatroid.Basic`** を追加 (Shannon.Polymatroid より前の位置)
- [ ] `lake env lean Common2026/Polymatroid/Basic.lean` で silent 化

### Done 条件

- `Polymatroid` structure が定義されている
- `lake env lean Common2026/Polymatroid/Basic.lean` が silent
...
ターゲット: `Common2026/Shannon/Polymatroid.lean` 末尾に `entropyPolymatroid : Polymatroid (Fin n)` を追加。

### スコープ (skeleton)

```lean
-- Common2026/Shannon/Polymatroid.lean 末尾追記:

import Common2026.Polymatroid.Basic  -- ファイル先頭に追加
...
- [ ] **`import Common2026.Polymatroid.Basic`** を `Common2026/Shannon/Polymatroid.lean` 先頭に追加 (HanD import の隣)
...
- `entropyPolymatroid` が `lake env lean Common2026/Shannon/Polymatroid.lean` で silent
...
- **`Common2026/Polymatroid/Basic.lean` の import policy 違反** (`import Mathlib` を入れてしまう) → `Mathlib.Data.Finset.Basic` / `Mathlib.Order.Monotone.Basic` / `Mathlib.Data.Real.Basic` の 3 本に絞る。`lake env lean` で silent 化を確認
...
- `Common2026/Polymatroid/Basic.lean` が新規作成、`structure Polymatroid` 定義済、`lake env lean Common2026/Polymatroid/Basic.lean` silent
- `Common2026/Shannon/Polymatroid.lean` に `entropyPolymatroid` 追加、`lake env lean Common2026/Shannon/Polymatroid.lean` silent
...
- `Common2026.lean` に `import Common2026.Polymatroid.Basic` 追加
- `lake build Common2026.Polymatroid.Basic` + `lake build Common2026.Shannon.Polymatroid` 緑通過
