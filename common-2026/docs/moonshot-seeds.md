...
>
> **E-8'' Phase α/β/γ 部分完了 (retreat lines 採用)**: Phase α `BackwardFiltration.lean` (新規 139 行、0 sorry) で `backwardFiltration : Filtration ℕᵒᵈ m₀` + `tailSigma` + `comap_T_tailSigma_le` を proven。Phase β `BackwardMartingale.lean` (新規 167 行、**2 sorry** retreat) で `Martingale (ι := ℕᵒᵈ)` を借用しつつ **`BackwardMartingale.ae_tendsto`** + `backward_upcrossings_finite_ae` を statement form で publish (Mathlib `Upcrossing.lean` が `ℕ` ハードコードのため reverse 写経 ~150 行は Mathlib PR 候補レベル、本セッション scope 外と判定)。Phase γ `BirkhoffErgodic.lean` (新規 198 行、0 sorry) で **`birkhoff_ergodic_ae_of_limit`** を Hopf 同型 + backward convergence を hypothesis 形で受け取り、`Ergodic.ae_eq_const_of_ae_eq_comp_ae` + 定数同定で discharge。SMB 主定理仮定なし形 (`shannon_mcmillan_breiman`) は Phase β の 2 sorry 解消後に Phase α/β/γ 合成で着地可能。
>
> **残 deferred**: D-2'' Phase B (`h_yother_zero` 派生は `IsMemorylessChannel` 強化必要、原理的不可)、E-3''' fully-discharged form (~500-800 行)、E-8'' Phase β backward upcrossing inequality 写経 (~150 行、Mathlib PR 候補) + γ.1 Hopf 同型完全証明 (~50 行)。
>
