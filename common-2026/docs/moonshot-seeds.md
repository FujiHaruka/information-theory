...
>
> **残 deferred**: D-2'' Phase B (`h_yother_zero` 派生は `IsMemorylessChannel` 強化必要、原理的不可)、E-3''' fully-discharged form (~500-800 行)、SMB 仮定なし形 `shannon_mcmillan_breiman` (Phase C.1+C.2 + D.1 完了済み、残: Algoet–Cover sandwich ~300-500 行で Phase D.2-D.5 → 4 仮説 discharge)。
>
