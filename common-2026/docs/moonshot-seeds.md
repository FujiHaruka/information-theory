...
>
> **E-8'' Phase γ 経路再選定 (2026-05-16)** — backward-martingale Hopf 経路 (Williams §14.4) は **i.i.d. 専用**で、汎用 `MeasurePreserving + Ergodic` では成立しないことが判明 (cyclic 反例: Ω = {0,1,2}, T(x) = (x+1) mod 3, f = id で `μ[f∘T | σ(S_3, S_4, …)] = f∘T ≠ f = μ[f | σ(S_3, …)]`)。`condExp_iterate_eq_condExp` は数学的に偽。Petersen Thm 2.2 で参照されているのは Hopf **maximal ergodic inequality** (組合せ的不等式、一般 MP で真) で、別物の Hopf rearrangement identity と取り違えていた。Phase α `BackwardFiltration.lean` (139 行、0 sorry) と Phase β `BackwardMartingale.lean` (837 行、0 sorry、forward proxy `reverseProxy` + `upcrossingsBefore_le_revPath_succ` で path-reversal 経由の Mathlib API 流用) は単独で valid な数学なので将来 i.i.d. SLLN 用に retain。Phase γ `BirkhoffErgodic.lean` は **Garsia (1965) maximal ergodic inequality 経路** に書き換え (現 562 行、新規 ~250 行)。完了済み: `maxPartialSum` + `maxPartialSum_sub_comp_T_le_indicator` (Garsia pointwise) + **`maximal_ergodic_inequality`** (∫_{M_n > 0} f dμ ≥ 0、Mathlib gap 自前 ~70 行) + **`birkhoff_neg_mean_sup_null`** (Banach principle "negative-mean ⟹ inf-often-positive cannot be full" via DCT + 上記、~80 行)。残 3 sorry: 上下 sandwich (`birkhoff_eventually_lt_integral_add` / `_gt_sub`、`{limsup A_n > 0}` の T-invariance + `Ergodic.ae_eq_const_of_ae_eq_comp_ae` on indicator で ergodic null、~150-200 行) + 主定理組み立て (`birkhoff_ergodic_ae`、rational-ε 経由で metric 収束、~50 行)。SMB 主定理仮定なし形 (`shannon_mcmillan_breiman`) はこれら完成で着地。
>
> **残 deferred**: D-2'' Phase B (`h_yother_zero` 派生は `IsMemorylessChannel` 強化必要、原理的不可)、E-3''' fully-discharged form (~500-800 行)、E-8'' Phase γ sandwich 完成 (上記 ~200-250 行)。
>
