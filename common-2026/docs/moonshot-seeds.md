...
>
> **E-8'' Phase α/β 完全 + γ ほぼ完了**: Phase α `BackwardFiltration.lean` (新規 139 行、0 sorry) で `backwardFiltration : Filtration ℕᵒᵈ m₀` + `tailSigma` + `comap_T_tailSigma_le` を proven。Phase β `BackwardMartingale.lean` (新規 167 → 837 行、**0 sorry**) で `Martingale (ι := ℕᵒᵈ)` を借用しつつ **`BackwardMartingale.ae_tendsto`** (β.4) + **`BackwardMartingale.upcrossings_ae_lt_top`** (β.2) を完全 discharge — Mathlib `Upcrossing.lean` の `ℕ`-ハードコードは **forward proxy** `reverseProxy N f k ω := f (toDual (N - k)) ω` + path-reversal 補題 `upcrossingsBefore_le_revPath_succ` (upcrossing と reversed-path downcrossing が高々 1 個差で interleave、witness-chain `upperCrossingTime_le_of_witness` 経由) で迂回し、Mathlib 既存 forward API をそのまま利用 (~150 行の reverse 写経も Mathlib PR も不要に)。Phase γ `BirkhoffErgodic.lean` (新規 198 行、現 663 行) で **`birkhoff_ergodic_ae_of_limit`** を `Ergodic.ae_eq_const_of_ae_eq_comp_ae` + 定数同定で discharge、`hopf_identity` 経由で Phase β の backward convergence を `birkhoffCondExpMartingale` に適用。**残 1 sorry**: γ.1 exchangeability `condExp_iterate_eq_condExp` (`BirkhoffErgodic.lean:365-373`、Petersen Lemma 2.2.1 / Williams §14.4、cylinder-set π-system + 対称性議論 ~70-150 行)。これが埋まれば SMB 主定理仮定なし形 (`shannon_mcmillan_breiman`) が Phase α/β/γ 合成で着地。
>
> **残 deferred**: D-2'' Phase B (`h_yother_zero` 派生は `IsMemorylessChannel` 強化必要、原理的不可)、E-3''' fully-discharged form (~500-800 行)、E-8'' γ.1 Hopf 同型 exchangeability ステップ (`condExp_iterate_eq_condExp`、cylinder-set π-system + joint-distribution 対称性、~70-150 行)。
>
