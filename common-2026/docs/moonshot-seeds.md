...
> **Status (2026-05-11)**: ✅ 5 シード本体 + A 節 deferred 全件完了。Loomis–Whitney → Slepian–Wolf → AEP (Phase A〜F unified) → Stein (achievability + converse 半分 + liminf/limsup sandwich) → Polymatroid (structure 化込) を **すべて 0 sorry** で通過。Strong Stein (`Tendsto → K` strict 形) のみ未着手 — strong converse が必要 (現行 DPI + log-sum 経路では `1/(1-ε)` 補正が構造的に残る)。次のムーンショット候補は末尾「次のシード候補 B 節」参照。
>
...
- ~~**Stein Tendsto 形統合 (Phase C 後段)**: `Common2026/Shannon/Stein.lean` の `stein_converse_finite_n` (concrete inequality) と achievability lower bound を合わせて `stein_lemma : Tendsto (fun n => -(1/n) * log (steinOptimalBeta P Q n ε)) atTop (𝓝 (klDiv P Q))` (または liminf 形 `liminf_n (-(1/n) * log β_n) = klDiv P Q`) を構築。新規 plumbing: (a) `steinOptimalBeta` def + sInf plumbing、(b) Filter.liminf squeeze + `inf_ε` で `1/(1-ε)` 補正項を absorb。proof 本体に新規数学なし (Filter / sInf API plumbing のみ)。見積 100〜200 行 / 低〜中リスク。起点完備 (`stein_converse_finite_n` + achievability 0 sorry)。~~ → ✅ 部分完了 (2026-05-11): `Common2026/Shannon/Stein.lean` (+359 行) に `steinBetaSet` / `steinOptimalBeta` 定義 + helper 7 本 (one_mem / nonempty / bddBelow / nonneg / le_one) + 両側 lift (`steinOptimalBeta_log_le_of_converse` / `steinOptimalBeta_log_ge_of_achievability`) + 主定理 `stein_lemma` を 0 sorry で実装。**Tendsto → K の strict 形は不達**: `stein_converse_finite_n` の `1/(1-ε)` 補正で `limsup → K/(1-ε) > K`、両端不一致のため `K ≤ liminf ≤ limsup ≤ K/(1-ε)` の **sandwich 形**に着地。`ε → 0+` で gap が closing (strong Stein には strong converse が必要、本経路では構造的に到達不可)。`IsCoboundedUnder` discharge は AEP Phase F の `Filter.Tendsto.bddAbove_range` trick を再利用 (上限項 `g(n) := K/(1-ε) + log 2/(n(1-ε))` の Tendsto から `BddAbove (Set.range g)` 抽出 → `isCoboundedUnder_ge_of_eventually_le`)。新規数学ゼロ、Filter / sInf API plumbing のみ → plan: `docs/shannon/stein-converse-plan.md` / 実装: `Common2026/Shannon/Stein.lean`
- ~~**AEP Phase F 両側等号 unified source coding theorem**: `Common2026/Shannon/AEP.lean` の `source_coding_weak_converse_aep` (Phase D, liminf ≥ H) と `source_coding_achievability` (Phase E, ∃ code with rate → R for any R > H) を合わせて両側等号 `inf_{achievable codes} liminf (log M_n / n) = entropy μ X` を構築。新規 plumbing: (a) "achievable code" の predicate (`∃ M_n c_n d_n, errorProb ... → 0`)、(b) sInf-over-achievable-codes wrapper、(c) Phase D の `hM_bdd` 仮定との整合 (achievable codes は実用 rate-bounded で常に成り立つ)。proof 本体に新規数学なし。見積 50〜100 行 / 低リスク。起点完備 (Phase D + E 0 sorry)。~~ → ✅ 完了 (2026-05-11): `Common2026/Shannon/AEP.lean` (+92 行) に `IsAchievableCode` (structure with `hM_pos` / `hPe_to_zero` / `hM_bdd`) + `achievableRates : Set ℝ` (liminf rate over achievable codes) + 3 主定理 (`entropy_le_of_mem_achievableRates` Phase D 直接 lift / `mem_achievableRates_of_gt_entropy` Phase E 直接 lift / `source_coding_theorem : sInf (achievableRates μ Xs) = entropy μ (Xs 0)`) を追加。`hM_bdd` 充足は `Filter.Tendsto.bddAbove_range` (Mathlib) で `Tendsto rate (𝓝 R)` から `BddAbove (Set.range rate)` を抽出する 2 行 trick で plumbing-free に処理 (achievability 副産物の `Tendsto` をそのまま使える)。`sInf` 等号は `le_csInf` + `csInf_le_of_le` + `le_of_forall_gt_imp_ge_of_dense` の標準展開で済み、新規数学ゼロ。
