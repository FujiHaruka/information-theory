...
>
> **E-8'' Phase γ 経路再選定 + sandwich 組み立て (2026-05-16)** — backward-martingale Hopf 経路 (Williams §14.4) は **i.i.d. 専用**で、汎用 `MeasurePreserving + Ergodic` では成立しない (cyclic 反例: Ω = {0,1,2}, T(x) = (x+1) mod 3, f = id で `μ[f∘T | σ(S_3, S_4, …)] = f∘T ≠ f = μ[f | σ(S_3, …)]`)。`condExp_iterate_eq_condExp` は数学的に偽。Petersen Thm 2.2 で参照されているのは Hopf **maximal ergodic inequality** (一般 MP で真) で、Hopf rearrangement identity と取り違えていた。Phase α/β (`BackwardFiltration.lean`/`BackwardMartingale.lean`、139+837 行、両方 0 sorry) は将来 i.i.d. SLLN 用に retain。Phase γ `BirkhoffErgodic.lean` は **Garsia (1965) maximal ergodic inequality 経路** に書き換え (現 752 行、新規 ~440 行)。完了済み: `maxPartialSum` + Garsia pointwise + **`maximal_ergodic_inequality`** (∫_{M_n > 0} f dμ ≥ 0、Mathlib gap 自前) + **`birkhoff_neg_mean_sup_null`** (Banach principle, DCT + maximal ergodic) + **`birkhoff_eventually_lt_integral_add`** (上 sandwich、limsup ≤ 0 → eventually < ε') + **`birkhoff_eventually_gt_integral_sub`** (下 sandwich、上を `-f` に適用) + **`birkhoff_ergodic_ae` 主定理組み立て** (rational-ε 経由 `Metric.tendsto_atTop`)。**残 3 sorry はすべて helper level**: (1) `birkhoffAverageReal_limsup_comp_T_ae` (limsup の T-invariance、Etemadi `f(T^n ω)/n → 0` 経由、~50-80 行)、(2) `birkhoffAverageReal_limsup_le_zero_of_int_neg` (上記 + `Ergodic.ae_eq_const_of_ae_eq_comp_ae` で c ≤ 0 限定、~60 行)、(3) 上 sandwich 内 inline boundedness (Doob-Hardy max ineq from `maximal_ergodic_inequality`、~30 行)。SMB 主定理仮定なし形 (`shannon_mcmillan_breiman`) はこれら 3 完成で着地。
>
> **残 deferred**: D-2'' Phase B (`h_yother_zero` 派生は `IsMemorylessChannel` 強化必要、原理的不可)、E-3''' fully-discharged form (~500-800 行)、E-8'' Phase γ helper 3 件 (`birkhoffAverageReal_limsup_comp_T_ae` via Etemadi 補題、`birkhoffAverageReal_limsup_le_zero_of_int_neg` via ergodic discharge、`birkhoffAverageReal_ae_bddAbove` via Hardy-Littlewood、合計 ~150-200 行、すべて named helper として独立化済み)。
>
