...
>
> **残 deferred**: D-2'' Phase B (`h_yother_zero` 派生は `IsMemorylessChannel` 強化必要、原理的不可)、E-3''' fully-discharged form (~500-800 行)、SMB 仮定なし形 `shannon_mcmillan_breiman` (Birkhoff 完成済みなので `shannon_mcmillan_breiman_of_sandwich` の 4 仮説供給で着地、~100-200 行)。
>
