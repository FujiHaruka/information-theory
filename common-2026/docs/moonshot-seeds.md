...
>
> **E-8'' Phase γ 経路再選定 + sandwich 組み立て + Hardy 補題 discharge (2026-05-16)** — backward-martingale Hopf 経路 (Williams §14.4) は **i.i.d. 専用**で、汎用 `MeasurePreserving + Ergodic` では成立しない (cyclic 反例: Ω = {0,1,2}, T(x) = (x+1) mod 3, f = id で `μ[f∘T | σ(S_3, S_4, …)] = f∘T ≠ f = μ[f | σ(S_3, …)]`)。`condExp_iterate_eq_condExp` は数学的に偽。Petersen Thm 2.2 で参照されているのは Hopf **maximal ergodic inequality** (一般 MP で真) で、Hopf rearrangement identity と取り違えていた。Phase α/β (`BackwardFiltration.lean`/`BackwardMartingale.lean`、139+837 行、両方 0 sorry) は将来 i.i.d. SLLN 用に retain。Phase γ `BirkhoffErgodic.lean` は **Garsia (1965) maximal ergodic inequality 経路** に書き換え (現 847 行、新規 ~540 行)。完了済み: `maxPartialSum` + Garsia pointwise + **`maximal_ergodic_inequality`** (∫_{M_n > 0} f dμ ≥ 0、Mathlib gap 自前) + **`maxPartialSum_meas_le`** (Hardy 有限-n、`m · μ({M_n > 0}) ≤ ‖g‖₁`) + **`maxPartialSum_meas_iUnion_le`** (Hardy union 経由) + **`birkhoffAverageReal_ae_bddAbove`** (a.e. BddAbove、Archimedean + Hardy) + **`birkhoff_neg_mean_sup_null`** + **`birkhoff_eventually_lt_integral_add`** + **`birkhoff_eventually_gt_integral_sub`** + **`birkhoff_ergodic_ae` 主定理組み立て** (rational-ε 経由 `Metric.tendsto_atTop`)。**残 2 sorry のみ**: (1) `birkhoffAverageReal_limsup_comp_T_ae` (limsup の T-invariance、**Etemadi 不要**で `birkhoffAverageReal_ae_bddAbove` を f と -f に適用して a.e. bounded を得て、recursion `A_n(f, Tω) - A_{n+1}(f, ω) = (A_{n+1}(f,ω) - f(ω))/(n+1) → 0` で discharge、~50 行)、(2) `birkhoffAverageReal_limsup_le_zero_of_int_neg` ((1) + `Ergodic.ae_eq_const_of_ae_eq_comp_ae` で c ≤ 0 確定、c > 0 case は `birkhoff_neg_mean_sup_null` で矛盾、~60 行)。SMB 主定理仮定なし形 (`shannon_mcmillan_breiman`) はこの 2 完成で着地。
>
> **残 deferred**: D-2'' Phase B (`h_yother_zero` 派生は `IsMemorylessChannel` 強化必要、原理的不可)、E-3''' fully-discharged form (~500-800 行)、E-8'' Phase γ helper 2 件 (`birkhoffAverageReal_limsup_comp_T_ae` via BddAbove + recursion、`birkhoffAverageReal_limsup_le_zero_of_int_neg` via ergodic discharge、合計 ~100-130 行)。
>
