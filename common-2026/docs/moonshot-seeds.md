...
- **E-3. Rate-distortion theorem achievability Phase A 完全形** ✅ (2026-05-14、初回 skeleton MVP → 同日 Phase A 完全形) →
  ...
  **E-3' Phase E MVP ✅ (2026-05-14、witness-form 主定理)** →
  ...
  - **後継 `E-3''` deferred (~360 行)**: fully-discharged 形への昇格 = (1) `measureToPmf` 抽出 (~30 行) + (2) entropy ↔ mutualInfoPmf bridge (~80 行) + (3) `iidAmbient` 構築 mirror (~150 行、`channel_coding_achievability` の verbatim ミラー) + (4) `h_codebook_avg_failure` を Phase B+C 合成で具体化 (~80 行) + (5) `h_failure_tendsto_zero` を Phase D.4' で discharge (~50 行)。**新数学なし、すべて mechanical fill**。
