...
- [ ] Phase 3 — 最終再実測 + 裾縮小確認 📋
- [ ] Phase 4 — **option C (>250 spine 攻略)** 🔨 (2026-06-14 ユーザー決定で着手。pilot = Mono Case-B 分解。下記 Phase 4 節)
...
### Pilot = Mono `differentialEntropyExt_mono_add_of_integrable` (289, 6 set, priv=0)

gateway-atom-first。Phase 1 が「不可分 Case-B spine」と判定した最小の priv=0 残留 = 難 (spine 再設計) 系の
decisive atom。構造: preamble(L144-192) + **Case B finite branch (L193-420, ~227 行)** + Case A(L421-432)。
Case-B 内に明確な seam: `hglob_abs_lint`(~23)/`hr_avg`(~17)/`hRHS_eq`(~20)/`hμWz_ac_vol`/`hKint`/
`htrans_ent`/`hgibbs`/`hsumdens`/`hofReal_fW`/`hcross_int` 等の自己完結ブロック (全て密度 `fW`/`rfun`・
測度 `ν`/`μV`/`μWz` 依存 → 純 math helper 化可能)。これらを <150 helper へ分解 → target は preamble + Case
dispatch で <250 へ。**成功なら難系の手法が確定、失敗なら option C を要再考。**

baseline axioms = `[propext, Classical.choice, Quot.sound]` (sorryAx-free)。target sig 不変を維持。
