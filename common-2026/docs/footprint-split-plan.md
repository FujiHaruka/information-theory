...
### 進捗 (option C) — 2026-06-14、3 本クリア、両機構検証済

>250: **15 → 12** (max 677 = Mass 不変)。Mathlib-shape の純 helper 抽出で **両機構が機械検証 PASS**:
...
helper は全 <150 (relabel 回避)。**両機構確定 = 残り 12 本は技術リスクなし、実行コストのみ。**

### 残り 12 本 (次セッション、易→難)

1. **mid-set spine 系 (priv=0、Mono/KF と同手法、最易)**: DensityForm(298,9set) /
   SmoothingLimit explicit(339)・smoothed(336)・add(264、同一ファイル=1 agent 直列) → 計 4 本。
   ※ SmoothingLimit は import fan-out 注意 (5 importer)、DensityForm も同様 (連結ビルド要)。
2. **private 系 (de-private、PB と同手法)**: ConvEntropyDensity(337,1priv `fibre_rnDeriv_integrable_iff`) /
   TwoSidedRatio(350,14priv = de-private 量多い)。
3. **set-heavy 最難 (15-25 set、spine 分解 + 一部 de-private、最後)**: Walls(323,16set,3priv) /
