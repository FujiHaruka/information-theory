...
| `lz78_phrase_count_fiber_card_le_nat` | GreedyParsingImpl.lean | 258→159 | 124 | ✅ body<150 (metric=次decl docstring artifact) |
| `lz78_parse_tiling_positions` | GreedyLongestPrefix.lean | 211→166 | 149 | ✅ body<150 (metric=file-final/docstring artifact) |
| `ziv_aseventual_le_condEntropyTail_bits` | GreedyParsingImpl.lean | 250→178 | 164 | ⚠ partial (set-heavy spine 残留、policy で停止) |
...
- **ARTIFACT-SKIP (未着手、判定正)**: `measurable_forwardEmbed` (TwoSidedExtension/Core.lean, body 3 — 次 decl docstring が
  metric を 178 に膨張) / `lz78_converse_bad_set_measure_le` (GreedyParsingImpl.lean, body 134 — 次 decl docstring が 165 に膨張)。
