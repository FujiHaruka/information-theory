...
- [ ] Phase 2 — 優先2 (>115 行 tier、159 本) を機会主義的に分解 📋 (dedup 候補は下記「Phase 2 への申し送り」参照)
- [ ] Phase 3 — 最終再実測 + 裾縮小確認 📋
...
純抽出で >250 を消せない代わり、**重複削除**は裾と総行数を両方下げる高価値手。Phase 1 で agent が
発見・未着手の候補 (全て cross-file = 第2ファイルを触るので `lake build` で連結確認が必須):

1. **`SmoothingLimit.lean` ⊃ DensityForm sibling 3 本** — `isHeatFlowEndpointRegular_of_map_eq_rnDeriv` /
   `integral_sub_integral_sq_smoothed_path_le` / `smoothed_path_absolutelyContinuous_and_negMulLog_integrable`
   は今や `DensityForm.lean` の canonical 版とほぼ byte 一致。SmoothingLimit から削除して DensityForm
   を呼べば、**SmoothingLimit の >250 残留 3 本 (338/336/264) が縮む**公算。最優先。
2. **Ext.lean の inline `hbound`** (`EntropyPower/Ext.lean` L79+) = Mono 抽出の
   `lintegral_ofReal_neg_ne_top_of_integrable`。差し替え可。
...
4. **`eq_sum_indicator_preimage_mul`** (TwoSidedRatio 抽出) は汎用の単座標 indicator 分解 — AlgoetCover/SMB
   の他帰納証明に inline 重複がある可能性。
