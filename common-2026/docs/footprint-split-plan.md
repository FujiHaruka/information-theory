...
**Status**: Phase 0 DONE (pilot 較正済) / 優先1 Wave 進行中 (25 本中 13 本 done) / **DoD 緩和済**
(2026-06-14、現実的版へ) / **Parent**: なし (standalone) /
...
DONE 12 本 (今セッション、検証済: signature byte-identical / `#print axioms` 不変 / タグ保存 /
compile clean / commit+push、計 42 本の再利用 public helper 抽出)。**DONE でも >250 残留分は現実的
DoD で許容**。footprint は before→after。

**DONE (今セッション 12 本 + pilot 1 本)**
...
|---|---|---|---|---|
| 4 | `EPI/G2/ConvEntropyDensity.lean:117` (629) | `negMulLog_convDensity_entropy_ge_density` | **1** | **デフォルト: sorry ブロックを inline 維持し sorry-free ブロックのみ抽出** (下記) |
| 10 | `EPI/DensityForm.lean:48` (430) | `entropy_power_inequality_of_density` | 0 | |
| 12 | `EPI/Blachman/GaussianWitness.lean:301` (390) | `isBlachmanConvReady_gaussianPDFReal` | 0 | |
| 13 | `SMB/AlgoetCover/TwoSidedRatio.lean:986` (382) | `integral_MRatioLowerZ_le_one` | 0 | |
| 14 | `EPI/G2/KLFatouLSC.lean:351` (374) | `negMulLog_convDensity_limsup_le` | 0 | |
| 16 | `EPI/Stam/SupplyTwoTime.lean:193` (364) | `isBlachmanConvReady_convDensityAdd_gaussian_asym` | 0 | |
| 17 | `HypercubeEdge/Boundary.lean:193` (355) | `two_sum_projection_eq` | 0 | |
| 18 | `EPI/Blachman/GeneralDensity.lean:201` (347) | `isBlachmanConvReady_convDensityAdd_gaussian` | 0 | |
| 19 | `EPI/Unconditional/TruncationLimit/Mono.lean:77` (330) | `differentialEntropyExt_mono_add_of_integrable` | 0 | |
| 21 | `ChannelCoding/Achievability/RandomCodebook.lean:507` (260) | `random_codebook_E1_swap` | 0 | |
...
- **デフォルト = sorry ブロックを inline のまま残す**こと。sorry を持たない周辺の自己完結
  `have` のみ抽出し、sorry ブロックは元証明に置いておく。この場合タグの移動は発生せず invariant 3 は
  自明に保たれる。残作業 #4 はこのデフォルトで進める。
