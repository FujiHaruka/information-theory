...
- **Wave 6** (`7cc5b52`/`0172774`/`c18ff0a`): `Shannon/DifferentialEntropy.lean` `klDiv_gaussianReal_gaussianReal_eq` 204→93 (Gaussian モーメント系 public helper 6 本、全 <30) + `Shannon/Pinsker/Basic.lean` `tvNorm_le_sqrt_klDiv` 194→143 (Hellinger²/CS pure-real helper 2 本、per-element BH は inline partial)。計 8 helper。>150 tier 73→71。
- **Wave 7** (`9130241`/`893aa89`): `EPI/.../TruncationLimit/Mono.lean` `differentialEntropyExt_mono_add_truncW` 205→124 (helper 2 本: W⊥V 保存 / truncated 密度負部 lintegral 有限性) + `EPI/InfiniteVariance/Capstone.lean` `integrable_negPart_negMulLog_map_sum` 231→137 (convolution-Jensen helper 3 本)。計 5 helper。>150 tier 71→69。
- **Wave 8** (`1a987b4`/`120490d`): `FisherInfo/.../Assembly.lean` `debruijnIdentityV2_holds_assembled_chain_hdiff` 231→126 (helper 1 本 = heatFlow_density σ-derivative domination) + `Shannon/RateDistortion/AchievabilityPhaseEStrong.lean` `jointStronglyTypicalSet_indep_prob_ge` 227→149 (helper 3 本: card_eq/perPair/sum_marginals)。計 4 helper。>150 tier 69→67。
- **Wave 9** (`4f343d6`/`604283e`): `Probability/TwoSidedExtension/Core.lean` `ergodic_shiftZ` 223→94 (helper 1 本 = cylinder 近似+Borel-Cantelli で posSigma 代表元存在、`omit` で section 切離) + `EPI/.../TruncationLimit/Limit.lean` `differentialEntropyExt_top_of_indep_add_unconditional` 202→130 (helper 1 本 = per-n Gibbs+測度 domination hub、`[IsProbabilityMeasure ν]` 明示)。計 2 helper。>150 tier 67→65。
- **Wave 10** (`00bb7ee`/`b04062e`): `EPI/G2/KLFatouLSC.lean` `negMulLog_convDensity_limsup_le` 228→120 (helper 2 本: density-form bridge / klFun-Fatou squeeze tail。1 本では 174 止まり) + `FisherInfo/V2DeBruijnPerTime.lean` `heatFlow_density_heat_equation` 208→143 (helper 1 本 = STEP D spatial-derivative)。計 3 helper。>150 tier 65→63。
- **Wave 11** (`3fcc99e`/`87ca1c7`): `ChannelCoding/.../OuterN.lean` `exists_N_for_smooth_achievability_uniform` 241→146 (private helper 3 本: variance bounds / log² bounds / smoothMinN 算術) + `EPI/.../Truncation/Construction.lean` `integrable_negPart_negMulLog_map_condTrunc_sum` 211→147 (helper 2 本: per-z Jensen / section integrability prod_right_ae 化)。計 5 helper。>150 tier 63→61。
- **Wave 12** (`637b2c6`/`2802899`): `FisherInfo/.../Derivatives.lean` `convDensityAdd_negMulLog_integrable` 187→119 (helper 1 本 = `Integrable (x²·p_t x)`、p_t/g を抽象関数+等式仮説で受領) + `ChannelCoding/.../SmoothInstantiation.lean` `channel_coding_achievability_smooth_at_N_le` 219→128 (helper 2 本: μ-regularity bundle→average_le / E1/E2+pigeonhole 集約)。計 3 helper。>150 tier 61→59。
...
- **Wave 18** (`2934f03`/`d432817`): `EPI/Unconditional/TruncationLimit/Core.lean` (private)
  `negPart_negMulLog_conv_single_ne_top` 191→138 (private helper 1 本
  `convDensity_jensen_negMulLog_ae_bound` 85 = per-z Jensen a.e. 境界 (hjensen) ブロック切出、
  μW μV + 型クラスを受け内部 set 再構築・a.e. 事実のみ仮説受領 = Wave 16 パターンで重 threading 解消) +
...
### 2026-06-21 追補 — LZ78 census 再実測 (asymptotic-optimality 完遂後の新規 target)

直近 2 日 (LZ78 asymptotic-optimality 完遂、`〜06-21`) に触れたファイルを再実測したところ decl-to-next metric >150
の decl が 10 本浮上し、うち 7 本が genuine な新規 target だった (残は計測 artifact)。**2026-06-14 の「枯渇」は
当初 census に対するもので、最近触れたファイルは枯渇 verdict でカバー済とみなさず必ず再実測すること** (新規
モノリスは census 後の実装で常に流入し得る)。

| target | file | metric before→after | body after | verdict |
|---|---|---|---|---|
| `ziv_achievability_composition` | ZivAchievabilityComposition.lean | 239→131 | 118 | ✅ clear |
| `empirical_entropy_le_log_mean` | EmpiricalEntropyMean.lean | 163→<150 | 75 | ✅ clear (helper `_of_lt` 122) |
| `birkhoffAverage_pmfLogCondMarkov_tendsto` | SMB/AlgoetCover/Core.lean | 154→90 | 80 | ✅ clear |
| `lz78_phrase_count_fiber_card_le_nat` | AsymptoticOptimality.lean | 258→159 | 124 | ✅ body<150 (metric=次decl docstring artifact) |
| `lz78_parse_tiling_positions` | GreedyLongestPrefix.lean | 211→166 | 149 | ✅ body<150 (metric=file-final/docstring artifact) |
| `ziv_aseventual_le_condEntropyTail_bits` | AsymptoticOptimality.lean | 250→178 | 164 | ⚠ partial (set-heavy spine 残留、policy で停止) |
| `condState_grouping_bound_mean` | ZivCondGrouping.lean | 233→158 | 156 | ⚠ set-heavy floor (8 set-local assembly core、これ以上は relabeling-adjacent) |

- **ARTIFACT-SKIP (未着手、判定正)**: `measurable_forwardEmbed` (TwoSidedExtension/Core.lean, body 3 — 次 decl docstring が
  metric を 178 に膨張) / `lz78_converse_bad_set_measure_le` (AsymptoticOptimality.lean, body 134 — 次 decl docstring が 165 に膨張)。
- ~18 本の新規 public helper を抽出 (全 <150、anti-relabel 遵守) + bonus dedup (`condState_grouping_bound` の byte-identical な
  `hgroup` block を共有 helper `condState_grouping_perFiber_logsum` へ配線し直し ~58 行削減)。
- 全 Hard invariant を orchestrator がファイル毎に機械検証 (target sig byte-identical / `#print axioms` =
  `[propext, Classical.choice, Quot.sound]` 不変 / sorry 数不変 (全 sorry-free) / `lake env lean` clean)。
  commits: `026c102` (census A) / `70f2f90` (census B) / `11569e3` (census C)。純リファクタ → honesty audit 不要。
