...
**Common2026 の measure-theoretic API は既に `Ω → α` 形 typed RV を引数に取って書かれている**。
...
5. `Common2026/Shannon/Bridge.lean` の `entropy` は `private` でない module-level `def` だが、
   **`Common2026.Fano.Measure.condEntropy` と同名衝突しない**ことを確認済み (前者は
   `InformationTheory.Shannon.entropy`, 後者は `InformationTheory.MeasureFano.condEntropy`)。

---

## A. Common2026 既存 typed RV API surface

### A-1. シャノンエントロピー / 条件付きエントロピー / 結合エントロピー

| 概念 | API (Common2026) | file:line | 引数順 | 戻り値 | 型クラス要件 (verbatim) |
|---|---|---|---|---|---|
| `H(X)` (有限) | `entropy (μ : Measure Ω) (Xs : Ω → X) : ℝ` | `Common2026/Shannon/Bridge.lean:43` | `μ → Xs` | `ℝ` | `[Fintype X] [DecidableEq X] [Nonempty X] [MeasurableSpace X] [MeasurableSingletonClass X]` (`Bridge.lean:38-39` の `variable` で section-wide), `[MeasurableSpace Ω]` (`Bridge.lean:37`) |
| `H(X) ≥ 0` | `entropy_nonneg (μ : Measure Ω) [IsProbabilityMeasure μ] (Xs : Ω → X) (hXs : Measurable Xs) : 0 ≤ entropy μ Xs` | `Common2026/Shannon/Bridge.lean:47` | `μ → Xs` | `Prop` | 上記 + `[IsProbabilityMeasure μ]` + `Measurable Xs` |
| `H(X \| Y)` (有限 X) | `def condEntropy (μ : Measure Ω) [IsFiniteMeasure μ] (Xs : Ω → X) (Yo : Ω → Y) : ℝ` | `Common2026/Fano/Measure.lean:68` | `μ → Xs → Yo` | `ℝ` | `[Fintype X] [DecidableEq X] [Nonempty X] [MeasurableSpace X] [MeasurableSingletonClass X]` (file-level variable `:58-59`), `[MeasurableSpace Y]` (`:60`), `[MeasurableSpace Ω]` (`:57`), `[IsFiniteMeasure μ]` |
| `H(Y \| X) ≤ H(Y)` | `entropy_ge_condEntropy (μ : Measure Ω) [IsProbabilityMeasure μ] (Ws : Ω → W) (Yo : Ω → Y) (hWs : Measurable Ws) (hYo : Measurable Yo) : condEntropy μ Ws Yo ≤ entropy μ Ws` | `Common2026/Shannon/SlepianWolf.lean:164` | `μ → Ws → Yo` | `Prop` | `[Fintype W] [DecidableEq W] [Nonempty W] [MeasurableSpace W] [MeasurableSingletonClass W]`, `[MeasurableSpace Y]`, `[IsProbabilityMeasure μ]` |
| Chain rule `H(X,Y) = H(X) + H(Y\|X)` | `entropy_pair_eq_entropy_add_condEntropy (μ : Measure Ω) [IsProbabilityMeasure μ] (Xs : Ω → X) (Yo : Ω → Y) (hXs : Measurable Xs) (hYo : Measurable Yo) : entropy μ (fun ω => (Xs ω, Yo ω)) = entropy μ Xs + InformationTheory.MeasureFano.condEntropy μ Yo Xs` | `Common2026/Shannon/Entropy.lean:41` | `μ → Xs → Yo` | `Prop` | `Bridge.lean` の variable block 全て (Fintype/DecEq/Nonempty/MS/MSClass × X,Y,Z) + `[IsProbabilityMeasure μ]` |
| Tower `H(X\|Y,Z)` | `condEntropy_tower` | `Common2026/Shannon/Entropy.lean:144` | `μ → Xs → Yo → Zo` | `Prop` | 同上 |
| Conditioning monotonicity | `condEntropy_le_condEntropy_of_pair` | `Common2026/Shannon/Entropy.lean:240` | `μ → Xs → Yo → Zo` | `Prop` | 同上 |
| Joint entropy (n-var) | `def jointEntropy (μ : Measure Ω) (Xs : Fin n → Ω → α) : ℝ` | `Common2026/Shannon/Han.lean:42` | `μ → Xs` | `ℝ` | `[Fintype α] [DecidableEq α] [Nonempty α] [MeasurableSpace α] [MeasurableSingletonClass α]`, `[MeasurableSpace Ω]`, `{n : ℕ}` |
| n-var chain rule | `jointEntropy_chain_rule (μ : Measure Ω) [IsProbabilityMeasure μ] (Xs : Fin n → Ω → α) (hXs : ∀ i, Measurable (Xs i)) : jointEntropy μ Xs = ∑ i : Fin n, InformationTheory.MeasureFano.condEntropy μ (Xs i) (fun ω (j : Fin i.val) => Xs ⟨j.val, j.isLt.trans i.isLt⟩ ω)` | `Common2026/Shannon/Han.lean:56` | `μ → Xs` | `Prop` | 同上 + `[IsProbabilityMeasure μ]` |
...
| 概念 | API (Common2026) | file:line | 引数順 | 戻り値 | 型クラス要件 (verbatim) |
|---|---|---|---|---|---|
| `I(X;Y)` (KL 形) | `noncomputable def mutualInfo (μ : Measure Ω) (Xs : Ω → X) (Yo : Ω → Y) : ℝ≥0∞` | `Common2026/Shannon/MutualInfo.lean:36` | `μ → Xs → Yo` | `ℝ≥0∞` | `[MeasurableSpace Ω]`, `[MeasurableSpace X]`, `[MeasurableSpace Y]` のみ (Fintype 不要) |
| `I(X;Y) ≥ 0` | `mutualInfo_nonneg (μ : Measure Ω) (Xs : Ω → X) (Yo : Ω → Y) : 0 ≤ mutualInfo μ Xs Yo` | `Common2026/Shannon/MutualInfo.lean:42` | `μ → Xs → Yo` | `Prop` | 同上 |
| `I(X;Y) = I(Y;X)` | `mutualInfo_comm (μ : Measure Ω) [IsFiniteMeasure μ] (Xs : Ω → X) (Yo : Ω → Y) (hXs : Measurable Xs) (hYo : Measurable Yo) : mutualInfo μ Xs Yo = mutualInfo μ Yo Xs` | `Common2026/Shannon/MutualInfo.lean:93` | `μ → Xs → Yo` | `Prop` | 上記 + `[IsFiniteMeasure μ]` |
| `I(X;Y) = 0 ↔ indep` | `mutualInfo_eq_zero_iff_indep (μ : Measure Ω) [IsProbabilityMeasure μ] (Xs : Ω → X) (Yo : Ω → Y) (hXs : Measurable Xs) (hYo : Measurable Yo) : mutualInfo μ Xs Yo = 0 ↔ IndepFun Xs Yo μ` | `Common2026/Shannon/MutualInfo.lean:109` | `μ → Xs → Yo` | `Prop` | 上記 + `[IsProbabilityMeasure μ]` |
| `I(X;Y) ≠ ∞` (有限) | `mutualInfo_ne_top [Fintype X] [MeasurableSingletonClass X] [Fintype Y] [MeasurableSingletonClass Y] (μ : Measure Ω) [IsProbabilityMeasure μ] (Xs : Ω → X) (Yo : Ω → Y) (hXs : Measurable Xs) (hYo : Measurable Yo) : mutualInfo μ Xs Yo ≠ ∞` | `Common2026/Shannon/MutualInfo.lean:192` | `μ → Xs → Yo` | `Prop` | 上記 + `[Fintype X] [MeasurableSingletonClass X] [Fintype Y] [MeasurableSingletonClass Y]` + `[IsProbabilityMeasure μ]` |
| Bridge `I = H - H(\|·)` | `mutualInfo_eq_entropy_sub_condEntropy (μ : Measure Ω) [IsProbabilityMeasure μ] (Xs : Ω → X) (Yo : Ω → Y) (hXs : Measurable Xs) (hYo : Measurable Yo) : (mutualInfo μ Xs Yo).toReal = entropy μ Xs - InformationTheory.MeasureFano.condEntropy μ Xs Yo` | `Common2026/Shannon/Bridge.lean:588` | `μ → Xs → Yo` | `Prop` | `Bridge.lean:37-40` の variable block (Fintype/etc on X), `[MeasurableSpace Y]`, `[IsProbabilityMeasure μ]` |
| `I(X;f∘Y) ≤ I(X;Y)` (DPI) | `mutualInfo_le_of_postprocess (μ : Measure Ω) [IsFiniteMeasure μ] (Xs : Ω → X) (Yo : Ω → Y) (hXs : Measurable Xs) (hYo : Measurable Yo) {f : Y → Z} (hf : Measurable f) : mutualInfo μ Xs (f ∘ Yo) ≤ mutualInfo μ Xs Yo` | `Common2026/Shannon/DPI.lean:139` | `μ → Xs → Yo → f` | `Prop` | `DPI.lean:34-37` の variable: `[MeasurableSpace Ω/X/Y/Z]`, `[IsFiniteMeasure μ]` |
| `I(X;Y\|Z)` (compProd 形) | `noncomputable def condMutualInfo (μ : Measure Ω) [IsFiniteMeasure μ] [StandardBorelSpace X] [Nonempty X] [StandardBorelSpace Y] [Nonempty Y] (Xs : Ω → X) (Yo : Ω → Y) (Zc : Ω → Z) : ℝ≥0∞` | `Common2026/Shannon/CondMutualInfo.lean:46` | `μ → Xs → Yo → Zc` | `ℝ≥0∞` | `[MeasurableSpace Ω/X/Y/Z]`, `[IsFiniteMeasure μ]`, `[StandardBorelSpace X] [Nonempty X]`, `[StandardBorelSpace Y] [Nonempty Y]` |
| `I(X;Y\|Z) ≥ 0` | `condMutualInfo_nonneg ... : 0 ≤ condMutualInfo μ Xs Yo Zc` | `Common2026/Shannon/CondMutualInfo.lean:55` | `μ → Xs → Yo → Zc` | `Prop` | 同上 |
...
| `I(X;Y\|Z) = I(Y;X\|Z)` | `condMutualInfo_comm ...` | `Common2026/Shannon/CondMutualInfo.lean:295` | `μ → Xs → Yo → Zc` | `Prop` | 同上 |
| `condMutualInfo_ne_top` (有限) | `condMutualInfo_ne_top [Fintype X] [MeasurableSingletonClass X] [Fintype Y] [MeasurableSingletonClass Y] [Fintype Z] [MeasurableSingletonClass Z] ...` | `Common2026/Shannon/CondMutualInfo.lean:331` | `μ → Xs → Yo → Zc` | `Prop` | 上記 + Fintype/MSClass triples + `[StandardBorelSpace X/Y] [Nonempty X/Y]` |
| 3-項橋 `I=H+H-H` | `mutualInfo_eq_entropy_add_entropy_sub_jointEntropy (joint : Measure (α × β)) [IsProbabilityMeasure joint] : (mutualInfo joint Prod.fst Prod.snd).toReal = entropy joint Prod.fst + entropy joint Prod.snd - entropy joint id` | `Common2026/Shannon/MIChainRule.lean:449` | `joint → Prod.fst → Prod.snd` | `Prop` | `[Fintype α] [DecidableEq α] [Nonempty α] [MeasurableSpace α] [MeasurableSingletonClass α]` × `α, β`, `[IsProbabilityMeasure joint]` |
| n-var MI chain rule (Fin n) | `mutualInfo_chain_rule_fin` | `Common2026/Shannon/MIChainRule.lean:117` | `μ → Xs → Yo` | `Prop` | (詳細は file 内 — `[StandardBorelSpace]` 系を含む) |
...
| `IsMarkovChain X→Z→Y` (γ-form) | `def IsMarkovChain (μ : Measure Ω) [IsFiniteMeasure μ] [StandardBorelSpace X] [Nonempty X] [StandardBorelSpace Y] [Nonempty Y] (Xs : Ω → X) (Zc : Ω → Z) (Yo : Ω → Y) : Prop := μ.map (fun ω => (Zc ω, Xs ω, Yo ω)) = (μ.map Zc) ⊗ₘ ((condDistrib Xs Zc μ) ×ₖ (condDistrib Yo Zc μ))` | `Common2026/Shannon/CondMutualInfo.lean:71` | `μ → Xs → Zc → Yo` | 上記 |
| Markov ⇒ MI 不等式 | `mutualInfo_le_of_markov (μ : Measure Ω) [IsProbabilityMeasure μ] [StandardBorelSpace X] [Nonempty X] [StandardBorelSpace Y] [Nonempty Y] (Xs : Ω → X) (Zc : Ω → Z) (Yo : Ω → Y) (hXs ...) (hmarkov : IsMarkovChain μ Xs Zc Yo) : mutualInfo μ Xs Yo ≤ mutualInfo μ Zc Yo` | `Common2026/Shannon/CondMutualInfo.lean:378` | `μ → Xs → Zc → Yo` | 同上 |

**観察**: Mathlib の `IdentDistrib` は **典型的な typed RV 形** (`f : α → γ`, `g : β → γ`,
`μ : Measure α := by volume_tac`, `ν : Measure β := by volume_tac`)。引数順は **RV 先、測度後**
(default `volume_tac`)。これは Common2026 の **測度先、RV 後** とは逆順だが、`volume_tac`
default のため typed RV 用 notation の参照点としては不適格。Common2026 の引数順 `μ → Xs (→ Yo)` を
維持するのが筋。
...
| Pushforward 不変 | **不在 (Common2026 自作)** — `klDiv_map_measurableEquiv {α β : Type*} [MeasurableSpace α] [MeasurableSpace β] (e : α ≃ᵐ β) (μ ν : Measure α) [IsFiniteMeasure μ] [IsFiniteMeasure ν] : klDiv (μ.map e) (ν.map e) = klDiv μ ν` | `Common2026/Shannon/MutualInfo.lean:52` | `e → μ → ν` | `[MeasurableSpace α] [MeasurableSpace β]`, `[IsFiniteMeasure μ] [IsFiniteMeasure ν]` |
| 一般 pushforward DPI | **不在 (Common2026 自作)** — `klDiv_map_le {α β : Type*} [MeasurableSpace α] [MeasurableSpace β] {f : α → β} (hf : Measurable f) (μ ν : Measure α) [IsFiniteMeasure μ] [IsFiniteMeasure ν] : klDiv (μ.map f) (ν.map f) ≤ klDiv μ ν` | `Common2026/Shannon/DPI.lean:52` | `f → μ → ν` | 同上 |
...
| 概念 | API (Common2026) | file:line | 引数順 | 戻り値 | 型クラス要件 (verbatim) |
|---|---|---|---|---|---|
| `h(μ)` (微分エントロピー、測度直) | `noncomputable def differentialEntropy (μ : Measure ℝ) : ℝ` | `Common2026/Shannon/DifferentialEntropy.lean:42` | `μ` | `ℝ` | — (`Measure ℝ` 固定) |
...
| 概念 | API (Common2026) | file:line | 備考 |
|---|---|---|---|
| Block joint RV | `def jointRV (Xs : ℕ → Ω → α) (n : ℕ) : Ω → (Fin n → α)` | `Common2026/Shannon/AEP.lean:55` | typed RV を `Fin n` 並べる helper |
| Per-symbol log-likelihood | `noncomputable def logLikelihood (μ : Measure Ω) (Xs : ℕ → Ω → α) (i : ℕ) : Ω → ℝ` | `Common2026/Shannon/AEP.lean:85` | typed RV 形 (Ω → ℝ) |
| Entropy of i.i.d. block | `theorem entropy_jointRV_eq_n_smul (μ : Measure Ω) [IsProbabilityMeasure μ] (Xs : ℕ → Ω → α) (hXs : ∀ i, Measurable (Xs i)) (hindep_full : iIndepFun (fun i => Xs i) μ) (hident : ∀ i, IdentDistrib (Xs i) (Xs 0) μ μ) (n : ℕ) : entropy μ (jointRV Xs n) = (n : ℝ) * entropy μ (Xs 0)` | `Common2026/Shannon/AEP.lean:527` | typed RV 形そのまま |
| Block entropy (stationary) | `noncomputable def blockEntropy (μ : Measure Ω) (p : StationaryProcess μ α) (n : ℕ) : ℝ := entropy μ (p.blockRV n)` | `Common2026/Shannon/EntropyRate.lean:58` | `entropy` を typed RV 形で直接呼ぶ |
| Conditional entropy tail | `noncomputable def conditionalEntropyTail (μ : Measure Ω) [IsFiniteMeasure μ] (p : StationaryProcess μ α) (n : ℕ) : ℝ := InformationTheory.MeasureFano.condEntropy μ (p.obs n) (p.blockRV n)` | `Common2026/Shannon/EntropyRate.lean:63` | `condEntropy` を typed RV 形で直接呼ぶ |
| i.i.d. ambient (`ℕ → α × β`) | `noncomputable def iidAmbientMeasure (p : Measure α) (W : Channel α β) : Measure (ℕ → α × β)` | `Common2026/Shannon/IIDProductInput.lean:48` | typed RV 用 ambient measure |
| `iidXs / iidYs` | `def iidXs : ℕ → (ℕ → α × β) → α := fun i ω => (ω i).1` | `Common2026/Shannon/IIDProductInput.lean:60` | 座標射影 typed RV |
...
### B-1. Common2026 内 notation

**結論: Common2026 内には information theory 関連の `notation` / `scoped notation` 宣言が
ゼロ件**。`rg -n "^(notation|scoped notation|prefix|infix)" Common2026/` の結果は空。
...
- Common2026 の measure 先・RV 後と引数順が**逆**だが、`IdentDistrib` 自体は標準的に
...
- **`Y` (取り出す側) と `X` (条件側) の引数順**は Mathlib 流に **`Y → X → μ`**。Common2026 の
  `condEntropy μ Xs Yo`、`mutualInfo μ Xs Yo` とは **意味的に逆**: Common2026 の `condEntropy μ Xs Yo`
  は `H(Xs | Yo)` だが、Mathlib `condDistrib Xs Yo μ` が「`Yo` 条件下の `Xs` の分布」。
- 既存 Common2026 コードは **正しくこの順序で使っている** (例: `Common2026/Fano/Measure.lean:70`
...
あくまで `Measure → Measure → ℝ≥0∞` であり、pushforward の bridge は Common2026 自作の
...
- **`condDistrib` の引数順は `(Y, X, μ)` で `H(Y | X)` を意味する** (Mathlib)。Common2026 の
  `condEntropy μ Xs Yo` = `H(Xs | Yo)` で `condDistrib Xs Yo μ` を内部で呼ぶ。
  typed RV notation `H(X | Y)` をどちらの順序で展開するか、設計判断が要る。Common2026 の
...
I-1 で「typed RV 形」として publish したい外向き API のうち、**現状の Common2026 が typed RV
...
- **notation `H(X)` / `I(X;Y)` / `D(X‖Y)`** → ❌ **完全不在** (Common2026・Mathlib ともに)
...
Common2026 の `klDiv_map_measurableEquiv` で全部足りる)。
...
`Common2026/InformationTheory/TypedRV.lean` (仮称) の出だし:
...
- 既存 Common2026 の measure-theoretic API は **すでに 100% typed RV 形** で書かれている
...
- **bridge lemma の新規追加はゼロ件**。Mathlib `Measure.map_apply`, `isProbabilityMeasure_map`,
  `integral_map`, `compProd_map_condDistrib` + Common2026 `klDiv_map_measurableEquiv` で全部足りる
...
- `Common2026/Shannon/Bridge.lean` — `entropy` 定義 + `mutualInfo ↔ entropy − condEntropy` 橋
- `Common2026/Shannon/MutualInfo.lean` — `mutualInfo` 定義 + KL pushforward 自作補題
- `Common2026/Shannon/CondMutualInfo.lean` — `condMutualInfo` / `IsMarkovChain` / chain rule
- `Common2026/Shannon/Entropy.lean` — chain rule / tower / conditioning monotonicity
- `Common2026/Shannon/Han.lean` — `jointEntropy` (n-var) + chain rule
- `Common2026/Shannon/DifferentialEntropy.lean` — measure-form 微分エントロピー
- `Common2026/Fano/Measure.lean` — `MeasureFano.condEntropy` / `errorProb` (Phase 3 達成)
- `Common2026/Shannon/DPI.lean` — `klDiv_map_le` (自作 pushforward DPI 核)
- `Common2026/Shannon/AEP.lean` — typed RV と `IdentDistrib` / `iIndepFun` の合流点
