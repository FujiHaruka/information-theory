...
| `epi-infinite-variance-classical` | 無限分散 a.c. 古典 EPI `N(X+Y) ≥ N(X) + N(Y)` (X, Y 両 a.c. + 両有限微分エントロピーだが **2次モーメント無限**)。集約先 `entropyPowerExt_add_ge_infinite_variance` (`EPICase1SmoothingLimit.lean`)。**2026-06-07 Pivot B closure で旧 `epi-finite-entropy-ac-classical` から分離**: 旧 bundled wall を (正則密度=Phase A 既閉) + (一般有限分散 a.c.=smoothing-limit で genuine 閉、Pivot B、残 `plan:epi-finitevar-smoothing-limit-plan` の `hent_sum` 1本のみ) + (無限分散=本 wall) に 3 分解。本 wall = genuine Mathlib 壁: 有限分散の smoothing→正則化 + endpoint 連続性ルートは 2次モーメント無限で発散 (Var(X+Y)=∞ ゆえ heat-flow 端点制御が効かない)、無限分散 EPI は Lieb-Young sharp Young inequality / Brascamp-Lieb / Rényi-Lp 経由が必須で Mathlib 完全不在 (loogle: `entropyPower`/`fisherInformation`/`Brascamp`/`sharpYoung`/`Renyi`/`differentialEntropy` 全 Found 0、`Lieb` は Lie 代数のみ、独立 honesty audit 2026-06-07 機械裏取り)。`epi-finite-entropy-ac-classical` (旧 bundled、本 closure で消滅) と区別: 本 wall は無限分散部のみ。`stam` (Fisher 側 residual) と semantic 区別: 本 wall は entropyPowerExt-level 古典 EPI 結論 | Ch.17 EPI (無条件化 case-1 無限分散部, `entropyPowerExt_add_ge_infinite_variance`) |
