...
| `minkowski-det-posdef` | Cover-Thomas 17.9.1: `det(A+B)^(1/n) ≥ det(A)^(1/n) + det(B)^(1/n)` for PosDef A B (Mathlib 0件、AM-GM + 同時対角化で組めるが congruence step 未 land)。隣接 `epi-n-dim` (entropic 形) / `bm-convex-body-sqrt` (geometric 形) と semantic 区別: 本 wall は algebraic 形 | Ch.17 EPI / 17.9 BM |
