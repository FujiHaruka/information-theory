...
| `lz78-aseventual-ziv` | LZ78 a.s.-eventual Ziv inequality `limsup (lz/n) ≤ entropyRate₂` (bit、achievability 上界、Cover-Thomas Lemma 13.5.5)。discharge = M3 (variable-depth tree-node AEP) research-level scope-out、codebase + Mathlib 不在。active = `lz78GreedyImpl_achievability_ae` (`GreedyParsingImpl.lean`、RHS = `entropyRate₂ μ p`)。現在 **honest tier-2 genuine M3 壁** (`entropyRate₂` target で TRUE-as-framed、a.s.-eventual Ziv 内容は未証明)。Ziv 組合せ核 (`c·log c ≤ K·n`、`lz78PhraseStrings_mul_log_le`) は sorryAx-free 確立済、残るは `entropyRate₂` との接続 (M3)。**経緯**: 符号長 def-fix (genuine longest-prefix parse `lz78PhraseStrings`、`c·bitLength c |α|`、rate `O(1)`、commit `5d08566`) で当初のダミー1シンボル parse degenerate defect を解消。続く独立監査で **bit-vs-nat units mismatch** (second defect) を発覚: `lz78GreedyImplEncodingLength/n` は bit-rate だが旧 RHS は nat-単位の `entropyRate`、正entropy源 A≥2 で `limsup = log₂A > logA` = 偽 (prior audit `9b09790` はこれを見落とし overturn)。`99acb58` で `@audit:defect(false-statement)` 確定 → `55e1cd9` で RHS を `entropyRate₂` (= `entropyRate/Real.log 2`、bit) 化して TRUE-as-framed に修正、再監査 PASS。cause:false-statement (過小評価系、units defect、entropyRate₂ 化で解消) | Ch.13 LZ78 (achievability) |
