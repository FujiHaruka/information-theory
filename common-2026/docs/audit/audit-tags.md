...
`@residual(wall:<name>)` の `<name>` は以下から選ぶ。新規追加時は本 register に PR で追記 (divergence 防止: 「stam」と「stam-inequality」が併存しないように)。
...
新規 wall を追加する時は: (1) loogle で 0件確認 (本当に Mathlib 不在か)、(2) 既存 register に類似がないか確認、(3) 本表に追記する PR を出す。
...
| Kind | 由来 defect |
|---|---|
| `circular` | 仮説型 ≡ 結論型 で body が `:= h` (旧 `@audit:defect(circular)`) |
| `prop-true` | `:True` slot に実 residual を隠す (旧 `@audit:defect(prop-true)`) |
| `launder` | name laundering (`*_discharged` / `*_full` 等で完成偽装、旧 `@audit:defect(launder)`) |
| `degenerate` | 退化定義悪用 (旧 `@audit:defect(degenerate)`) |
| `false-statement` | mathlib_wall_misuse / 実は偽の statement (旧 `@audit:defect(false-statement)`) |
| `false-hypothesis` | 仮説自体が偽だった (例: Huffman MergedHuffmanAuxIdentHypothesis FALSE)。signature 修正 + sorry 化必要 |
...
| `@audit:retract-candidate(REASON)` | 削除候補。circular passthrough で honest 経路が他にあるケース等 | REASON 短文 (kebab-case) | YES | `@audit:retract-candidate(circular-passthrough)` |
