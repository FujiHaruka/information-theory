...
| `general-alpha-rate-≠-E₂` | 固定 `alpha` の rate 列 (`-(1/n) log steinTypeII_at_level_pmf`) が Hoeffding tradeoff curve `E₂(alpha)` を一般に target しない (Stein's lemma の反例: `alpha = 0` で `rate ≡ 0 ≠ E₂(0) = D(P₁‖P₂) > 0`、`0 < alpha < 1` で `rate → D(P₁‖P₂) > E₂(α)`)。genuine 後継は exponential-level `hoeffding_tradeoff_exp` (`HoeffdingTradeoffExp.lean`) | **2026-05-28**: 旧 5 declarations (HoeffdingTradeoff / HoeffdingSandwich / HoeffdingSandwichDischarge / HoeffdingSandwichBody) は un-instantiable + `hoeffding_tradeoff_exp` で既に置換済のため **削除** (retraction)。現 carrier は Draft の interior wrapper 3 件 (`HoeffdingInteriorBody.lean`: `hoeffding_tradeoff_sandwich_at_interior_via_predicate` / `_via_gradient`、`HoeffdingInteriorGradientBody.lean`: `hoeffding_tradeoff_sandwich_at_lagrange`、Draft sweep 待ち) — 同型 false fixed-`alpha` 結論 |
