...
| `lz78-converse-aseventual` | LZ78 a.s.-eventual converse 下界 `entropyRate₂ ≤ liminf (lz/n)` (bit、Cover-Thomas Thm 13.5.3 lower bound、harder SMB-lower + Kraft 方向)。**[RECLASSIFIED 2026-06-21 → `plan:lz78-m4-plan`、もはや wall ではない]**: 旧「research-level scope-out wall」判定を gateway-atom-first 在庫が overturn (cause:single-route + gateway-atom-untried)。M4 converse は既存 sorry-free ergodic machinery (G4 SMB-liminf / G3 Z-side Barron テンプレ + Markov/BC) の配線 + 単一の genuine 組合せ brick `lz78_block_kraft_poly` (G2、polynomial n-block Kraft `∑ 2^{−Lₙ} ≤ (n+1)²`、~150–300 行) に分解、closeable plan あり (`docs/shannon/lz78-m4-plan.md`)。**唯一の active sorry は G2 brick** (`AsymptoticOptimality.lean:502`、`@residual(plan:lz78-m4-plan)`)。consumer chain `lz78_converse_bad_set_measure_le` / `blockLogAvg₂_minus_error_le_rate_ae` / `lz78GreedyImpl_converse_ae` (LHS = `entropyRate₂ μ p`) は G2 を forward-reference 消費する sorry-free helper (bundling なし、signature は `(μ, p)` + regularity のみ、独立監査 2026-06-21 PASS)。G2 statement は TRUE-as-framed (polynomial 上界を α=Bool / Fin 8, n≤6 で数値裏取り、large slack、`n=0` 境界 `1 ≤ 1`)。closability は依然 `human-judgment` 見積 (G2 body は未証明 `sorry`)。**経緯**: genuine longest-prefix def-fix (`5d08566`、rate `O(1)`) で旧ダミー parse 時代の rate 発散を解消後、独立監査で同じ **bit-vs-nat units mismatch** を発覚 (nat `entropyRate` を bit-rate と sandwich する LHS が誤 unit、prior audit `9b09790` overturn、`99acb58` で `@audit:defect(false-statement)` 確定) → `55e1cd9` で LHS を `entropyRate₂` (bit) 化、再監査 PASS。cause:false-statement (過小評価系、units defect、entropyRate₂ 化で解消) | Ch.13 LZ78 (converse) |
