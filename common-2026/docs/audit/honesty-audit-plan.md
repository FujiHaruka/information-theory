...
# ★ calibration gate（必須）: docs/audit/calibration-set.md のラベル済み8件で
#   Sonnet ワーカー(doctrine入り)を試走 → LB 6件が全て status!=ok なら合格。
#   1件でも ok（偽陰性）なら doctrine 修正して再走。合格まで本番に進まない。

# --- 第1波: Sonnet 全件 ---
# unaudited>0 の間、Agent(model:"sonnet") を N=4–8 個 並列起動。各ワーカー = 1 シフト。
...
#   単一 Opus: そのまま Read で処理 / 並列 Opus: claim --status suspect --agent opus-K で再リース
...
- m=0 → 健全。サンプルを変えて随時継続。
- m≥1 で **load-bearing の明白な見逃し** → **停止**。flip 事例から doctrine を補強 → calibration gate 再走（合格まで）→ 影響母集団（同じ flag/同種述語の `ok`）を `reaudit-stale` 相当で再監査。
