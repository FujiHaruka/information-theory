...
2. `worker-prompts.md`「TASK B」（CORE doctrine ＋ calibration 指示、`Agent(model:"sonnet", subagent_type:"general-purpose")`）に上表8件の id を差し込んで起動。`show → 本体 Read → 層C → verdict` を全件 `--db /tmp/cal.db` で実行させる。
