...
| ~~**PG legacy retract**~~ | ~~`rg "@audit:defer\(pg-legacy-retract\)" Common2026/`~~ | **完了 (2026-05-24)** — 4 legacy passthrough theorem を削除し、3 caller (`ParallelGaussianKKT` / `ParallelGaussianWFCertBody` / `ParallelGaussianWFStationarityBody`) を `isParallelGaussianPerCoordReduction_of_bundle` 直 `exact` (def-unfolding 経由) に inline-rewire。honest headline は `ParallelGaussianPerCoord.parallel_gaussian_capacity_formula` (sup-sandwich) のまま。defect declaration 8 → 4、案件 3 → 2。|
