...
波0/1/2 + 後続 retract/rebrand を経た **post-cleanup snapshot (2026-05-24)**:

| status | count |
|---|---:|
| ok | 2483 |
| suspect (honest 🟢ʰ remaining tasks) | 381 |
| **defect** | **3** |
| unaudited | 581 |
| total | 3448 (`theorem + lemma + def`、parser-scope 拡張後) |

defect 101 → 3 まで圧縮。残 3 件はいずれも **intentional staged Mathlib 壁案件**で、本 cleanup plan の scope 外 (200-500 LoC 規模の analytic discharge 案件で別 plan に切り出し):

| # | id | 種別 | 想定 discharge route |
|---|---|---|---|
| 1 | `Common2026/Shannon/AWGNAchievability.lean::awgn_achievability` | theorem (load-bearing hypothesis wrapper) | [`awgn-achievability-typicality-plan.md`](../shannon/awgn-achievability-typicality-plan.md) (Tier-3, Cover-Thomas 9.2) |
| 2 | `Common2026/Shannon/AWGNAchievability.lean::IsAwgnTypicalityHypothesis` | def (type ≡ conclusion predicate) | 同上 |
| 3 | `Common2026/Shannon/BrunnMinkowski.lean::IsBrunnMinkowskiEntropyHypothesis` | def (type ≡ conclusion predicate) | [`brunn-minkowski-from-epi-discharge-plan.md`](../shannon/brunn-minkowski-from-epi-discharge-plan.md) (Tier-3, EPI route) |

(1) は (2) を消費する passthrough wrapper なので、(2) の analytic discharge と同時に閉じる。
