...
> - `InformationTheory/Shannon/MultipleAccessChannel.lean` — T3-B MAC publish (637 行, L-MAC1〜5 全 pass-through) — **本 plan の直接の verbatim 雛形**
> - `InformationTheory/Shannon/ChannelCoding.lean` — `Channel`, `Code`, `errorProbAt`, `averageErrorProb`
> - `InformationTheory/Shannon/CondMutualInfo.lean` — `condMutualInfo`, `IsMarkovChain` (γ-form), `mutualInfo_le_of_markov`
...
> - `InformationTheory/Shannon/RelayCutset.lean` — converse hypothesis pass-through 雛形 (signature 流用)
> - `InformationTheory/Shannon/WynerZivAchievability.lean` — achievability existence pass-through 雛形 + auxiliary RV `U` 流用
>
> **Goal (短形)**: 新規 1 ファイル `InformationTheory/Shannon/BroadcastChannel.lean` で
...
- [ ] Phase 0 — Mathlib + 既存 InformationTheory 在庫 + 設計確定 📋 → [`broadcast-channel-mathlib-inventory.md`](./broadcast-channel-mathlib-inventory.md)
...
InformationTheory.lean              ← `import InformationTheory.Shannon.BroadcastChannel` 追記
```
...
- [x] `InformationTheory/Shannon/MultipleAccessChannel.lean` — **本 plan の verbatim 雛形**
- [x] `InformationTheory/Shannon/CondMutualInfo.lean:46, :71, :219, :378, :652` — `condMutualInfo`, `IsMarkovChain`, chain rule, DPI
- [x] `InformationTheory/Shannon/MIChainRule.lean:117` — `mutualInfo_chain_rule_fin`
- [x] `InformationTheory/Shannon/MutualInfo.lean:36, :93, :192` — `mutualInfo`, `_comm`, `_ne_top`
...
- 撤退ライン 5 本 (L-BC1 / L-BC2 / L-BC3 / L-BC4 / L-BC5) を inventory + 本 plan に append-only
  で記録
- Phase A skeleton (`InformationTheory/Shannon/BroadcastChannel.lean` の sorry-driven 出だし 80 行) が
...
- `lake env lean InformationTheory/Shannon/BroadcastChannel.lean` clean (Phase B/C/D は別 section で
...
- [ ] **A-5 `lake env lean InformationTheory/Shannon/BroadcastChannel.lean`** clean 確認
...
- `lake env lean InformationTheory/Shannon/BroadcastChannel.lean` clean
...
- [ ] **V-1**: `InformationTheory.lean` の既存 `import InformationTheory.Shannon.MultipleAccessChannel` の
  後に
  ```lean
  import InformationTheory.Shannon.BroadcastChannel
  ```
  を追記 (**オーケストレータが後で実施 — 本 plan では行わない**)
- [ ] **V-2**: `lake env lean InformationTheory/Shannon/BroadcastChannel.lean` silent
...
| `InformationTheory.lean` の import 順序で circular dependency | 低 | 中 | `BroadcastChannel.lean` は `InformationTheory.Shannon.{CondMutualInfo, MIChainRule, ChannelCoding}` のみ import (MAC と同一)、`MultipleAccessChannel` に依存しない (pattern のみ流用) |
...
- 既存実装 (黒箱 reuse):
  - `InformationTheory/Shannon/MultipleAccessChannel.lean` (637 行) — pattern verbatim 雛形
  - `InformationTheory/Shannon/RelayCutset.lean` (386 行) — converse pass-through pattern
...
7. **(2026-05-20) 単一ファイル戦略確定** — T3-B MAC publish (637 行, 単一 file) と同様、両側
   ...
   10-15 秒の inner loop に収まる。分離不要。`InformationTheory/Shannon/BroadcastChannel.lean` 単一
   file で publish。
