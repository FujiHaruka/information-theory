...
- **Gap 2 = `epi-phaseA-sum-reparam`** (新 slug、Gap 1 後): sum-pair `(X+Y, Z_X+Z_Y)`、雑音 `~gaussianReal 0 2`。producer `isDeBruijnRegularityHyp_of_methodX_unitnoise` (`:1950`) の `hZX_law` を `gaussianReal 0 v` (v≠0) に一般化 (body は `gaussianPDFReal 0 ⟨t·v,_⟩`、`isRegularDensityV2_convDensityAdd_gaussian` が任意 s>0 受けるので bundle 再利用可)。既存 X'/Y' caller は `v:=1` 明示。Mathlib `gaussianReal_conv_gaussianReal` (variance 加法則、`EPIBlachmanGaussianWitness.lean:220` 使用実績) で reparam。**producer signature 変更 + caller 2 箇所、~30-50 行、新 lemma 0**。producer の unit hardcode は構造的制約でなく PB-1 の惰性 (producer docstring `:1860` 自認)。
- 注: residual 5 (sum Fisher 正値) は Gap 1 (Fisher lemma) と Gap 2 (h_reg_sum 供給) 両依存。residual 6 (conv-pin) は Gap 2 連動。
