...
| **G2** | `heatFlowEntropyPower_continuousOn` (共有壁補題) | `EPIG2HeatFlowContinuity.lean:78` | **真 Mathlib 壁 確定 (2026-06-03 GATE NO-GO、独立 honesty audit OK)** | `entropyPower ∘ P.map` の heat-flow path 上 `√t→0` 連続性。DCT 機構 (`continuousWithinAt_of_dominated`) は Mathlib に在るが、**t=0⁺ 一様 integrable pointwise majorant が `IsDeBruijnRegularityHyp` field から組めない** (既存 envelope は全て `s∈Ioo(t/2,2t)` fixed-t で t→0⁺ 発散)。`@residual(wall:heatflow-continuity)` で honest 据置。2 consumer (`csiszarLogRatioGap_continuousOn`/`csiszarGap1Source_continuousOn`) は壁補題呼出 + 外側合成で **genuine 結線済** (自身 sorry なし)。詳細 → [`epi-g2-continuity-plan.md`](epi-g2-continuity-plan.md) |
...
| **G4** | `IndepFun (X+Y) (Z_X+Z_Y)` joint independence | `EPIStamToBridge.lean:1360` | **richness ~~gap~~ → lift で closable (2026-06-04)** | in-place は pairwise のみ。**lift 後継 `indepFun_add_add_on_lift` genuine 着地** (`EPINoiseExtension.lean`、`indepFun_prod` 直接 1 行、`@audit:ok`)。in-place re-wire (B2) は G2 待ちで保留。詳細 → [`epi-richness-route-b-plan.md`](epi-richness-route-b-plan.md) |
...
| W2 | ~~`stamScalingNoise_exists` (shared sorry)~~ | `EPIStamToBridge.lean:398` | ⚠ **false-statement → lift で closable (2026-06-04)** | **in-place existential は atomic P (dirac) で偽** (独立 audit 確認、`@audit:defect(false-statement) @audit:closed-by-successor(epi-richness-route-b-plan)`)。「Mathlib noise-extension 不在の壁」判定は誤り — **lift 後継 `stamScalingNoise_exists_on_lift` genuine 着地** (`@audit:ok`)。richness は真の Mathlib 壁ではなかった |
