...
#### 2026-06-06 着手前 scoping 確定 (Explore 在庫 + pivot-advisor 裏取り、commit `9d7d067` 時点)

two-time signature `entropyPower_add_ge_case1_of_regular_twotime` (`EPICase1TwoTime.lean:1620`) の ~30 引数を在庫照合した結果、roadmap が未列挙だった 2 点を確定:

- **(A) sum 側 input density 正則性 = honest 前提追加 (advisor PASS、load-bearing でない)**。`h_reg_sum`/`h_endpt_sum`/`h_scale_sum`/`h_rescale_S` + h_stam_supply の J_S 正値は、X/Y 側と同じ producer (`isDeBruijnRegularityHyp_of_methodX_unitnoise` `:1950` / `isRescaledPathRegular_of_methodX` `:562` / `endpt_of` helper / `fisherInfoOfDensityReal_convDensityAdd_pos`) で供給できる。但しこれら producer は **input density `p_{X+Y}=(P.map(X+Y)).rnDeriv.toReal = convDensityAdd pX pY` (general∗general)** の `IsRegularDensityV2`/`IsBlachmanConvReady(∀v)`/Fisher 有限/normalized/entropy 有限 を要求する。在庫に **general∗general 畳込み正則性 producer はゼロ** (全て general∗Gaussian `convDensityAdd p g_t`)。→ pX/pY 既存前提と同型の **sum 正則性前提 5 本 (`h_fisher_XY`/`hreg_pXY`/`hnorm_pXY`/`hready_pXY`/`hent_pXY`) を `entropy_power_inequality_of_density` signature に追加**。pivot-advisor 2026-06-06: core-reconstruction test PASS、`IsRegularDensityV2`/`IsBlachmanConvReady` は純 regularity (Fisher 不等式核を encode せず)、既存 pX/pY 前提 (`@audit:ok` EPIDensityForm.lean:51-60) と同 category。Phase A の「仮定多」scope 内 (Phase B で discharge)。これにより sum 側は X/Y 側 producer をそのまま再利用、新規 general∗general producer 不要。
- **(B) h_stam_supply の唯一の新 seam = `p_{X+Y} = convDensityAdd pX pY` (time-0 sum input density = 入力畳込み)**。two-time `h_stam_supply` (`:1667-1679`) は single-t `h_pos_stam` から law-match conjunct (`density_sum = convDensityAdd density_X density_Y`) を **strip 済**なので、producer 内部で自前 discharge 要。資産確定: `n_eq_map_conv_map` (`P.map(X+Y) = P.map X ∗ P.map Y`、measure-level、`EPIUncondMixedCase.lean`/`EPICase1TwoTime.lean` で使用) + `convDensityAdd_eq_n` (`EPIConvDensityAssoc.lean`、`convDensityAdd = ⋆[mul,volume]`) + withDensity 操作。regularity-level、rfl でない real work だが在庫あり・壁なし。step3 (`isStamInequalityHyp_via_step3` は measurability+indep のみ要求) + DONE bridges (`convDensityAdd_convGaussian_interchange_asym` `:271` / `convDensityAdd_gaussian_variance_add` `:195`) で接続。
- **着手順序 (advisor 推奨)**: (1) `h_stam_supply` producer を standalone lemma で skeleton 化 (seam を 1 sorry + `@residual(plan:epi-debruijn-pertime-closure)`)、type-check 通過確認後 fill。(2) 3-noise lift。(3) EPIDensityForm 再構成 (sum 前提 5 本追加 + twotime 呼出)。h_stam_supply は honesty-load-bearing → 実装後 **独立 honesty-auditor 必須** (CLAUDE.md brief checklist 項目4: 直接埋込 mechanism、a.e.-pin 不可)。
