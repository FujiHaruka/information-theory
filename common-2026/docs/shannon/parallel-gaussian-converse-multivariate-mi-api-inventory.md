...
> (`Common2026/Shannon/ParallelGaussianConverse.lean:1093`, `@residual(wall:multivariate-mi)`).
...
を全て自動 `infer_instance`（project context `Common2026.Shannon.ParallelGaussianConverse` import 下）。
...
| `Fin n → ℝ` product withDensity | `Common2026 pi_withDensity_fin` | `Common2026/Draft/Shannon/MultivariateDiffEntropy.lean:260` | ✅ 自作既存 | `∏ gaussianPDF = rnDeriv (Measure.pi 𝒩)` |
...
| product withDensity → 積密度 | `Common2026 pi_withDensity_fin` | `MultivariateDiffEntropy.lean:260` | ✅ 自作既存 | noise rnDeriv = ∏ pdf |
...
| 概念 | Mathlib / Common2026 API | file:line | 状態 | #5 での扱い |
|---|---|---|---|---|
...
| **`gaussianPDFReal_le_sup`（上界 `≤(√2πv)⁻¹`）** | `Common2026 (AWGN)` | `Common2026/Draft/Shannon/AwgnCapacityConverseMaxent.lean:65` | ✅ 自作既存（1-D） | log 上界、座標ごとに適用 |
...
| 概念 | Common2026 API | file:line | 状態 | #5 での扱い |
|---|---|---|---|---|
| 座標ごと second-moment integrability + Bochner 和 | `parallelGaussianPowerConstraintSet_mem_iff_integrable` | `Common2026/Shannon/ParallelGaussian.lean:191` | ✅ 自作既存・sorryAx-free | 各座標 `∫(xᵢ)² ≤ P` → 座標箱 Chebyshev の入力 |
...
**Mathlib gap 判定:** Common2026 内で完備。**自作必要 0**（建材として既存）。
...
- joint conv 出力等式 / quadratic 包絡 / 座標箱 Chebyshev: いずれも **Mathlib 不在だが
  Common2026 自作タスク**（1-D 手本 + 既存 helper の座標積）。`wall` ではなく `plan`/`self-buildable`。
