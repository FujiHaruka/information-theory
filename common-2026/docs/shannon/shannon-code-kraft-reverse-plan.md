...
> Status (2026-05-12): **完了 ✅**。`InformationTheory/Shannon/ShannonCodeKraftReverse.lean` (498 行、0 sorry / 0 error / 0 warning)。B-8 期待長 sandwich (`docs/shannon/shannon-code-moonshot-plan.md`) と並立 publish。

> 実態整合 (2026-05-20): DONE-UNCOND — `exists_prefix_code_of_kraft` (`InformationTheory/Shannon/ShannonCodeKraftReverse.lean:482`、`{D}(hD : 2 ≤ D)(l)(hl : ∀ a, 0 < l a)(hk : Σ D^(-l a) ≤ 1) : ∃ c, Injective c ∧ (∀ a, (c a).length = l a) ∧ IsPrefixFree c`) を 0 sorry で証明、`shannonFanoCode_injective` / `shannonFanoCode_prefixFree` 経由。pass-through / `Prop := True` 不在。
...
- **ファイル**: `InformationTheory/Shannon/ShannonCodeKraftReverse.lean` (498 行)
...
- **主定理**:
  ...
  ```
- **検証**: `lake env lean InformationTheory/Shannon/ShannonCodeKraftReverse.lean` clean — **0 sorry / 0 error / 0 warning**.
- **既存 B-8** (`InformationTheory/Shannon/ShannonCode.lean` 354 行) は **完全に並立** (touch せず)。
...
- **既存 `kraftSum`** (`InformationTheory/Shannon/ShannonCode.lean`) を入力形に採用:
...
`InformationTheory.lean` 末尾に `import InformationTheory.Shannon.ShannonCodeKraftReverse` 追加。
...
- 検証: `lake env lean InformationTheory/Shannon/ShannonCodeKraftReverse.lean` clean、0 sorry。
