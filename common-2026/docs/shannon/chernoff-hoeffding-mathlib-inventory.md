# Chernoff / Hoeffding (T1-B + T1-D) Mathlib + InformationTheory inventory
...
(Mathlib `Measure.tilted` family + InformationTheory Stein/Sanov plumbing + Mathlib `inner_le_Lp_mul_Lq_of_nonneg`/`geom_mean_le_arith_mean_weighted` 完備)。
...
| 数値 | 値 |
|---|---|
| 既存 API カバレッジ (実体ベース) | **約 90%** (Mathlib + InformationTheory) |
...
Lean 風 signature (InformationTheory 既存 convention に合わせる):
...
| klFun continuity | `lemma continuous_klFun : Continuous klFun` — `KLFun.lean:76` | ✅ 既存 | `klDivPmf P Q` の `P` 連続性 (既に `InformationTheory/Shannon/CsiszarProjection.lean:71` で利用) |
...
| KL ⊗ left invariance | `lemma klDiv_compProd_left (μ ν : Measure α) (κ : Kernel α β) : klDiv (μ ⊗ₘ κ) (ν ⊗ₘ κ) = klDiv μ ν` — `ChainRule.lean:182` (`@[simp]`) | ✅ 既存 | InformationTheory `MutualInfo.lean:88` `klDiv_prod_const_left` で既に再利用済 |
...
### D. InformationTheory Stein / Sanov / LLR plumbing (proof-log 70-80% 主張の実体)

#### D-1. Stein 系 (`InformationTheory/Shannon/Stein.lean`)
...
#### D-2. Sanov 系 (`InformationTheory/Shannon/Sanov.lean`, `SanovLDP.lean`, `SanovLDPEquality.lean`)
...
#### D-3. Csiszar projection (`InformationTheory/Shannon/CsiszarProjection.lean`)
...
### H. n-IID infrastructure (`InformationTheory/Shannon/Stein.lean` の Pi 仕組み + Mathlib `Measure.pi`)
...
- Sanov LDP equality (`InformationTheory/Shannon/SanovLDPEquality.lean:1243`) が Chernoff lower bound に **直接** 流用可能
- Csiszar projection (`InformationTheory/Shannon/CsiszarProjection.lean`) が Hoeffding `min` 達成性に **直接** 流用可能
...
`InformationTheory/Shannon/Chernoff.lean` (T1-B + T1-D 一括):
...
- InformationTheory Stein/Sanov/Csiszar plumbing: 9 項目 (D 群 hot path)

分子 (既存): 35 項目
- 全項目が既存 (Mathlib + InformationTheory)

→ **既存率 100%** (実体ベース)。InformationTheory Stein/Sanov plumbing の **再利用率は roadmap 主張の 70-80% と整合**:
...
- **InformationTheory 既存 plumbing 再利用率 70-80% (proof-log 主張)** は **実態は分野毎に変動** (Csiszar 90% / Sanov 60-70% / Stein 70-80%) するが、合算では妥当
