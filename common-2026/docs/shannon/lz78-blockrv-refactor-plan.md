...
> (`Common2026/Shannon/LZ78AchievabilityLimsup.lean:254`) が現在 honest load-bearing 入力として
...
- [ ] Phase V — 無仮定 headline publish + `Common2026.lean` 編入 + clean check 📋
...
| Phase V | **20 行** | 10–40 | 無仮定 headline + `Common2026.lean` import + clean check | no |
...
Common2026/Shannon/
  ...
  Common2026.lean               ← import 1–2 行追記
```
...
`rg "blockRV|StationaryProcess|ErgodicProcess"` で Common2026 全体を走査した結果:
...
**Common2026 内に `StationaryProcess`/`ErgodicProcess` の anonymous constructor / `.mk` /
...
- [ ] **新規 `StationaryKernel.lean` skeleton**: imports (pinpoint、`Mathlib.Probability.Kernel.*` +
      `Common2026.Shannon.Stationary` + `ShannonMcMillanBreiman` + `LZ78ZivEntropyBridge`)、namespace、
...
- [ ] **検証**: `lake env lean Common2026/Shannon/StationaryKernel.lean` が sorry warning のみ。
...
- [ ] **olean refresh**: 新 public symbol を下流が拾うなら `lake build Common2026.Shannon.StationaryKernel`
...
- [ ] **`Common2026.lean` 編入**: `import Common2026.Shannon.StationaryKernel` を `Stationary` import の後ろ、
      LZ78 系 import の前に追記。
- [ ] **検証**: `lake env lean Common2026/Shannon/StationaryKernel.lean` /
...
- **inner loop**: 各 fill 後 `lake env lean Common2026/Shannon/<file>.lean` が silent (CLAUDE.md
...
2. **(2026-05-21) blast radius 判定: additive で Ch.4 非破壊と確定** — `rg` 全数確認で
   ...
   Common2026 内にゼロ (唯一のヒット `TwoSidedExtension.lean:959` は別名束縛で構築ではない)。
