...
>
> **重要な事実 (verbatim 確認済)**: `huffmanCost` は **まだ存在しない** (`rg huffmanCost Common2026/ docs/` → 0 hit)。本調査は新規定義 `huffmanCost` + 漸化式補題のための在庫であり、既存コードの監査ではない。
...
注: `len` の漸化 (子の長さ = 親の長さ + 1) は `huffmanLengthAux_eq_step` (`Common2026/Shannon/Huffman.lean:427`) の `fun a => if a ∈ A ∨ a ∈ B then g a + 1 else g a` 構造に対応する。per-group 版 `len` を新規に定義する必要がある (§5)。
...
| card 減少 | `huffmanStep_card_lt` (Common2026) | `Common2026/Shannon/Huffman.lean:389` | ✅ 既存 (自作済) | `s''.card < s.card`、IH 適用条件 |
...
**接続の形 (verbatim 確認済の橋)**: `expectedLength P l = ∑ a : α, P.real {a} * (l a : ℝ)`（`Common2026/Shannon/ShannonCode.lean:56`）。`initMultiset P = (Finset.univ).val.map (fun a => ({a}, P.real {a}))`（`Common2026/Shannon/Huffman.lean:420`）。よって:
...
- **`Multiset.sum_map_erase`** (category B 主役):
  ...
  - `(h : a ∈ m)` — **membership 仮説が必須**。二重 erase の 1 段目は `x2 ∈ s.erase x1`（`huffmanStep_spec` の `.val.2.1 ∈ s.erase .val.1` が供給、`Common2026/Shannon/Huffman.lean:273`）、2 段目は `x1 ∈ s`（同 spec `.val.1 ∈ s`, line 272）。membership は既存 spec から取れる。
...
Mathlib lemma の不足はゼロ。自作対象は全て **Common2026 側の定義 / 補題**。
...
判定: **Mathlib lemma 自作要 = 0 件。Common2026 定義/補題 自作要 = 4 件（うち定義 2、補題 2）**。二重 erase の sum 分解は専用 lemma を Mathlib に求めず `sum_map_erase` 2 連で導出（bridge 自作不要）。
...
`Common2026/Shannon/HuffmanCost.lean`（新規）の出だし:

```lean
import Common2026.Shannon.Huffman          -- huffmanStep / huffmanLengthAux / initMultiset / HuffmanGrouping
import Common2026.Shannon.ShannonCode       -- expectedLength
...
- **自作必要 4 件**（Mathlib lemma ではなく Common2026 定義/補題: `len`/`huffmanCost`/`huffmanCost_step`/`expectedLength_eq_huffmanCost`）
