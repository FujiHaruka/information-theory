...
3. **cross-family entanglement が 2 件 (Wyner–Ziv side)** (verbatim 確認済、後述「Cross-family
   ...
   - `RelayCFBinningBody.lean:2` の `import Common2026.Shannon.WynerZivBinningCovering` → 3 predicate
   ...
   - `RelayInnerBodyDischarge.lean:2` の `import Common2026.Shannon.WynerZivBinningBody` →
...
- 各 file `lake env lean Common2026/Shannon/Relay<X>.lean` が 0 errors、
...
|---|---|---|---|
| `RelayCFBinningBody.lean:2` | `import Common2026.Shannon.WynerZivBinningCovering` | re-namespacing import | 未決事項 #2 (cross-family entanglement) |
...
| `RelayInnerBodyDischarge.lean:2` | `import Common2026.Shannon.WynerZivBinningBody` | re-namespacing import | 未決事項 #2 |
...
`rg '⚠|HONESTY ALERT|FALSE' Common2026/Shannon/Relay{InnerBodyDischarge,Cutset,InnerBound,CFBinningBody,DFBlockMarkovBody}.lean`
...
- [ ] **2.1.3** `relay_cutset_outer_bound` (line 536)
  ...
  - body → `by sorry`。
  - docstring refine。
- [ ] **2.1.4** Phase 2.1 完了時 `lake env lean Common2026/Shannon/RelayCutset.lean` 0 errors 確認。
  signature 改変したため olean refresh が必要 (Pilot Pattern A):
  ```bash
  lake build Common2026.Shannon.RelayCutset
  for f in Common2026/Shannon/RelayInnerBound.lean Common2026/Shannon/RelayInnerBodyDischarge.lean Common2026/Shannon/RelayCFBinningBody.lean; do
    lake env lean "$f"
  done
  ```
...
- `lake env lean Common2026/Shannon/RelayCutset.lean` 0 errors。
...
- [ ] **2.2.6** Phase 2.2 完了時 olean refresh + dependent verify:
  ```bash
  lake build Common2026.Shannon.RelayInnerBound
  for f in Common2026/Shannon/RelayInnerBodyDischarge.lean Common2026/Shannon/RelayCFBinningBody.lean Common2026/Shannon/RelayDFBlockMarkovBody.lean; do
    lake env lean "$f"
  done
  ```
...
- 散文 🟢ʰ 4 件 → 0 件、
- `lake env lean Common2026/Shannon/RelayInnerBound.lean` 0 errors。
...
- [ ] **2.3.7** Phase 2.3 完了時 olean refresh:
  ```bash
  lake build Common2026.Shannon.RelayInnerBodyDischarge
  for f in Common2026/Shannon/RelayCFBinningBody.lean Common2026/Shannon/RelayDFBlockMarkovBody.lean; do
    lake env lean "$f"
  done
  ```
...
- `RelayInnerBodyDischarge.lean` で `@audit:suspect` 0 件、`@residual(plan:relay-inner-bound-moonshot-plan)`
  15 件、sorry 15 件、
- `lake env lean Common2026/Shannon/RelayInnerBodyDischarge.lean` 0 errors。
...
- [ ] **2.4.3** Phase 2.4 完了時 `lake env lean Common2026/Shannon/RelayDFBlockMarkovBody.lean` 0 errors 確認。
...
- `lake env lean Common2026/Shannon/RelayDFBlockMarkovBody.lean` 0 errors。
...
- [ ] **2.5.7** Phase 2.5 完了時 olean refresh + verify:
  ```bash
  lake build Common2026.Shannon.RelayCFBinningBody
...
- [ ] **V.1** 全 5 file (+ family 内 transitive 影響受ける suspect-無し file) で `lake env lean` 確認:
  ```bash
  for f in Common2026/Shannon/RelayCutset.lean \
           Common2026/Shannon/RelayInnerBound.lean \
           Common2026/Shannon/RelayInnerBodyDischarge.lean \
           Common2026/Shannon/RelayDFBlockMarkovBody.lean \
           Common2026/Shannon/RelayCFBinningBody.lean; do
    echo "=== $f ==="
    lake env lean "$f"
  done
  ```
  signature 改変があった file (RelayCutset / RelayInnerBound / RelayInnerBodyDischarge / RelayCFBinningBody)
  は事前に `lake build Common2026.Shannon.Relay<X>` で olean refresh (Pilot Pattern A)。
  Phase 2.4 default なら RelayDFBlockMarkovBody は signature 改変なし → olean refresh 不要。

- [ ] **V.2** 集計コマンド実行:
  ```bash
  rg -c '@audit:suspect' Common2026/Shannon/Relay*.lean | awk -F: '{s+=$2} END {print "suspect:", s}'   # = 0
  rg -c '🟢ʰ' Common2026/Shannon/Relay*.lean | awk -F: '{s+=$2} END {print "🟢ʰ:", s}'                 # = 0
  rg -c '@residual\(plan:relay-inner-bound-moonshot-plan\)' Common2026/Shannon/Relay*.lean | awk -F: '{s+=$2} END {print "residual(IB):", s}'
  rg -c '@residual\(plan:relay-cutset-moonshot-plan\)' Common2026/Shannon/Relay*.lean | awk -F: '{s+=$2} END {print "residual(CS):", s}'
  rg -c '@audit:retract-candidate\(load-bearing-predicate\)' Common2026/Shannon/Relay*.lean | awk -F: '{s+=$2} END {print "retract:", s}'
  # word-boundary は backtick-quoted docstring 内 sorry も拾うので参考値、warning 数を信頼:
  rg -nw 'sorry' Common2026/Shannon/Relay*.lean | wc -l
  ```
...
- [ ] **V.4** **WynerZiv sweep の最新状態を確認** (`docs/shannon/wynerziv-sorry-migration-plan.md` の Phase V
  完了タイムスタンプ + `rg @audit:retract-candidate Common2026/Shannon/WynerZivBinningCovering.lean` の付与
...
1. **2026-05-25 plan 起草**: lean-planner (本 session、docs-only) が `Common2026/Shannon/Relay{Cutset,InnerBound,
