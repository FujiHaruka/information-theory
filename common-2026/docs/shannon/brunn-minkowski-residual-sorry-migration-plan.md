...
rg -n '@audit:suspect\(' Common2026/Shannon/<file>.lean \
...
- `lake env lean Common2026/Shannon/BrunnMinkowskiClosure.lean` が 0 errors、
...
| Pattern | 該当性 | 対処 |
|---|---|---|
| **A** (stale olean で type error 見逃し) | △ 弱該当 — 本 plan は signature / body 改変なし、docstring 編集のみ | Phase V で `lake env lean Common2026/Shannon/BrunnMinkowskiClosure.lean` 0 errors 確認のみで十分 (olean refresh 不要) |
...
|---|---|---|
| `BrunnMinkowskiClosure.lean:372 brunn_minkowski_volume_indicator` | **なし (in-family closed)** | `rg brunn_minkowski_volume_indicator Common2026/` で同 file 内のみ (`:373 theorem` 定義 + `:656/:825/:852/:856 in-file caller` のみ)、cross-family consumer 0 件 |
| `BrunnMinkowskiClosure.lean:493 brunn_minkowski_entropy_jointPi` | **なし (in-family closed)** | `rg brunn_minkowski_entropy_jointPi Common2026/` で同 file 内のみ (`:494 theorem` 定義 + `:548/:711 caller` のみ)、cross-family consumer 0 件 |
| `BrunnMinkowskiClosure.lean:441 IsBMEntropyPowerVolumeHyp` (Phase 2.X 注記対象) | **なし (in-family closed)** | `rg IsBMEntropyPowerVolumeHyp Common2026/` で同 file 内のみ、cross-family consumer 0 件 |
...
verbatim 確認方法: `Common2026/Shannon/BrunnMinkowskiClosure.lean` を Read で
...
- [x] Round 2 BM commit (`419be86` + `8c31ff6`) + Round 1 WynerZiv commit
  ...
  declaration を特定。
- [x] cross-family 検出 (`rg brunn_minkowski_volume_indicator|brunn_minkowski_entropy_jointPi|IsBMEntropyPowerVolumeHyp Common2026/`)
...
- [ ] **1.1** `WynerZiv.lean` の grep `@audit:suspect` 2 ヒットを **verbatim
  確認**:
  ```bash
  rg -n '@audit:suspect' Common2026/Shannon/WynerZiv.lean
  ```
...
- [ ] **1.3** Phase 1 完了時 `lake env lean Common2026/Shannon/WynerZiv.lean`
...
- [ ] **2.BM.1.3** docstring 内に **追加散文** で次を明示 (`audit-tags.md`
  ...
  ```
- [ ] **2.BM.1.4** `lake env lean Common2026/Shannon/BrunnMinkowskiClosure.lean`
...
- [ ] **2.BM.2.4** docstring 末尾に追加散文 (Phase 2.BM.1.3 と同様の Phase V
  ...
  自体は維持。
  ```
- [ ] **2.BM.2.5** `lake env lean Common2026/Shannon/BrunnMinkowskiClosure.lean`
  で 0 errors 確認。
...
- [ ] **2.X.1** `BrunnMinkowskiClosure.lean:441` の `def IsBMEntropyPowerVolumeHyp`
  ...
  n-dim PL route) に委ねる。
  ```
- [ ] **2.X.2** `lake env lean Common2026/Shannon/BrunnMinkowskiClosure.lean`
  で 0 errors 確認。
...
- [ ] **V.1** `lake env lean Common2026/Shannon/BrunnMinkowskiClosure.lean` 確認
  (0 errors):
  ```bash
  lake env lean Common2026/Shannon/BrunnMinkowskiClosure.lean
  ```
  ...
  lake env lean Common2026/Shannon/BrunnMinkowskiFunctional.lean
  ```
- [ ] **V.2** 集計コマンド実行:
  ```bash
  rg -c '@audit:suspect' Common2026/Shannon/BrunnMinkowskiClosure.lean
  # 期待値: 0 (本 plan Phase 2.BM.1 + 2.BM.2 で削除)

  rg -c '🟢ʰ' Common2026/Shannon/BrunnMinkowskiClosure.lean
  # 期待値: 0 (本 plan Phase 2.BM.2 で削除)

  rg -c '@audit:retract-candidate\(closure-plan-completed\)' Common2026/Shannon/BrunnMinkowskiClosure.lean
  # 期待値: 2 (Phase 2.BM.1 + 2.BM.2 で付与)

  rg -nw 'sorry' Common2026/Shannon/BrunnMinkowskiClosure.lean | wc -l
  # 期待値: 不変 (本 plan は新規 sorry 0 件、既存 sorry 件数も変えない)

  rg -c '@audit:suspect' Common2026/Shannon/WynerZiv.lean
  # 期待値: 0 (Round 1 完了状態を維持、本 plan touch しない)

  rg -c '@audit:suspect\(' Common2026/Shannon/BrunnMinkowskiFunctional.lean
  # 期待値: 0 (Round 2 完了状態を維持、本 plan touch しない、`\(` 直接タグ pattern)
  ```
- [ ] **V.3** **closure plan banner 更新**: `brunn-minkowski-closure-plan.md`
  の進捗 L16 を編集:
  ```
  - [x] Phase V — clean ✅ (`lake env lean Common2026/Shannon/BrunnMinkowskiClosure.lean` silent +
...
1. **2026-05-26 plan 起草**: lean-planner (本 session、docs-only) が
   `Common2026/Shannon/BrunnMinkowski{Functional,Closure}.lean` +
   `Common2026/Shannon/WynerZiv.lean` 3 file の `@audit:suspect` / 🟢ʰ 残置を
