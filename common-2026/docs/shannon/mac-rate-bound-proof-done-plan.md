...
`InformationTheory/Shannon/MultipleAccessChannel.lean` の 3 declaration (single user 1 / single user 2 / sum) の body `by sorry` を term-mode `mac_rate_le_of_fano hn R ... h_fano h_chain h_cleanup` の 1 行呼出に置換。**3 sorry → 0 sorry / 0 @residual / proof done** に到達 (`@residual(plan:mac-bc-sorry-migration-plan)` も削除、auditor pass 後 `@audit:ok` 付与)。
...
`InformationTheory/Shannon/MultipleAccessChannel.lean:396-420` 現状 (verbatim):
...
```bash
rg -n 'mac_single_rate_bound₁|mac_single_rate_bound₂|mac_sum_rate_bound' InformationTheory/
```

結果 (verbatim、上から):

- `InformationTheory/Shannon/MultipleAccessChannel.lean:37` (docstring 散文、自身)
- `InformationTheory/Shannon/MultipleAccessChannel.lean:105` (docstring 散文、自身)
- `InformationTheory/Shannon/MultipleAccessChannel.lean:450, 474, 501` (3 declaration 自身)
- `InformationTheory/Shannon/MultipleAccessChannel.lean:638` (`mac_capacity_region_outer_bound_three_bounds` docstring 散文「`_with_body` 経由で genuine」紹介、コード参照無し)
- `InformationTheory/Shannon/MACBodyDischarge.lean:45, 68, 575` (docstring 散文 3 件、`mac_sum_rate_bound` を「parent file exit」と紹介、コード参照無し)
- `InformationTheory/Shannon/MACL2Discharge.lean:10, 11, 47, 322, 331, 341, 345-354, 357, 359, 363-372` —
...
- `InformationTheory/Shannon/MACFanoConverseBody.lean:62, 63, 261, 266, 277, 280` — `_with_fano` wrappers 系の docstring + 自身 (両 `_with_fano` は `_with_body` 経由で MAC 3 declaration を呼ぶ間接 caller、しかし MAC 3 declaration 自身を直接 import せず `_with_body` を経由するため、引数順序整合は `_with_body` の 1 段でのみ verify すれば良い)
...
- 触る file: **1 file (`InformationTheory/Shannon/MultipleAccessChannel.lean`)**
...
- **L-MAC-PD-1 (想定外 consumer 出現)**: Phase 0.3 で実コード caller 0 〜 1 件
  ...
  InformationTheory/` を回し、新規 caller が出現していないか再確認。signature 不変
...
- **L-MAC-PD-3 (kernel verbatim drift)**: 本 plan 進行中に MAC 側
  ...
  Phase 1 着手前に `git log --oneline -- InformationTheory/Shannon/MultipleAccessChannel.lean
...
- [ ] **1.1** Phase 0 撤退ライン L-MAC-PD-1 再確認 (`rg -n 'mac_single_rate_bound₁|mac_single_rate_bound₂|mac_sum_rate_bound' InformationTheory/` で新規 caller 出現無し確認)
- [ ] **1.2** L-MAC-PD-3 再確認 (`git log --oneline -- InformationTheory/Shannon/MultipleAccessChannel.lean | head -5`)
- [ ] **1.3** `InformationTheory/Shannon/MultipleAccessChannel.lean:422-458` 周辺 docstring + body の 1 件 `Edit`:
...
- [ ] **1.4** `lake env lean InformationTheory/Shannon/MultipleAccessChannel.lean` を実行、0 errors / 0 sorry warning 確認
...
- [ ] **2.2** `InformationTheory/Shannon/MultipleAccessChannel.lean:460-482` 周辺 docstring + body の 1 件 `Edit` (Phase 1.3 mirror、indices: `R₁ → R₂`, `I_marg₁ → I_marg₂`, `I₁ → I₂`, `Pe₁ → Pe₂`, `M₁ → M₂`)
- [ ] **2.3** `lake env lean InformationTheory/Shannon/MultipleAccessChannel.lean` 再 verify
...
- [ ] **3.2** `InformationTheory/Shannon/MultipleAccessChannel.lean:484-510` 周辺 docstring + body の 1 件 `Edit`:
  - docstring の `@residual(plan:mac-bc-sorry-migration-plan)` を削除
...
- [ ] **3.3** `lake env lean InformationTheory/Shannon/MultipleAccessChannel.lean` 再 verify
...
- [ ] **V.1.1** body 置換ゆえ public symbol 変更無し、ただし `MultipleAccessChannel.lean` の `.olean` は body 変更で再生成必要。`lake build InformationTheory.Shannon.MultipleAccessChannel` で olean refresh
- [ ] **V.1.2** dependent 再 verify (signature 不変ゆえ 0 ripple 想定):
  - `lake env lean InformationTheory/Shannon/MACL2Discharge.lean` — `_with_body` × 2 件が `mac_single_rate_bound₁/₂` を呼出、引数順序不変ゆえ通る想定
  - `lake env lean InformationTheory/Shannon/MACFanoConverseBody.lean` — `_with_fano` × 2 件 (間接 caller via `_with_body`) も通る想定
  - `lake env lean InformationTheory/Shannon/MACBodyDischarge.lean` — docstring 散文言及のみ、コード参照無し
  - `lake env lean InformationTheory/Shannon/BroadcastChannel.lean` — BC peer の docstring が MAC を MAC analogue として言及 (`bc_common_rate_bound` `:479` 等)、本 plan の MAC peer proof done 到達後に docstring 更新の opportunity あり (V.3.1 で扱う)

### V.2 集計コマンド (target file)

```bash
TARGET="InformationTheory/Shannon/MultipleAccessChannel.lean"
...
- [ ] **V.3.3** `InformationTheory/Shannon/BroadcastChannel.lean` BC docstring 更新の
