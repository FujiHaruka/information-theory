...
**Status**: CLOSED ✅ — 無限分散 a.c. 古典 EPI を route T (conditioning truncation + Gibbs + DCT で crux usc を閉じ R→∞) で genuine closure。epi-infinite-variance-classical 壁は FALSE WALL (sharp Young/Brascamp-Lieb 不要) と判明し general unconditional EPI を完成。
...
## 要点 (≤5 行)
- 壁 epi-infinite-variance-classical は本ルートで body genuine 化 + legacy route 削除で code から消滅 = CLOSED (`#print axioms` sorryAx-free)。
