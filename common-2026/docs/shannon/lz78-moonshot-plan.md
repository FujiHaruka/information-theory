...
> - `Common2026/Shannon/Stationary.lean` — `StationaryProcess`, `ErgodicProcess`, `blockRV`
> - `Common2026/Shannon/EntropyRate.lean` — `entropyRate`, `entropyRate_exists_of_stationary`, `entropyRate_eq_lim_condEntropy`
> - `Common2026/Shannon/ShannonMcMillanBreiman.lean` — `blockLogAvg`, `shannon_mcmillan_breiman_of_sandwich`, `tendsto_expected_blockLogAvg`
> - `Common2026/Shannon/SMBChainRule.lean`, `SMBAlgoetCover.lean` — SMB の上下境界 chain rule
> - `Common2026/Shannon/BirkhoffErgodic.lean` — Birkhoff ergodic theorem
> - `Common2026/Shannon/AEPRate.lean`, `AEP.lean` — AEP a.s. convergence
>
> **Pattern 雛形**:
> - `Common2026/Shannon/RelayCutset.lean` (T3-F outer bound; 5 hypothesis pass-through 全発動 pattern の直接の雛形 — 主定理 body `:= h_rate_bound` の 1 行 wrap)
> - `Common2026/Shannon/WynerZivConverse.lean` (T3-D `wyner_ziv_converse_n_letter` の statement-level pass-through pattern)
>
> **Goal (短形)**: 新規 1 ファイル `Common2026/Shannon/LempelZiv78.lean` で Cover-Thomas
...
- [ ] Phase 0 — Mathlib + 既存 Common2026 在庫 + 設計確定 📋 → [`lz78-mathlib-inventory.md`](./lz78-mathlib-inventory.md)
...
- [ ] Phase V — `Common2026.lean` 編入 + textbook-roadmap.md / moonshot-seeds.md 更新 📋

## ゴール / Approach

### 最終到達点 (Phase F 完成形)

新規 1 ファイル `Common2026/Shannon/LempelZiv78.lean` の主合流:
...
[T3-F Relay cut-set pattern (雛形)]            [T4-A LZ78 pattern]
  ...
  relayCutsetBound : 2 数の min (scalar)        entropyRate : 既存 Common2026 definition
                ▼                                       ▼
...
Common2026.lean            ← `import Common2026.Shannon.LempelZiv78` 追記
```
...
- [x] `Common2026/Shannon/Stationary.lean:45` — `structure StationaryProcess`
- [x] `Common2026/Shannon/Stationary.lean:81` — `def StationaryProcess.blockRV`
- [x] `Common2026/Shannon/Stationary.lean:114` — `structure ErgodicProcess extends StationaryProcess`
- [x] `Common2026/Shannon/EntropyRate.lean:69` — `noncomputable def entropyRate`
- [x] `Common2026/Shannon/EntropyRate.lean:432` — `theorem entropyRate_exists_of_stationary`
- [x] `Common2026/Shannon/ShannonMcMillanBreiman.lean:55` — `noncomputable def blockLogAvg`
- [x] `Common2026/Shannon/ShannonMcMillanBreiman.lean:85` — `theorem shannon_mcmillan_breiman_of_sandwich`
- [x] `Common2026/Shannon/ShannonMcMillanBreiman.lean:162` — `theorem tendsto_expected_blockLogAvg`
...
- `lake env lean Common2026/Shannon/LempelZiv78.lean` clean (Phase B/C/D/E/F は forward declaration の `sorry` 許容)
...
- `lake env lean Common2026/Shannon/LempelZiv78.lean` 完全 clean (0 sorry / 0 warning)
...
- [ ] **V-1 `Common2026.lean` 編入**: 既存 `Common2026/Shannon/ShannonMcMillanBreiman` の **後** に
  ```lean
  import Common2026.Shannon.LempelZiv78
  ```
  を追記
- [ ] **V-2 ファイル clean 確認**: `lake env lean Common2026/Shannon/LempelZiv78.lean` silent
...
**0.05-0.15 セッション (~5-10 行 of Common2026.lean diff + ~30-50 行 of docs diff)**。
...
- **L-RP3**: **既存 SMB / EntropyRate の symbol 名が namespace 衝突**
  ...
  - 緩和: `open Common2026.Shannon` ではなく fully-qualified で参照 (本 plan の現方針通り)
...
| **`Common2026.lean` の import 順序で circular dependency** | 低 | 中 (file 移動が必要) | `LempelZiv78.lean` は `Common2026.Shannon.{Stationary, EntropyRate, ShannonMcMillanBreiman}` のみ import、SMB chain rule / Birkhoff / AEPRate には依存しない (型レベルで間接 reuse のみ) |
...
1. **Phase 0 (Mathlib + Common2026 在庫 + 設計確定)** — `lz78-mathlib-inventory.md` 本 plan と並行起草中。完了次第 Phase A 着手判定。
2. **Phase A skeleton 作成** ← Phase 0 完了後の次これ
   - `Common2026/Shannon/LempelZiv78.lean` 新規 (在庫 §7 の 120 行 skeleton を基盤に)
...
8. **Phase D-docs + Phase V `Common2026.lean` 編入 + roadmap 更新** (~0.2 セッション)
...
- 既存実装 (黒箱 reuse):
  - `Common2026/Shannon/Stationary.lean:45, :81, :114` — `StationaryProcess`, `blockRV`, `ErgodicProcess`
  - `Common2026/Shannon/EntropyRate.lean:69, :432` — `entropyRate`, `entropyRate_exists_of_stationary`
  - `Common2026/Shannon/ShannonMcMillanBreiman.lean:55, :85, :162` — `blockLogAvg`, `shannon_mcmillan_breiman_of_sandwich`, `tendsto_expected_blockLogAvg`
...
1. **実装 agent は worktree isolation 環境で動作** — `Common2026.lean` 編入は実装 agent が直接行う (Phase V)
...
3. **(2026-05-19) 撤退ライン L-LZ3 確定発動** — 既存 `shannon_mcmillan_breiman_of_sandwich` は sandwich hypothesis (`h_liminf`, `h_limsup`, `h_bdd_above`, `h_bdd_below`) を取る形 (`Common2026/Shannon/ShannonMcMillanBreiman.lean:85`)。本 plan で SMB の a.s. 結論を使うには sandwich を更に Birkhoff + chain rule で discharge する必要があり ~500-800 行。`IsSMBSandwichPassthrough μ p : Prop` を `True` placeholder で publish、別 plan `lz78-smb-sandwich-discharge-*` で。
...
6. **(2026-05-19) 単一ファイル戦略確定** — Wyner-Ziv 3 ファイル分離と異なり、T4-A LZ78 (5 撤退ライン全発動) は ~1200-1700 行で `lake env lean` 単一 file 10-20 秒の inner loop に収まる。分離不要。`Common2026/Shannon/LempelZiv78.lean` 単一 file で publish。
