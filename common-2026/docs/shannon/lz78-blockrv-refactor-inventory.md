...
|---|---|---|---|---|
| Gibbs/Kraft 下界（期待値版） | `entropyD_le_expectedLength_of_kraft` | `Common2026/Shannon/ShannonCode.lean:164` | ✅ genuine | 下記 verbatim |
...
| Birkhoff 個別エルゴード定理 | `birkhoff_ergodic_ae` | `Common2026/Shannon/BirkhoffErgodic.lean:1031` | ✅ genuine | 下記 verbatim |
| SMB a.s. 収束 | `shannon_mcmillan_breiman` | `Common2026/Shannon/SMBAlgoetCover.lean:2840` | ✅ genuine | 下記 verbatim |
...
`rg` 全数確認（Common2026 全体）:
- `StationaryProcess.mk` / `ErgodicProcess.mk` / anonymous constructor / 全 field 供給サイト: **ゼロ**。
- 唯一のヒット `Common2026/Probability/TwoSidedExtension.lean:959`
  （`set q : StationaryProcess μ α := p.toStationaryProcess`）は **別名束縛**で構築ではない。
  **※ 計画は `Common2026/Shannon/TwoSidedExtension.lean` と記すが、実体は `Common2026/Probability/`
...
## §12. Phase K1 着手 skeleton（`Common2026/Shannon/StationaryKernel.lean`）
