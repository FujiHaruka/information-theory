...
`InformationTheory/Shannon/HuffmanOptimality.lean` 末尾追記 (新規 file は H2/H1 の規模次第、§設計判断):
...
- **証明済の道具 (黒箱 reuse)**:
  - `exists_two_equal_longest` (`HuffmanSwapNormProof.lean:110`) — Kraft = 1 完全符号は
    ...
    parity 論法) + `kraft_one_nat_sum` (`:34`)。HuffmanSwapNormProof.lean は InformationTheory
...
- 実際の再検証対象 (`Huffman.lean` を transitively import する file):
  ...
  `HuffmanSwapNormalizationBody.lean`。`HuffmanSwapNormProof.lean` は InformationTheory 依存ゼロで
  insulated。
...
- [ ] `lake build InformationTheory.Shannon.Huffman` で olean refresh 後、下流 6 Huffman* file を
...
- [ ] `huffmanLength_optimal` (hypothesis 引数なし) を
      ...
      `InformationTheory.lean` の import は既存 (Huffman* は登録済)、新規 file 作成時のみ追記。
...
2. **2026-05-21 起草 — C1 blast radius が prompt 想定と相違 (Ch.5 へ波及しない)**:
   ...
   のみ。`HuffmanSwapNormProof.lean` (keystone) は InformationTheory 依存ゼロで insulated。
