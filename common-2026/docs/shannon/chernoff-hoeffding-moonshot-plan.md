...
> achievability side は `chernoff_lemma_achievability` (`InformationTheory/Shannon/Chernoff.lean:1059`,
...
> **Status (2026-05-19)**: **Tier 0 (定義 + 基本性質) publish 済** (`InformationTheory/Shannon/Chernoff.lean`, ~280 行, 0 sorry, library root 編入完了)。`docs/textbook-roadmap.md` §T1-B (Chernoff) + §T1-D (Hoeffding) を **一括** で扱う 1〜2 セッション (~3-5h) ムーンショット。Stein/Sanov/Csiszar plumbing (`Stein.lean` / `StrongStein.lean` / `SanovLDPEquality.lean` / `CsiszarProjection.lean`) からの **70-80% 再利用** を前提に、新規ファイル `InformationTheory/Shannon/Chernoff.lean` (T1-B + T1-D 一括、~600-900 行) に publish。
...
- [x] Phase 0 — Mathlib + InformationTheory API 在庫 ✅ → [`chernoff-hoeffding-mathlib-inventory.md`](chernoff-hoeffding-mathlib-inventory.md)
- [x] **Tier 0 publish — Chernoff/Hoeffding 定義 + 基本性質 (`chernoffInfo`, `chernoffInfo_attained`, `chernoffInfo_nonneg`, `hoeffdingE2`, `hoeffdingE2_attained`, `hoeffdingE2_nonneg`, `klDivPmf_self_eq_zero`) ✅** (2026-05-19, 0 sorry, library root 編入済, `InformationTheory/Shannon/Chernoff.lean` ~280 行)
- [x] **Phase A 残 — `convexOn_chernoffLogZ` (Hölder 凸性) + `chernoffMediator` pmf bridge ✅** (2026-05-19, 0 sorry, +307 行, `InformationTheory/Shannon/Chernoff.lean` ~663 行)
- [x] **Phase C — Chernoff achievability (rate-side lower bound, `liminf ≥ chernoffInfo`) ✅** (2026-05-19, 0 sorry, +403 行, `InformationTheory/Shannon/Chernoff.lean` ~1066 行)。`bayesErrorMinPmf` 自前 pmf 形定義 + per-point Hölder min-bound + n-IID sum reshape ⇒ `Filter.le_liminf_of_le` で結論。
...
新規ファイル `InformationTheory/Shannon/Chernoff.lean` で 2 主定理 publish:
...
Phase E  : 主定理 wrapper + InformationTheory.lean 編入               ← 0.25 セッション (0.5h)
...
- [x] `InformationTheory/Shannon/Stein.lean` (`llrPmf`, `logLikelihoodRatio`, `steinTypicalSet_*`, `steinOptimalBeta`, `klDiv_pi_eq_n_smul`, `stein_strong_law`, `jointRV`)
- [x] `InformationTheory/Shannon/StrongStein.lean` (`stein_strong_lemma`, strict `Tendsto` 形)
- [x] `InformationTheory/Shannon/SanovLDPEquality.lean` (`sanov_ldp_equality`, `roundedTypeIndex`, `klDivIndex`)
...
- **既存 API カバレッジ 100%** (実体ベース 35 項目): Mathlib `Measure.tilted` + `LogLikelihoodRatio` + InformationTheory Stein/Sanov/Csiszar plumbing で完備
...
- `lake env lean InformationTheory/Shannon/Chernoff.lean` で Phase A までの skeleton + Phase A 本体が clean (Phase B-E は `sorry`)
...
- [ ] **C-1 statement 形確定** (judgement #1 で確定): 「`Tendsto` 直書き」 vs 「`DotEq` 形 (`bayesErrorMin ≐ exp(-n · chernoffInfo)`)」 vs 「achievability/converse pair」のいずれを最終形にするかを **Phase A 終了時に決定**。`dotEq_iff_tendsto_log_div` (`Asymptotic.lean:116`) で往復可能、`Tendsto` 形 を main + `DotEq` corollary が現状の InformationTheory style と整合。
...
`chernoff_lemma` (T1-B) + `hoeffding_tradeoff` (T1-D) の **最終 statement form を確定** し、`InformationTheory.lean` に `import InformationTheory.Shannon.Chernoff` を追記。
...
- `InformationTheory.lean` 更新、`lake env lean InformationTheory/Shannon/Chernoff.lean` clean
...
- [ ] **E-4 library root 編入**:
  - `InformationTheory.lean` に `import InformationTheory.Shannon.Chernoff` 追記
...
1. **(2026-05-19) statement 形は `Tendsto` 直書きを main、`DotEq` を corollary に**: InformationTheory 既存 Stein / Sanov / Pinsker の主定理が全て `Tendsto` 直書き形を採用しており (`stein_lemma`, `stein_strong_lemma`, `sanov_ldp_equality`)、Asymptotic.lean の `DotEq` は **wrapper notation** として後付け corollary 化する style が既に確立。本 plan も同 style を採用、`chernoff_lemma` / `hoeffding_tradeoff` は `Tendsto` を main, `DotEq` を corollary。Phase E で `dotEq_iff_tendsto_log_div` (`Asymptotic.lean:116`) を呼ぶだけで往復。
