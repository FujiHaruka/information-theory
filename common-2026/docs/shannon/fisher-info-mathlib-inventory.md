# T2-F Fisher Information + de Bruijn Identity — Mathlib + Common2026 在庫調査

> **対象**: `docs/textbook-roadmap.md` §T2-F (Cover-Thomas Ch.17 / EPI 経路の入口、~600-800 行)。
> 着手前の Mathlib + Common2026 在庫を構造化テーブルで網羅し、自作要箇所の工数概算を付ける。
...
(`Common2026/Shannon/DifferentialEntropy.lean` 1010 行) はすべて既存。
...
- **`F x₀` の Integrable**: `differentialEntropy` の被積分関数 `pdf log pdf` の可積分性 →
  `Common2026/Shannon/DifferentialEntropy.lean:81 integrable_density_log_density_of_gaussian` 拡張要
...
| `Measure.rnDeriv_pos`, `Measure.rnDeriv_lt_top` | `Mathlib/MeasureTheory/Measure/Decomposition/Lebesgue.lean` | (既知、Common2026 DifferentialEntropy.lean:825 で使用) | ✅ | `pdf > 0` a.e., `pdf < ∞` a.e. — Fisher info の `1/pdf` 安全性 |
| `Measure.rnDeriv_mul_rnDeriv` | (既知、DifferentialEntropy.lean:818 で使用) | (cf. Common2026 内で実証済) | ✅ | チェイン RN |
...
## §E. Differential entropy 微分可能性 (Common2026 既存) — DifferentialEntropy.lean 再利用候補

`Common2026/Shannon/DifferentialEntropy.lean` (1010 行) から T2-F 再利用候補のみ抜粋。

| 補題名 | file:line | signature (要点) | 状態 | T2-F での扱い |
|---|---|---|---|---|
| **`differentialEntropy`** | `Common2026/Shannon/DifferentialEntropy.lean:42-43` | `noncomputable def differentialEntropy (μ : Measure ℝ) : ℝ := ∫ x, Real.negMulLog ((μ.rnDeriv volume x).toReal) ∂volume` | ✅ Common2026 既存 | **`h(P_t)` の定義そのもの**。T2-F で再利用 |
| **`differentialEntropy_eq_integral_density`** | `Common2026/Shannon/DifferentialEntropy.lean:60-72` | `theorem differentialEntropy_eq_integral_density {f : ℝ → ℝ} (hf : Measurable f) (hf_nn : ∀ x, 0 ≤ f x) (μ : Measure ℝ) (hμ : μ = volume.withDensity (fun x => ENNReal.ofReal (f x))) : differentialEntropy μ = -∫ x, f x * Real.log (f x) ∂volume` | ✅ Common2026 既存 | **`h(P_t) = -∫ p_t log p_t dx`** 形書き換え。これが de Bruijn の `d/dt` を取る対象 |
| `differentialEntropy_eq_integral_withDensity` | `Common2026/Shannon/DifferentialEntropy.lean:47-55` | `theorem ... {f : ℝ → ℝ≥0∞} (hf : Measurable f) : differentialEntropy (volume.withDensity f) = ∫ x, Real.negMulLog (f x).toReal ∂volume` | ✅ | `ℝ≥0∞`-density 形 |
| `differentialEntropy_dirac` | `Common2026/Shannon/DifferentialEntropy.lean:149-150` | `theorem differentialEntropy_dirac (m : ℝ) : differentialEntropy (Measure.dirac m) = 0` | ✅ | `t = 0` の boundary 処理 |
| `differentialEntropy_map_add_const`, `..._mul_const`, `..._affine` | `Common2026/Shannon/DifferentialEntropy.lean:165, 195, 344` | translation / scaling | ✅ | `Real.sqrt t · Z` の scaling 処理 |
| `integrable_density_log_density_of_gaussian` | `Common2026/Shannon/DifferentialEntropy.lean:81-83` | `theorem ... (m : ℝ) {v : ℝ≥0} (hv : v ≠ 0) : Integrable (fun x => gaussianPDFReal m v x * Real.log (gaussianPDFReal m v x)) volume` | ✅ | **Gaussian PDF の log-product 可積分性**。Fisher info の有限性 (Gaussian の場合) の核 |
| `log_gaussianPDFReal_eq` | `Common2026/Shannon/DifferentialEntropy.lean:391-403` | `theorem ... (m : ℝ) {v : ℝ≥0} (hv : v ≠ 0) (x : ℝ) : Real.log (gaussianPDFReal m v x) = -(1/2) * Real.log (2 * Real.pi * v) - (x - m)^2 / (2 * v)` | ✅ | Gaussian の log 展開 (score 計算で `deriv` を取ると `-(x - m)/v` が出る、Gaussian の Fisher info `J = 1/v` の根拠) |
| `differentialEntropy_gaussianReal` | `Common2026/Shannon/DifferentialEntropy.lean:406-409` | `theorem ... (m : ℝ) {v : ℝ≥0} (hv : v ≠ 0) : differentialEntropy (gaussianReal m v) = (1/2) * Real.log (2 * Real.pi * Real.exp 1 * v)` | ✅ | **`h(𝒩) = (1/2) log(2πev)`** — de Bruijn を `X = 0` で sanity check |
| `klDiv_gaussianReal_gaussianReal_eq` | `Common2026/Shannon/DifferentialEntropy.lean:791-795` | `theorem ... (m₁ m₂ : ℝ) {v₁ v₂ : ℝ≥0} (hv₁ : v₁ ≠ 0) (hv₂ : v₂ ≠ 0) : (klDiv (gaussianReal m₁ v₁) (gaussianReal m₂ v₂)).toReal = (1/2) * (Real.log ((v₂ : ℝ) / v₁) + (v₁ : ℝ) / v₂ + (m₁ - m₂)^2 / v₂ - 1)` | ✅ | de Bruijn の検算 (`d/dt` を 2 重で取ると Fisher info) |
| **`deriv_t (differentialEntropy (P.map (X + √t · Z))) = (1/2) · J(P_t)`** | — | — | ❌ **T2-F 主定理** | **自作必須 — 本 seed のゴール** |

---

## §F. Gaussian distribution / convolution (Mathlib + Common2026 既存)
...
| **Common2026 既存 Gaussian-related** | — | — | — | **T2-F 着手前は `DifferentialEntropy.lean` 内のみ**。`Gaussian.lean` という独立ファイルは無い |

---

## §G. 自作要 (Mathlib + Common2026 ともに無いもの) — 自作工数概算付き
...
- **`pdf` を直接呼ばず `μ.rnDeriv volume` で書く** — `HasPDF X ℙ volume` を要求するか `μ : Measure ℝ` 引数 mid-level かは設計判断。後者の方が `differentialEntropy : Measure ℝ → ℝ` と整合 (cf. Common2026 既存 §E)
...
Common2026 `DifferentialEntropy.lean` の Gaussian 補助インフラを大量に流用できる**。
...
`differentialEntropy_eq_integral_density` (Common2026 §E) — ただし `f := (pdf Y ℙ volume).toReal`
...
| **Differential entropy (Common2026)** | **80%** | `differentialEntropy`, `..._eq_integral_density`, `..._gaussianReal`, `integrable_density_log_density_of_gaussian`, `klDiv_gaussianReal_gaussianReal_eq` |
| **Fisher info 概念** | **0%** | Mathlib 不在、Common2026 不在 |
| **de Bruijn identity** | **0%** | Mathlib 不在、Common2026 不在 |
...
4. **§E の Common2026 既存 `differentialEntropy` が再利用可能** — 主定理の左辺は
...
- **`hasDerivAt_integral_of_dominated_loc_of_deriv_le`** (§C-4):
  ...
  - **`Integrable (F x₀) μ`** — `F (t, x) := -p_t(x) log p_t(x)` の `t = t₀` での可積分性。
    Gaussian convolution の場合 `Common2026/Shannon/DifferentialEntropy.lean:81`
...
## 着手 skeleton (`Common2026/Shannon/FisherInfo.lean`)
...
Cover-Thomas Ch.17 の Fisher info 定義 + de Bruijn identity。Common2026 T2-F
...
- 前提 inventory: `Common2026/Shannon/DifferentialEntropy.lean` (1010 行、Common2026 既存)
