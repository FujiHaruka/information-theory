...
## 残る genuine core (次 leg、route LOCK = markovFactor、leg 7 後)

- **(i) k-state threading**: **markovFactor 位置不変性 (`markovFactor_eq_of_window_eq`、ca78b10) 閉鎖**。残るは parse 配線 = negLogQk の各絶対位置 markovFactor を condQkState の position に同定する threading。
