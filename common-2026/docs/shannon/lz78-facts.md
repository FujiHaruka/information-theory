...
| `negLogQk_phrase_threading` = **block 分解 (1 sorry)**: tiling 入力下で `negLogQk = boundary + ∑_phrases -log condQkState` | (sorry) | `rg -n '@residual\(wall:lz78-aseventual-ziv\)' InformationTheory/Shannon/LZ78/ZivThreading.lean` | bf78de9 | `ZivThreading.lean:335`、`@residual(wall:lz78-aseventual-ziv)`。**tiling (`N`/`hmono`/`hstart`/`hNb`/`hNn`) は明示引数 = regularity (監査 PASS、非 bundling・sufficiency TRUE-as-framed、`bd198e2`)**。残 gap = (a) body fill (tiling 下の sum-split、tractable、要 `hposfac`/`k<` regularity) + (b) tiling 材料化 (genuine blocker、List↔Fin) |
...
- **(i) k-state threading**: **gateway atom `markovFactor_blockRV_eq_window` + factor correspondence 5 補題 sorryAx-free 閉鎖 (leg 8、`bf78de9`)**。残るは (a) `negLogQk_phrase_threading` body fill (tiling 下の sum-split) + (b) tiling 材料化 (`lz78PhraseStrings` → 絶対位置 `N : Fin (c+1)→ℕ`、unfinished tail、genuine blocker)。
