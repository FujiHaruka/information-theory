...
- decoder measurability の壁 → Phase 0 Axis 5 で `Classical.choose` +
  `measurable_to_countable'` ルートが確定 (`Function.argmin` は `WellFounded (·<·)`
  要求のため `ℝ` で詰む、Mathlib `Measurable.argmin` も不在)。genuine discharge
  ~15-25 行で済む、hyp 化は不要
...
Axis 2 inventory ([`awgn-achievability-typicality-mathlib-inventory-axis2-aep.md`](awgn-achievability-typicality-mathlib-inventory-axis2-aep.md))
で Mathlib + Common2026 を網羅調査:

- ✅ 既存: `strong_law_ae_real` / `_Lp` (SLLN)、`memLp_id_gaussianReal'` /
  `variance_fun_id_gaussianReal` (1-d Gaussian moments)、`Filter.Tendsto.log`、
  `iIndepFun_pi`
- 🔴 不在: **continuous AEP 本体** / **n-d differentialEntropy** /
  **continuous SMB (Shannon-McMillan-Breiman)**

T-2 不採用ルート: Phase B 単独 ~390 行、Phase A-E 合計 750-810 行で T-4 (plan 2 分割)
高確率発動 → 採用却下。

T-2 採用ルート: Phase B ~50 行 (predicate のみ)、achievability core (codebook +
union bound + expurgation) は Phase C-D で本物 discharge を維持。`@audit:staged
(continuous-aep-gaussian)` タグで parallel-gaussian / EPI と同型 staged pattern。

honesty 規律 (Phase B 着手時 4 条件、CLAUDE.md「Mathlib 壁の 4 分類」):
(a) `IsContinuousAEPGaussian P N` の型 ≠ `IsAwgnTypicalityHypothesis` 結論型 /
(b) docstring で "Mathlib gap, NOT load-bearing for achievability core" 明示 /
(c) Phase C-D で genuine discharge / (d) audit:staged タグ付与。

### #2 (2026-05-24) Phase 0 完了、codebook 測度 = Option A (2 段 `Measure.pi`)

Axis 1 inventory ([`awgn-achievability-typicality-mathlib-inventory-axis1-codebook.md`](awgn-achievability-typicality-mathlib-inventory-axis1-codebook.md))
で 6 サブ項目 100% Mathlib 既存確認:
...
- `AwgnCode.encoder : Fin M → (Fin n → ℝ)` と型 defeq、追加 instance 不要
- `iIndepFun_pi` (Basic.lean:784) + `measurePreserving_eval` (Pi.lean:407) で 2-3 行
- T-1 (Measure.pi 型クラス壁) 不発動

T-2 採用ルートでは Phase B で SLLN を使わないため、Axis 2 の `strong_law_ae` Banach
版 `[BorelSpace E]` 要求 (`EuclideanSpace` flatten 推奨) は無効 → Option A で整合維持。

**trap 1**: `iIndepFun_pi` の `[∀ i, IsProbabilityMeasure (μ i)]` がファイル変数継承
で signature に現れない。`νₙ` を prob measure のまま維持必須、非正規化版に書き換える
と static unify failure。

**trap 2**: `pi_map_eval` (Pi.lean:379) 一般版は `(∏ μ j univ) • μ i` scalar 付き、
prob でない測度で projection を取ると trap。**必ず `measurePreserving_eval` (line
407、prob 専用、scalar が消える形) を選ぶ**。

### #3 (2026-05-24) Phase 0 完了、typical set 定義形 = Option γ (`klDiv` 形)

Axis 3 inventory ([`awgn-achievability-typicality-mathlib-inventory-axis3-density.md`](awgn-achievability-typicality-mathlib-inventory-axis3-density.md))
で 3 形 (α `rnDeriv` / β `differentialEntropy` / γ `klDiv`) を比較:

- **Option α (`rnDeriv` 形)**: `Measure.pi × Measure.rnDeriv` は loogle **0
  declarations** → bridge 大量自作 (50-100 行)、却下
- **Option β (`differentialEntropy` 形)**: Common2026 既存 `jointDifferentialEntropyPi_le_sum`
  が load-bearing `h_llr_split` (`@audit:suspect(differential-entropy-plan)`) を
  含む → 本 plan が既存負債を継承する形になる、却下
- **Option γ (`klDiv` 形)**: Mathlib `klDiv_compProd_eq_add` (無条件等号) +
  Common2026 既存 `klDiv_pi_eq_sum` / `klDiv_gaussianReal_gaussianReal_eq` +
  `mutualInfo := klDiv (joint) (prod marginal)` で完全に乗る、負債完全断ち切り、
  **採用**

採用形:

- typical set / AEP bound を `klDiv` 経由で表現
- `mutualInfo` 既存定義と直結 (Phase C の union bound で `I` の closed form 不要)
...
T-2 採用方針確定済 (判断 #1)、Phase B 着手時に Option γ (`klDiv` 形) に整合する
predicate 具体形を確定 + audit:staged タグ付与確認 append 予定。

### #7 (2026-05-24) E-1 staged hyp `IsAwgnPowerConstraintRealizable` に false-statement defect 発覚 → sibling pivot plan で bundle 形に pivot 完了

Phase E-1 で導入した `IsAwgnPowerConstraintRealizable P N` (判断 #6 で追加 staged) が、後続の Mathlib 壁 discharge 検討段階で **false-statement defect** であることが判明。原因は chi-square mass の bound `P(∑ Xᵢ² ≤ nP) ≥ 1 − ε` が、`Xᵢ ~ N(0, P)` i.i.d. のとき `S := ∑Xᵢ²/P ~ χ²(n)` の中央値が ≈ n − 2/3 で右偏なので `P(S ≤ n) → 0.5⁺` から上、1 にはならない (CLT)。`Fin M` 結合で `≤ (0.5 + o(1))^M`、unsatisfiable。

**標準解 (Cover-Thomas 9.2)**: codebook 生成 variance を `P' < P` に下げる → SLLN で `(1/n)∑Xᵢ² → P' < P` a.s. → `P(∑Xᵢ² ≤ nP) → 1`。これを Lean predicate 側に押し付ける。

**Pivot (sibling plan [`awgn-power-constraint-realizable-pivot-plan.md`])**:

- Option C (bundled) 採用: 3 staged hyp (`h_aep` + `h_rand` + `h_power`) を 1 bundle hyp `IsAwgnRandomCodingFeasible P N h_meas` に統合、bundle が `∃ P' ∈ (0, P]` の witness を共有
- 新規 predicate 2 件 (Phase 2 commit `9dcef00`):
  - `IsAwgnPowerConstraintHonest (P_cb P_target : ℝ) (N : ℝ≥0)` — codebook 生成 P_cb / constraint target n·P_target 分離形
  - `IsAwgnRandomCodingFeasible (P : ℝ) (N : ℝ≥0) (h_meas : IsAwgnChannelMeasurable N)` — `∀ R, ∃ P' ∈ (0, P], rate-margin + AEP + RC-integral + PowerConstraintHonest` bundle
- 旧 `IsAwgnPowerConstraintRealizable` は orphan 化、`@audit:defect(false-statement)` タグ + body 完全不変で残置 (honesty record)
- Phase 3 (commit `2ace40b`): consumer `isAwgnTypicalityHypothesis` body 1641 行 / 0 sorry / silent 復元、bundle destructure + `gaussianCodebook P → P'` 15 箇所 sed + `IsAwgnPowerConstraintHonest` の rate-bound P→P' bridge 20 行 (`Real.log_le_log` 経由)
- 独立 honesty audit (Phase 2 で general-purpose fresh subagent 起動): bundle predicate 2 件とも `load_bearing_hyp / honest 🟢ʰ`、4 条件遵守、name laundering なし
- 完了状態 (commit `4fdf403`): `Common2026/Shannon/AWGNAchievabilityDischarge.lean` 1641 行 / 0 sorry / silent、`@audit:suspect(awgn-power-constraint-realizable-pivot)` closure 済

**Soft caveat**: bundle の `P' ≤ P` (non-strict) は `P' = P` 退化を許容、その場合 `IsAwgnPowerConstraintHonest P P N` が v1 の unsatisfiable form に戻る。Phase 3 body は `P' = P` でも形式的に通る (defect ではない) が、bundle の genuine discharger (Mathlib PR or 別 session) で `P' < P` を必ず選ばせる責務が残る。

**E-1 staged 総数の変化**: 3 hyp 並列 → 1 bundle hyp、Mathlib 壁 (b) 系の analytic 3 gap (continuous SMB / n-d differentialEntropy / chi-square SLLN) は不変。consumer signature 1/3 に縮約、wrapper 2 件 (F1 / F1F4) は term-mode で transitively 継承。

**proof-log**: `docs/proof-logs/proof-log-awgn-power-constraint-realizable-pivot-phase2.md` / `proof-log-awgn-power-constraint-realizable-pivot-phase3.md`。判断ログは sibling plan `#1`-`#3` に詳細。

### #6 (2026-05-24) Phase D 完了 + Phase E signature refine、E-1 body は次セッションへ defer

`Common2026/Shannon/AWGNAchievabilityDischarge.lean` (716 行) で Phase D の 3
declarations + Phase E の 3 declarations signature を全 refine + Phase D body
fill 0 sorry + E-2 / E-3 body 完成 (1 行 wrapper)。**E-1 body のみ `sorry`
1 個残置**。

#### Phase D 確定 (publication, 0 sorry)

- **D-1** `awgn_exists_codebook_le_avg`: `MeasureTheory.exists_le_lintegral`
  (Average.lean:738) 直接 1 行 + `le_trans` — inventory axis 4.1.1 通り。signature
  は `(σsq : ℝ≥0) (Pe : (Fin M → Fin n → ℝ) → ℝ≥0∞) (hPe_aemeas) {B} (h_avg)`
  で `∃ c, Pe c ≤ B` (B を抽象化、E-1 で `ENNReal.ofReal (2 * ε)` 等に具体化).
- **D-2** `awgn_expurgate_worst_half`: pure `Finset` arithmetic ~40 行
  (`Finset.card_filter_add_card_filter_not` で good/bad 分割 +
  `Finset.sum_lt_sum_of_nonempty` で sum lower bound + `nlinarith`). 
  `M / 2 ≤ S.card` (not `M / 2 ≤ S.card` の等号) で worst-half throw-away を表現。
  inventory axis 4.2 「~10-15 行」見積もりに対し +25 行は `Nat → ℝ` cast
  plumbing (2*card vs M*2 の整数除算境界条件) で消費。
- **D-3** `awgn_extract_AwgnCode`: `AwgnCode` constructor 直接 +
  `jointTypicalDecoder` + `jointTypicalDecoder_measurable` + ENNReal `.toReal`
  slack (`4ε → 5ε`). h_power と h_max_Pe を仮定として受ける形 (E-1 で供給予定)。

#### 追加 staged hyp `IsAwgnPowerConstraintRealizable` (Phase D 拡張、4 条件遵守)

D-3 の `h_power : ∀ m, (∑ i, (codebook m i)^2) ≤ n * P` は **gaussianCodebook
からは deterministic に成立しない** (Gaussian の support は ℝ 全体) — 確率版で
高確率成立 (Cover-Thomas 9.2 の SLLN による margin 議論)。これを E-1 で抽出する
ための staged hyp として `IsAwgnPowerConstraintRealizable P N` を新規導入。

- 型: `∀ ε > 0, ∀ R, 0 < R → R < C → ∃ N₀, ∀ n ≥ N₀, ∀ M ≤ ⌈exp(nR)⌉,
    gaussianCodebook M n P.toNNReal {c | ∀ m, ∑ (c m i)^2 ≤ n*P} ≥ 1 - ε`
- 4 条件:
  (a) 型 ≠ `IsAwgnTypicalityHypothesis` 結論 (`AwgnCode` / `errorProbAt` を含まない、
      raw codebook 上の measure-of-set 形)
  (b) docstring で "NOT load-bearing" + Mathlib gap (n-d Gaussian SLLN
      coordinate-wise + union bound across `Fin M`) 明記
  (c) E-1 body (deferred) が D-3 を invoke してこの hyp の measure-of-set
      から deterministic codebook を抽出する設計
  (d) `@audit:staged(awgn-power-constraint-realizable)` タグ付与

これで Phase E-1 の staged hyp 総数は **3 本** (h_aep + h_rand + h_power)。当初想定 1 本 (h_aep
のみ) から +2 本だが、各 hyp は独立な analytic 壁 (連続 AEP / random-coding
integral / power-constraint SLLN) を表現し、全て同型の Mathlib gap (n-d
Gaussian SLLN / 連続 SMB 不在) に紐づく。

#### E-2 / E-3 確定 (publication, 0 sorry, isAwgnTypicalityHypothesis 呼び出し 1 行)

- **E-2** `awgn_achievability_F1_discharged`: `awgn_achievability` の
  `h_typicality` を `isAwgnTypicalityHypothesis P hP N hN h_meas h_aep h_rand
  h_power` で埋める 1 行 wrapper。
- **E-3** `awgn_theorem_F1F4_discharged`: `awgn_channel_coding_theorem` の
  `h_meas` を `isAwgnChannelMeasurable N` (F-4 discharge from `AWGNF1Discharge`)、
  `h_typicality` を E-1 で埋める。残 hyp = `h_aep` + `h_rand` + `h_power` +
  `h_mi_bridge` + `h_converse` の 5 本 (F-1 staged 3 + F-2 + F-3)。

#### E-1 body 残置 (1 sorry、次セッション)

`isAwgnTypicalityHypothesis` body は **意図的に `sorry` 1 個残置**。理由:

- skeleton (docstring 内に明示) は ~7 ステップ (ε scaling → 3 hyp で max N₀ →
  M = 2 * ⌈exp(nR)⌉ → AEP で typical set 取得 → h_rand で per-m bound + h_power
  で power-OK set → **Fubini 形で「∫⁻ codebook, ∑_m Pe(c, m) ∂(gauss) ≤ M * 2ε」
  を per-m bound 線形和で構築** + 「power-OK set ∩ Pe-sum 低い codebook」を
  `awgn_exists_codebook_le_avg` で抽出 → `awgn_expurgate_worst_half` で半数の m
  で Pe ≤ 4ε → `Finset.equivFin` で `Fin M_target` reindex → `awgn_extract_AwgnCode`
  で AwgnCode 化 + ε scaling で `< ε` 成立).
- Lean 化規模は ~100-200 行。本セッションでは D-1/D-2/D-3 (~150 行) + signature
  refine + proof-log で時間枠を消費、E-1 body の Fubini + reindex 詳細を実装する
  余地が無い。
- **rebrand 回避**: E-1 を「`IsAwgnExpurgationFeasible` 等の追加 staged hyp」で
  1 行 passthrough する案を検討したが、これは Phase D-1/D-2 を hyp 化することに
  なり、CLAUDE.md §「honesty 撤退ライン」(name laundering 禁止) に抵触する
  ため **採用しない**。E-1 body の Fubini + reindex は本物の analytic 工程と
  して inline で書くのが honest。
- code 内 `@audit:residual(awgn-typicality-E1-body)` タグで current sorry を
  明示。docstring 内に full skeleton (7 ステップ) を pseudocode で記載済。

#### 検証

`lake env lean Common2026/Shannon/AWGNAchievabilityDischarge.lean`: error 0、
warning 1 (`declaration uses 'sorry'` at line 670, E-1 body). Phase D 全 3
declarations + E-2 + E-3 は silent.

#### 次セッション TODO (Phase E-1 closure session)

1. E-1 body inline 実装 (~100-200 行、上記 7 ステップ)、特に Fubini 部分は
   `lintegral_finset_sum` + 線形性で per-m bound を summing する形
2. `Finset.equivFin` で `Fin M_target` ≃ `S ⊆ Fin M` 翻訳 ~10-15 行
3. `Phase V` (verify + Common2026.lean import 追加 + honesty 再 audit)
4. plan の進捗ブロックを ✅ に更新
