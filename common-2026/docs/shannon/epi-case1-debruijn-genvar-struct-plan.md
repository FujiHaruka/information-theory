...
> **Status**: 📋 draft (起草のみ、実装は別 session で `lean-implementer` dispatch)。**2026-06-06 確定状態 (機械実証で固定): B-τ・case-A の両 route は REFUTED。真の closure = factor-2 ratio-Stam 再導出 (2 consumer lemma、標準 de Bruijn EPI 数学、壁でない・trivial でない)。plumbing は機械的。** 詳細は冒頭「## 確定状態」section (本文の thrash した route 比較より優先)。
...
## 確定状態 (2026-06-06、機械実証で固定 — 本文 route 比較より優先される superseding 要約)
...
- [ ] **GS-A3' (最初に着手すべき gate、load-bearing 解析核)**: factor-2 ratio-Stam 再導出。2 consumer lemma を正しい factor で書き直す — (a) `csiszarLogRatioGap_hasDerivAt` の `hN_*` lift (`:766-803`、sum 項 factor を `(1/2)` から `(v_sum/2)·J=J` に)、(b) `csiszar_ratio_deriv_le_zero_arith` の `α²≤α` weights を factor-2 sum numerator で再導出。最小 probe = sum 項 lift 書き直し + arith ≤0 機械確認 (plumbing に投資する前) 📋
