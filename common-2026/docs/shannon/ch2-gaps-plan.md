...
- [ ] WI-1 — 充足統計量 (2.9) 新規実装 `SufficientStatistic.lean` 📋 ※唯一の新規 Lean
- [ ] WI-2 — ch02-entropy.md に 2.6 + 2.7 の Verified 再リンク 📋 (docs のみ、独立並行可)
- [ ] WI-3 — ch02-entropy.md に 2.9 節追加 + 未形式化リストから 2.9 削除 📋 (WI-1 依存)
...
- [ ] WI-1.0 在庫差分確認 + skeleton (`IsSufficientStatistic` def + 主定理 sorry) 📋
- [ ] WI-1.1 主定理 body 実装 (le_antisymm 2 方向) 📋
- [ ] WI-1.2 (stretch) Neyman-Fisher 因子分解 同値補題 📋

### ゴール

新規 file `Common2026/Shannon/SufficientStatistic.lean`。Cover-Thomas 2.9:
T が θ に対し sufficient (= chain `X → T(X) → θ` が Markov) ⟹ **I(θ; X) = I(θ; T(X))**。

### 実装者向け brief

**触る file**:
- 新規: `Common2026/Shannon/SufficientStatistic.lean`
- 完了時: `Common2026.lean` に `import Common2026.Shannon.SufficientStatistic` を 1 行追加。
...
-- Common2026/Shannon/MutualInfo.lean:96
...
**検証コマンド**: `lake env lean Common2026/Shannon/SufficientStatistic.lean` が 0 errors
...
- 既存定理 1: ``klDivPmf_eq_zero_iff_pmf`` (`Common2026/Shannon/MaxEntropyConstrained.lean:287`)
  ```lean
...
- 既存定理 2: ``klDivPmf_nonneg`` (`Common2026/Shannon/CsiszarProjection.lean:62`)
  ```lean
...
- 既存定理: ``log_sum_inequality`` (`Common2026/Shannon/LZ78ZivEntropyBridge.lean:71`)
  ```lean
...
- 補足リンク (任意): negMulLog 形 ``log_sum_inequality_negMulLog`` (`Common2026/Fano/DPI.lean:45`、
...
- `IsSufficientStatistic` (`Common2026/Shannon/SufficientStatistic.lean`) — markov-form 定義。
  本文で「`IsMarkovChain` の compProd 等式形で sufficiency を定義した」旨を 1 行補足
  (在庫 §所見 #2: 教科書のマルコフ連鎖直感とのギャップ説明が要る)。
- `mutualInfo_eq_of_sufficient` (`Common2026/Shannon/SufficientStatistic.lean`) — 主定理
  `I(θ; X) = I(θ; T(X))`。
