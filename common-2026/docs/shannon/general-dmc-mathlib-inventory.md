...
### A. 既存 `Common2026/Shannon/` channel 抽象

| 概念 | API (project 内) | file:line | 状態 | I-2 での扱い |
|---|---|---|---|---|
| `Channel α β := Kernel α β` (abbrev) | `Common2026.Shannon.ChannelCoding.Channel` | `ChannelCoding.lean:49` | ✅ 既存 (abbrev) | `BlockwiseChannel` の構成要素として **再利用** |
...
### E. 既存 `Common2026/Shannon/` の MI / chain rule API (block input MI bound 用)
...
新規ファイル `Common2026/Shannon/BlockwiseChannel.lean` の出だし:
...
import Common2026.Shannon.EntropyRate -- for limit form template
...
着手準備 OK。新規ファイル `Common2026/Shannon/BlockwiseChannel.lean` (400-500 行見込み) + 既存 `Common2026/Shannon/ChannelCoding.lean` 系の 0-sorry 維持 (callsite 13 箇所は touch しない方針)。
