...
- **Headline (改善対象)**: `lz78_asymptotic_optimality`
  (`InformationTheory/Shannon/LempelZiv78.lean:382-419`) — Cover-Thomas Theorem
...
- **Goal**: 4 hyp → 2 hyp に削減した中間 headline
  ...
  (`lz78PhraseStrings_count_isBigO` `InformationTheory/Shannon/LZ78ZivCountingBody.lean:408`、
...
archeology で確認 — 削除済 `InformationTheory/Draft/Shannon/LZ78DistinctEncoding.lean`
...
`InformationTheory/Shannon/` 配下に sorry なし live**。scope expansion (削除済
...
| 削除済 `lz78DistinctEncodingLength` 旧 def | `git show "f67ec8a^:common-2026/InformationTheory/Draft/Shannon/LZ78DistinctEncoding.lean":128-129` | `noncomputable def lz78DistinctEncodingLength (n : ℕ) (x : Fin n → α) : ℕ := (lz78DistinctParsing (List.ofFn x)).encodingLength (Fintype.card α)` |
| 削除済 `lz78DistinctEncodingLength_isBoundedUnder_le` 旧 body サイズ | `git show "f67ec8a^:common-2026/InformationTheory/Draft/Shannon/LZ78DistinctEncoding.lean":331-372` | 41 行 (`Filter.Eventually.of_forall` + `lz78DistinctEncodingLength_rate_isBigO_one` 経由)、sorry なし |
| 削除済 `lz78DistinctEncodingLength_isBoundedUnder_ge` 旧 body サイズ | `git show "f67ec8a^:common-2026/InformationTheory/Draft/Shannon/LZ78DistinctEncoding.lean":374-388` | 14 行 (`isBoundedUnder_of_eventually_ge (a := 0)` + `_per_symbol_nonneg`)、sorry なし |
| 旧 §4 headline (load-bearing bundling defect、本 plan は復活させない) | `git show "f67ec8a^:common-2026/InformationTheory/Draft/Shannon/LZ78DistinctEncoding.lean":414-432` | `IsLZ78AchievabilityChainHyp` + `IsLZ78ConverseChainHyp` 述語を hypothesis に取って body `sorry`、tier-5 load-bearing hypothesis bundling defect。本 plan は **代わりに `h_lower` / `h_upper` 直接 hyp 形** で再構築 |
...
| 対象 | 役割 | 行数見積もり |
|---|---|---|
| `InformationTheory/Shannon/LempelZiv78.lean` (既存拡張) or `InformationTheory/Shannon/LZ78DistinctEncoding.lean` (新規) | (A) `lz78DistinctEncodingLength` 定義復活 (削除済 `Draft/.../LZ78DistinctEncoding.lean:128` から cherry-pick) + (B) Phase B 計数包絡経由の rate `=O 1` 補題 + (C) `_isBoundedUnder_le` / `_ge` 補題 2 件 + (D) 中間 headline `lz78_asymptotic_optimality_bdd_free` | ~250-350 (削除済 §1-§3 ~280 行 + 中間 headline ~30 行) |
| `InformationTheory.lean` | 新規 file 追加時のみ import 1 行追加 | +1 |
| `InformationTheory/Shannon/LempelZiv78.lean` 既存 (line 380-529) | 変更なし (`lz78_asymptotic_optimality` / `_two_sided` / `_of_bounds` は不変、本 plan は **新規 headline** を追加するだけ) | 0 |
...
[既存 main 在庫 (sorry 0)]                    [削除済 §1-§3 から cherry-pick]    [既存 main headline]
  ...
  Real.log / Nat.log Mathlib API                lz78DistinctEncodingLength_rate_isBigO_one
       ...
       Phase V  lake env lean silent + `InformationTheory.lean` 編入 + honesty audit + proof-log
```
...
- **新規 file `LZ78DistinctEncoding.lean`** (推奨): 削除済 file との対応が
  ...
  `InformationTheory.lean` に新 import 1 行追加、依存は `LempelZiv78.lean` + 既存
...
Phase 3 完了 → 最終 commit + InformationTheory.lean 編入 + honesty audit。

### 規模見積もり

| Sub-step | 内容 | 行数 | 依存 | Mathlib / InformationTheory 在庫 |
|---|---|---|---|---|
...
| Phase V | lake env lean silent + `InformationTheory.lean` 編入 + honesty audit dispatch + proof-log | 0-10 | Phase 3 | — |
...
- [ ] Phase V — verify + `InformationTheory.lean` 編入 + honesty audit + proof-log 📋
...
- [ ] (P0-1) `git show "f67ec8a^:common-2026/InformationTheory/Draft/Shannon/LZ78DistinctEncoding.lean"`
...
- [ ] (P0-4) **判断ログ #1 (file 配置)**: 新規 `LZ78DistinctEncoding.lean` vs
      `LempelZiv78.lean` 拡張。新規 file 路線で `InformationTheory.lean` に import 1 行
...
- [ ] (P3-2) `@[entry_point]` マーキング (`InformationTheory/Meta/EntryPoint.lean`)
- [ ] (P3-V) lake env lean silent + `InformationTheory.lean` 編入 (新 file の import 1 行追加)

### Phase V — 検証 + closure

- [ ] (PV-1) lake env lean 新 file silent (0 errors / 0 sorry warning、proof done
      直行を verify)
- [ ] (PV-2) `InformationTheory.lean` 編入後、`lake env lean InformationTheory.lean` または
      `lake build InformationTheory.<new-file>` で olean refresh
...
- `lz78_asymptotic_optimality_bdd_free` (2 hyp form、distinct 固定) は **新規追加**、
  既存 file に追加 import が入る (`InformationTheory.lean` に 1 行) のみ
...
1. **(Phase 0 で確定予定) file 配置**: 新規 `InformationTheory/Shannon/LZ78DistinctEncoding.lean`
...
- (Q3) `InformationTheory.lean` の現行 import 順序に新規 `LZ78DistinctEncoding.lean` を
...
| `LZ78TreeInducedAEP.lean` | 288 | — | 不要 (`InformationTheory/Shannon/` 直下から削除) | 削除済据置 |
| `LZ78ZivTreeBridge.lean` | 158 | — | 不要 (同上) | 削除済据置 |

| 現存 main file (依存先) | 行数 | sorry 数 | 本 plan で参照 |
|---|---|---|---|
| `InformationTheory/Shannon/LempelZiv78.lean` | 532 | 0 | 既存 `lz78_asymptotic_optimality` を Phase 3 で internal call、変更なし |
| `InformationTheory/Shannon/LZ78GreedyParsing.lean` | 544 | 0 | `LZ78Phrase.bitLength` / `LZ78Parsing.encodingLength` 経由参照 |
| `InformationTheory/Shannon/LZ78GreedyParsingImpl.lean` | 474 | 0 | 直接参照なし (delete file 当時の import) |
| `InformationTheory/Shannon/LZ78GreedyLongestPrefix.lean` | 271 | 0 | `lz78PhraseStrings_count_le` 経由参照 |
| `InformationTheory/Shannon/LZ78ZivCountingBody.lean` | 420 | 0 | `lz78PhraseStrings_count_isBigO` 経由参照 (Cover-Thomas Eq. 13.124、本 plan の Phase 1-G の鍵) |
| `InformationTheory/Shannon/LZ78PhraseCountAsymptoticBody.lean` | 243 | 0 | 間接参照 (`LZ78ZivCountingBody.lean` 経由) |
| `InformationTheory/Shannon/LZ78ZivInequality.lean` | 307 | 0 | 直接参照なし |
