...
> - `InformationTheory/Shannon/HuffmanOptimality.lean` (1054 行 / 0 sorry / weak form publish 済)
> - `InformationTheory/Shannon/Huffman.lean` (961 行 / 0 sorry、`huffmanLength_kraft_eq_one` 含む)
> - `InformationTheory/Shannon/ShannonCode.lean`, `ShannonCodeKraftReverse.lean`.
>
...
> `InformationTheory/Shannon/HuffmanOptimality.lean` **末尾追記** で publish (新規 file 不要、
> 同 namespace 内完結)。
...
```lean
-- `InformationTheory/Shannon/HuffmanOptimality.lean` 末尾に追記 (新規 file 不要)
...
- **完了済 (黒箱 reuse、書き換えない)**:
  - **T1-A**: `InformationTheory/Shannon/Huffman.lean` (961 行 / 0 sorry).
  ...
  - **T1-A'**: `InformationTheory/Shannon/HuffmanOptimality.lean` (1054 行 / 0 sorry).
...
- **InformationTheory.lean**: import 追記不要 (HuffmanOptimality.lean 末尾追記のため import 関係不変)。

## File / module layout

### 編集対象: `InformationTheory/Shannon/HuffmanOptimality.lean` (末尾追記のみ)
...
`InformationTheory.lean` への変更: **なし**。
...
- [ ] **V.1** `lake env lean InformationTheory/Shannon/HuffmanOptimality.lean` で 0 sorry / 0 error /
  最小 warning. 想定最終 ~1450 行.
- [ ] **V.2** regression check (既存 0-sorry ファイル):
  - `InformationTheory/Shannon/Huffman.lean` (961 行 / 0 sorry を維持).
...
- [ ] **V.3** `lake build InformationTheory` で全 silent (~1 回のみ、最終 sanity check).
...
- [ ] T1-A 既存 `InformationTheory/Shannon/Huffman.lean` は **不変** (961 行 / 0 sorry).
...
- [ ] `InformationTheory.lean` import 不変 (新規 file 追加なし).
...
> `InformationTheory/Shannon/HuffmanOptimality.lean` 末尾 (line 1054 `end ...` 直前) に
...
- 既存実装 (T1-A): `InformationTheory/Shannon/Huffman.lean` (961 行 / 0 sorry).
- 既存実装 (T1-A'): `InformationTheory/Shannon/HuffmanOptimality.lean` (1054 行 / 0 sorry, weak form).
- T1-A' 既存 API key 群 (`InformationTheory/Shannon/HuffmanOptimality.lean`):
  - `mergedMeasure`: `:244`
...
- T1-A 既存 API key 群 (`InformationTheory/Shannon/Huffman.lean`):
...
2. **2026-05-19 着手評価 — 案 Y (no-op) 採用、partial publish も見送り、技術メモを残す**:
   ...
   化が必要だが、InformationTheory / Mathlib に該当 API なし. 代替は kraft sum identity の
   ...
   **Definition of Done 状態**: 既存 `lake env lean InformationTheory/Shannon/HuffmanOptimality.lean`
...
3. **2026-05-19 再着手評価 #2 — 案 Y 再採用、no-op 維持**: 判断ログ #2 以後の Mathlib 状態と
   ...
   **Definition of Done 状態**: 既存 `InformationTheory/Shannon/HuffmanOptimality.lean` は不変
