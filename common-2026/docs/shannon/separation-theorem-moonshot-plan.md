...
> - `InformationTheory/Shannon/AEP.lean:1138` — `source_coding_achievability`
>   (IID source achievability, `Tendsto rate (𝓝 R)` + `Tendsto errorProb (𝓝 0)`)
> - `InformationTheory/Shannon/AEP.lean:704` — `source_coding_converse`
>   (IID source converse, `entropy ≤ Filter.liminf (log M_n / n)`)
> - `InformationTheory/Shannon/AEP.lean:580` — `source_coding_per_n_bound` (per-n Fano 形)
> - `InformationTheory/Shannon/AEPRate.lean` — rate scaling 用の `Tendsto` lemma 群
> - `InformationTheory/Shannon/ChannelCodingShannonTheoremFullDischarge.lean:1588` —
>   `shannon_noisy_channel_coding_theorem_general_full` (DMC achievability, max-error 形,
>   `hW_pos` 全 discharge 済)
> - `InformationTheory/Shannon/ChannelCodingConverseMemorylessPure.lean:650` —
>   `channel_coding_converse_general_memoryless_pure` (DMC converse, semi-pure Fano 形)
> - `InformationTheory/Shannon/ChannelCoding.lean` — `Code M n α β` bundle, `Code.errorProbAt`,
>   `Code.averageErrorProb`
> - `InformationTheory/Shannon/BlockwiseChannel.lean:111, :1181` — `BlockwiseChannel.ofMemoryless`,
>   `capacity_lim_eq_capacity_of_memoryless`
> - `InformationTheory/Shannon/ChannelCodingShannonTheorem.lean:102` — `capacity W` 定義
>
> **Goal (短形)**: 新規ファイル `InformationTheory/Shannon/SeparationTheorem.lean` で
...
rate scaling / Tendsto↔liminf bridge の 4 件が **Mathlib + InformationTheory 完全不在の novel 構造**
として残るが、それ以外は既存 publish 済 black-box の合成で組める。Tier 1 (achievability) →
Tier 2 (converse) の段階的 publish 設計で着手準備完了。

## 進捗

- [x] Phase 0 — Mathlib + InformationTheory 在庫 ✅ → [`separation-theorem-mathlib-inventory.md`](./separation-theorem-mathlib-inventory.md)
...
- [x] Phase C — Converse 主定理 (Tier 2) ✅ (`separation_converse_iid`、hypothesis pass-through 形、+~120 行)
- [x] Phase D — 主定理 wrapper + library 編入 ✅ (InformationTheory.lean に import 追記済)
...
新規ファイル `InformationTheory/Shannon/SeparationTheorem.lean` で 4 件 publish:
...
論証は 3 段に分解できるが、Mathlib + InformationTheory では **(1) source side / (2) channel side
...
InformationTheory.lean          ← `import InformationTheory.Shannon.SeparationTheorem` 追記
```
...
- [x] `InformationTheory/Shannon/AEP.lean:1138` — `source_coding_achievability` (IID source 完成形)
- [x] `InformationTheory/Shannon/AEP.lean:704` — `source_coding_converse` (IID source converse、`liminf` 形)
- [x] `InformationTheory/Shannon/AEP.lean:580` — `source_coding_per_n_bound` (per-n Fano 形)
- [x] `InformationTheory/Shannon/AEPRate.lean` — rate scaling 用の `Tendsto` 補題群
- [x] `InformationTheory/Shannon/ChannelCodingShannonTheoremFullDischarge.lean:1588` —
  `shannon_noisy_channel_coding_theorem_general_full` (DMC achievability、`hW_pos` 全 discharge 済)
- [x] `InformationTheory/Shannon/ChannelCodingConverseMemorylessPure.lean:650` —
  `channel_coding_converse_general_memoryless_pure` (DMC converse、semi-pure Fano 形)
- [x] `InformationTheory/Shannon/ChannelCoding.lean` — `Code M n α β` bundle, `Code.errorProbAt`,
  `Code.averageErrorProb`, `Channel α β`
- [x] `InformationTheory/Shannon/BlockwiseChannel.lean:111, :1181` — blockwise kernel + per-letter / blockwise
  capacity 同一性
- [x] `InformationTheory/Shannon/ChannelCodingShannonTheorem.lean:102` — `capacity` 定義
...
- `lake env lean InformationTheory/Shannon/SeparationTheorem.lean` で skeleton clean
...
- [ ] **A-0 skeleton**: `SeparationTheorem.lean` 新規ファイルに全主定理 + 補助補題を
  `:= by sorry` で並べた skeleton を Write。`import InformationTheory.Shannon.{AEP, AEPRate,
...
- `lake env lean InformationTheory/Shannon/SeparationTheorem.lean` clean (Phase C / D は sorry 残し可)
...
- `lake env lean InformationTheory/Shannon/SeparationTheorem.lean` clean
...
- `InformationTheory.lean` に `import InformationTheory.Shannon.SeparationTheorem` 追記
...
- [ ] **D-2 `InformationTheory.lean` 編入**: `import InformationTheory.Shannon.SeparationTheorem` 追記

- [ ] **D-3 最終 verify**: `lake env lean InformationTheory.lean` clean 確認 + `InformationTheory/Shannon/`
  全体回帰チェック (`lake env lean InformationTheory/Shannon/AEP.lean` 等が dependent 経由で
  壊れていないか)
...
- **D-3 で dependent 経由 break** → 該当 file に oleans refresh
  (`lake build InformationTheory.Shannon.SeparationTheorem` 1 回)。CLAUDE.md "After upstream edits"
  節参照。
...
2. **(2026-05-19) Composition primitive 自作を確定 (roadmap の「composition なので新数学なし」
   ...
   が **definition-level で measure space が完全不一致**、Mathlib にも InformationTheory にも source–channel
...
3. **(2026-05-19) 規模見積を roadmap 300–500 → ~400 行 (Tier 2 / IID 限定) に再評価**: 在庫調査
   §K-L で「source side / channel side / capacity / entropy 基盤は 100% 既存 (Mathlib + InformationTheory
...
4. **(2026-05-19) Tier 1 (achievability) ship 完了、撤退ライン L-S3 採用 (avg-error 形)**:
   ...
   **Tier 1 公開 API** (`InformationTheory.Shannon.SeparationTheorem`):
...
5. **(2026-05-19) Tier 2 (converse) ship 完了、hypothesis pass-through 形採用**:
   `separation_converse_iid` を「composed error → 0 かつ channel-side rate ≤ capacity ⇒
   entropy ≤ capacity」の hypothesis pass-through 形で publish。channel-side 仮説
   `hM_ch_bdd : ∀ n, log (M_ch n) / n ≤ R_ch` + `hR_ch_le_C : R_ch ≤ capacity W` は
   **future channel converse plan** (`separation-theorem-converse-channel-bridge-*`) で
   `channel_coding_converse_general_memoryless_pure` 経由で discharge 可能 (撤退ライン L-S2
   部分発動: uniform-message bridge は本 plan 外)。証明は **source_coding_converse** (AEP.lean:704)
   + `Filter.liminf_le_liminf` (log M_src n / n ≤ log M_ch n / n を per-n で取って liminf 単調性)
   + `Filter.liminf_le_of_frequently_le` (liminf ≤ R_ch from per-n upper bound) の 3 piece
   composition。**Tier 2 規模実測**: +120 行 (中央予測 +100 と整合)、`SeparationTheorem.lean`
   全体 506 行 (Tier 0 + Tier 1 + Tier 2)、plan 中央予測 350-500 行に収まる。
   `sourceError_tendsto_zero_of_composed` (composed → 0 から source-side → 0 への non-negative
   sandwich) を helper として切り出し、`sourceError_lt_eventually` (achievability 用) と対称形。

   **Tier 2 公開 API 追加** (`InformationTheory.Shannon.SeparationTheorem`):
   - `separation_converse_iid`: 主定理 (pass-through 形、channel-side rate ≤ capacity は仮説)

   **本 plan で残された future work**: channel-side rate ≤ capacity を IID source-encoded index
   に対して discharge する uniform-message bridge (撤退ライン L-S2、`hMsg_uniform` 近似)。
   別 seed `separation-theorem-converse-channel-bridge-*` で扱う。
