...
`sorry-migration-runbook.md`「並列実行候補 family」Round 3 設計時に **PGPC は EPI/Stam 依存ありの可能性** と注記されていたが、実コード `rg -n 'import InformationTheory.Shannon.EPI|import InformationTheory.Shannon.Stam|wall:stam|wall:n-dim-gaussian-aep' InformationTheory/Shannon/ParallelGaussianPerCoord*.lean` で **0 hit**。逆方向 `rg -n 'ParallelGaussianPerCoord' InformationTheory/Shannon/EPI*.lean InformationTheory/Shannon/Stam*.lean` も 0 hit。
...
`rg -n 'rate_distortion_converse_n_letter_singleLetter|rateDistortionFunction_convexOn|rate_distortion_achievability_witness_form|rate_distortion_achievability_partial_discharge|codebookAvgFailureStrong_tendsto_zero|rate_distortion_achievability\b' InformationTheory/ --type=lean | grep -v RateDistortion` で **1 hit**:

- `InformationTheory/Shannon/WynerZivConverseChain.lean:10` — docstring 散文 `... in the same style as rate_distortion_converse_n_letter_singleLetter ...` (import なし)。これは runbook Pattern G 3-段判定の **S1 (散文 reference)** = 本 sweep 単独で実施可能、Phase 2.RD で散文を更新するのみ (実際には WynerZiv 側のみが該当 declaration 名に言及しているので RD sweep が deprecate / signature 変更しても WynerZiv 側 prose の更新だけで済む)。
...
これら **同 family 内の S2 import 依存**は、本 plan の Phase 2.PGPC でタグ削除 (signature 改変なし) だけを行うため**波及なし**。olean refresh も不要 (signature 維持)。Pattern A (stale olean) 回避策は formally 不要だが、念のため Phase V で `lake env lean InformationTheory/Shannon/ParallelGaussianKKT.lean` 1 件のみ再 verify。
...
- [ ] **1.1** `RateDistortionAchievabilityPhaseEStrongFinal.lean:1638` `rate_distortion_achievability` (V wrapper alias) の `@audit:suspect()` タグ削除。signature 改変不要、`_strong` 版へ delegation する body もそのまま。
  - `lake env lean InformationTheory/Shannon/RateDistortionAchievabilityPhaseEStrongFinal.lean` で type-check done 確認。
...
**Phase 1 DoD**: 1 件で `@audit:suspect()` 0 件、新規 `sorry` 0 件、`lake env lean InformationTheory/Shannon/RateDistortionAchievabilityPhaseEStrongFinal.lean` 0 errors。
...
- [ ] **2.RD.1.a** `rate_distortion_converse_n_letter_singleLetter` (`:262`):
  ...
  - `lake env lean InformationTheory/Shannon/RateDistortionConverseNLetter.lean` 0 errors 確認。
...
- [ ] **2.RD.2.a** `rateDistortionFunction_convexOn` (`:138`):
  ...
  - `lake env lean InformationTheory/Shannon/RateDistortionConvexity.lean` 0 errors 確認。
...
- [ ] **2.RD.3.a** `rate_distortion_achievability_witness_form` (`:87`):
  ...
  - `lake env lean InformationTheory/Shannon/RateDistortionAchievabilityPhaseE.lean` 0 errors 確認。
...
- [ ] **2.RD.4.a** `rate_distortion_achievability_partial_discharge` (`:285`):
  ...
  - `lake env lean InformationTheory/Shannon/RateDistortionAchievabilityPhaseEDischarge.lean` 0 errors 確認。
...
- [ ] **2.RD.5.a** `codebookAvgFailureStrong_tendsto_zero` (`:737`):
  ...
  - `lake env lean InformationTheory/Shannon/RateDistortionAchievabilityPhaseEStrongFinal.lean` 0 errors 確認。
...
- [ ] **2.PGPC.d** `lake env lean InformationTheory/Shannon/ParallelGaussianPerCoord.lean` + `InformationTheory/Shannon/ParallelGaussianPerCoordRegularity.lean` 0 errors 確認。同 family 内 S2 import 依存 file の olean refresh: `lake env lean InformationTheory/Shannon/ParallelGaussianKKT.lean` 1 件のみ念のため再 verify (タグ削除のみで signature 変更なし、波及は想定上 0)。

**Phase 2.PGPC DoD**: 5 件で `@audit:closed-by-successor(...)` 0 件、新規 `sorry` 0 件、`lake env lean` 0 errors。`InformationTheory.lean` import 行変更なし。
...
- [ ] **V.1** 全 7 file で `lake env lean` 確認。Phase 2.RD で signature 改変 4 件 (Phase 2.RD.1〜2.RD.4) があったため、dependent file の olean refresh:
  ```bash
  lake build InformationTheory.Shannon.RateDistortionConverseNLetter
  lake build InformationTheory.Shannon.RateDistortionConvexity
  lake build InformationTheory.Shannon.RateDistortionAchievabilityPhaseE
  lake build InformationTheory.Shannon.RateDistortionAchievabilityPhaseEDischarge
  ```
  その後 `RateDistortionAchievabilityPhaseEStrongFinal.lean` (Phase 2.RD.5 の上流) を再 verify。
- [ ] **V.2** 集計コマンド実行:
  ```bash
  rg '@audit:suspect' InformationTheory/Shannon/RateDistortion*.lean | wc -l   # = 0
  rg '@audit:closed-by-successor' InformationTheory/Shannon/ParallelGaussianPerCoord*.lean | wc -l   # = 0
  rg '@residual\(plan:rate-distortion-' InformationTheory/Shannon/RateDistortion*.lean | wc -l
  rg -nw 'sorry' InformationTheory/Shannon/RateDistortion*.lean InformationTheory/Shannon/ParallelGaussianPerCoord*.lean
  ```
...
2. **2026-05-26 Round 3 PGPC dependency 仮定の verbatim 逆検証**: runbook が Round 3 設計時に「PGPC は EPI/Stam dependency 要確認」と注記していたが、`InformationTheory/Shannon/ParallelGaussianPerCoord*.lean` の `import` 文 + 逆方向 `rg` で **依存 0** と verbatim 確認。PGPC は独立 sweep 可能、escalate 不要と判定。`CLAUDE.md`「同じ verbatim 確認義務は依存方向 / Phase 順序 / wrapper 呼出方向 / import cycle にも適用」の実例として handoff finding に反映。
