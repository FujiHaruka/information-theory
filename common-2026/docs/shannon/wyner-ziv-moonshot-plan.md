...
> - `Common2026/Shannon/SlepianWolfBinning.lean` — random binning + collision bound
> - `Common2026/Shannon/SlepianWolfConditionalTypicalSlice.lean` — `T_{X|Y}^n(y)` 濃度
> - `Common2026/Shannon/SlepianWolfFullRateRegion.lean` — 4-way error decomposition + jointly typical decoder
> - `Common2026/Shannon/RateDistortionAchievability.lean` — `LossyCode`, `mutualInfoPmf`, `RDConstraint`, `rateDistortionFunctionPmf`, `rateDistortionFunctionPmf_attained`, `continuous_mutualInfoPmf`
> - `Common2026/Shannon/RateDistortionConverse.lean` — single-shot RD converse 4-step chain
> - `Common2026/Shannon/RateDistortionConverseNLetter.lean` — `rateDistortionFunction_le_mutualInfo_perLetter`, n-letter Jensen + antitone hypothesis pass-through pattern
...
> - `Common2026/Shannon/ChannelCoding.lean` — `jointSequence`, `jointlyTypicalSet`, `jointlyTypicalSet_card_le`
> - `Common2026/Shannon/AEP.lean` + `AEPRate.lean` — IID source AEP + rate scaling Tendsto 補題
> - `Common2026/Fano/Measure.lean` — `fano_inequality_measure_theoretic`, `errorProb`
>
> **Goal (短形)**: 新規 3 ファイル `Common2026/Shannon/WynerZiv{,Achievability,Converse}.lean`
...
- [x] Phase 0 — Mathlib + 既存 Common2026 在庫 + 設計確定 📋 → [`wyner-ziv-mathlib-inventory.md`](./wyner-ziv-mathlib-inventory.md)
...
```
Phase 0  : Mathlib + Common2026 在庫 + SBS instance 戦略確定           ← 完了予定 (本 plan 起草と並行)
           ────────────────────────────────────────────────────────────
...
Phase V  : lake env lean 3 ファイル clean + Common2026.lean 編入        ← ~10 行
```
...
| Phase V | **10 行** | 5-15 | `Common2026.lean` 追記 |
...
Common2026.lean               ← `import Common2026.Shannon.WynerZiv{,Achievability,Converse}` 追記
```
...
- [x] `Common2026/Shannon/SlepianWolfBinning.lean` — `binningMeasure`, `binningMeasure_singleton_real`, `binning_collision_prob`
- [x] `Common2026/Shannon/SlepianWolfConditionalTypicalSlice.lean` — `conditionalTypicalSlice` + card bound
- [x] `Common2026/Shannon/SlepianWolfFullRateRegion.lean` — `swJointTypicalDecoder`, 4-way error decomposition
- [x] `Common2026/Shannon/RateDistortionAchievability.lean` — `LossyCode`, `mutualInfoPmf`, `RDConstraint`, `rateDistortionFunctionPmf`, `rateDistortionFunctionPmf_attained`, `continuous_mutualInfoPmf`
- [x] `Common2026/Shannon/RateDistortionConverse.lean:133` — `rate_distortion_converse_single_shot` (4-step chain 雛形)
- [x] `Common2026/Shannon/RateDistortionConverseNLetter.lean:207` — `rateDistortionFunction_le_mutualInfo_perLetter`
- [x] `Common2026/Shannon/CondMutualInfo.lean:46` — `condMutualInfo` (`[StandardBorelSpace X][StandardBorelSpace Y]`)
- [x] `Common2026/Shannon/CondMutualInfo.lean:71` — `IsMarkovChain` (γ-form, 条件付け side が真ん中)
...
- [x] `Common2026/Shannon/MutualInfo.lean:36` — `mutualInfo` (KL form)
- [x] `Common2026/Shannon/MutualInfo.lean:93` — `mutualInfo_comm`
- [x] `Common2026/Shannon/MutualInfo.lean:192` — `mutualInfo_ne_top`
- [x] `Common2026/Shannon/ChannelCoding.lean:275, :301, :340` — `jointSequence`, `jointlyTypicalSet`, `jointlyTypicalSet_card_le`
- [x] `Common2026/Shannon/AEPRate.lean:395` — `jointlyTypicalSet_prob_ge_of_rate`
- [x] `Common2026/Shannon/Entropy.lean:41, :208, :240` — `entropy_pair_eq_entropy_add_condEntropy`, `condMutualInfo_eq_condEntropy_sub_condEntropy`, `condEntropy_le_condEntropy_of_pair`
...
- [x] `Common2026/Fano/Measure.lean:224` — `fano_inequality_measure_theoretic`
...
- 軸 2: `[StandardBorelSpace U]` instance を `[Fintype + MSC]` から derive する戦略確定
  (`Common2026/Shannon/WynerZiv.lean` 内で local instance 化 vs 別 file `WynerZivStandardBorel.lean`
...
- Phase A skeleton (`Common2026/Shannon/WynerZiv.lean` の sorry-driven 出だし 95 行) が
...
- `lake env lean Common2026/Shannon/WynerZiv.lean` clean (Phase B/C は別 file で `sorry`)
...
- [ ] **A-9 `lake env lean Common2026/Shannon/WynerZiv.lean`** clean 確認 (Phase A 完了)
...
- `lake env lean Common2026/Shannon/WynerZivAchievability.lean` clean

### ステップ

- [ ] **B-0 skeleton**: `WynerZivAchievability.lean` 新規ファイル。`import Common2026.Shannon.WynerZiv`
...
- [ ] **B-6 `lake env lean Common2026/Shannon/WynerZivAchievability.lean`** clean (Phase B 完了)
...
- `lake env lean Common2026/Shannon/WynerZivConverse.lean` clean

### ステップ

- [ ] **C-0 skeleton**: `WynerZivConverse.lean` 新規ファイル。`import Common2026.Shannon.WynerZiv`
...
- [ ] **C-6 `lake env lean Common2026/Shannon/WynerZivConverse.lean`** clean (Phase C 完了)
...
## Phase V — `lake env lean` 3 ファイル clean + `Common2026.lean` 編入 📋
...
- [ ] **V-1 `Common2026.lean` 編入**: 既存の `import Common2026.Shannon.RateDistortion*` /
  `import Common2026.Shannon.SlepianWolf*` の **後** に
  ```lean
  ...
  ```
  を追記 (オーケストレータが実施)。
- [ ] **V-2 3 ファイル clean 確認**: `lake env lean Common2026/Shannon/WynerZiv.lean`,
  `WynerZivAchievability.lean`, `WynerZivConverse.lean` 全て silent。
- [ ] **V-3 全体回帰チェック**: `lake env lean Common2026.lean` clean を確認 (依存 file の
  olean refresh が必要な場合は `lake build Common2026.Shannon.WynerZiv*` 1 回)。

### 工数感

**0.1-0.2 セッション (~5-15 行 of Common2026.lean diff)**。proof-log: no。

### Done 条件

- `Common2026.lean` に 3 ファイル import 追記済
...
- **L-WP1**: **`[StandardBorelSpace U]` instance の local 配置が既存 file に衝突**
  ...
  - 縮退案 (2 段):
    ...
    - **(L-WP1b)** instance 不要な形に statement を書き換え (`Common2026/Shannon/Entropy.lean`
...
1. **Phase 0 (Mathlib + Common2026 在庫 + 設計確定)** — `wyner-ziv-mathlib-inventory.md` 本 plan
   と並行起草中。完了次第 Phase A 着手判定。
2. **Phase A skeleton 作成** ← Phase 0 完了後の次これ
   - `Common2026/Shannon/WynerZiv.lean` 新規 (在庫 §7 の 95 行 skeleton を基盤に)
...
6. **Phase D 主定理 wrapper + Phase V `Common2026.lean` 編入** (0.5-0.7 セッション)
...
- 既存実装 (黒箱 reuse):
  - `Common2026/Shannon/SlepianWolfBinning.lean:62` `binningMeasure`
  - `Common2026/Shannon/SlepianWolfFullRateRegion.lean:35` `swJointTypicalDecoder`
  - `Common2026/Shannon/RateDistortionAchievability.lean:293` `rateDistortionFunctionPmf`
  - `Common2026/Shannon/RateDistortionConverseNLetter.lean:207` `rateDistortionFunction_le_mutualInfo_perLetter`
...
1. **実装 agent は `Common2026.lean` ルートを編集しない** — Phase V `Common2026.lean` 編入は
