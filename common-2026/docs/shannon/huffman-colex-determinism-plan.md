...
`Common2026/Shannon/HuffmanStrongForm.lean` 末尾追記 (または同 namespace 新規 file、§file 配置):
...
- `HuffmanSwapNormProof.lean` は Common2026 依存ゼロで insulated (決定化の影響を受けない)。
...
- Phase M wrapper は `HuffmanStrongForm.lean` 末尾 (`mergedHuffmanAuxIdent_proof` +
  `huffmanLength_optimal`)。`Common2026.lean` の import は Huffman* 登録済、新規 file 時のみ追記。
...
- [ ] `lake build Common2026.Shannon.Huffman` で olean refresh 後、Huffman* 系を個別
