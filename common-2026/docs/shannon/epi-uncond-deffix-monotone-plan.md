...
- [~] **P2 拡張単調性** `entropyPowerExt_mono_add` (`EPIUncondMonotone.lean`、gateway atom) — **restructure 済 (§7-6) → 第一 chunk 着地 (§7-7)**: mono/top 伝播/(i-a) は ①③ 合成で genuine modulo ②、唯一の sorry = ② EReal chain rule (`EPIUncondCondEntropyExt.lean`、`@residual(plan:...)`、独立監査 honest_residual PASS)。**② 攻略は §7-6/§7-7 が SoT** (道 A self-build、multi-session moonshot、型壁で 2-3 session 詰まれば §7-4 で `wall:` 昇格)。
  - **✅ 2026-06-08: finite-entropy 版 ② genuine 着地**。step a'/b'/c' (`klDiv_compProd_lintegral` / `lintegral_condDistrib_cross_eq`+`_marginal_eq` / `klDiv_negMulLog_cross_balance_ennreal`、全 `@audit:ok`) + EReal helper `ereal_sub_eq_sub_add_of_ennreal_balance` を結線し、`differentialEntropyExt_eq_condEntExt_add_klDiv_of_finite` (`EPIUncondCondEntropyExt.lean:262`、11 regularity 仮説、sorryAx-free、`@audit:ok`) として完成。**finiteness-free 版 ② は本ルート (per-fibre (☆) の mass 相殺) で証明不能と確定** (proof-pivot-advisor 裏取り、反例 `KL=∞` fibre)。無条件版 ② (`:234`、sorry 残置) は **route β' (truncation+LSC) で別途攻略** (L-Uncond-Y-roi / `:153` T2)。**次**: route β' plan 起草、または finite ② を消費する finite-entropy gateway wiring (consumer chain に `hκ_*` regularity を thread)。詳細 SoT → `epi-uncond-chainrule-ext-inventory.md` 冒頭「✅ 完了状態 (2026-06-08)」。
