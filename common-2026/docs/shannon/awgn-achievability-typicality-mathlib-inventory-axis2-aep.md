...
持っていく統合 lemma** はいずれも Mathlib 不在 + Common2026 不在。Phase B 全体を本物
...
- **🔴 不在**: Mathlib + Common2026 ともに無し、自作必要
...
| n-d differential entropy | — | — | 🔴 | Common2026 にも n-d 版なし。1-d しかない (`Common2026/Shannon/DifferentialEntropy.lean`)。本物 discharge には自作必要 |
...
### Cat-5 `differentialEntropy` AEP 系 (Common2026 既存 / Mathlib gap)

| 概念 | API | file:line | 状態 | Phase B での扱い |
|---|---|---|---|---|
| 1-d `differentialEntropy` 定義 | `InformationTheory.Shannon.differentialEntropy` | `Common2026/Shannon/DifferentialEntropy.lean:42` | 🟢 (Common2026) | Gaussian の場合の値 `(1/2) log (2πe v)` を `differentialEntropy_gaussianReal` で取得 |
| 1-d Gaussian entropy 値 | `InformationTheory.Shannon.differentialEntropy_gaussianReal` | `Common2026/Shannon/DifferentialEntropy.lean:406` | 🟢 (Common2026) | B-3 で SLLN 極限を `(1/2) log (2πe v)` に同定 |
| n-d differential entropy 定義 | — | — | 🔴 | Common2026 にも n-d 版なし、joint entropy `h(X^n, Y^n)` を直接扱える Mathlib-shape predicate は無い。B-3 直 discharge には自作 ~50-100 行 |
| (1/n) log p(X^n) → h(X) 連続 AEP | — | — | 🔴 | Mathlib 全体に **continuous AEP** 単体定理は存在せず。discrete 版 `aep_ae` (Common2026) を continuous に転用するには `[Fintype α]` を外す必要があり、`logLikelihood` も `(μ.map X).real {x}` から `rnDeriv` 経由に書き換える必要、API 不適合 |
| Shannon-McMillan-Breiman (continuous) | — | — | 🔴 | Mathlib 不在、Common2026 にも discrete sandwich のみ (`ShannonMcMillanBreiman.lean`, `[Fintype α]` 要求) |
...
- **file:line**: `Common2026/Shannon/DifferentialEntropy.lean:42`
- **signature** (verbatim):
  ```lean
...
4. **n-d differential entropy 定義** (~50 行、判断 #3 で形を確定)
   ...
   - Common2026 既存 1-d を Pi 形に lift
...
`Common2026/Shannon/AWGNAchievabilityDischarge.lean` の Phase B 部分 (skeleton 形、Phase A
