...
> InformationTheory 直 `rg` で本定理の Lean 化に必要な 7 軸 (R_WZ 定義 / auxiliary RV / Markov chain /
...
> `InformationTheory/Shannon/WynerZiv*.lean`) で別途立てる。

## サマリ (一行)

Phase achievability + converse のうち、**既存 InformationTheory / Mathlib で実体が既にある API は
...
|---|---|---|---|---|
| 相互情報量 `I(X;Y)` (KL form) | `mutualInfo` | `InformationTheory/Shannon/MutualInfo.lean:36` | ✅ 完備 | Phase A `I(X;U) - I(Y;U)` の RHS 両項を直接書ける |
| MI 対称性 | `mutualInfo_comm` | `InformationTheory/Shannon/MutualInfo.lean:93` | ✅ | Phase B / C で頻出 |
| MI 有限性 | `mutualInfo_ne_top` | `InformationTheory/Shannon/MutualInfo.lean:192` | ✅ (`[Fintype X][Fintype Y]`) | Phase C 全段 (`ENNReal.toReal_mono`) |
| 条件付き MI `I(X;Y\|Z)` | `condMutualInfo` | `InformationTheory/Shannon/CondMutualInfo.lean:46` | ✅ (`[StandardBorelSpace X][StandardBorelSpace Y]`) | Phase C converse の n-letter chain rule に必須 |
| 2 変数 MI chain rule | `mutualInfo_chain_rule` | `InformationTheory/Shannon/CondMutualInfo.lean:219` | ✅ | Csiszár's sum identity の 2 変数 base |
| n 変数 MI chain rule | `mutualInfo_chain_rule_fin` | `InformationTheory/Shannon/MIChainRule.lean:117` | ✅ (Fintype α `Fin n → α`) | Phase C n-letter converse |
| Markov chain `X → Z → Y` (γ-form) | `IsMarkovChain` | `InformationTheory/Shannon/CondMutualInfo.lean:71` | ✅ | `U − X − Y` 制約の Lean 化に直接対応 |
| Markov ⇒ MI bound | `mutualInfo_le_of_markov` | `InformationTheory/Shannon/CondMutualInfo.lean:378` | ✅ | DPI: `I(X;U) ≤ I(X;Z)` 形 |
| Markov post-processing | `isMarkovChain_map_left` | `InformationTheory/Shannon/CondMutualInfo.lean:652` | ✅ | Phase A auxiliary RV `U = f(X)` の Markov 化 |
| DPI (post-processing) | `mutualInfo_le_of_postprocess` | `InformationTheory/Shannon/DPI.lean:139` | ✅ | Phase C `I(W;X̂) ≤ I(W;X)` |
| MI ↔ entropy 橋 | `mutualInfo_eq_entropy_sub_condEntropy` | `InformationTheory/Shannon/Bridge.lean:588` | ✅ | Phase C `I(X;U) - I(Y;U)` 展開 |
| 3 項 entropy 橋 | `mutualInfo_eq_entropy_add_entropy_sub_jointEntropy` | `InformationTheory/Shannon/MIChainRule.lean:449` | ✅ | optional, single-letterization 簡略形 |
| 条件付き entropy chain | `entropy_pair_eq_entropy_add_condEntropy` | `InformationTheory/Shannon/Entropy.lean:41` | ✅ (`[Fintype X][Fintype Y]`) | `H(X^n,U^n) = H(X^n) + H(U^n\|X^n)` |
| conditioning ≤ | `condEntropy_le_condEntropy_of_pair` | `InformationTheory/Shannon/Entropy.lean:240` | ✅ | Phase C side info 単調性 |
...
|---|---|---|---|---|
| Random binning measure | `binningMeasure` | `InformationTheory/Shannon/SlepianWolfBinning.lean:62` | ✅ | Phase B auxiliary `U^n` 上の bin index に流用 (input space を `Fin n → U` に置換) |
| Binning collision (singleton mass) | `binningMeasure_singleton_real` | `InformationTheory/Shannon/SlepianWolfBinning.lean:78` | ✅ | Phase B 誤り bound 計算 |
| Binning collision prob | `binning_collision_prob` | `InformationTheory/Shannon/SlepianWolfBinning.lean` (~150) | ✅ | Phase B `Pr[same bin] = 1/M` |
| Joint sequence | `ChannelCoding.jointSequence` | `InformationTheory/Shannon/ChannelCoding.lean:275` | ✅ | Phase B 三項 typicality `(X^n, Y^n, U^n)` を `((X,Y), U)` でネスト |
| Joint typical set | `ChannelCoding.jointlyTypicalSet` | `InformationTheory/Shannon/ChannelCoding.lean:301` | ✅ (二項 `α × β`) | Phase B `T^n_{ε}(X, Y, U)` を `((X,Y), U)` reshape で構築 |
| Conditional typical slice | `ChannelCoding.conditionalTypicalSlice` | `InformationTheory/Shannon/SlepianWolfConditionalTypicalSlice.lean:59` | ✅ | Phase B `\|T_{X\|Y}^n(y)\| ≤ exp(n(H(X\|Y)+2ε))` を `(U\|Y)` に流用 |
| Joint typical set card bound | `jointlyTypicalSet_card_le` | `InformationTheory/Shannon/ChannelCoding.lean:340` | ✅ | Phase B 誤り bound |
| Joint typical set prob → 1 | (in `AEPRate.lean`) `jointlyTypicalSet_prob_ge_of_rate` | `InformationTheory/Shannon/AEPRate.lean:395` | ✅ | Phase B AEP 経由 |
| Joint typical decoder (SW form) | `swJointTypicalDecoder` | `InformationTheory/Shannon/SlepianWolfFullRateRegion.lean:35` | ✅ | Phase B side info Y 入り decoder に reshape (`decoder : Fin M × (Fin n → β) → (Fin n → α)`) |
| 4-way error event decomposition | `swError_E0/EX/EY/EXY` | `InformationTheory/Shannon/SlepianWolfFullRateRegion.lean:51-83` | ✅ | Wyner-Ziv では **2-way 縮退** (E_typ + E_bin only)。SW 4-way の `EX` (= bin collision on `U^n`) と `E0` (= joint not typical) を回す |

### 2.3 既存 InformationTheory — Rate-Distortion (Wyner-Ziv で再利用)

| 概念 | API | file:line | 状態 | Wyner-Ziv での扱い |
|---|---|---|---|---|
| Distortion function `α → γ → NNReal` | `DistortionFn` | `InformationTheory/Shannon/RateDistortionAchievability.lean:59` | ✅ | そのまま流用 |
| Block distortion `(1/n)∑ d(x_i, y_i)` | `blockDistortion` | `InformationTheory/Shannon/RateDistortionAchievability.lean:62` | ✅ | そのまま流用 |
| Lossy code structure | `LossyCode` | `InformationTheory/Shannon/RateDistortionAchievability.lean:83` | ✅ (decoder `Fin M → Fin n → β`) | Wyner-Ziv では decoder を `Fin M × (Fin n → β) → Fin n → γ` に拡張 — 新規 structure `WynerZivCode` を立てる (~10 行) |
| Expected block distortion | `LossyCode.expectedBlockDistortion` | `InformationTheory/Shannon/RateDistortionAchievability.lean:93` | ✅ | 同上、side info 引数を追加 |
| `R(D)` pmf 形 | `rateDistortionFunctionPmf` | `InformationTheory/Shannon/RateDistortionAchievability.lean:293` | ✅ (`negMulLog` 連続) | **流用不可** — `R_WZ(D)` は MI 差 `I(X;U) − I(Y;U)` で auxiliary `U` を回す min が要る。**新規定義** `wynerZivRatePmf` (~50 行) |
| `R(D)` 達成性 (compactness) | `rateDistortionFunctionPmf_attained` | `InformationTheory/Shannon/RateDistortionAchievability.lean:301` | ✅ | Phase A `wynerZivRatePmf` の min 達成性に同形式の戦略 (compact constraint + continuous obj) |
| Per-letter feasibility | `rateDistortionFunction_le_mutualInfo_perLetter` | `InformationTheory/Shannon/RateDistortionConverseNLetter.lean:207` | ✅ | Phase C per-letter `R_WZ(D_i) ≤ I(X_i; U_i) - I(Y_i; U_i)` |
| n-letter Jensen + antitone | (hypothesis pass-through in) `rate_distortion_converse_n_letter_singleLetter` | `InformationTheory/Shannon/RateDistortionConverseNLetter.lean:260` | ✅ | Phase C 同パターンで `R_WZ` も hypothesis pass-through 採用 (L-WZ3 撤退ライン) |
| Single-shot RD converse | `rate_distortion_converse_single_shot` | `InformationTheory/Shannon/RateDistortionConverse.lean:133` | ✅ | Phase C 骨格 — 4 step chain (entropy → MI → DPI → R(D̃)) は Wyner-Ziv でも同型 |
| RD pmf convexity | `RDConstraint_convex` | `InformationTheory/Shannon/RateDistortionAchievability.lean:217` | ✅ | Phase A `wynerZivRatePmf` 凸性は **新規証明** が必要 (auxiliary `U` domain mixing は別 strategy) |
| RD pmf MI 連続 | `continuous_mutualInfoPmf` | `InformationTheory/Shannon/RateDistortionAchievability.lean:267` | ✅ | Phase A `wynerZivRatePmf` の連続性証明に流用 (但し `I(X;U) − I(Y;U)` は **連続だが凸ではない**) |
...
| KL invariant under measurable equiv | `klDiv_map_measurableEquiv` | `InformationTheory/Shannon/MutualInfo.lean:52` | ✅ | (InformationTheory 自作) Phase A `U ≃ U'` reshape |
...
### 3.3 `mutualInfo_chain_rule_fin` (`InformationTheory/Shannon/MIChainRule.lean:117`)
...
### 3.4 `rate_distortion_converse_single_shot` (`InformationTheory/Shannon/RateDistortionConverse.lean:133`)
...
- **目的**: Phase C n-letter converse で per-letter `I(X_i; U_i) - I(Y_i; U_i)` を出すための
  auxiliary RV 構成。
- **推奨**: `InformationTheory/Shannon/WynerZivConverseAux.lean` (新規) で
...
- **推奨**: `mutualInfo_map_left_measurableEquiv` (InformationTheory) と同型の証明 (~80 行)。
- **工数感**: ~80-100 行。
...
```
を locally 立てる (既存ファイル汚染を避けるため `InformationTheory/Shannon/WynerZivStandardBorel.lean`
...
既存 `IsMarkovChain` は **γ-form** (`InformationTheory/Shannon/CondMutualInfo.lean:71`):
```lean
...
**argument order**: `InformationTheory` の `IsMarkovChain μ Xs Zc Yo` は「`Zc` を介して `Xs` と `Yo` が
...
既存 `jointlyTypicalSet` (`InformationTheory/Shannon/ChannelCoding.lean:301`) は **二項
...
## 7. 着手 skeleton (`InformationTheory/Shannon/WynerZiv.lean` 出だし)
...
import InformationTheory.Shannon.RateDistortionConverseNLetter
