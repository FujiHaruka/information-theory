...
>
> **Scope**: Phase D の expurgation 2 ステップを実現するための Mathlib + Common2026
> API 在庫:
...
>
> **T-3 判定**: **T-3 不採用 (= D-1/D-2/D-3 すべて既存 Mathlib + Common2026 既存
> パターンで直接構成可能)**。手書きは「Markov pigeonhole 2 行」相当のみで
> ~30 行新規にはならない。詳細 §最終判定。

## 一行サマリ

Phase D で使う API は **D-1: 100% 既存** (Mathlib `exists_le_lintegral` + Common2026
...
| 概念 | Mathlib / Common2026 API | file:line | 状態 | Phase D での扱い |
|---|---|---|---|---|
...
| Common2026 既存類似 (random codebook) | `exists_codebook_le_avg` | `Common2026/Shannon/ChannelCodingAchievability.lean:1479` | ✅ **既存** | **D-1 の直接 reference 実装**、写しで構成 |
| Common2026 既存類似 (BC abstract) | `bc_exists_codebook_of_avg_le` | `Common2026/Shannon/BroadcastChannelRandomCodebook.lean:208` | ✅ **既存** | D-1 の structural 形 (Finset 上の weighted-sum) |
| Common2026 既存類似 (uniform weight) | `bc_exists_codebook_of_sum_le` | `Common2026/Shannon/BroadcastChannelRandomCodebook.lean:256` | ✅ **既存** | D-1 の uniform 版 |
...
#### 4.1.3 — Common2026 `exists_codebook_le_avg` 詳細 (D-1 の reference 実装)

- **file:line**: `Common2026/Shannon/ChannelCodingAchievability.lean:1479`
- **signature (verbatim)**:
  ```lean
...
| Markov div 形 (`measure ≤ integral / ε`) | `MeasureTheory.meas_ge_le_lintegral_div` | `Mathlib/MeasureTheory/Integral/Lebesgue/Markov.lean:104` | ✅ **既存** | Common2026 で `SMBAlgoetCover.lean:754, 2425` で利用実績有 |
...
- **D-1**: `MeasureTheory.exists_le_lintegral` / `exists_le_integral` が
  ...
  さらに Common2026 内 `exists_codebook_le_avg` (CCAchievability.lean:1479)
...
## 着手 skeleton (`Common2026/Shannon/AWGNAchievabilityDischarge.lean` Phase D 部のみ抜粋)
...
Common2026 既存 `exists_codebook_le_avg` パターンで D-1 は 5 行で完成、D-2 は
