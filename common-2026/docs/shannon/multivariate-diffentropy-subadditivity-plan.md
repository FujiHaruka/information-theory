...
- [ ] Phase 1 — 2 変数 subadditivity bridge の honest hyp 縮約 (`h_llr_split` Bayes density split を `prod_withDensity` + `volume_eq_prod` で discharge) 📋
- [ ] Phase 2 — n 変数 subadditivity bridge の honest hyp 縮約 (`pi_withDensity` 不在の壁、2 変数 induction 帰着) 📋
- [ ] Phase 3 — AWGN `IsContinuousAEPGaussian` predicate の plan-side reflection (Mathlib 壁 = continuous SMB / n-dim differentialEntropy、staged 維持) 📋
- [ ] Phase V — clean + slug 付け替え (`@audit:suspect(differential-entropy-plan)` → `@audit:suspect(multivariate-diffentropy-subadditivity-plan)`) 📋
...
## Phase 1 — 2 変数 subadditivity bridge の honest hyp discharge 📋

`proof-log: yes`。**推定 ~80-150 行**。本 plan の最重 step。
...
## Phase 2 — n 変数 subadditivity bridge の honest hyp discharge 📋
