...
- [ ] Phase 0 — Mathlib + Common2026 既存 EPI / BM API 在庫 + CT 17.7.4 textbook flow 固定 📋 → [`brunn-minkowski-from-epi-mathlib-inventory.md`](brunn-minkowski-from-epi-mathlib-inventory.md) (未起草、Phase 0 で着手)
...
`Common2026/Shannon/BrunnMinkowski.lean:188` の `brunn_minkowski_entropy_inequality`
(現在 `:= h_bm_entropy_assumed` で結論を仮説として受け取る pass-through) を、
**EPI 経由の chain** から genuine に導出する形に rewire する。chain の形:

```
[1-D EPI]                              ← Common2026 既存 (Gaussian case full、
...
- `Common2026/Shannon/BrunnMinkowskiFromEPI.lean` (新規, 推定 ~400-700 行) に
  上記 chain を publish。
- 旧 `brunn_minkowski_entropy_inequality` (`BrunnMinkowski.lean:188`、L-BM1
  pass-through) は **deprecated として残す** (signature 互換、過去参照のため)。
- `Common2026.lean` に `import Common2026.Shannon.BrunnMinkowskiFromEPI` 1 行追加。
...
1. **1-D EPI を black-box 化** (Phase 1)。`Common2026/Shannon/EntropyPowerInequality.lean`
...
- `Common2026/Shannon/BrunnMinkowskiFromEPI.lean` (新規, 推定 **~400-700 行**)
...
### Phase 0 — Mathlib + Common2026 既存 EPI / BM API 在庫 + CT 17.7.4 textbook flow 固定 📋
...
##### Common2026 既存 BM 群 (signature 整合先)
...
##### Common2026 既存 multivariate entropy
...
- [ ] **independent case 等式**: independent coordinate での `h_n = Σ h_1` 等式
  (subadditivity の等号成立条件)。Common2026 に既存か Mathlib に既存か Phase 0
...
- [ ] **step case (n → n+1)**: `Fin.snoc` で 1 coordinate 拡張。帰納仮定 + 残り
  ...
  - **independent coordinate 等式が Mathlib/Common2026 にあれば**: それを使い、
...
- **independent coordinate 等式が Mathlib/Common2026 双方に不在**: max-entropy
...
- [ ] `lake env lean Common2026/Shannon/BrunnMinkowskiFromEPI.lean` silent (0
  error / 0 sorry / 最小 warning)。
- [ ] `Common2026.lean` に `import Common2026.Shannon.BrunnMinkowskiFromEPI`
  1 行追加。
