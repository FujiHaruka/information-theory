...
検証 bash (段別検証は `lake env lean InformationTheory/Shannon/<file>.lean` を該当 file ごとに、段 1 後は `lake build InformationTheory.Shannon.FisherInfoV2DeBruijn` で olean refresh):

```bash
# launder pattern 撲滅確認 (Done 条件)
rg -n ':= h_ibp|:= h_reg\.derivAt_entropy_eq_half_fisher_v2' InformationTheory/Shannon/Fisher*.lean
# 0 hit が Done 条件
```
...
#### 3 declaration consumer 表 (verbatim、`InformationTheory/Shannon/EntropyPowerInequality.lean`)
...
Done: 全 file 検証 PASS + audit PASS + `InformationTheory.lean` import 確認 + 判断ログに closure entry (Phase 別件数集計) + 既存親 plan 群の進捗ブロック update + `docs/textbook-roadmap.md` Ch.17 EPI frontier 行 update。
...
Phase 別: 該当 Cluster の file (Phase 1 = Cluster C 9 file / Phase 2 = Cluster B 7 file / Phase 3 = Cluster D 1 file) を `lake env lean` 個別実行。signature 改変後は `lake build InformationTheory.Shannon.<refactored-file>` で olean refresh。
...
for f in FisherInfo FisherInfoV2 FisherInfoV2DeBruijn FisherInfoV2DeBruijnBody \
  ...
  lake env lean InformationTheory/Shannon/$f.lean
done

# 残課題集計 + 分類漏れ + deprecated タグ残置確認 (audit-tags.md recipe)
rg "@residual" InformationTheory/Shannon/ | wc -l
rg -o "@residual\([a-z]+:" InformationTheory/Shannon/ | sort | uniq -c | sort -rn
rg '@audit:suspect\(|@audit:staged\(|@audit:defer\(|@audit:closed-by-successor\(|🟢ʰ' \
  InformationTheory/Shannon/EPI*.lean InformationTheory/Shannon/Fisher*.lean \
  InformationTheory/Shannon/Stam*.lean InformationTheory/Shannon/EntropyPowerInequality.lean
```
...
- Cluster 主 file: `InformationTheory/Shannon/EntropyPowerInequality.lean` (D anchor)、`EPIStamDischarge.lean` / `EPIStamToBridge.lean` / `EPIL3Integration.lean` (C)、`FisherInfoV2DeBruijn.lean` / `FisherInfoV2DeBruijnBody.lean` / `FisherInfoV2HeatFlowBody.lean` (B tier 5 defect 所在)
