...
>
> **Status (2026-05-10):** 起草。loogle index (`.lake/build/loogle.index`) + Mathlib / Common2026 直 grep
...
5 軸の調査結果を **Phase A skeleton (`Common2026/Shannon/SlepianWolf.lean` の sorry-driven 出だし) が書ける状態** に持っていく。各軸で「Mathlib にあるか / ないか / 既存補題で代用可」の 1 行結論 + 採用する具体補題シグネチャを verbatim 記録。
...
| (3) chain rule | 2 変数版 `entropy_pair_eq_entropy_add_condEntropy` ✅、`condEntropy_le_condEntropy_of_pair` ✅ ともに `Common2026/Shannon/Entropy.lean` 既存 | そのまま流用 |
...
- **位置**: `Common2026/Shannon/Converse.lean:81`
- **完全シグネチャ**:
  ```lean
...
- **位置**: `Common2026/Fano/Measure.lean:224`
- **完全シグネチャ**:
  ```lean
...
- **位置**: `Common2026/Fano/Measure.lean:73`
- **完全シグネチャ**:
  ```lean
...
**既存 `Common2026/Shannon/Entropy.lean` の `entropy_pair_eq_entropy_add_condEntropy` + `condEntropy_le_condEntropy_of_pair` でカバー。新規補題不要。**
...
**`entropy μ (e ∘ Xs) ≤ log (Fintype.card (Fin M))` の任意 μ 版は Mathlib・Common2026 両方に不在。Phase A で 30〜50 行の新規補題として立てる必要あり。** Loomis–Whitney の `entropy_le_log_image_card` は uniform 専用で代用不可。
...
- **位置**: `Common2026/Shannon/LoomisWhitney.lean:125`
- **完全シグネチャ**:
  ```lean
