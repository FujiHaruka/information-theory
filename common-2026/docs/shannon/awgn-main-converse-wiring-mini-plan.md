...
> - `Common2026/Shannon/AWGNConverse.lean:59-70` `awgn_converse` body
>   (現状 `sorry` のみ、tag `@residual(plan:awgn-converse-aux-plan)` +
>   `@audit:closed-by-successor(awgn-converse-aux-plan)`)
> - `Common2026/Shannon/AWGNMain.lean` の consumer ripple 評価 (現状は
>   `awgn_converse` 未呼出 = ripple 0、後述で verbatim 確定)
>
> **Status (2026-05-27)**: 起草。**M3 完了**で `AWGNConverseDischarge.lean:1259-1277`
> `awgn_converse_F3_discharged` wrapper が ready (verbatim Read 済)、
> `awgnConverseJoint_pair_mi_ne_top` (line 406-413、private、`@residual(wall:multivariate-mi)`、
> tier 2 honest) 1 件のみが残置。本 mini-plan は wrapper を **直接呼出**して
> `awgn_converse` body の `sorry` を埋める purely-wiring 作業。新規 sorry / 新規
> @residual は導入しない (wall:multivariate-mi の単一 sorry は M4 完成後も discharge
> 経路上で間接伝播するのみ、新規導入なし)。

## 進捗

- [ ] M0 — caller verbatim 確認 + signature 整合判定 (本 plan 内で済、§「signature 整合 verbatim」) ✅
- [ ] M1 — `awgn_converse` body skeleton publish (新引数 3 件追加: `h_feasible` / `h_mi_bridge_per_letter` / `hn_pos`、body は `awgn_converse_F3_discharged` 1 行呼出) 📋
- [ ] M2 — `AWGNMain.lean` consumer ripple 解消 (現状 0 件確認済、§「consumer ripple 評価」) 📋
- [ ] M3 — verify (`lake env lean` 3 file、type-check done 維持、proof done は wall:multivariate-mi 残置 = 単一 sorry 透過状態) 📋
- [ ] M4 — `@audit:closed-by-successor` tag 撤去 + `@residual(plan:awgn-converse-aux-plan)` reclassify (`@residual(wall:multivariate-mi)` 透過、shared lemma 経由) 📋
...
```lean
-- Common2026/Shannon/AWGNConverse.lean:59-70 (改変後)
...
`rg -n 'awgn_converse[^_]' Common2026/` 結果 verbatim 確認 (本起草 turn で実行済):
...
本 mini-plan は **既存 Common2026 declaration の薄い wrapper 呼出のみ**。
...
- `import Common2026.Shannon.AWGNConverseDischarge` — `awgn_converse_F3_discharged`
  + `IsAwgnConverseFeasible` + `perLetterMI` + `perLetterYLaw` (新引数の型に必要)
- 既存 `import Common2026.Shannon.AWGN` は維持 (`IsAwgnChannelMeasurable` /
...
`import Common2026.Shannon.AWGNConverse` (verbatim Read 済) — `AWGNConverse.lean`
...
```
ここに `import Common2026.Shannon.AWGNConverseDischarge` を追加すると、
`AWGNConverseDischarge.lean:2` の逆向き import と循環。

→ **M1 で確実に確認 + 解消方針**:
- 方針 (i) `AWGNConverseDischarge.lean:2` の `import Common2026.Shannon.AWGNConverse`
...
scope: `Common2026/Shannon/AWGNConverse.lean` のみ改変。

- [ ] M1-α: `AWGNConverseDischarge.lean:2` の `import Common2026.Shannon.AWGNConverse`
...
- [ ] M1-β (M1-α の結論次第):
  - **方針 (i) 採用** (import 削除可) なら `AWGNConverseDischarge.lean:2` の import 1 行削除
    + `AWGNConverse.lean` に `import Common2026.Shannon.AWGNConverseDischarge` 追加
...
- [ ] `lake env lean Common2026/Shannon/AWGNConverse.lean` clean (0 errors / 0 sorry)
- [ ] `lake env lean Common2026/Shannon/AWGNConverseDischarge.lean` clean
  ...
  しないことを確認)
- [ ] `lake env lean Common2026/Shannon/AWGNMain.lean` clean (docstring 更新で
  type-check 影響なし)
- [ ] `rg -n '@residual|@audit:' Common2026/Shannon/AWGNConverse.lean` で deprecated
...
`lake env lean` 3 file (Common2026 root の追加は不要、既に `Common2026.lean:106-110`
に `AWGNConverse` / `AWGNMain` / `AWGNConverseDischarge` 全 3 件 import 済 verbatim
確認):

```bash
lake env lean Common2026/Shannon/AWGNConverse.lean
lake env lean Common2026/Shannon/AWGNConverseDischarge.lean
lake env lean Common2026/Shannon/AWGNMain.lean
```
...
`rg -n '@audit:|@residual|🟢ʰ' Common2026/Shannon/AWGNConverse.lean` で deprecated
