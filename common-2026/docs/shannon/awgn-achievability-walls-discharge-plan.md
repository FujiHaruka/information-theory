...
>
> **Status (2026-06-12)**: **CLOSED — achievability stack 全体 proof done / 解析核 genuine**。
> deep atoms 5 件 + degenerate 残件すべて閉鎖、3 壁の honest 後継 2 本 + 縮小 Wall 1 が
> genuine、consumer (`isAwgnTypicalityHypothesis` / 主要 wrapper 2 本) まで genuine。
> 計 6 件の false-statement (Wall1 (ii)/(iii)、D2 term2 #5 Fix B、degenerate corner #6) を
> いずれも honest 化済。**機械検証状態 (sorryAx 依存 + 再検証コマンド) は
> [`awgn-facts.md`](awgn-facts.md) の達成テーブル + false-statement 台帳が SoT** (prose にキャッシュしない)。

## 進捗

- [x] M0 — decomposition 再設計の feasible 確定 (D1-D4、2 subagent 独立検証) ✅
- [x] Phase 1 — Wall 1 (D1): (ii) 削除 + (iii) statement-fix + change-of-measure/`hφ_memLp` genuine、`continuousAepGaussian_holds` genuine closure ✅ (commits eab36aa→cd5419c→2036bfa→ce9d1bf)
- [x] Phase 4 — Wall 2 (D2): 新 lemma `awgn_random_coding_union_bound` genuine + term1/term2 collapse + Fix B (#5) + Fix #6 ✅ (commits 7c26322→e4587aa→b6d0089→8405696→c543ae3→f69cfea)
- [x] Phase 5a — Wall 3 (D3): 新 lemma `awgnPowerConstraintPerCodeword_holds` proof done ✅ (commit e4587aa)
- [x] D4 — consumer rewire: 旧 Wall2/3 retire + per-codeword barrier 再構築、`isAwgnTypicalityHypothesis` genuine ✅ (commit b6d0089)
...
transitively sorry 非依存にする。**達成**: deep atoms 全閉鎖後、achievability 解析核が genuine
(機械検証状態は → [`awgn-facts.md`](awgn-facts.md) 達成テーブル)。converse は既に genuine 完了済
(`awgn_converse`)。本 plan 完走で AWGN channel coding theorem の achievability 解析核が genuine。
...
- **D3 (Wall 3)** = `awgnPowerConstraintPerCodeword_holds` (per-codeword expurgation 形、engine φ=x²、proof done)。
- **D4 (consumer)** = `isAwgnTypicalityHypothesis` を新 decomposition に rewire、per-codeword 合算 barrier 再構築、旧 false 補題 retire。consumer body genuine。

#### bridge ① 共有 lemma 2 本 (Phase 1 で新設、genuine)
...
機械検証状態 (sorryAx 依存・再検証コマンド) は → [`awgn-facts.md`](awgn-facts.md) 達成テーブルが SoT (本表は依存先の役割のみ列挙、prose にキャッシュしない)。

| 資産 | file:line | 用途 | 状態タグ |
|---|---|---|---|
| `pi_empirical_mean_concentration` | `AWGN/AchievabilityAEP.lean:38` | (i)/(D3) engine: 有限-n Chebyshev 集中 (abstract μ+φ) | → facts ledger |
| `pi_empirical_mean_typical_mass` | `AWGN/AchievabilityAEP.lean:130` | (i)/(D3) engine: ∃N₀ で mass `≥ 1−η` の存在形 | → facts ledger |
| `klDiv_pi_eq_sum` | `Shannon/MIChainRule.lean:249` | (iii)/1b: `klDiv (pi P) (pi Q) = ∑ klDiv P Q` (型前提 = `IsProbabilityMeasure` のみ) | → facts ledger (M0 表) |
| `klDiv_prod_eq_add` | `Shannon/MIChainRule.lean:230` | (iii): prod の KL 加法分解 | → facts ledger (M0 表) |
...
| `awgn_expurgate_worst_half` | `AWGN/AchievabilityDischarge.lean:526` | D4: worst-half throwaway、signature 不変で combined penalty に再利用 | `@audit:ok` (code SoT) |
| `awgnPowerWitness_exists` | `AWGN/AchievabilityDischarge.lean:614` | D3: strict slack `P' < P` witness (4次モーメント有限) | `@audit:ok` (code SoT) |

> **converse 側資産 (achievability では不要)**: `jointDifferentialEntropyPi` 系
> (`Shannon/MultivariateDiffEntropy.lean` / `ParallelGaussian/Converse/Core.lean:145`)
...
### Phase 1 — Wall 1 (D1): engine mass (i) + klDiv indep-pair (iii)、(ii) 削除 ✅ genuine closure (proof-log: yes)
...
(`klDiv_perLetter_eq_capacity` / `klDiv_nFold_eq_nsmul`) を新設して (iii) を klDiv
...
分解) で genuine 化。**全 deep atom 閉鎖、`continuousAepGaussian_holds` genuine closure**
(commits eab36aa / cd5419c / 2036bfa / ce9d1bf、機械検証状態 → [`awgn-facts.md`](awgn-facts.md))。

### Phase 4 — Wall 2 (D2): genuine union-bound lemma ✅ genuine closure (proof-log: yes)
...
(hN : (N:ℝ)≠0)` 追加で矛盾 discharge、consumer 2 件 rewire。**genuine closure** (false-statement #5/#6
解消、独立 honesty 監査 PASS、機械検証状態 → [`awgn-facts.md`](awgn-facts.md))。

### Phase 5a — Wall 3 (D3): per-codeword expurgation 形 ✅ proof done (commit e4587aa、proof-log: yes)

新 lemma `awgnPowerConstraintPerCodeword_holds` (`Walls.lean:370`) を genuine で追加 (機械検証状態
→ [`awgn-facts.md`](awgn-facts.md) 達成テーブル)。
...
**consumer body genuine** (機械検証状態 → [`awgn-facts.md`](awgn-facts.md))、wrapper pass-through 自動追従。
...
- `#print axioms` で achievability stack の headline 系 (`isAwgnTypicalityHypothesis` / 主要 wrapper 2 本) の sorryAx 依存を検証 (再検証コマンド + 結果は → [`awgn-facts.md`](awgn-facts.md) 達成テーブルに記録、prose にはキャッシュしない)。
- 親 `awgn-moonshot-plan.md` の F-1 撤退ライン + 進捗ブロックを本子の状態に同期 (child SoT)、facts ledger 達成テーブルに achievability stack + false-statement #6 + bridge ① 2 本 + 教訓行を追記。
