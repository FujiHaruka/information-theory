...
> (`Common2026/Shannon/LZ78DistinctEncoding.lean:412`) が現在 honest 入力として受ける
...
- [ ] Phase V — `Common2026.lean` 編入 + hyp-free distinct headline publish + clean check 📋
...
| Phase V | **15 行** | 10–25 | `Common2026.lean` import + hyp-free distinct headline | no |
...
- [ ] imports: `Common2026.Shannon.ShannonCode` (Kraft 資産), `LZ78ConverseDischarge`
...
- [ ] **検証**: `lake env lean Common2026/Shannon/LZ78ConverseKraft.lean` が sorry warning のみで type-check。
...
- [ ] imports: `Common2026.Shannon.LZ78ZivInequality` (`card_phraseSet_le_pow`),
...
- [ ] **検証**: `lake env lean Common2026/Shannon/LZ78ZivEntropyBridge.lean` が sorry warning のみ。
- **撤退ライン**: なし (skeleton のみ)。
...
## Phase V — `Common2026.lean` 編入 + hyp-free distinct headline publish 📋

- [ ] `Common2026.lean` に `import Common2026.Shannon.LZ78ConverseKraft` と
      `import Common2026.Shannon.LZ78ZivEntropyBridge` を追記 (`LZ78DistinctEncoding` import の後ろ)。
...
- [ ] **検証**: `lake env lean Common2026/Shannon/LZ78ConverseKraft.lean` と
      `Common2026/Shannon/LZ78ZivEntropyBridge.lean` が silent (0 error / 0 sorry / 0 warning)。
- [ ] 既存 genuine 補題 (`card_phraseSet_le_pow`, `lz78DistinctEncodingLength_eq`,
      ...
      `rg` で横断確認 (新規追加のみ)。
- [ ] upstream olean refresh: 新 public symbol を下流が拾うなら `lake build Common2026.Shannon.LZ78ConverseKraft`
...
- **新規 lemma 投入先**: `Common2026/Shannon/LZ78ConverseKraft.lean` (新規, ~330 行, L-LZ2 群) +
  `Common2026/Shannon/LZ78ZivEntropyBridge.lean` (新規, ~410 行, L-LZ1 群)。
- **編集される既存ファイル**: `Common2026.lean` (import 2 行追記)。`LZ78DistinctEncoding.lean` は
...
6. **Phase V** `Common2026.lean` 編入 + hyp-free distinct headline publish + clean check。
