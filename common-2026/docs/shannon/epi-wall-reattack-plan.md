...
- [x] **Phase 3d — assemble `stam_step2_density_wall` = genuine ✅ (2026-05-30、独立監査 @audit:ok)**。案 b' (`IsBlachmanConvReady` bundle + `convex_fisher_bound_of_ready` を最適 λ 適用 + `stam_lambda_min` inline) で genuine 0-sorry closure。precondition gap (L-EPIW-3-密度-β) は **案 b = 新 structure `IsBlachmanConvReady fX fY` (19 field の regularity bundle) + pointwise `hconv` を lockstep 述語 (`IsStamCondExpCSHyp`/`IsStamCauchySchwarzOptimal`/`IsStamCauchySchwarz`) に追加**で解消。`IsStamCauchySchwarzOptimal` が **SOUND な Prop** であることが sorryAx-free 証明で確定 (旧 false-statement defect 完全解消)。import cycle 回避で λ 最適化 inline。残 sorry 2 (`isBlachmanConvReady_symm` reflection plumbing / `isStamInequalityHyp_via_body` regularity signature gap、両 `@residual(plan:...)`、honest)。
- [x] **Phase 3e — Gaussian witness `IsBlachmanConvReady` = proof done ✅ (2026-05-31、独立監査 @audit:ok)**。新 file `EPIBlachmanGaussianWitness.lean`。linchpin `convDensityAdd_gaussian_closed_form` (Gaussian 密度畳み込み = Gaussian(sum)、pointwise、measure-level 5-step + 連続性 a.e.→pointwise) + `isRegularDensityV2_gaussianPDFReal` + witness `isBlachmanConvReady_gaussianPDFReal` (19 field 全 genuine、`int_prod*`/`int_inner` は shear `measurePreserving_prod_sub_swap`) すべて genuine 0-sorry/sorryAx-free。**`IsBlachmanConvReady (gaussian)(gaussian)` の proven inhabitant 成立 = density route 非vacuousness 機械確証**。capstone `convex_fisher_bound_gaussian_via_density_route` (+ closed-form route 整合系) で witness を operationally 消費。**🎯 EPI density route は 3c-fin/3d/3e/capstone まで全 @audit:ok で完成**。
- [x] **2 honest sorry 完済 ✅ (2026-05-31、独立監査 @audit:ok)**:
  - `isBlachmanConvReady_symm` (`EPIBlachmanDensity.lean:792`) = genuine 0-sorry。19-field reflection transport (`convDensityAdd_comm` + shear `x↦z-x` + skew map `(z,x)↦(z,z-x)` を `MeasurePreserving.skew_product` で自前構築)。sorryAx-free。
  - `isStamInequalityHyp_via_body` (`EPIStamInequalityBody.lean:446`) = genuine 0-sorry。**owner-level pivot**: `IsStamInequalityHyp`/`IsStamInequalityResidual` を lockstep で pivot し最終 conv hyp を `=ᵐ` → pointwise `∀x` に強化 + `IsBlachmanConvReady fX fY` bundle 追加 (8-file ripple、defeq site は pure pass-through で commit diff にすら含まれず、Gaussian end-to-end EPI も sorryAx-free 維持)。2 新 hyp は regularity precondition (core-reconstruction test pass、非vacuous は `isBlachmanConvReady_gaussianPDFReal` で確証)。
- [ ] predicate-level Gaussian wiring (低優先、**ほぼ moot**): pivot 監査が Gaussian 経路は `IsStamInequalityHyp` を witness 構築せず `entropyPower_gaussian_additivity` 直通と確認、非vacuousness も witness で確証済 → 残価値は低い。
- [~] **Phase 4 — 壁2 (per-time de Bruijn + FTC 積分形)**: 独立 proof-pivot-advisor (2026-05-31) で **per-time `debruijnIdentityV2_holds` = NO-GO 真壁確定** (loogle で heat/Fokker/semigroup/PDE 全 Found 0、`differentialEntropy μ := ∫ negMulLog(μ.rnDeriv volume)` が measure 本体依存で density-collapse 不可 = 壁1 を解体した rfl pivot が構造的に効かない、L-EPIW-4-α 発動)。**現状の `sorry`+`@residual(wall:debruijn-integration)` 据置が正しい honest state**。
      - [x] **積分形 `debruijnIntegrationIdentity_holds` = 構造的 closure ✅ (2026-05-31、独立監査 honest_residual)**。旧「独立 sorry」を FTC reduction (`intervalIntegral.integral_eq_sub_of_hasDerivAt_of_le` + per-time `debruijnIdentityV2_holds` 各点呼出) で genuine 本体に置換。新 `structure IsDeBruijnPathRegular X Z P T` (4 field: fPath/reg_t/cont/integrable、全 regularity precondition、`@audit:ok`) を precondition に追加、consumer (`EPIStamDischarge.isDeBruijnIntegrationHyp_holds`) ripple。**2 独立 sorry → per-time 壁 1 個に集約** (local 0 sorry、transitive `wall:debruijn-integration`)。core (heat eq IBP) は per-time wall に残る。
      - [ ] F prep (`_hX/_hZ/_hXZ` signature 復元) = **caller 都合で見送り** (caller `csiszarGap1Source_hasDerivAt` が `IsDeBruijnRegularityHyp` bundle しか持たず `Measurable`/`IndepFun` を作れない、forward-looking note 予期通り)。per-time 壁 closure 着手時に caller 側で当該 regularity hyp を持つ設計が要る。
      - [ ] 残 incidental: `IsIBPHypothesis` (`FisherInfoV2DeBruijnBody.lean:199`) = tier-4 name-laundering retract-candidate (自認済、load-bearing でない、consumer 既に `_h_ibp` unused 化)。当該 file touch 時に incidental 削除候補。
...
`IsRegularDensityV2 := InformationTheory.Shannon.FisherInfoV2.IsRegularDensityV2`
...
1. `EPIStamStep12Body.lean` に `import InformationTheory.Shannon.EPIConvDensity` 追加 (cycle 無し:
...
4. 緑 gate: `lake env lean InformationTheory.Shannon.EPIStamStep12Body`。Commit。

**Step 2 — `IsStamCauchySchwarzOptimal`。**
1. `EPIStamInequalityBody.lean` に `import InformationTheory.Shannon.EPIConvDensity` 追加。
...
5. 緑 gate: `lake env lean InformationTheory.Shannon.EPIStamInequalityBody` + transitive
...
1. `EntropyPowerInequality.lean` に `import InformationTheory.Shannon.EPIConvDensity` 追加。
...
- **壁1 `wall:stam-step2-density`** — `InformationTheory/Shannon/EPIStamInequalityBody.lean:283`
...
- **壁2 `wall:debruijn-integration`** — `InformationTheory/Shannon/FisherInfoV2DeBruijn.lean:245`
...
| 対象 file | 役割 | Phase |
|---|---|---|
| `InformationTheory/Shannon/EPIConvDensity.lean` (新規) | gateway atom `convDensity_add_differentiable` + logDeriv 表現 | Phase 1 |
| `InformationTheory/Shannon/EPIScoreCrossTermOrth.lean` (新規) | cross-term orthogonality (inventory §着手 skeleton) | Phase 2 |
| `InformationTheory/Shannon/EPIStamInequalityBody.lean` (既存拡張) | `stam_step2_density_wall:283` 本体充足 | Phase 3 |
| `InformationTheory/Shannon/FisherInfoV2DeBruijn.lean` (既存拡張) | per-time `:245` + 積分形 `:310` 充足 + `_hX/_hZ/_hXZ` signature 復元 | Phase 4 |
...
```lean
-- InformationTheory/Shannon/EPIConvDensity.lean (新規)
...
```lean
-- InformationTheory/Shannon/EPIScoreCrossTermOrth.lean (新規)
...
`InformationTheory/Shannon/EPIBlachmanConvScore.lean` (新規、inventory 着手 skeleton の file 名を流用、ただし
...
> closure 対象: `InformationTheory/Shannon/EPIStamInequalityBody.lean:376` `stam_step2_density_wall`
...
|---|---|---|
| `IsRegularDensityV2` **構築サイト** (`⟨…⟩` / `.mk` / `isRegularDensityV2_*` 生成補題) | **0 件** | `rg 'IsRegularDensityV2' InformationTheory/` 全件が hypothesis (consumption) のみ。構築補題・anonymous ctor・`{ diff := … }` ゼロ |
...
> **closure 対象**: `IsBlachmanConvReady` (`InformationTheory/Shannon/EPIBlachmanDensity.lean:708`, 19 field) +
> `IsRegularDensityV2` (`InformationTheory/Shannon/FisherInfoV2.lean:124`, 6 field) の **Gaussian proven inhabitant**。
...
**決定: 新規 file `InformationTheory/Shannon/EPIBlachmanGaussianWitness.lean`。private 補題 2 件は public 化する。**
...
- **type-check done** (commit OK): `isBlachmanConvReady_of_gaussian` + `isRegularDensityV2_of_gaussian` が
  新規 file で `lake env lean InformationTheory.Shannon.EPIBlachmanGaussianWitness` 0 errors。未完 field は
  `sorry` + `@residual(plan:epi-wall-reattack-plan)`。`InformationTheory.lean` に import 1 行追加。private→public
...
2. **2026-05-30 Phase 3-pre 設計確定 (§A–D 書換、scoping doc 消費)**: `epi-3predicate-pivot-scoping.md`
   ...
   - **軸2 = 抽象 Prop carrier**: `fisherInfoOfMeasureV2_eq_of_pdf_ae_eq` は Mathlib/InformationTheory とも不在
