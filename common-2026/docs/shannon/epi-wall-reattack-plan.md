...
- [ ] Phase 3c-fin — `convex_fisher_bound` の残 sorry (Tonelli `integral_integral_swap`/`lintegral_lintegral_swap` + 3 項評価 + cross-term=0 `integral_logDeriv_density_eq_zero`/Phase 2) を genuine closure 📋 ← **次の一手** (既知の壁なし)
- [ ] Phase 3d — assemble: `stam_step2_density_wall` の sorry を `convex_fisher_bound` + `stam_lambda_min` (λ最適化既済) で genuine 充足 📋。**precondition gap 整合 (L-EPIW-3-密度-β: deriv 有界 + J 可積分性) を先に決定**
