...
- [ ] Phase 6 — verify (already passing) + Common2026 編入 (already 編入: 行 186 + 187) + roadmap 判断ログ 📋
...
- [x] **`typeClassByCount_Qn_ge`** — `Common2026/Shannon/SanovLDPEquality.lean:918`
  ```
  theorem typeClassByCount_Qn_ge
      (Q : Measure α) [IsProbabilityMeasure Q]
      (hQpos : ∀ a : α, 0 < Q.real {a})
...
- [x] **`sanov_ldp_lower_bound_pointwise`** — `Common2026/Shannon/SanovLDPEquality.lean:1071`
  ```
  theorem sanov_ldp_lower_bound_pointwise
      (Q : Measure α) [IsProbabilityMeasure Q] (hQpos : ∀ a : α, 0 < Q.real {a})
      (P : α → ℝ) (hP_prob : (∑ a, P a) = 1) (hP_full : ∀ a, 0 < P a)
...
- [x] **`steinTypicalSet_P_prob_tendsto_one`** — `Common2026/Shannon/Stein.lean:275`
  ```
...
- [x] **`steinTypicalSet_Q_prob_le`** — `Common2026/Shannon/Stein.lean:341`
  ```
...
- [x] **`chernoffMediatorMeasure_isProbabilityMeasure`** — `Common2026/Shannon/ChernoffPerTiltDischarge.lean:464`
  ```
...
- [x] **`min_le_rpow_mul_rpow`** — `Common2026/Shannon/Chernoff.lean:699`
  ```
...
- [x] **`sum_prod_rpow_eq_Z_pow`** — `Common2026/Shannon/Chernoff.lean:751`
  ```
...
- [x] **`chernoffZSum_pow_eq_sum_prod`** — `Common2026/Shannon/ChernoffNLetterZSum.lean:36`
  ```
...
- [x] 新規 file `Common2026/Shannon/ChernoffSanovDischarge.lean` 作成、pinpoint import:
...
- Cramér 側の既存 `iIndepFun_eval_under_infinitePi` / `identDistrib_eval_under_infinitePi` (`Common2026/Shannon/Cramer/Discharge.lean`、ChernoffBandMassDischarge.lean:429-433 で reuse) が **adaptation infrastructure を 1 行で供給**。Stein 経路を新規移植する 80-120 行より圧倒的に短い。
...
- `iIndepFun_eval_under_infinitePi`, `identDistrib_eval_under_infinitePi` — `Common2026/Shannon/Cramer/Discharge.lean` (既存)
...
Common2026.lean は変更不要 (既設 import 186/187)。

### Phase 6 — verify + Common2026 編入 + roadmap 📋

- [x] `lake env lean Common2026/Shannon/ChernoffSanovDischarge.lean` silent (実装完了済)。
- [x] `lake env lean Common2026/Shannon/ChernoffBandMassDischarge.lean` silent (実装完了済)。
- [x] `Common2026.lean` 行 186-187 に既に編入済:
  ```
  186:import Common2026.Shannon.ChernoffSanovDischarge
  187:import Common2026.Shannon.ChernoffBandMassDischarge
  ```
...
- 各 Phase: `lake env lean Common2026/Shannon/ChernoffSanovDischarge.lean`,
  `lake env lean Common2026/Shannon/ChernoffBandMassDischarge.lean` が silent
...
- Phase 5 後、predecessor file (`ChernoffPerTiltSanov.lean`, `ChernoffPerTiltDischarge.lean`,
  `ChernoffConverse.lean`) の docstring 編集後は `lake build Common2026.Shannon.ChernoffPerTiltSanov`
  などで olean refresh。
...
3. **2026-05-24 Wave 2 planner refine (本セッション)**: 実装ベースで plan を全面 update。
   ...
   - Phase 6 import 行確認: 既に `Common2026.lean:186-187` に `ChernoffSanovDischarge` / `ChernoffBandMassDischarge` 編入済 (`ChernoffPerTiltSanov` 行 185 の直後)。
