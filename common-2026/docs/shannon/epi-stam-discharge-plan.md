...
> **Status**: **38/39 ok (97%)** (Wave 3 second batch, 2026-05-25 commit
> `0fe2ad4`)。Phase A noop 確認 + Phase B Tier 0/1 完了 + Phase C/D 既完了 (Wave 2 以前)。
> 残 1 件は `IsStamToEPIScalingHyp` (`EPIStamToBridge.lean:147`、launder 疑い defect として
> sister plan `epi-stam-to-conclusion-plan` Phase 0 で cleanup 予定)。詳細は §残 1 件 +
> §Upstream defect 参照。
...
- [x] Phase A — V1→V2 張替 + Cauchy-Schwarz inventory ✅ (noop 確認、Wave 3 second batch)
- [x] Phase B — Blachman score-of-convolution identity ✅ (Tier 0/1 完了、2026-05-25)
- [x] Phase C — λ-optimization + Cauchy-Schwarz application ✅ (Wave 2 以前完了、再確認 2026-05-25)
- [x] Phase D — inverse-Fisher harmonic mean 合成 ✅ (Wave 2 以前完了、再確認 2026-05-25)
- [x] Phase V — verify (`lake env lean ...`) + Common2026.lean 編入 ✅ (38/39 ok、2026-05-25 commit `0fe2ad4`)
- [ ] **残 1 件**: `IsStamToEPIScalingHyp` (`EPIStamToBridge.lean:147`) は **sister plan
      `epi-stam-to-conclusion-plan` Phase 0** で defect cleanup (本 plan 所管外、§Upstream defect 参照) 📋
...
> **完了** (2026-05-25, Wave 3 second batch commit `0fe2ad4`)。実観測は **noop** —
> V1 keyed primitive はすでに V2 へ移行済 (Wave 2 以前)、または対応 declaration は
> 後段で再構成されており Phase A 設計時の張替対象は存在しなかった。
> Cauchy-Schwarz / AM-GM verbatim signature の inventory 追記は Phase B-C 進行中に
> 必要箇所で個別に取り込み済 (`condVar_ae_le_condExp_sq` 等は本 plan §Approach 図に既掲)。
...
> **完了 Tier 0/1** (2026-05-25, Wave 3 second batch commit `0fe2ad4`)。
> `IsStamScoreConvolution` 等の Step12 関連は `EPIStamStep12Body.lean` で 5 件すべて
> `@audit:ok` に降格済 (line 214/253/273/295/309)。本 plan 設計時に B-α 撤退ライン
> (smooth-density witness) で守られていた箇所も honest hypothesis に整理した上で ok。
...
> **完了** (Wave 2 以前、Wave 3 second batch 2026-05-25 再確認 commit `0fe2ad4`)。
> `EPIStamInequalityBody.lean` 7 件 (`IsStamCauchySchwarz` / `IsStamCauchySchwarzOptimal`
> 系、line 113/129/287/307/449/472/485) すべて `@audit:ok`。`stam_lambda_min` の閉形
> により L-Stam-C-β は不要だった (設計通り)。
...
> **完了** (Wave 2 以前、Wave 3 second batch 2026-05-25 再確認 commit `0fe2ad4`)。
> `EPIStamDischarge.lean` 15 件 (line 292-698) すべて `@audit:ok` に降格。`EPIStamToBridge.lean`
> は 13/14 件 `@audit:ok` (残 1 = `IsStamToEPIScalingHyp` line 147、§Upstream defect 参照)。
...
> **完了** (2026-05-25, Wave 3 second batch commit `0fe2ad4`)。5 file 全て
> `lake env lean` clean、`Common2026.lean` 編入済。38/39 件 `@audit:ok` 降格達成。
> 残 1 件 (`IsStamToEPIScalingHyp`) は本 plan 所管外として sister plan に re-route。
...
## 残 1 件 — `IsStamToEPIScalingHyp` (sister plan 所管) 📋

| 項目 | 値 |
|---|---|
| declaration | `IsStamToEPIScalingHyp` |
| location | `Common2026/Shannon/EPIStamToBridge.lean:147` |
| current tag | `@audit:suspect(epi-stam-to-conclusion-plan)` (line 138) |
| 発見 | 2026-05-25 (Wave 3 EPI-Stam discharge agent、本 batch session 内) |
| 種別 | **launder 疑い defect** (本物の Csiszár scaling content 不在) |
| 所管 | sister plan `epi-stam-to-conclusion-plan` Phase 0 (本 plan 所管外) |

**defect 概要** (docstring line 139-146 より要約): predicate body は `g1 = 0` を固定
しているため、実際は `entropyPower (X+Y) − entropyPower X − entropyPower Y ≥ 0`、つまり
EPI 結論そのものに帰着し、Csiszár scaling-monotonicity content を運んでいない。命名
(`Scaling`) が型の語る内容と一致しない (name laundering 兆候)。

**修正方針** (sister plan 所管): 実 `g1` parameter (path endpoint からの値) を導入するか、
monotonicity premise + endpoint identification の 2 つに分割。本 plan では これ以上
触らない (separation of concerns)。

---

## Upstream defect — `IsStamToEPIScalingHyp` (本 plan 配下で発見、cleanup は sister plan)

本 plan 配下 (`EPIStamToBridge.lean`) で Wave 3 second batch session 中に **upstream defect**
を 1 件発見し、`@audit:suspect(epi-stam-to-conclusion-plan)` タグを付与した:

- **declaration**: `IsStamToEPIScalingHyp` (line 147-154)
- **tag 付与 commit**: `0fe2ad4` (Wave 3 second batch、2026-05-25)
- **discharge plan**: **sister plan** [`epi-stam-to-conclusion-plan.md`](./epi-stam-to-conclusion-plan.md)
  Phase 0 で cleanup (refactor or 撤退)

**本 plan では cleanup しない**:

- 本 plan (`epi-stam-discharge-plan`) は **Stam inequality `1/J(X+Y) ≥ 1/J(X) + 1/J(Y)` の
  genuine discharge** を担う。`IsStamToEPIScalingHyp` は **Stam → EPI の bridge** に属し、
  topology 上 sister plan の責務。タグの slug (`epi-stam-to-conclusion-plan`) もそれを反映。
- separation of concerns 上、本 plan の 38/39 ok 完了と独立に進行可能。
- sister plan 側で `IsStamToEPIScalingHyp` を正規 refactor (real `g1` parameter 導入 or
  premise 分割) すると、`EPIStamToBridge.lean:147` の signature が変わり、本 plan の
  `@audit:ok` 群への影響は **ない** (本 plan 配下の declaration は `IsStamToEPIScalingHyp`
  を consumer として参照していない — sister plan 側の bridge wrappers のみ consumer)。
