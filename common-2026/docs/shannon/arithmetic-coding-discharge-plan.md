...
> - `InformationTheory/Shannon/ShannonCode.lean` — `entropyD`, `expectedLength`, `shannonLength`, `kraftSum`,
...
> - `InformationTheory/Shannon/ShannonCodeKraftReverse.lean` — `exists_prefix_code_of_kraft` (`:482`, Kraft 逆向き prefix code 構成, genuine),
>   `IsPrefixFree` (`:47`)
...
>
> **Goal (短形)**: 現 `InformationTheory/Shannon/ArithmeticCoding.lean` (288 行、完全 pass-through:
...
> 依存先は無い (`InformationTheory.lean:132` の import のみ) ため自由書き換え可。
>
> **撤退ライン (本 plan)**:
...
- [ ] Phase V — `InformationTheory.lean` import 確認 + clean check 📋

## ゴール / Approach

### 最終到達点 (Phase 6 完成形)

`InformationTheory/Shannon/ArithmeticCoding.lean` の主合流 (signature は inventory「genuine 目標」と整合):
...
| Phase V | **0-1 行** | `InformationTheory.lean` import 確認 (既に line 132 にある、変更不要) | no |
...
InformationTheory.lean           ← import は既に line 132 にある (変更不要)
```
...
- [ ] imports: `InformationTheory.Shannon.ShannonCode`, `InformationTheory.Shannon.ShannonCodeKraftReverse`,
...
- [ ] `InformationTheory.lean:132` の `import InformationTheory.Shannon.ArithmeticCoding` は既存 (file 名不変のため変更不要、確認のみ)。
- [ ] `lake env lean InformationTheory/Shannon/ArithmeticCoding.lean` silent (0 error / 0 sorry / 0 warning)。
