...
`InformationTheory/Shannon/EPIL3Integration.lean` には EPI L3 integrated pipeline が pass-through
...
|---|---|---|---|
| `InformationTheory/Shannon/EPIL3Integration.lean` | EPI L3 integrated pipeline (de Bruijn 積分経路の集約 file) | 14 | 500 |
...
| V | verify + InformationTheory.lean 編入 + roadmap | ~5-10 |
| **合計** | | **~385-640** |
...
- [ ] Phase V — verify (`lake env lean ...`) + InformationTheory.lean 編入 🚧 (`InformationTheory.lean` 編入は完了、14 件 `@audit:suspect` 降格は sister Phase A 完了後 post-merge cleanup task に移管 — 本 plan 責務外)
...
- [ ] **C-4**: `IsDeBruijnIntegrationHyp X Z P T` の bounded T genuine discharge
      (Gaussian 限定):
  ~30-50 行
- [x] **C-5**: 一般 `X` 用の honest hypothesis `IsDeBruijnTailHyp X Z P` 導入
      (`T → ∞` 部分、Phase D で扱う) — **Wave 3 third batch で試行、独立 audit が
      DEFECT verdict (vacuous-bypass via `Z = 0` + `h_inf : ℝ` で semantically
      empty) を出し retract**。**2026-05-25 honest 再導入完了** (sub-plan
      `epi-debruijn-tail-reintroduction-plan.md`、EReal lift + `Z_law` field 追加、
      Gaussian discharge `isDeBruijnTailHyp_of_gaussian` 5 段 compose、独立 audit
      PASS verdict)。タグ `@audit:suspect(epi-debruijn-tail-reintroduction-plan)`。
      実測 ~139 行 (見積 ~40-60 行から上振れ、Gaussian discharge 5 段 compose 詳細
      展開分): ✅
...
- `lake env lean InformationTheory/Shannon/EPIL3Integration.lean` clean (0 errors / 0 sorry /
  警告最小限)
- `InformationTheory.lean` import 確認 (既に import 済み)
...
- **File:line**: `InformationTheory/Shannon/EPIStamDischarge.lean:177`
...
- **File:line**: `InformationTheory/Shannon/EPIStamDischarge.lean:143`
- **タグ**: `@audit:defect(false-statement)` `@audit:suspect(epi-debruijn-integration-plan)`
- **旧 field**:
  ```
...
- **修正主体**: Wave 3 third batch、Agent A1 (進行中)
- **2026-05-25 caveat 解消完了** (sub-plan
  `epi-debruijn-regularity-refactor-plan.md`、Track B): Wave 3 third batch closure
  audit が追加した caveat
  `@audit:caveat(epi-debruijn-regularity-integrable-deriv-decoupled)` を構造的に
  解消。`density_path` を top-level structure field に昇格 + `density_t_eq` 一致強制
  field 追加で、`reg_at` 内蔵 `density_t` と top-level `density_path t` を pin。
  退化 witness `density_path := 0` は `density_t_eq` 経由で `reg_at` 側
  `derivAt_entropy_eq_half_fisher_v2` の RHS を `0` 固定 → Gaussian の真の
  derivative `1/(2(v+t)) ≠ 0` と矛盾 → 構造的に構成不能。独立 audit
  (`general-purpose` subagent + CORE doctrine inline) Tier 1/2/3 PASS verdict。
  caveat タグ削除済、`@audit:staged(epi-debruijn-regularity)` 維持。
  実測 +47/-28 = net +19 行 (見積 ~40-80 行の下限内)。

### Defect #3 — `IsDeBruijnTailHyp` (Phase C-5 honest hypothesis) — **retracted**

- **File:line**: 元 `InformationTheory/Shannon/EPIL3Integration.lean:589` (現在は撤回コメント)
...
- **2026-05-25 honest 再導入完了**: sub-plan
  `epi-debruijn-tail-reintroduction-plan.md` Phase T-1〜T-V 全完了。`EReal` lift + `Z_law`
  field 両方採用、Gaussian discharge `isDeBruijnTailHyp_of_gaussian` (5 段 compose:
  `differentialEntropy_gaussianConvolution_of_gaussian` → `Real.tendsto_log_atTop` →
  `Tendsto.const_mul_atTop` → `tendsto_atTop_add_const_left` →
  `EReal.tendsto_coe_nhds_top_iff.mpr`) で `h_inf = ⊤` を構成、
  独立 audit (`general-purpose` subagent + CORE doctrine inline) Tier 1/2/3 PASS verdict。
  タグ `@audit:suspect(epi-debruijn-tail-reintroduction-plan)` (SLUG 末尾 `-plan` は
  audit-tags.md convention に従う、normalize 済)。
  retract コメント (`EPIL3Integration.lean:597-615`) は history record として保存。
...
| L-DB-C-β | C | `T → ∞` tail analysis (non-Gaussian) | `IsDeBruijnTailHyp X Z P` (新規 honest、EReal lift + Z_law field、2026-05-25 honest 再導入完了、`@audit:suspect(epi-debruijn-tail-reintroduction-plan)`、Gaussian discharge `isDeBruijnTailHyp_of_gaussian` 提供) | sub-plan `epi-debruijn-tail-reintroduction-plan.md` 完了済、L-DB-C-β は **解除** |
