...
4. **cross-family entanglement は 2 件**:
   ...
   - 本 sweep scope 内: `ChernoffPerTiltSanov.lean:5` + `ChernoffPerTiltDischarge.lean:4`
     の `import Common2026.InformationTheory.Asymptotic` (= utility import、family
     entanglement ではない)
...
- 各 file `lake env lean Common2026/Shannon/Chernoff<X>.lean` が 0 errors、
...
|---|---|---|---|
| `ChernoffConverse.lean:1` | `import Common2026.Shannon.Chernoff` | family 内 (S0) | scope 内 |
| `ChernoffConverse.lean:2` | `import Common2026.Shannon.ChernoffInformation` | family 内 (S0) | scope 内 |
| `ChernoffPerTiltDischarge.lean:1-4` | family 内 + `import Common2026.InformationTheory.Asymptotic` | family 内 + S1 (utility) | scope 内 (DotEq) |
| `ChernoffPerTiltSanov.lean:1-5` | family 内 + `Common2026.InformationTheory.Asymptotic` | 同上 | scope 内 |
| `ChernoffSanovDischarge.lean:1-5` | family 内 + Mathlib のみ | scope 内 | scope 内 |
| `ChernoffInformation.lean:1-2` | family 内 + `Common2026.InformationTheory.Asymptotic` | 同上 | scope 内 |
| (scope 外参考) `ChernoffBandMassDischarge.lean:7` | `import Common2026.Shannon.CramerLC2Discharge` | **S2 (cross-family Cramér)** | **本 sweep scope 外** — successor file で sweep 対象外 |
...
`rg '⚠|HONESTY ALERT|FALSE' Common2026/Shannon/Chernoff{SanovDischarge,PerTilt-
...
- [ ] **2.1.5** Phase 2.1 完了時 olean refresh + verify:
  ```bash
  lake build Common2026.Shannon.ChernoffSanovDischarge
  lake env lean Common2026/Shannon/ChernoffSanovDischarge.lean
  # downstream verification (Phase V で全 file 一括だが暫定確認):
  lake env lean Common2026/Shannon/ChernoffBandMassDischarge.lean
  ```
...
- `lake env lean Common2026/Shannon/ChernoffSanovDischarge.lean` 0 errors。
...
- [ ] **2.2.3** Phase 2.2 完了時 olean refresh + verify:
  ```bash
  lake build Common2026.Shannon.ChernoffInformation
  for f in Common2026/Shannon/ChernoffPerTiltDischarge.lean \
           Common2026/Shannon/ChernoffPerTiltSanov.lean \
           Common2026/Shannon/ChernoffConverse.lean \
           Common2026/Shannon/ChernoffSanovDischarge.lean \
           Common2026/Shannon/ChernoffBandMassDischarge.lean; do
    lake env lean "$f"
  done
  ```
...
- `lake env lean Common2026/Shannon/ChernoffInformation.lean` 0 errors。
...
- [ ] **2.3.5** Phase 2.3 完了時 olean refresh + verify:
  ```bash
  lake build Common2026.Shannon.ChernoffConverse
  for f in Common2026/Shannon/ChernoffPerTiltDischarge.lean \
           Common2026/Shannon/ChernoffPerTiltSanov.lean \
           Common2026/Shannon/ChernoffSanovDischarge.lean \
           Common2026/Shannon/ChernoffBandMassDischarge.lean; do
...
- `lake env lean Common2026/Shannon/ChernoffConverse.lean` 0 errors。
...
- [ ] **2.4.9** Phase 2.4 完了時 olean refresh:
  ```bash
  lake build Common2026.Shannon.ChernoffPerTiltDischarge
  for f in Common2026/Shannon/ChernoffPerTiltSanov.lean \
           Common2026/Shannon/ChernoffSanovDischarge.lean \
           Common2026/Shannon/ChernoffBandMassDischarge.lean; do
    lake env lean "$f"
  done
...
- `lake env lean Common2026/Shannon/ChernoffPerTiltDischarge.lean` 0 errors。
...
- [ ] **2.5.11** Phase 2.5 完了時 olean refresh:
  ```bash
  lake build Common2026.Shannon.ChernoffPerTiltSanov
  for f in Common2026/Shannon/ChernoffSanovDischarge.lean \
           Common2026/Shannon/ChernoffBandMassDischarge.lean; do
    lake env lean "$f"
  done
  ```
...
- `lake env lean Common2026/Shannon/ChernoffPerTiltSanov.lean` 0 errors。
...
- [ ] **2.x.2** **family 外 caller (scope 外 `ChernoffBandMassDischarge.lean`)**:
  ...
  ```bash
  rg -n 'chernoff_lemma_tendsto|chernoff_dotEq_tendsto|chernoff_converse_of_per_tilt_existential|bayesErrorMinPmf_ge_exp_neg_mul_Z_pow|chernoff_converse_from_eps_relaxed|chernoff_converse_of_bandMass|chernoff_converse_from_predicate|chernoff_converse_discharged_from_predicate|chernoff_lemma_tendsto_from_predicate|chernoff_lemma_tendsto_of_per_tilt|chernoff_dotEq_tendsto_of_per_tilt|isChernoffNLetterRN_of_isBayesErrorPerTiltLowerBound|chernoff_lemma_tendsto_via_RN|isChernoffPerTiltDischargeable_of_RN|chernoff_converse_via_RN_forall|chernoff_lemma_tendsto_via_RN_forall|chernoff_dotEq_tendsto_via_RN|chernoff_converse_from_per_tilt|chernoff_lemma_tendsto_from_per_tilt' Common2026/Shannon/ChernoffBandMassDischarge.lean
  ```
...
- [ ] **V.1** 全 5 file (+ scope 外 transitive 受け先 `ChernoffBandMassDischarge.lean`) で `lake env lean` 確認:
  ...
  lake build Common2026.Shannon.ChernoffInformation
  lake build Common2026.Shannon.Chernoff   # base, scope 外だが downstream の base
  lake build Common2026.Shannon.ChernoffConverse
  lake build Common2026.Shannon.ChernoffPerTiltDischarge
  lake build Common2026.Shannon.ChernoffPerTiltSanov
  lake build Common2026.Shannon.ChernoffSanovDischarge
  lake build Common2026.Shannon.ChernoffBandMassDischarge   # scope 外 + transitive

  # 逆 import 順で verify (最 dependent から):
  for f in Common2026/Shannon/ChernoffBandMassDischarge.lean \
           Common2026/Shannon/ChernoffSanovDischarge.lean \
           Common2026/Shannon/ChernoffPerTiltSanov.lean \
           Common2026/Shannon/ChernoffPerTiltDischarge.lean \
           Common2026/Shannon/ChernoffConverse.lean \
           Common2026/Shannon/ChernoffInformation.lean; do
    echo "=== $f ==="
    lake env lean "$f"
  done
  ```
...
- [ ] **V.2** 集計コマンド実行:
  ```bash
  rg -c '@audit:closed-by-successor' Common2026/Shannon/Chernoff{SanovDischarge,PerTiltDischarge,PerTiltSanov,Converse,Information}.lean | awk -F: '{s+=$2} END {print "closed-by-successor:", s}'   # = 0
  rg -c '🟢ʰ' Common2026/Shannon/Chernoff{SanovDischarge,PerTiltDischarge,PerTiltSanov,Converse,Information}.lean | awk -F: '{s+=$2} END {print "🟢ʰ:", s}'   # = 0
  rg -c '@residual\(plan:chernoff-converse-sanov-discharge\)' Common2026/Shannon/Chernoff{SanovDischarge,PerTiltDischarge,PerTiltSanov,Converse,Information}.lean | awk -F: '{s+=$2} END {print "residual:", s}'   # = 19
  rg -c '@audit:retract-candidate' Common2026/Shannon/Chernoff{SanovDischarge,PerTiltDischarge,PerTiltSanov,Converse,Information}.lean | awk -F: '{s+=$2} END {print "retract-candidate:", s}'   # = 3 (既存 2 + 新規 1)
  rg -c '@audit:defect' Common2026/Shannon/Chernoff{SanovDischarge,PerTiltDischarge,PerTiltSanov,Converse,Information}.lean | awk -F: '{s+=$2} END {print "defect (existing tier 5 markers):", s}'   # = 3 (既存維持)
  rg -nw 'sorry' Common2026/Shannon/Chernoff{SanovDischarge,PerTiltDischarge,PerTiltSanov,Converse,Information}.lean | wc -l   # = 19
  ```
...
3. **scope 外 `ChernoffBandMassDischarge.lean` の transitive sorry handling**
   ...
   Phase V.1 で `lake env lean Common2026/Shannon/ChernoffBandMassDischarge.lean`
...
1. **2026-05-26 plan 起草**: lean-planner (本 session、docs-only) が
   `Common2026/Shannon/Chernoff{SanovDischarge,PerTiltDischarge,PerTiltSanov,Converse,Information}.lean`
