...
| `WynerZivConverseChain.lean:611` | `wyner_ziv_tendsto_chain` | `h_ach : wynerZivRatePmf U P_XY d D ≤ R`<br>`h_chain_conv : R ≤ wynerZivRatePmf U P_XY d D` | **境界判定** (auditor 委任) — 2 hyp とも `≤` order claim だが、`le_antisymm` 合成 = 構造的 non-load-bearing。Round 1 Phase 1.5 段階で auditor が「pure forwarder、`@residual` 不要」と判定する可能性あり (Round 1 plan 未決事項 2 で言及済)。本 Phase 2.x.1 では **暫定的に 2 hyp 維持** (auditor 判定後に再評価) | なし |
...
- **境界判定 (auditor 委任)**: **2 件**
  - `wyner_ziv_tendsto_chain` (ConverseChain.lean:611) — 純 forwarder 性 vs.
    `≤` order claim の load-bearing 性
  - `wzAchievability_random_binning_body` (BinningBody.lean:492) — regularity
    probability bound vs. load-bearing claim
...
- [ ] **2.x.1.a.6** `wyner_ziv_tendsto_chain` (line 611): **境界判定** — 暫定で
...
- [ ] **2.x.1.e.1** `wzAchievability_random_binning_body` (line 492): **境界判定**
  ...
  A3 §2 直接編集に委譲。
- [ ] **2.x.1.e.2** 改変なしのため `lake env lean` 確認のみ (0 errors を維持)。
...
- 境界判定 2 件 (`wyner_ziv_tendsto_chain` + `wzAchievability_random_binning_body`)
  は暫定維持、Phase 2.x.4 audit-2 verdict 待ち
...
- [ ] **2.x.4.3** 境界判定 2 件の最終分類:
  - `wyner_ziv_tendsto_chain`: auditor が「pure forwarder、`@residual` 不要、
    ...
    委譲。本 plan 完了時点では `sorry` + `@residual` 維持。
  - `wzAchievability_random_binning_body`: 同様に auditor 判定で scope 外
    降格 + handoff §6 A3 §2 への委譲を確認。
