...
2. **コード上の依存関係 (内部 cross-family)** — `BroadcastChannelSuperposition.lean` が
   `import InformationTheory.Shannon.MACL1Discharge` し、`BroadcastChannelSuperpositionBody.lean`
   が `import InformationTheory.Shannon.MACBodyDischarge` (verbatim 確認)。BC superposition の
...
verbatim 確認方法: `rg -nl <MAC/BC 主要 symbol>` で当該 file 外の use site を `InformationTheory.Shannon`
内で検索、各 file の `rg -n '^import InformationTheory' <target>` で import declaration を確認。
...
| `InformationTheory/Shannon/RelayCutset.lean` / `RelayInnerBound.lean` references `MACInnerBoundExistence` / `BCInnerBoundExistence` / `BCSuperpositionAchievable` / `MACJointTypicalityAchievable` 等 | **docstring 内 prose 言及のみ** (verbatim: `mirrors the MAC ...`、`is the relay analogue of MACCode ...` 等) | **実コード上の symbol 引用なし、`InformationTheory.Shannon.MultipleAccessChannel` / `InformationTheory.Shannon.BroadcastChannel` import なし** (verbatim 確認、Relay import は `ChannelCoding` / `CondMutualInfo` / `MIChainRule` のみ)。entanglement なし。docstring 内 prose を touch しない方針で本 sweep scope を MAC/BC 内部に閉じる |
...
を持たない (verbatim 確認、`rg -B5 'bc_capacity_region_outer_bound\b' InformationTheory/Shannon/BroadcastChannel.lean`)。
...
verbatim 検証: `rg -n '⚠|HONESTY ALERT|FALSE' InformationTheory/Shannon/MultipleAccessChannel.lean
InformationTheory/Shannon/BroadcastChannel.lean InformationTheory/Shannon/MACFanoConverseBody.lean
InformationTheory/Shannon/BroadcastChannelExistenceBridgeBody.lean InformationTheory/Shannon/MACBodyDischarge.lean
InformationTheory/Shannon/MACCornerPoint.lean InformationTheory/Shannon/BroadcastChannelSuperposition.lean
InformationTheory/Shannon/BroadcastChannelSuperpositionBody.lean InformationTheory/Shannon/MACL1Discharge.lean
InformationTheory/Shannon/MACPerEventAEPDecay.lean`
...
- [ ] **1.1** `MultipleAccessChannel.lean` の散文 `🟢ʰ` 言及 9 件 (line 42 / 47 / 50 / 86 / 443 / 534 / 558 / 563 / 747) を docstring refine:
  ...
  - 本体 type-check 影響なし
  - signature 改変なし
  - `lake env lean InformationTheory/Shannon/MultipleAccessChannel.lean` で type-check done 確認 (docstring refine だけなので 0 errors 期待)
...
- [ ] **1.5** Phase 1 完了時:
  ```bash
  rg -n '🟢ʰ' InformationTheory/Shannon/MultipleAccessChannel.lean InformationTheory/Shannon/MACFanoConverseBody.lean InformationTheory/Shannon/BroadcastChannel.lean InformationTheory/Shannon/BroadcastChannelExistenceBridgeBody.lean | wc -l
  ```
...
- [ ] **2.1.7** olean refresh (Pilot Pattern A):
  ```bash
  lake build InformationTheory.Shannon.MultipleAccessChannel
  lake build InformationTheory.Shannon.MACBodyDischarge
  lake build InformationTheory.Shannon.MACFanoConverseBody
  lake build InformationTheory.Shannon.MACCornerPoint
  lake build InformationTheory.Shannon.MACL1Discharge
  lake build InformationTheory.Shannon.MACPerEventAEPDecay
  ```
...
- [ ] **2.2.5** olean refresh:
  ```bash
  lake build InformationTheory.Shannon.BroadcastChannel
  lake build InformationTheory.Shannon.BroadcastChannelExistenceBridgeBody
  lake build InformationTheory.Shannon.BroadcastChannelSuperposition
  lake build InformationTheory.Shannon.BroadcastChannelSuperpositionBody
  ```
...
- [ ] **2.3.4** olean refresh (`BroadcastChannel` / `BroadcastChannelExistenceBridgeBody` を `lake build`)、続いて両 file + 依存先 `BroadcastChannelSuperposition` / `BroadcastChannelSuperpositionBody` を `lake env lean` で再 verify。signature 改変は `bc_common_rate_bound` / `bc_private_rate_bound` のみで dependent caller の rg 検索結果に依存:
  ```bash
  rg -l 'bc_common_rate_bound\|bc_private_rate_bound' InformationTheory/Shannon/
  ```
...
- [ ] **V.1** 全 10 file で `lake env lean` 確認:
  ```bash
  for f in InformationTheory/Shannon/MultipleAccessChannel.lean \
           InformationTheory/Shannon/BroadcastChannel.lean \
           InformationTheory/Shannon/MACFanoConverseBody.lean \
           InformationTheory/Shannon/BroadcastChannelExistenceBridgeBody.lean \
           InformationTheory/Shannon/MACBodyDischarge.lean \
           InformationTheory/Shannon/MACCornerPoint.lean \
           InformationTheory/Shannon/BroadcastChannelSuperposition.lean \
           InformationTheory/Shannon/BroadcastChannelSuperpositionBody.lean \
           InformationTheory/Shannon/MACL1Discharge.lean \
           InformationTheory/Shannon/MACPerEventAEPDecay.lean; do
    echo "=== $f ==="
    lake env lean "$f"
  done
  ```
...
- [ ] **V.2** 集計コマンド実行:
  ```bash
  TARGETS="InformationTheory/Shannon/MultipleAccessChannel.lean InformationTheory/Shannon/BroadcastChannel.lean InformationTheory/Shannon/MACFanoConverseBody.lean InformationTheory/Shannon/BroadcastChannelExistenceBridgeBody.lean InformationTheory/Shannon/MACBodyDischarge.lean InformationTheory/Shannon/MACCornerPoint.lean InformationTheory/Shannon/BroadcastChannelSuperposition.lean InformationTheory/Shannon/BroadcastChannelSuperpositionBody.lean InformationTheory/Shannon/MACL1Discharge.lean InformationTheory/Shannon/MACPerEventAEPDecay.lean"
...
1. **2026-05-25 plan 起草**: lean-planner (本 session、docs-only) が
   `InformationTheory/Shannon/{MultipleAccessChannel, BroadcastChannel, MACFanoConverseBody,
