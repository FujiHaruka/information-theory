...
- **`klDiv (μ.prod ν₁) (μ.prod ν₂) = klDiv ν₁ ν₂` (右因子)** — `InformationTheory/Shannon/MutualInfo.lean:80` `klDiv_prod_const_left` で**既存** (本 project 内、`[IsProbabilityMeasure μ]` 仮定付き)。Stein の i.i.d. 拡張で再利用予定
...
- **本 plan での要点**:
  ...
  - `α : Fintype` のもとでは `(P.rnDeriv Q x).toReal = P.real {x} / Q.real {x}` (`hQpos : Q.real{x} > 0` のとき) — `InformationTheory/Shannon/Bridge.lean` の `klDiv_discrete_toReal_eq_sum` 内で `h_rnD_div : (Q.rnDeriv P x).toReal = Q.real {x} / P.real {x}` として導出済み (再利用可)
...
- **本 plan で使う補題**:
  - `Measure.withDensity_rnDeriv_eq` — `Q ≪ P ⇒ P.withDensity (Q.rnDeriv P) = Q`、`InformationTheory/Shannon/Bridge.lean:248` で再利用前例あり
...
`InformationTheory/Shannon/AEP.lean` で確立済 (432 行、0 sorry) のうち、Stein で再利用するもの:
...
1. **`InformationTheory/Shannon/Stein.lean`** に Stein 全体を書く (新規ファイル、`AEP.lean` を import)。AEP 補題は `open` で取り込み
...
- [x] Phase A skeleton (`InformationTheory/Shannon/Stein.lean` の sorry-driven 出だし) が書ける状態
