...
**ゴール**: `Common2026/Shannon/AWGN.lean:125`
...
- [ ] **import DAG** (`rg -n '^import Common2026' Common2026/Shannon/AWGN*.lean
      Common2026/Draft/Shannon/ContChannelMIDecomp.lean`):
...
```
  ...
  - 兄弟関係の核心: `AWGNBindConvBody` と `ContChannelMIDecomp` は **どちらも import
    していない** (`rg -rln 'AWGNBindConvBody' Common2026/` → 0 件、
...
- **案 A (第一候補) — NEW file `Common2026/Shannon/AWGNMIClosedForm.lean`**:
  `import Common2026.Shannon.AWGNBindConvBody` +
  `import Common2026.Draft.Shannon.ContChannelMIDecomp`。
  `Common2026.lean` の import 順は **`AWGNMIDecompBody` (line 172) と
...
- [ ] (案 A 採用時) `Common2026/Shannon/AWGNMIClosedForm.lean` を skeleton で作成
      (namespace `InformationTheory.Shannon.AWGN`、import 2 本、定理 1 本 `:= by sorry`)。
- [ ] `Common2026.lean` に import 行 1 本追記 (line 173 `ContChannelMIDecomp` の直後)。
...
- [ ] `lake env lean Common2026/Shannon/AWGNMIClosedForm.lean` で 0 errors
      (sorry 0、warning 0)。
...
Common2026 本体内; Draft `ContChannelMIDecomp` 側は既に `_of_out` 経由で独立):
...
- [ ] 旧 closed-form 削除後、`lake env lean` で AWGNMIBridge.lean / AWGNMIDecompBody.lean /
      ...
      (olean refresh 必要、`lake build Common2026.Shannon.AWGN` 1 回)。
