...
`InformationTheory/Shannon/LZ78TreeInducedAEP.lean` に集約 (private helper 共有のため 1 file)。
...
- [ ] `InformationTheory.lean` に `import InformationTheory.Shannon.LZ78TreeInducedAEP` 追記。
- [ ] `lake env lean InformationTheory/Shannon/LZ78TreeInducedAEP.lean` silent (0 sorry / 0 warning)。
...
- `lake env lean InformationTheory/Shannon/LZ78TreeInducedAEP.lean` が silent (0 sorry / 0 warning)。
...
- **library-scale build**: 本 plan は ~500-900 行の major ergodic build。最終 1 回
  `lake build InformationTheory.Shannon.LZ78TreeInducedAEP` で olean 整合確認 (per-fill は
  `lake env lean`)。
