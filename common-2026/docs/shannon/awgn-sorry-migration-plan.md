...
|---|---|---|---|
| `AWGN.lean:1-5` | `InformationTheory.Shannon.ChannelCoding` + `DifferentialEntropy` + Mathlib | family 内 + S0 (utility) | scope 内 |
| `AWGNAchievability.lean:1` | `InformationTheory.Shannon.AWGN` | family 内 (S0) | scope 内 |
...
```bash
rg -l 'IsContChannelMIDecompHyp\|cont_chain_of_awgn_midecomp' InformationTheory/Shannon/
```
...
`rg '⚠|HONESTY ALERT|FALSE' InformationTheory/Shannon/AWGN*.lean InformationTheory/Shannon/ShannonHartley.lean InformationTheory/Shannon/MultivariateDiffEntropy.lean InformationTheory/Shannon/ParallelGaussianPerCoord*.lean` 結果 (19 hit):
...
**downstream consumer 確認**: `rg -l 'IsContChannelMIDecompHyp\|cont_chain_of_awgn_midecomp' InformationTheory/` で `AWGNMIDecompBody.lean` のみ (verbatim 確認済)、cross-family / cross-file ripple **0 件**。
...
- [ ] **1A.3** `AWGNMIDecompBody.lean:161-169` の `theorem awgn_midecomp_of_cont_chain` を以下に書換:
  ...
  theorem awgn_midecomp_of_cont_chain
    ...
    sorry
  ```
- [ ] **1A.4** `lake build InformationTheory.Shannon.AWGNMIDecompBody` で olean refresh (signature 改変 = predicate 削除のため)。
- [ ] **1A.5** dependent file の re-verify (parent .olean 再使用は worktree → main 切替時に必要、CLAUDE.md「After upstream edits」):
  ```bash
  rg -l 'AWGNMIDecompBody\|IsContChannelMIDecompHyp\|awgn_midecomp_of_cont_chain' InformationTheory/Shannon/
  for f in $(rg -l 'AWGNMIDecompBody' InformationTheory/Shannon/ -t lean); do
    lake env lean "$f"
  done
  ```
  期待: family 内 file (AWGNMIBridgeDischarge.lean / AWGNBindConvBody.lean 等の AWGNMIDecompBody 経由 consumer) で 0 errors。
- [ ] **1A.6** `lake env lean InformationTheory/Shannon/AWGNMIDecompBody.lean` で 0 errors (1 sorry warning 許容)。
...
- [ ] **1B.4** `lake env lean InformationTheory/Shannon/AWGNAchievability.lean` で 0 errors 確認。
...
- [ ] **1C.5** `lake env lean InformationTheory/Shannon/AWGN.lean` で 0 errors 確認。
...
- [ ] **1D.5** `lake env lean InformationTheory/Shannon/AWGNAchievabilityDischarge.lean` で 0 errors 確認 (大型 file 1641 行のため re-verify に時間要、Phase 1D は 1 worktree 専有推奨)。
...
- [ ] **1E.3** `lake env lean InformationTheory/Shannon/AWGNMain.lean` で 0 errors 確認。
...
- [ ] **1F.6** 5 file の olean refresh + dependent re-verify (signature 改変 0 件のため olean refresh 不要、ただし lake env lean で個別 verify):
  ```bash
  for f in InformationTheory/Shannon/AWGNF1Discharge.lean InformationTheory/Shannon/AWGNMIBridge.lean InformationTheory/Shannon/AWGNMIBridgeDischarge.lean InformationTheory/Shannon/AWGNBindConvBody.lean InformationTheory/Shannon/AWGNF2F3Discharge.lean; do
    lake env lean "$f"
  done
  ```
...
- [ ] **1G.4** `lake env lean InformationTheory/Shannon/AWGNConverse.lean` で 0 errors 確認。
...
- [ ] **1H.4** `lake env lean InformationTheory/Shannon/ShannonHartley.lean` で 0 errors 確認。
...
- [ ] **V.1** 14 file の `lake env lean` 全件確認 (0 errors、sorry warnings 許容):
  ```bash
  for f in InformationTheory/Shannon/AWGN.lean InformationTheory/Shannon/AWGNAchievability.lean InformationTheory/Shannon/AWGNAchievabilityDischarge.lean InformationTheory/Shannon/AWGNBindConvBody.lean InformationTheory/Shannon/AWGNConverse.lean InformationTheory/Shannon/AWGNF1Discharge.lean InformationTheory/Shannon/AWGNF2F3Discharge.lean InformationTheory/Shannon/AWGNMain.lean InformationTheory/Shannon/AWGNMIBridge.lean InformationTheory/Shannon/AWGNMIBridgeDischarge.lean InformationTheory/Shannon/AWGNMIDecompBody.lean InformationTheory/Shannon/MultivariateDiffEntropy.lean InformationTheory/Shannon/ParallelGaussianPerCoord.lean InformationTheory/Shannon/ParallelGaussianPerCoordRegularity.lean InformationTheory/Shannon/ShannonHartley.lean; do
    echo "===== $f ====="
    lake env lean "$f"
  done
  ```
- [ ] **V.2** 集計コマンド実行 (per-file):
  ...
  rg -c '@audit:suspect\(' InformationTheory/Shannon/AWGN*.lean InformationTheory/Shannon/ShannonHartley.lean InformationTheory/Shannon/MultivariateDiffEntropy.lean InformationTheory/Shannon/ParallelGaussianPerCoord*.lean
  ...
  rg -c '@audit:defer\(' InformationTheory/Shannon/AWGN*.lean
  # 期待値: 0 (Phase 1B で 2 件削除)

  rg -c '🟢ʰ' InformationTheory/Shannon/AWGN*.lean InformationTheory/Shannon/ShannonHartley.lean
  # 期待値: 0 (Phase 1B/1D/1G/1H で全 9 件削除)

  rg -c '@audit:closed-by-successor\(' InformationTheory/Shannon/AWGN*.lean InformationTheory/Shannon/ShannonHartley.lean
  ...
  rg -c '@audit:retract-candidate\(' InformationTheory/Shannon/AWGNF2F3Discharge.lean InformationTheory/Shannon/AWGNAchievabilityDischarge.lean
  # 期待値: 3 (Phase 1F の 2 alias + Phase 2.X auditor 委任の 1 件、auditor verdict ok 時)

  rg -c '@audit:defect\(circular\)' InformationTheory/Shannon/AWGN*.lean
  # 期待値: 2 (`IsAwgnTypicalityHypothesis` 維持 + 新規 `IsAwgnConverseHypothesis` 付与 = Phase 2.audit ok 時)

  rg -nw 'sorry' InformationTheory/Shannon/AWGN*.lean InformationTheory/Shannon/ShannonHartley.lean | wc -l
  # 期待値: 1 (Phase 1A の `awgn_midecomp_of_cont_chain` のみ)

  rg '@residual\(' InformationTheory/Shannon/AWGN*.lean InformationTheory/Shannon/ShannonHartley.lean | wc -l
...
- [ ] **V.3** **parent moonshot banner 更新**: `awgn-moonshot-plan.md` の実態整合 banner (line 2-10) を更新:
  ...
  > (`InformationTheory/Shannon/AWGNMain.lean:60`) は achievability (F-1 `IsAwgnTypicalityHypothesis`) +
...
- **L-MIG-4 expansion (Chernoff pilot 由来、inverted dependency)**: downstream genuine proof が migrated wrapper の old hypothesis を thread しているケース。本 plan の Phase 1A 開始前に grep 確認:
  ```bash
  rg -l 'IsContChannelMIDecompHyp' InformationTheory/
  ```
...
1. **2026-05-26 plan 起草**: lean-planner (本 session、docs-only) が
   `InformationTheory/Shannon/AWGN*.lean` + `ShannonHartley.lean` + `MultivariateDiffEntropy.lean` +
