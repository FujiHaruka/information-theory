...
> Phase 着手前 Mathlib API 在庫調査 (2026-05-20)。対象は `Common2026/Shannon/WynerZivObjectiveConvexityBody.lean:212` の primitive predicate `WynerZivCondEntDiffConvex` を 0-sorry まで discharge できるか。
>
...
**Verdict (b)。** core を直接当てる Mathlib 高レベル補題は **0% 存在** (`klDiv` の joint convexity も `perspective` 関数も log-sum 不等式も Mathlib 不在を loogle で確認)。しかし discharge に必要な**素材 (negMulLog 凹性 + Jensen + 自前 log-sum 不等式) は 100% 既存**で、決定打となる `log_sum_inequality_negMulLog` (Real 値・有限・negMulLog 形・**完全証明済**) が **`Common2026/Fano/DPI.lean:44`** に既にある。これを per-`u` に当てて組み立てる路線で自前 **150〜300 行** 見込み。撤退ライン (joint-convexity-of-KL が Mathlib 不在) は**踏み抜く** が、Common2026 内に等価な自前部品があるため**致命傷ではない**。

---

## 主定理の最終形 (再掲)

`Common2026/Shannon/WynerZivObjectiveConvexityBody.lean:212`:
...
| negMulLog 連続 | `Real.continuous_negMulLog` | NegMulLog.lean | 既存 | block 連続性 (既に Common2026 で活用) |
...
## C. log-sum 不等式 (Mathlib: **不在** / Common2026: **既存・完全証明済**)

| 概念 | API | file:line | 状態 | core での扱い |
|---|---|---|---|---|
| log-sum 不等式 (Mathlib) | — | — | ❌ **不在** | loogle `"log_sum"` → `Real.posLog_sum` のみ (無関係: log の正部分の和) |
| **log-sum 不等式 (Common2026, negMulLog 形)** | `InformationTheory.log_sum_inequality_negMulLog` | **`Common2026/Fano/DPI.lean:44`** | ✅ **既存・proven** | **本 core discharge の決定打** |
| perspective 変換 (negMulLog 形) | `InformationTheory.mul_negMulLog_div` | `Common2026/Fano/BinaryJensen.lean:43` | ✅ **既存・proven** | log-sum 内部で per-atom 変換に使う |

**signature 逐語 (最重要):**

```lean
-- Common2026/Fano/DPI.lean:44
...
```lean
-- Common2026/Fano/BinaryJensen.lean:43
...
> この 2 補題は `Common2026/Fano/DPI.lean` で **DPI (`condEntropy_le_pushforward_condEntropy`, `:184`)** を per-fiber 集約で証明するのに既に使われている (`:217` で fiber `F` に当てている)。本 core では fiber の代わりに **per-`u` block** に同じ engine を当てる。**先行事例として証明テンプレートが既にある**のが大きい。
...
## F. 既存 Common2026 資産 (再利用最優先)

| 概念 | API | file:line | 状態 | core での扱い |
|---|---|---|---|---|
| **log-sum 不等式** | `log_sum_inequality_negMulLog` | `Common2026/Fano/DPI.lean:44` | ✅ proven | **核 engine** (§C) |
| **perspective per-atom 変換** | `mul_negMulLog_div` | `Common2026/Fano/BinaryJensen.lean:43` | ✅ proven | log-sum 補助 |
| log-sum を per-block 集約する型 | `condEntropy_le_pushforward_condEntropy` (証明本体) | `Common2026/Fano/DPI.lean:184` | ✅ proven (構造のみ参照) | **集約テンプレート** (fiber→u block に翻訳) |
| pmf 形 KL (1変数凸) | `klDivPmf` + `klDivPmf_strictConvexOn_left` | `Common2026/Shannon/CsiszarProjection.lean:55,93` | ✅ proven | **1変数凸のみ**。joint convexity ではない → 直接は使えないが per-atom klFun 凸の使い方の手本 |
| pmf 形 MI (entropy 形) | `mutualInfoPmf` | `Common2026/Shannon/RateDistortionAchievability.lean:261` | ✅ 既存 | core の上位 `wzMutualInfo*` の本体定義 |
| R(D) 凸性 (joint-KL を **仮定で**取り回し) | `rateDistortionFunction_convexOn` | `Common2026/Shannon/RateDistortionConvexity.lean:136` | ⚠ proven **but** `h_klDiv_conv` を **hypothesis 化** | **前例: この codebase は joint-KL 凸性を一度も自前 discharge していない** (docstring `:135`「specializations can discharge ... via log-sum inequality」と明記) |
...
| 定義 | file:line | 形 |
|---|---|---|
| `wzMarginalXY` | `Common2026/Shannon/WynerZiv.lean:94` | `(q : α × β × U → ℝ) : α × β → ℝ := fun p => ∑ u, q (p.1, p.2, u)` |
| `wzMarginalXU` | `Common2026/Shannon/WynerZiv.lean:98` | `(q : α × β × U → ℝ) : α × U → ℝ := fun p => ∑ y, q (p.1, y, p.2)` |
| `wzMarginalYU` | `Common2026/Shannon/WynerZiv.lean:102` | `(q : α × β × U → ℝ) : β × U → ℝ := fun p => ∑ x, q (x, p.1, p.2)` |
| `wzMutualInfoXU` | `Common2026/Shannon/WynerZiv.lean:107` | `:= mutualInfoPmf (wzMarginalXU U q)` |
| `wzMutualInfoYU` | `Common2026/Shannon/WynerZiv.lean:112` | `:= mutualInfoPmf (wzMarginalYU U q)` |
| `marginalFst` | `Common2026/Shannon/RateDistortionAchievability.lean:121` | `(q : α × β → ℝ) : α → ℝ := fun a => ∑ b, q (a, b)` |
| `marginalSnd` | `Common2026/Shannon/RateDistortionAchievability.lean:125` | `(q : α × β → ℝ) : β → ℝ := fun b => ∑ a, q (a, b)` |
| `mutualInfoPmf` | `Common2026/Shannon/RateDistortionAchievability.lean:261` | `:= (∑ a, negMulLog (marginalFst q a)) + (∑ b, negMulLog (marginalSnd q b)) − (∑ p, negMulLog (q p))` |
| `wzJointEntXU` | `Common2026/Shannon/WynerZivObjectiveConvexityBody.lean:78` | `:= ∑ p : α × U, negMulLog (wzMarginalXU U q p)` |
| `wzJointEntYU` | `Common2026/Shannon/WynerZivObjectiveConvexityBody.lean:83` | `:= ∑ p : β × U, negMulLog (wzMarginalYU U q p)` |
| `IsWynerZivFactorizable` | `Common2026/Shannon/WynerZivConvexityBody.lean:97` | `(P_XY q) : Prop := ∃ κ : α → U → ℝ, (∀ x u, 0 ≤ κ x u) ∧ (∀ x, ∑ u, κ x u = 1) ∧ (∀ x y u, q (x,y,u) = κ x u * P_XY (x,y))` |
| `IsWynerZivFactorizable_convex_combination` | `Common2026/Shannon/WynerZivConvexityBody.lean:253` | 凸結合が factorisable を保つ (assembly で既に使用) |
...
`Common2026/Shannon/WynerZivCondEntDiffConvexDischarge.lean` の出だし:
...
**(b) Mathlib + 既存 Common2026 部品の組み立てでいける。**

組み立て路線 (1 パラグラフ): core `H(m_YU) − H(m_XU)` の凸性は、(i) `wzMarginalXU/YU` が `q` について affine であること (Mathlib の `Finset.sum_add_distrib`/`Finset.mul_sum` で自前 ~30 行) を使って凸結合を marginal の凸結合に翻訳し、(ii) 各 `u` block について差 `(∑_x negMulLog m_XU(x,u)) − (∑_y negMulLog m_YU(y,u))` の凸性を **`Common2026/Fano/DPI.lean:44` の `log_sum_inequality_negMulLog`** (Real 値・有限・negMulLog 形・**完全証明済**, `mul_negMulLog_div` 補助) を **per-`u` block に当てて** 示し (Fano の per-fiber DPI 証明 `:184-227` がそのままテンプレート)、(iii) `∑_u` で集約して二点 Jensen に縮約 (`Finset.sum_le_sum` + `nlinarith`) する。Mathlib の `klDiv` joint convexity も `perspective` 凸性も log-sum 不等式も**不在** (loogle で全て `Found 0` 確認) だが、それらの **per-atom 等価物は Common2026 内に既証**で、measure 形 (`klDiv : ℝ≥0∞`) で詰んだ R(D) 凸性と違い本 core は **pmf 形 (Real・有限) なので log-sum 不等式がそのまま当たる**。
