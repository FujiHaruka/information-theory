...
- core の上の層 (Z3 base-2 Ziv 不等式 `ziv_count_mul_logb_le_neg_logb_blockProb`、
  ...
  **全て genuine 済** (`InformationTheory/Shannon/LZ78ZivCombinatorics.lean`, sorryAx 非依存)。
...
`InformationTheory/Shannon/LZ78ZivTreeNode.lean`** に集約 (private helper を共有するため
1 file)。
...
- [ ] `InformationTheory.lean` に `import InformationTheory.Shannon.LZ78ZivTreeNode` 追記。
- [ ] `lake env lean InformationTheory/Shannon/LZ78ZivTreeNode.lean` silent (0 sorry / 0 warning)。
...
- `lake env lean InformationTheory/Shannon/LZ78ZivTreeNode.lean` が silent (0 sorry / 0 warning)。
