...
`InformationTheory/Shannon/BroadcastChannel.lean` の 2 declaration を MAC analogue
...
1. **consumer 0 件 (verbatim 確認済)** — `rg -n 'bc_common_rate_bound|bc_private_rate_bound' InformationTheory/`
...
`InformationTheory/Shannon/BroadcastChannel.lean:453-458` 現状:
...
`InformationTheory/Shannon/BroadcastChannel.lean:492-497` 現状 (mirror):
...
```bash
rg -n 'bc_common_rate_bound|bc_private_rate_bound' InformationTheory/ docs/
```

結果 (verbatim):
- `InformationTheory/Shannon/BroadcastChannel.lean:37` (docstring 散文、`bc_*_rate_bound` を thin wrapper として紹介)
- `InformationTheory/Shannon/BroadcastChannel.lean:109` (docstring 散文、同上)
- `InformationTheory/Shannon/BroadcastChannel.lean:453` (`bc_common_rate_bound` 自身)
- `InformationTheory/Shannon/BroadcastChannel.lean:492` (`bc_private_rate_bound` 自身)
- 他 `.lean` 0 件
...
- 触る file: **1 file (`InformationTheory/Shannon/BroadcastChannel.lean`)**
...
- **L-SIGRW-1 (想定外 consumer 出現)**: Phase 0.3 で consumer 0 件を確認済だが、
  ...
  InformationTheory/` を回し、もし新規 caller (本 session 開始から作成された別 file 等)
...
- **L-SIGRW-3 (MAC analogue が signature drift する)**: 本 plan 進行中に MAC 側
  ...
  InformationTheory/Shannon/MultipleAccessChannel.lean | head -10` で直近変更を確認、
...
- [ ] **1.1** Phase 0 撤退ライン L-SIGRW-1 再確認 (`rg -n 'bc_common_rate_bound|bc_private_rate_bound' InformationTheory/` で consumer 0 件継続確認)
- [ ] **1.2** L-SIGRW-3 再確認 (`git log --oneline -- InformationTheory/Shannon/MultipleAccessChannel.lean | head -5`)
- [ ] **1.3** `InformationTheory/Shannon/BroadcastChannel.lean:417-458` 周辺 docstring + signature + body の 1 件 `Edit`:
...
- [ ] **1.4** `lake env lean InformationTheory/Shannon/BroadcastChannel.lean` を実行、0 errors / 0 sorry warning 確認
...
- [ ] **2.2** `InformationTheory/Shannon/BroadcastChannel.lean:460-497` 周辺 docstring + signature + body の 1 件 `Edit` (Phase 1.3 の mirror、indices: `R₂ → R₁`, `I_u → I_xy`, `I_marg_u → I_marg_xy`, `Pe₂ → Pe₁`, `M₂ → M₁`)
- [ ] **2.3** `lake env lean InformationTheory/Shannon/BroadcastChannel.lean` 再 verify
...
- [ ] **V.1** signature 改変があったので `lake build InformationTheory.Shannon.BroadcastChannel` で olean refresh、続いて dependent (`BroadcastChannelSuperposition.lean` / `BroadcastChannelSuperpositionBody.lean` / `BroadcastChannelRandomCodebook*` 系) を `lake env lean` で再 verify
- [ ] **V.2** consumer 0 件 (Phase 0.3 verbatim 確認) ゆえ追加 caller の transitive sorry drift は発生しない想定。dependent ファイルでも `import InformationTheory.Shannon.BroadcastChannel` 経由の symbol 解決のみ、本質的な再 verify は BroadcastChannel.lean 自身

### V.2 集計コマンド (target file)

```bash
TARGET="InformationTheory/Shannon/BroadcastChannel.lean"
...
1. **plan 起草時 (2026-05-26、本 plan 作成 turn)**:
   - 当初 brief「2 declaration の signature rewrite を別 plan として正式起草」を
     受領した時点で、2 declaration の現状を verbatim 確認 (`InformationTheory/Shannon/BroadcastChannel.lean:453, 492`) →
