...
- [x] 軸 5: 既存 InformationTheory 資産の再利用判定 (`shannon_converse_single_shot` 3 形 / `entropy_le_log_card` / Han chain rule)

## ゴール / Approach

AEP Phase D (源符号化定理 weak converse) 着手前に、5 軸で **新規自前構築量 vs Mathlib / InformationTheory 既存資産** を固定する。
...
または同値の `iIndepFun (fun i => Xs i) μ` (InformationTheory 内の流儀に合わせる)。
...
- **rg**: `entropy.*Pi\.|entropy_pi|jointEntropy.*iid|entropy.*nsmul|entropy.*n_smul` in `InformationTheory/` and `.lake/packages/mathlib/` → 0 件 (InformationTheory 内に `klDiv_pi_eq_n_smul` (Stein converse 由来) はあるが entropy 版は無し)
- **loogle**: InformationTheory 内 `entropy` 系で `Pi` を含む補題は `entropy_measurableEquiv_comp` のみ (= reshape のみ、加法則無し)
...
- **`InformationTheory.Shannon.jointEntropy_chain_rule`** (`InformationTheory/Shannon/Han.lean:56`):
  ```lean
...
- **`InformationTheory.Shannon.entropy_pair_eq_entropy_add_condEntropy`** (`InformationTheory/Shannon/Entropy.lean:41`):
  ```lean
...
戦略: `mutualInfo_eq_entropy_sub_condEntropy μ X Y hX hY` + `mutualInfo_eq_zero_iff_indep` (`InformationTheory/Shannon/MutualInfo.lean:109`、両方向) で `mutualInfo = 0` → `condEntropy = entropy`。30〜50 行。
...
### 既存 InformationTheory / 親 inventory での近似形

- **`InformationTheory.MeasureFano.errorProb`** (`InformationTheory/Fano/Measure.lean:73`):
  ```lean
...
## 軸 5: 既存 InformationTheory 資産の再利用判定

### `shannon_converse_single_shot` (3 形)

- **file:line**: `InformationTheory/Shannon/Converse.lean:81` (基本) / `:141` (injective encoder) / `:207` (Markov encoder)
...
- **file:line**: `InformationTheory/Fano/Measure.lean:224`
- **完全署名 verbatim**:
  ```lean
...
- [x] Pi 化 entropy chain rule (`H(X^n) = n · H(X)`) は InformationTheory / Mathlib 共に不在を裏取り
...
- [x] Phase D skeleton (`InformationTheory/Shannon/AEP.lean` 末尾 append、または `InformationTheory/Shannon/SourceCoding.lean` 新ファイル) が書ける状態
