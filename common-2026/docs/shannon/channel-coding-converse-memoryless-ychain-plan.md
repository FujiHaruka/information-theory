...
新規ファイル `Common2026/Shannon/ChannelCodingConverseMemorylessPure.lean` を推奨。
...
- `import Common2026.Shannon.ChannelCodingConverseGeneralStrong` 1 行のみ追加。
...
| C | `Common2026.lean` import 追記 + lake env lean smoke | 5 |
| **合計** | | **~230-400 行** |

**proof-log: yes** (graphoid 経路の構造変換が泥沼化リスク中、Phase A の plumbing 詳細を残す価値あり)。

## Phase 0 — Mathlib / Common2026 在庫確認

### 既存 Common2026 API (再利用)
...
- [ ] (0.2) `loogle "IsMarkovChain"` / `rg "IsMarkovChain"` で Common2026 内既存補題を網羅。
...
- [ ] C.1 `Common2026.lean` に `import Common2026.Shannon.ChannelCodingConverseMemorylessPure` を追記
- [ ] C.2 `lake env lean Common2026/Shannon/ChannelCodingConverseMemorylessPure.lean` で clean を確認
- [ ] C.3 `lake build Common2026.Shannon.ChannelCodingConverseMemorylessPure` で olean 確定 (依存 module の olean 更新確認)
...
**対処**: ChannelCodingConverseMemorylessPure.lean 内で同名の `private lemma` を再宣言 (Common2026 全体の API 表面を増やさず、file-scoped で再利用)。CondMutualInfo.lean に昇格する案もあるが scope 拡大なので保留。
...
- 既存実装: `Common2026/Shannon/ChannelCodingConverseGeneralStrong.lean` (272-321), `Common2026/Shannon/ChannelCodingConverseGeneralComplete.lean` (92-99)
- 補助 API: `Common2026/Shannon/CondMutualInfo.lean:652` (`isMarkovChain_map_left`), `Common2026/Shannon/CondEntropyMemoryless.lean:316,353` (private `isMarkovChain_swap`, `_map_right`)
