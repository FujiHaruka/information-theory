...
| S5 | `epi-uncond-deffix-monotone-plan` (W-Y1) / `epi-uncond-truncation-lsc-plan` (W-Y2) | **方針 Y クリティカルパス (2026-06-07 再評価、smoothing FALSE WALL)**: W-Y1 拡張単調性 + +∞ 伝播 (plan-closeable plumbing、gateway atom 着手済) / W-Y2 entropy 弱収束半連続性 (`wall:gaussian-approx-identity-weak` genuine 残置)。smoothing ルートの「t>0 平滑側無前提 EPI + t→0⁺ 極限」は route T が迂回し不要に。 | case1 (route T closure 済) + S1 | 🚧 W-Y1 着手 (mono_add genuine、2 sorry plan-park) / W-Y2 wall 1 本 |
...
W-Y2 (有限エントロピーすら無い a.c. を救う層、`wall:gaussian-approx-identity-weak` genuine 残置) に分離。
...
6. **2026-06-07 方針 Y 再評価 — smoothing 壁 FALSE WALL 判明 + W-Y1 gateway atom 着手**: 判断ログ 5 末尾「完全無条件は別 moonshot として park」を **ユーザー指示で再評価**し、§方針 Y feasibility verdict (2026-06-05、SUPERSEDED 済) を覆した。
   ...
   - **W-Y1/W-Y2 分離** (`epi-uncond-monotone-inventory.md`): 方針 Y の残壁を 2 本に分離。
     - **W-Y1 = 拡張単調性 + +∞ 伝播** (`h=±∞ 含む a.c.`、両部発散除く): `plan:epi-uncond-deffix-monotone-plan` 所有の **plan-closeable plumbing** (wall でない)。gateway atom `entropyPowerExt_mono_add` (`EPIUncondMonotone.lean:135`、新規 file) を着手 — **−∞ 枝 (`bot_le`) + EReal 単調性 lift (`EReal.exp_monotone`) genuine**、+∞ 伝播 (`differentialEntropyExt_top_of_indep_add` `:77`) + 有限枝 (`:120`) は sorry (`@residual(plan:epi-uncond-deffix-monotone-plan)`)。独立 honesty audit (commit `834af6d`) 4-check 全 PASS = 両 sorry とも honest_residual (循環/バンドル/退化/under-hyp なし、classification `plan:` 妥当 = wall でない)。
   ...
   - **+∞ 伝播の signature 設計 pivot** (W-Y1 closure の鍵): 現 signature は V 特異を許すが、それだと `rnDeriv_map_sum_ae` (V a.c. 要) が呼べず closeable でない。**dispatch 用途で V を a.c. 制限できることを確認** (無条件 a.c. dispatch は X,Y 両 a.c. ゆえ V 常に a.c.)。ただし残 obstruction = 悪い組合せ (h(W)=+∞ ∧ h(V)=−∞) で両 summand とも有限エントロピー無し → 既存 capstone 負部補題 `integrable_negPart_negMulLog_map_sum` (V 有限エントロピー要) が呼べず B_{W+V}<⊤ が直接出せない。Jensen `φ(f∗g)≤(φf)∗g` 経由の V 非依存 B<⊤ bound or EReal 一括単調性の代替 route を proof-pivot-advisor に諮問中 (本セッション)。verdict 次第で W-Y1 closure or honest park を確定。
   - **honest 後退口**: W-Y1 が 1-2 session で closeable なら ±∞ a.c. 無条件化が中間到達点。closeable でなければ dispatch headline は `entropy_power_inequality_of_ac` (a.c.+有限エントロピー、既 proof-done) に留め、W-Y1 を `plan:` のまま park (方針 X より strictly 強い honest 中間形)。
