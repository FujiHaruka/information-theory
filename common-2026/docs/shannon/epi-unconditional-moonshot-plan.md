...
| S5 | `epi-uncond-deffix-monotone-plan` (W-Y1) / `epi-uncond-truncation-lsc-plan` (W-Y2) | **方針 Y クリティカルパス (2026-06-07 再評価、smoothing FALSE WALL)**: W-Y1 拡張単調性 + +∞ 伝播 (plan-closeable plumbing、gateway atom 着手済) / W-Y2 entropy 弱収束半連続性 (`wall:gaussian-approx-identity-weak` genuine 残置)。smoothing ルートの「t>0 平滑側無前提 EPI + t→0⁺ 極限」は route T が迂回し不要に。 | case1 (route T closure 済) + S1 | 🚧 W-Y1 feasibility gate done → restructure (§7-6) → **第一 chunk 着地 (§7-7)**: 新 file `EPIUncondCondEntropyExt.lean` で ①fibre同定/def genuine + (i-a) sorry 消滅 (③合成)、crux を **② EReal chain rule 1 本に局所化** (`@residual(plan:...)`、独立監査 honest_residual PASS)。② closure で W-Y1 一括 proof-done / W-Y2 wall 1 本。**✅ 2026-06-08: ② finite-entropy 版 `differentialEntropyExt_eq_condEntExt_add_klDiv_of_finite` genuine 着地** (step a'/b'/c' + EReal helper 結線、sorryAx-free、`@audit:ok`)。**finiteness-free 版②は per-fibre mass 相殺で証明不能と確定** (advisor 裏取り) → 無条件版は **route β' (truncation+LSC、有限近似の極限)** で別途。次: route β' plan or finite-entropy gateway wiring。SoT → `epi-uncond-chainrule-ext-inventory.md` 冒頭「✅ 完了状態」 |
