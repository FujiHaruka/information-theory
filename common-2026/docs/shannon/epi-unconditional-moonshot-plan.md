...
- [~] Phase 5 — 主定理 assembly = **dispatch endgame** 🚧 → [sub-plan: epi-uncond-dispatch-endgame-plan]: 方針 X 段は達成済 (`entropyPowerExt_add_ge_dispatch_skeleton` 全 4 枝 proof-done だが **21 precondition** 保持、実数版 `entropy_power_inequality_of_ac` proof-done `@audit:ok`)。**残務 = headline wire endgame**: S5 (method-Y gateway、proof-done) を使い、21 precondition を全除去し `hX hY hXY` のみの完全無条件 dispatch `entropyPowerExt_add_ge_unconditional` を新 file `EPIUncondDispatchFull.lean` に別建て (`epi-uncond-dispatch-endgame-plan`、柱 A/B/C/D)。**真の `_unconditional` 命名は ℝ≥0∞ 版でのみ可** (実数版は `EReal.exp_coe` 変換に a.c.+有限を要し完全無条件にならない、論点 → 子 Phase 4) → 判断ログ 7
