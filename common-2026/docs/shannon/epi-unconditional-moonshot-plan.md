...
- [~] Phase 3 — a.c. EPI core (case 1) 🔄 **walled 判定を訂正 (2026-06-05 独立壁再確認)**: difference G3 経路は walled でない — `wall:blachman-general-density` は FALSE WALL (一般密度 producer 実在)、G3 carrier は genuine ratio antitone (sorryAx-free)、残るは 6 AC/integrability assembly + joint-indep under-hyp + density-witness precondition = pure assembly。ratio-limit のみ NO-GO 維持 (entropic CLT 壁)。**方針 X (a.c.+有限分散+有限エントロピー precondition) で closable** → [sub-plan: epi-case1-difference-g3-closure-plan]
...
- [~] Phase 5 — 無条件主定理 assembly 🚧 **方針 X partial 達成**: `entropyPowerExt_add_ge_dispatch_skeleton` が 3-case dispatch (case 2/3 genuine + case 1 park)。**真の `_unconditional` 命名は name laundering ゆえ不可** (16 integrability precondition + case 1 sorry を持つ)。完全達成は case 1 closure (walled) + 方針 Y (別 moonshot) 待ち
...
4. **2026-06-06 S1 retype の false-statement defect 発見 + def-fix** (orchestrator 機械検証 + 独立 honesty audit): Phase 1 (S1 retype, commit `3421948`) で "genuine 0 sorry" とした `entropyPowerExt` は、a.c. 枝 `ofReal(exp(2·differentialEntropy μ))` が Bochner `differentialEntropy` の非可積分時 garbage `0` 返しにより **infinite-entropy a.c. 入力で FALSE-as-stated** だった (機械検証 `/tmp/test_garbage.lean`: 密度 ∝ 1/(x log²x) 等 h=±∞ の a.c. を entropyPowerExt=1 に潰し、dispatch case-1 obligation を偽に)。退化トラップ除去 (特異→0) は正しかったが a.c. 枝の ±∞ 表現が欠けていた。
   ...
   - **dispatch (`EPIUncondMixedCase`)**: case-2 に finite-entropy 前提追加 + `_of_ac_integrable` 化 (genuine 維持)、case-1 の false bare sorry を named wall `entropyPowerExt_add_ge_finite_ac` (`@residual(wall:epi-finite-entropy-ac-classical)`) に置換。**headline TRUE-as-stated 化 + 唯一の sorry を精密 named wall 1 本に局所化**。独立 honesty audit 0 defect。
