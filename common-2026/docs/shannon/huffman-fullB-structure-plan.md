...
> - measure 層剥離 + Hyp2 正確 statement: [`HuffmanMergedIdentBody.lean`](../../InformationTheory/Shannon/HuffmanMergedIdentBody.lean)
>
...
`InformationTheory/Shannon/HuffmanStrongForm.lean` 末尾追記:
...
- `InformationTheory.lean` に `import InformationTheory.Shannon.HuffmanFullBStructure` を 1 行追加。
...
- [ ] `lake build InformationTheory.Shannon.HuffmanFullBStructure` で olean refresh 後、`HuffmanStrongForm` を
...
- [ ] `InformationTheory.lean` に import 1 行追加済を確認。
...
4. **2026-05-21 起草 — file 配置 = 新規 `HuffmanFullBStructure.lean`**: full-B が ~300-400 行と見込まれ、
   ...
   file が自然。Hyp2 + 無引数 publish は `HuffmanStrongForm.lean` 末尾。`InformationTheory.lean` に import 1 行追加。
