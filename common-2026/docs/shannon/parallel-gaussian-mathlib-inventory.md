...
>
> 出力先: Lean 実装は `Common2026/Shannon/ParallelGaussian.lean` (新規)。
> 規約: `CLAUDE.md` の "Subagent Inventory of Mathlib Lemmas" + "Mathlib-shape-driven
> Definitions" に従う。
>
> **Status (2026-05-19)**: 着手前在庫。本ファイルは Phase 0 (在庫調査) の成果物。
...
|---|---|---|---|---|
| `IsAwgnChannelMeasurable` | `def IsAwgnChannelMeasurable (N : ℝ≥0) : Prop := Measurable (fun x : ℝ => gaussianReal x N)` | `Common2026/Shannon/AWGN.lean:63` | ✅ 既存 | per-coordinate `N i` で `∀ i, IsAwgnChannelMeasurable (N i)` を hypothesis pass-through |
...
## C. Common2026 既存資産 (per-coordinate MI / chain rule)

### C.1 — `mutualInfoOfChannel` (本 plan で per-coordinate 利用)

| 概念 | API | file:line | 状態 | T2-B での扱い |
|---|---|---|---|---|
| `Channel α β := Kernel α β` | `abbrev Channel (α β : Type*) [MeasurableSpace α] [MeasurableSpace β] := Kernel α β` | `Common2026/Shannon/ChannelCoding.lean:49` | ✅ 既存 | `parallelGaussianChannel N : Channel (Fin n → ℝ) (Fin n → ℝ)` |
...
| 概念 | API | file:line | 状態 | T2-B での扱い |
|---|---|---|---|---|
| `condMutualInfo_chain_rule_X_2var` | `Common2026/Shannon/MIChainRule.lean` (chain rule 2 var 形) | `MIChainRule.lean` (調査要) | ✅ 既存 | 本 plan では使わず、L-PG1 内部に集約 (`I(X^n; Y^n) = ∑ I(X_i; Y_i)` は per-coordinate chain rule の memoryless specialization、L-PG1 hypothesis に bundle) |

**判断**: parallel ⇒ per-coordinate 分解部分は Mathlib + Common2026 既存 chain rule
で原理的には組めるが、本 plan の scope (~400-600 行) を超える可能性が高い。
**L-PG1 (per-coordinate AWGN F-* hypothesis bundle)** に含める。

### C.3 — `Common2026/Shannon/DifferentialEntropy.lean` (max-entropy)
...
- **discharge plan**: `parallel-gaussian-chain-rule-plan.md` (Tier 3、Common2026
...
| 4 | per-coordinate chain rule `I(X^n;Y^n) = ∑ I(X_i;Y_i)` の memoryless specialization | Mathlib + Common2026 共に直接 lemma 不在の可能性 | L-PG1 hypothesis bundle に含める |
