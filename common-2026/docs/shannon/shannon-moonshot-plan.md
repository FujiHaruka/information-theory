...
> 実態整合 (2026-05-20): DONE-HONEST-HYPS — 主定理 `shannon_converse_single_shot` (`InformationTheory/Shannon/Converse.lean:81`) は 0 sorry で publish。hypothesis は標準 measurability + uniform-message (`hMsg_uniform`) + MI 有限性 (`hMI_finite : mutualInfo μ Msg Yo ≠ ∞`) のみで genuine analytic、pass-through / `Prop := True` 不在。

> **Status (2026-05-10): 🌙 ムーンショット達成 (Phase 4-γ 完了)**。`InformationTheory/Shannon/Converse.lean` で `shannon_converse_single_shot` が sorry ゼロで通過。Phase 4-M0 〜 γ の全段が活性。インベントリは [`docs/shannon/shannon-mathlib-inventory.md`](shannon-mathlib-inventory.md)、最終振り返りは [Phase 4-γ 結果](#phase-4-γ-結果-2026-05-10) 節を参照。
...
- **我々の側**: `InformationTheory/Fano/Measure.lean` で測度論版 Fano (X Fintype, Y 任意可測, deterministic decoder) が完成。`condEntropy` / `errorProb` / `pointwiseErrorProb` を自前定義済み。
...
1. **Phase 4-α だけで成果が立つ**。「Mathlib の `klDiv` に Shannon の MI と DPI を接続した」だけで `InformationTheory/Shannon/MutualInfo.lean` が生まれ、proof-log が取れる。Phase 4-γ に届かなくても価値が残る
...
`InformationTheory.lean` (library root) に対応する `import InformationTheory.Shannon.*` を順次追記。
...
- Phase 4-α の skeleton (`InformationTheory/Shannon/MutualInfo.lean` の出だし) が書ける状態
...
- `InformationTheory.lean` に `import InformationTheory.Shannon.MutualInfo` / `InformationTheory.Shannon.DPI` 追記
...
- ✅ `InformationTheory/Shannon/Converse.lean` が `lake env lean` で silent (124 行 / 0 errors / 0 sorry)
...
- ✅ proof-log + metrics 取得済み ([proof-log-shannon-converse.md](proof-logs/proof-log-shannon-converse.md), [metrics/shannon-converse.metrics.md](metrics/shannon-converse.metrics.md))
...
| **Phase 4-γ (converse)** | `InformationTheory/Shannon/Converse.lean` | **124 行** | (組み合わせのみ) | **plumbing** |
...
- `InformationTheory/Shannon/Converse.lean` (124 行、主定理 + private helper `entropy_of_uniform_msg`)
- `InformationTheory.lean` に `import InformationTheory.Shannon.Converse` 追記
- [`docs/proof-logs/proof-log-shannon-converse.md`](proof-logs/proof-log-shannon-converse.md) — 質的観察 (encoder 落とし判断、`.olean` 鮮度ハマり、PATH ハマり、`omit [Inst] in` パターン)
...
5. **Phase 4-α の skeleton 作成** — `InformationTheory/Shannon/MutualInfo.lean` の sorry-driven 出だし ([`shannon-mathlib-inventory.md`](shannon-mathlib-inventory.md) 末尾の skeleton をそのまま採用) ← **次これ**
