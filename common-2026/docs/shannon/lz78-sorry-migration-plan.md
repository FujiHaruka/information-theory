...
|---|---:|---:|---|
| `@audit:suspect` total | 26 | **30** | `rg -c '@audit:suspect' Common2026/Shannon/LZ78*.lean Common2026/Shannon/LempelZiv78.lean` |
| `@audit:staged` total | 0 | **0** | `rg -c '@audit:staged' Common2026/Shannon/LZ78*.lean` |
| 散文 `🟢ʰ` | 0 | **1** | `rg -c '🟢ʰ' Common2026/Shannon/LZ78*.lean` (`LZ78ZivTreeNode.lean:631`) |
| `@audit:defect(*)` 残存 | (未集計) | **3** | `rg -nc '@audit:defect' Common2026/Shannon/LZ78*.lean` (`LZ78ZivTreeNode.lean:651/712` `degenerate` + `LZ78SMBSandwich.lean:298` `launder`) |
| `@audit:defer` / `@audit:closed-by-successor` | 0 | 0 | (該当なし) |
| 既存 word-boundary `sorry` (実 body) | 0 | **0** | `rg -nw 'sorry' Common2026/Shannon/LZ78*.lean Common2026/Shannon/LempelZiv78.lean` — 該当 0、Pilot Pattern D 計数 |
...
- 各 file `lake env lean Common2026/Shannon/LZ78<X>.lean` が 0 errors、
...
(`rg -n '⚠|HONESTY ALERT|FALSE' Common2026/Shannon/LZ78*.lean`):
...
`rg -n '^import Common2026' Common2026/Shannon/LZ78*.lean Common2026/Shannon/LempelZiv78.lean`
verbatim 確認結果:

- LZ78 group の全 `import` は `Common2026.Shannon.*` 内部のみ (他 family
  namespace への直接 import なし)。
- `LZ78ConverseDischarge.lean` / `LZ78SMBSandwich.lean` / `LempelZiv78.lean`
  は `Common2026.Shannon.ShannonMcMillanBreiman` / `SMBAlgoetCover` /
...
ブリーフは LempelZiv78.lean に 3 件記載していたが verbatim 確認で 0 件 (`rg -c '@audit:suspect' Common2026/Shannon/LempelZiv78.lean` = 0)。本 plan touch 対象外。
...
- [x] cross-family 依存 確認 (`rg '^import Common2026' Common2026/Shannon/LZ78*.lean Common2026/Shannon/LempelZiv78.lean`) → **0 件、`LZ78ConverseDischarge.lean:67` は散文 reference のみで非 entanglement**
...
- [ ] **1.5.1** `LZ78AchievabilityLimsup.lean` 2 件 (line 154 / 236) 書換:
  ...
  - `lake env lean Common2026/Shannon/LZ78AchievabilityLimsup.lean` で 0 errors 確認。
...
- [ ] **1.5.7** Phase 1.5 完了時 各 file で `lake env lean` 確認 + 集計:
  ```bash
  rg -c '@audit:suspect' Common2026/Shannon/LZ78AchievabilityLimsup.lean \
                         Common2026/Shannon/LZ78AsEventualAchievability.lean \
                         Common2026/Shannon/LZ78DistinctEncoding.lean \
                         Common2026/Shannon/LZ78GreedyParsing.lean \
                         Common2026/Shannon/LZ78GreedyParsingImpl.lean \
                         Common2026/Shannon/LZ78SMBSandwich.lean   # = 6 file の合計が次 phase 件数まで段階 0 可能
  rg -c '@residual\(plan:lz78-achievability-converse-plan\)' Common2026/Shannon/LZ78*.lean
  rg -c '@residual\(plan:lz78-blockrv-refactor-plan\)' Common2026/Shannon/LZ78*.lean
  rg -c '@audit:retract-candidate' Common2026/Shannon/LZ78SMBSandwich.lean   # ≥ 2 (alias + @simp lemma)
  ```
...
- [ ] **2.0.5** Phase 2.0 完了時 `lake env lean Common2026/Shannon/LZ78ZivTreeNode.lean` 確認 + olean refresh (`lake build Common2026.Shannon.LZ78ZivTreeNode`) + dependent file (`LZ78ZivTreeBridge.lean` / `LZ78AsEventualAchievability.lean` / `LZ78ZivCombinatorics.lean`) 再 verify (Pilot Pattern A)。
...
- `lake env lean Common2026/Shannon/LZ78ZivTreeNode.lean` 0 errors。
...
- [ ] **2.1.A.6** `lake build Common2026.Shannon.LZ78ConverseDischarge` で olean refresh、`lake env lean Common2026/Shannon/LZ78ConverseDischarge.lean` 0 errors 確認。
- [ ] **2.1.A.7** dependent file (`LZ78FinalGlue.lean` / `LZ78SMBSandwich.lean` / `LZ78ConverseKraft.lean` 等) の type drift を `rg -l 'lz78_converse_lower_bound_(pmfBased|of_pointwise|with_chain|discharge|greedy)' Common2026/Shannon/` で列挙 + 個別 `lake env lean` 再 verify (Pilot Pattern A)。
...
- [ ] **2.1.B.6** `lake build Common2026.Shannon.LZ78FinalGlue` で olean refresh、`lake env lean Common2026/Shannon/LZ78FinalGlue.lean` 0 errors 確認。
...
- [ ] **2.2.5** `lake build Common2026.Shannon.LZ78SMBSandwich` + `lake build Common2026.Shannon.LZ78ConverseKraft` で olean refresh、`lake env lean` 0 errors 確認 + dependent (`LZ78AchievabilityLimsup.lean` 等) 再 verify。
...
- [ ] **2.3.6** `lake build Common2026.Shannon.LZ78ZivCombinatorics` で olean refresh、`lake env lean Common2026/Shannon/LZ78ZivCombinatorics.lean` 0 errors 確認 + dependent (`LZ78ZivTreeNode.lean` / `LZ78AchievabilityLimsup.lean` 等) 再 verify。
...
- [ ] **2.x.2** transitive sorry を引き継ぐ caller を `rg -l 'lz78_two_sided_optimality_(ergodic|greedy_impl|distinct_genuine|distinct_aseventual|distinct_ziv_core_wired)' Common2026/Shannon/` で列挙、各 caller の docstring に **transitive sorry の散文** を追加 (Pilot Pattern C):
  ```
...
- [ ] **V.1** 全 LZ78 file で `lake env lean` 確認 (signature 改変があった file は事前 olean refresh、Pilot Pattern A):
  ```bash
  for f in Common2026/Shannon/LZ78*.lean; do
    echo "=== $f ==="
    lake env lean "$f"
  done
  ```
...
- [ ] **V.2** 集計コマンド実行:
  ```bash
  rg -c '@audit:suspect' Common2026/Shannon/LZ78*.lean | awk -F: '{s+=$2} END {print "suspect:", s}'                       # = 0
  rg -c '@audit:defect\(degenerate\)' Common2026/Shannon/LZ78*.lean | awk -F: '{s+=$2} END {print "defect(degen):", s}'  # = 0 (rename 完了確認)
  rg -c '@audit:defect\(launder\)' Common2026/Shannon/LZ78*.lean | awk -F: '{s+=$2} END {print "defect(laund):", s}'      # = 1 (alias 残置のため)
  rg -c '🟢ʰ' Common2026/Shannon/LZ78*.lean | awk -F: '{s+=$2} END {print "🟢ʰ:", s}'                                       # = 0
  rg -c '@residual\(plan:lz78-residual-discharge-plan\)' Common2026/Shannon/LZ78*.lean | awk -F: '{s+=$2} END {print "residual(residual):", s}'
  rg -c '@residual\(plan:lz78-ziv-inequality-discharge-moonshot-plan\)' Common2026/Shannon/LZ78*.lean | awk -F: '{s+=$2} END {print "residual(ziv-ineq):", s}'
  rg -c '@residual\(plan:lz78-blockrv-refactor-plan\)' Common2026/Shannon/LZ78*.lean | awk -F: '{s+=$2} END {print "residual(blockrv):", s}'
  rg -c '@residual\(plan:lz78-achievability-converse-plan\)' Common2026/Shannon/LZ78*.lean | awk -F: '{s+=$2} END {print "residual(achiev-conv):", s}'
  rg -c '@residual\(defect:false-hypothesis\)' Common2026/Shannon/LZ78*.lean | awk -F: '{s+=$2} END {print "residual(false-hyp):", s}'   # = 3
  rg -c '@audit:retract-candidate' Common2026/Shannon/LZ78*.lean | awk -F: '{s+=$2} END {print "retract-candidate:", s}'   # ≥ 4 (alias + @simp lemma + IsLZ78ZivCombinatorialCoreOverhead def + load-bearing predicate retract判断)
  rg -nw 'sorry' Common2026/Shannon/LZ78*.lean | wc -l
  ```
...
1. **2026-05-25 plan 起草**: lean-planner (本 session、docs-only) が
   `Common2026/Shannon/LZ78*.lean` 20 file + `LempelZiv78.lean` 1 file の
