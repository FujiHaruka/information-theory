...
> 0 sorry 完了済 (`Common2026/Shannon/ChernoffInformation.lean` 241 行)。本 plan は
...
>
> `Common2026/Shannon/ChernoffConverse.lean` (~200-300 行) で:
>
...
- [x] Phase 0 — Mathlib + Common2026 API 在庫再確認 ✅ (既存
...
- [ ] Phase 6 — verify + Common2026 編入 + roadmap 更新 (judgement log) ✅ target
...
- `h_bdd_le` (`IsBoundedUnder (· ≤ ·)`): `chernoff_rate_le_aux_upper` (Chernoff.lean:898) は
  ...
  **問題**: `private` なので外部から呼べない。再構築する: bayesErrorMinPmf の lower bound
  `≥ (1/2) · p_min^n` (Chernoff.lean:953 構成) を Common2026 内で再現する短い lemma
...
Phase 6  : verify + Common2026 編入 + roadmap 更新 (~10 行)        ← ~0.05 セッション
```
...
Common2026.lean                  ← `import Common2026.Shannon.ChernoffConverse` 追記
docs/shannon/
...
- [x] **Common2026/Shannon/ChernoffInformation.lean** (241 行, 0 sorry):
...
- `Common2026/Shannon/SanovLDP.lean` / `SanovLDPEquality.lean` (per-tilt Sanov LDP 完全 discharge
  は L-CC2 撤退で別 plan)
