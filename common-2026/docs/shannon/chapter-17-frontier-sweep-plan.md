...
2. **(B) Minkowski promote**: CT 17.9 Minkowski determinant inequality を新規 declaration として登録する。**ただし Mathlib 在庫 hint (Phase 0 で verbatim 確認) は negative** — `Matrix.det_add` 不在 + `gaussianMultivariate` 不在の **二重壁** で、Phase 2 inventory 結果次第で「shared sorry 補題化 + `@residual(wall:n-dim-gaussian-minkowski)` 撤退」ルートを default に格上げする可能性が高い。1D univariate Gaussian additivity ((A) の rename target) から **直接** 導出可能、というのは 1D (n=1) 退化形のみ; n≥2 では multivariate Gaussian の `differentialEntropy` 表式 (Mathlib 不在) を要求するため、**新規 wall 候補** として正式登録する見込み。
...
- [ ] **0-A-1**: `rg -nF 'entropy_power_inequality_gaussian_saturation' Common2026/ docs/` で全 occurrence 列挙
...
前 plan で記録された **27 件 (Common2026 内) + docs 内** の breakdown:

```
Common2026/Shannon/EntropyPowerInequality.lean    : 3 件 (def 1 + docstring 2)
Common2026/Shannon/EPIStamStep12Body.lean         : 1 件 (docstring)
Common2026/Shannon/EPIStamInequalityBody.lean     : 1 件 (docstring)
Common2026/Shannon/EPIStamToBridge.lean           : 6 件 (docstring 4 + call 2)
Common2026/Shannon/EPIStamDischarge.lean          : 4 件 (docstring 2 + call 2)
Common2026/Shannon/EPIStamStep3Body.lean          : 1 件 (docstring)
Common2026/Shannon/EPIStamDeBruijnConclusion.lean : 2 件 (docstring 1 + call 1)
Common2026/Shannon/EPIL3Integration.lean          : 6 件 (docstring 4 + call 2)
...
- `loogle "ProbabilityTheory.gaussianMultivariate"` → **unknown identifier** (multivariate Gaussian 自体が Mathlib 未収録、univariate `gaussianReal` のみ)。
...
- 既存 declaration: `Common2026/Shannon/EntropyPowerInequality.lean:301` (`@audit:ok` Tier 1、body 22 行 genuine、proof done)
...
- deprecated alias は本 project の他 sweep で導入例なし (`rg '@\[deprecated\]' Common2026/Shannon/` で 0 hit を Phase 1 で再確認)。新規導入はプロジェクト規約から逸脱気味。
- Mathlib 等の external 依存者がいないため後方互換義務もない (`Common2026` namespace 内自己完結)。
...
- [ ] **1-2**: `lean-implementer` 1 件 sequential dispatch (worktree 不要、single-file 系列 edit、main 直接編集)
  - dispatch brief 内容:
    ...
    - 検証: `lake env lean Common2026/Shannon/EntropyPowerInequality.lean` + 各 consumer file `lake env lean Common2026/Shannon/<file>.lean` で 0 errors
    - **継承タグ check** (CLAUDE.md「継承タグの語彙整合 inline check」): rename 後に `rg -n '@audit:|@residual|🟢ʰ' Common2026/Shannon/EntropyPowerInequality.lean` で deprecated タグ残置 0 件を確認 (本 declaration は `@audit:ok` の Tier 1 で、本体タグの変動は想定外)
...
- `lake env lean Common2026/Shannon/EntropyPowerInequality.lean` 0 errors
- 全 consumer file (8 件) `lake env lean` 0 errors
- `rg -nF 'entropy_power_inequality_gaussian_saturation' Common2026/` → 0 hit
- `rg -nF 'entropyPower_gaussian_additivity' Common2026/` → 9+ hit (本体 + consumer)
...
を Common2026 で landing するために必要な Mathlib API を **構造化 per-lemma 形式** (CLAUDE.md「Subagent Inventory of Mathlib Lemmas」: file:line / 完全 signature / 型クラス前提 verbatim / 結論形 verbatim) で網羅する。
...
を Common2026 で landing するための Mathlib API 在庫を per-lemma 形式で網羅する。
...
| 4. multivariate Gaussian lift | **NEGATIVE** | `gaussianMultivariate` 不在、別 plan が必要 |
...
- **(α) 新規 file** `Common2026/Shannon/MinkowskiDeterminant.lean` — multivariate Gaussian / matrix 系の新 family を立てる、将来の凸体 BM / 多次元 EPI と接続しやすい
- **(β) 既存 file** `Common2026/Shannon/EntropyPowerInequality.lean` 末尾追記 — Gaussian additivity の直接 corollary 位置付け、import 増やさない
...
- [ ] **3-3**: skeleton type-check (`lake env lean Common2026/Shannon/MinkowskiDeterminant.lean`) 0 errors
- [ ] **3-4**: 1 sorry ずつ fill、可能な範囲で proof done、不可なら `@residual` 撤退
- [ ] **3-5**: `Common2026.lean` に import 1 行追加
...
| Wall name | 意味 | 関連 textbook 節 |
|---|---|---|
| `n-dim-gaussian-multivariate` | 多次元 Gaussian の Mathlib 立て上げ (`gaussianMultivariate` 不在、PDF + `differentialEntropy` ベクトル版) | Ch.17.9 / Ch.9 AWGN multivariate |
...
- [ ] **V-2**: `Common2026.lean` import 追加箇所確認
- [ ] **V-3**: `rg -nF 'entropy_power_inequality_gaussian_saturation' Common2026/` → 0 hit (rename 完遂確認)
- [ ] **V-4**: `rg "@residual" Common2026/Shannon/MinkowskiDeterminant.lean` で新規 residual を列挙、Phase 3 で意図したものと一致するか確認
