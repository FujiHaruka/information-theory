...
> **Status (2026-05-19)**: 兄弟 plan `chernoff-hoeffding-moonshot-plan.md` で **T1-D Tendsto 形を撤退ライン L-S1 適用で defer**、本 plan に切り出して継続。`hoeffdingE2` 自体の **定義 + min 達成性 + 一意性 + 非負性 + constraint set 凸性** は既に `Common2026/Shannon/Chernoff.lean` (1066 行, 0 sorry, library root 編入済) で publish 済。**残るは Tendsto 形主定理 `hoeffding_tradeoff` のみ**。
> **Predecessor**:
...
> **Goal**: Cover-Thomas Theorem 11.7.x の **n-IID Type II at Type I level `alpha` の指数収束 rate = `hoeffdingE2`** を `Tendsto` 形で publish。新規ファイル `Common2026/Shannon/HoeffdingTradeoff.lean` (~310-410 行) として独立 publish。
...
- [x] Phase 0 — Mathlib + Common2026 API 在庫 ✅ → [`hoeffding-tradeoff-mathlib-inventory.md`](hoeffding-tradeoff-mathlib-inventory.md)
...
- [ ] Phase V — verify + `Common2026.lean` 編入 (オーケストレータ側) 📋

## ゴール / Approach

### 最終到達点

新規ファイル `Common2026/Shannon/HoeffdingTradeoff.lean` で主定理 publish:
...
`hoeffdingConstraintSet` family は **既に `Common2026/Shannon/Chernoff.lean` で publish 済** — 本 plan ではそのまま import して使う。)

判断ログ #1 で **statement 形 (`Tendsto` 直書き vs `DotEq` corollary)** を確定する。
現時点では Common2026 既存スタイル (`stein_lemma`, `chernoff_lemma_achievability`, `sanov_ldp_equality`)
...
**既存 Common2026 plumbing (Csiszar projection / Sanov LDP equality / Stein typicality template) の
...
Phase V   : verify + Common2026.lean 編入 (オーケストレータ)         ← 5 分
```
...
Common2026.lean              ← `import Common2026.Shannon.HoeffdingTradeoff` を追記 (Phase V, オーケストレータ)
```
...
- [x] `Common2026/Shannon/Chernoff.lean` (1066 行, 0 sorry)
  - `hoeffdingE2`, `hoeffdingE2_attained`, `hoeffdingE2_unique`, `hoeffdingE2_nonneg`
...
- [x] `Common2026/Shannon/CsiszarProjection.lean` (487 行)
...
- [x] `Common2026/Shannon/Stein.lean` (1481 行)
...
新規ファイル `Common2026/Shannon/HoeffdingTradeoff.lean` の skeleton (全主定理 + 補助補題を
`:= by sorry` で並べた状態) を Write、LSP 診断で type-check OK 確認 → `steinBetaSet_pmf` /
`steinTypeII_at_level_pmf` 定義 + 基本性質を fill in。

### Done 条件

- `Common2026/Shannon/HoeffdingTradeoff.lean` skeleton (~150-200 行, sorry 多数) が `lake env lean` で
...
- [ ] **A-0 skeleton Write** (CLAUDE.md §「Skeleton-driven Development」):
  ...
  - import 行: `Common2026.Shannon.Chernoff` / `Stein` / `StrongStein` / `SanovLDPEquality` /
    `CsiszarProjection` / `KLDivContinuous` / `Common2026.InformationTheory.Asymptotic` /
...
`Common2026/Shannon/HoeffdingTradeoff.lean` を 0 sorry で確定し、library root に編入。

### Done 条件

- `lake env lean Common2026/Shannon/HoeffdingTradeoff.lean` silent (0 errors, 0 sorry, minimal warnings)
- `Common2026.lean` の Shannon section に `import Common2026.Shannon.HoeffdingTradeoff` 追記 (順序は `Common2026.Shannon.Chernoff` の後ろ、依存上)
- `lake env lean Common2026.lean` silent

### ステップ (オーケストレータ側、本 plan の実装 agent は触らない)

- [ ] **V-1**: `lake env lean Common2026/Shannon/HoeffdingTradeoff.lean` で 0 sorry 確認
- [ ] **V-2**: `Common2026.lean` に import 行追記 (位置: `import Common2026.Shannon.Chernoff` の直後)
...
- **実装 agent は `Common2026.lean` ルートを編集しない**: Phase V で本 plan が完成したあと、オーケストレータが
  まとめて `import Common2026.Shannon.HoeffdingTradeoff` を追加し `lake env lean Common2026.lean` で
  全体 silent を確認する
