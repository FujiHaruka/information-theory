...
2. **(2026-06-05) B2 trigger 発火 + joint-indep 閉鎖路として route-B が再浮上**: 判断ログ 1 の B2 昇格
   trigger 「G2 が genuine 化したら」は **発火済** — G2 系 3 wall (`heatflow-continuity` /
   `approx-identity-L1` / `cond-diff-entropy`) は 2026-06-05 全 CLOSED (`audit-tags.md` 確認、layer-2
   sandwich)。ただし case 1 headline は G3 difference rescale + joint-indep `hXYZXY` で別途 block ゆえ、
   richness 単独での headline ROI は依然低い。**新たな B2 の用途**: case 1 closure (新規 plan
   `epi-case1-difference-g3-closure-plan`) の Phase 2 で `IndepFun (X+Y) (Z_X+Z_Y) P` joint-indep を閉じる
   2 ルートのうち、ルート II が本 plan の `indepFun_add_add_on_lift` (`EPINoiseExtension.lean:122`,
   `@audit:ok`) + `entropy_power_inequality_via_lift` (`:145`, `@audit:ok`) による lift 再配線。ルート I
   (in-place `iIndepFun.indepFun_add_add` 経由、`IsStamScalingNoiseHyp` を 4-tuple joint に強化) が現
   Mathlib で成立すれば I 優先 (blast radius I < II)、不成立なら本 plan B2 のフル lift 再配線が採用される。
   B2 (偽 in-place `stamScalingNoise_exists` 完全除去) は case 1 closure plan が消費する形で実行。
