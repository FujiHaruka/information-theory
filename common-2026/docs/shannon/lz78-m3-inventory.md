...
> `InformationTheory/Shannon/LZ78/GreedyParsingImpl.lean:442`）。
...
## M3 壁の最終形（再掲、verbatim from `GreedyParsingImpl.lean:442`）
...
|---|---|---|---|---|---|
| 符号長 def | `lz78GreedyImplEncodingLength`<br>`GreedyParsingImpl.lean:78` | `(n : ℕ) (x : Fin n → α) : ℕ`、section vars `{α : Type*} [Fintype α] [DecidableEq α]` | `let c := (lz78PhraseStrings (List.ofFn x)).length; c * LZ78Phrase.bitLength c (Fintype.card α)` | **bit**（`bitLength = Nat.log 2 …`） | Step A の被分子 |
| 定数 rate 上界 | `lz78_impl_rate_le_const`<br>`GreedyParsingImpl.lean:168` | `[Nonempty α] (n : ℕ) (x : Fin n → α)` | `(lz78GreedyImplEncodingLength n x : ℝ) / (n : ℝ) ≤ (1 + 8 * Real.log (Fintype.card α + 1) / Real.log 2) + ((Nat.log 2 (Fintype.card α) : ℝ) + 2)` | bit | headline の `h_bdd_above` 内製に使用済。M3 では limsup 有界性（cobounded）に再利用可 |
| per-symbol 上界 | `lz78_impl_encoding_length_per_symbol_le`<br>`GreedyParsingImpl.lean:125` | `(n : ℕ) (hn : 0 < n) (x : Fin n → α)` | `(lz78GreedyImplEncodingLength n x : ℝ) / (n : ℝ) ≤ (Nat.log 2 (n + 1) : ℝ) + (Nat.log 2 (Fintype.card α) : ℝ) + 2` | bit | 参考 |
| per-symbol 非負 | `lz78_impl_encoding_length_per_symbol_nonneg`<br>`GreedyParsingImpl.lean:144` | `(n : ℕ) (x : Fin n → α)` | `(0 : ℝ) ≤ (lz78GreedyImplEncodingLength n x : ℝ) / (n : ℝ)` | — | limsup 下界・cobounded |
| `Nat.log 2 → Real.log` 橋 | `lz78_impl_natLog_mul_log_two_le`<br>`GreedyParsingImpl.lean:152` | `(m : ℕ)` | `(Nat.log 2 m : ℝ) * Real.log 2 ≤ Real.log m` | nat↔bit 変換 | **Step A/C の単位整合の核**。`Nat.log 2 (c+1)` を `Real.log (c+1)/Real.log 2` に降ろす |
...
`ZivCountingBody.lean` / `GreedyParsingImpl.lean` に追記）の出だし:
...
import InformationTheory.Shannon.LZ78.GreedyParsingImpl   -- lz78GreedyImplEncodingLength, lz78_impl_rate_le_const
