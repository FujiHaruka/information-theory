...
`rg -n '@audit:retract-candidate\(load-bearing-predicate\)' Common2026/Shannon/AWGN*.lean`
(commit `37284f1` 時点):
...
`rg -n '^import' Common2026/Shannon/AWGN*.lean Common2026.lean` および
`rg -nc '<predname>' Common2026/Shannon/` で確認 (2026-05-28):
...
- **Route B — shared sorry 補題化 (共有 Mathlib 壁、`docs/audit/audit-tags.md`
  ...
  `Common2026/Shannon/AwgnWalls.lean` の **shared sorry 補題** に格上げする
...
- **Phase 2 — shared sorry 補題 file 新設** (`Common2026/Shannon/AwgnWalls.lean`):
...
- [ ] **0-2**: 各 declaration の **hypothesis-form consumer** を
      `rg -n '<PredName>' Common2026/Shannon/` で列挙、本 plan の
...
- [ ] **0-5**: import 依存 verbatim 確認 — `Common2026/Shannon/AwgnWalls.lean` (新規) が
      持つべき import = `Common2026.Shannon.AWGN` (`gaussianCodebook` / `awgnChannel`
      access) + `Common2026.Shannon.ChannelCoding` (`AwgnCode` / `errorProbAt`) +
...
- [ ] **2-5**: `Common2026.lean` に `import Common2026.Shannon.AwgnWalls` を 1 行追加
- [ ] **2-6**: `lake env lean Common2026/Shannon/AwgnWalls.lean` で 0 errors / 3 sorry
      warning 確認
...
- [ ] **3α-8**: file 単独で `lake env lean Common2026/Shannon/AWGNConverseDischarge.lean`
      + `lake env lean Common2026/Shannon/AWGNConverse.lean` の 2 file 検証 0 errors
...
- [ ] **3β-8**: `awgn_theorem_F4_discharged_F1_via_staged` (`line 1622`) signature 書換:
      同上
- [ ] **3β-9**: file 単独で `lake env lean Common2026/Shannon/AWGNAchievabilityDischarge.lean`
      検証 0 errors
...
- [ ] **V-1**: 全 touched file の `lake env lean` 検証 0 errors:
      ...
      `AWGNAchievabilityDischarge.lean` / `Common2026.lean` (import 1 行追加)
...
1. `Common2026/Shannon/AwgnWalls.lean` が新設され、3-6 shared sorry 補題が
