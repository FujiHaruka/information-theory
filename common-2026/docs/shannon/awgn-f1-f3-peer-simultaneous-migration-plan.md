...
| Mathlib re-build | `lake build Common2026.Shannon.AWGNAchievability` 1 回 + dependent | `lake build Common2026.Shannon.AWGNConverse` 1 回 + dependent | 2 回 |
...
| file | 触る種別 | 内容 | Phase |
|---|---|---|---|
| `Common2026/Shannon/AWGNAchievability.lean` | predicate 削除 + wrapper sorry 化 | F-1 primary | Phase 1 |
| `Common2026/Shannon/AWGNConverse.lean` | predicate 削除 + wrapper sorry 化 | F-3 primary | Phase 2 |
| `Common2026/Shannon/AWGNF2F3Discharge.lean` | F-2 + F-3 alias 削除 + cascade wrapper signature 縮約 | F-1 + F-3 cascade | Phase 1 + Phase 2 |
| `Common2026/Shannon/AWGNMain.lean` | `awgn_channel_coding_theorem` の `h_typicality` + `h_converse` 削除 | downstream | Phase 1 + Phase 2 |
| `Common2026/Shannon/AWGNF1Discharge.lean` | `awgn_theorem_F1_discharged` の 2 hyp 削除 | downstream | Phase 1 + Phase 2 |
| `Common2026/Shannon/AWGNBindConvBody.lean` | `awgn_theorem_of_typicality_converse_bindconv_discharged` の 2 hyp 削除 | downstream | Phase 1 + Phase 2 |
| `Common2026/Shannon/AWGNMIBridge.lean` | `awgn_theorem_F2_discharged` の 2 hyp 削除 | downstream | Phase 1 + Phase 2 |
| `Common2026/Shannon/AWGNMIBridgeDischarge.lean` | `awgn_theorem_of_typicality_converse_bindconv` の 2 hyp 削除 | downstream | Phase 1 + Phase 2 |
| `Common2026/Shannon/AWGNAchievabilityDischarge.lean` | `awgn_theorem_F4_discharged_F1_via_staged` の `h_converse` 削除 + `isAwgnTypicalityHypothesis` constructor (line 931) の取扱判定 | downstream + analytic discharge artifact | Phase 1 + Phase 2 |
...
- olean refresh の順序 (Phase 1 完了後 `lake build Common2026.Shannon.AWGNAchievability` →
  全 downstream 個別 verify、Phase 2 完了後 `lake build Common2026.Shannon.AWGNConverse` →
...
`Common2026/Shannon/AWGNAchievability.lean:47-53` (verbatim):
...
`Common2026/Shannon/AWGNConverse.lean:69-77` (verbatim):
...
`Common2026/Shannon/AWGNF2F3Discharge.lean:194-200` (verbatim、`IsAwgnTypicalityHypothesis` と同型):
...
`Common2026/Shannon/AWGNF2F3Discharge.lean:253-261` (verbatim、`IsAwgnConverseHypothesis` と同型):
...
`rg -n 'IsAwgnTypicalityHypothesis|IsAwgnConverseHypothesis|IsAwgnF2DecodingHypothesis|IsAwgnF3ChainHypothesis' Common2026/Shannon/`
...
- [ ] `lake env lean Common2026/Shannon/AWGNAchievability.lean` 0 errors (1 sorry warning 許容)
- [ ] `lake build Common2026.Shannon.AWGNAchievability` で olean refresh (predicate 削除のため)
...
- [ ] `lake env lean Common2026/Shannon/AWGNConverse.lean` 0 errors (1 sorry warning 許容)
- [ ] `lake build Common2026.Shannon.AWGNConverse` で olean refresh
...
1. **全 9 file `lake env lean` 0 errors 確認** (Phase 0 で baseline 確認済、Phase 1 + Phase 2
   完了時に再 verify)。
2. **`lake env lean Common2026/Shannon/AWGN*.lean ShannonHartley.lean` 横断 silent 確認** (AWGN
...
| **olean refresh 連鎖破綻** (Phase 1 完了後 Phase 2 で predicate 削除 olean refresh が AWGN family 全 14 file に波及) | 低 | 中 (`lake build Common2026.Shannon.<家系>` 個別 invoke で吸収) | Phase 1 完了直後 `lake build` 1 回 + dependent 個別 verify |
...
| (新規候補) `wall:fano-data-processing-continuous` | ✗ achievability には不要 | ✅ converse の Fano + data processing chain | F-3 専用 wall、shared 化不要、`Common2026/Fano/Measure.lean` 既存資産 reuse 検討 |
| (新規候補) `wall:per-letter-gaussian-max-entropy` | ✗ | ✅ converse の per-letter MI bound | F-3 専用、`differentialEntropy_le_gaussian_of_variance_le` の Common2026 既存資産 reuse |
