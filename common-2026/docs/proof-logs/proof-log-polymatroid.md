...
`InformationTheory/Shannon/Polymatroid.lean` で entropy の **polymatroid rank function**
...
**最終定理** (`InformationTheory/Shannon/Polymatroid.lean`):
...
- `InformationTheory/Shannon/Polymatroid.lean` — 288 行、0 errors / 0 sorry / 0 warning
  - 主定理 3 本 (Phase A〜C)
  - 補助 helper 2 本 (再利用可):
...
- `InformationTheory/Shannon/HanD.lean` — 既存 3 declarations (`subsetIdxEquiv`,
  `subsetSplitMEquiv`, `subsetSplitMEquiv_apply`) を `private` から **公開化**
  (Polymatroid.lean からの再利用のため)
- `InformationTheory.lean` に `import InformationTheory.Shannon.Polymatroid` 追記
- `docs/han/polymatroid-moonshot-plan.md` Phase A〜C ✅ + Phase D 判断ログ
- Phase D は **(D-b)「別 plan に切り出し」採用**、本 plan は core delivery で close

`lake env lean InformationTheory/Shannon/Polymatroid.lean` silent / `lake build` 全体緑通過
(2772 jobs)。
...
**観察**: 本 project の `InformationTheory/Shannon/` は moonshot ごとに **並列の枝** で
...
- **`subsetIdxEquiv` / `subsetSplitMEquiv` / `subsetSplitMEquiv_apply` を `Pi.lean`
  ...
  再利用される。`InformationTheory/Shannon/Pi.lean` の Pi 値 reshape 補題 family に
  自然に収まる
