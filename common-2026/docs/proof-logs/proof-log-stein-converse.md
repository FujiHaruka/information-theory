...
`docs/moonshot-seeds.md` 「A. 直接 deferred」項目 "Stein converse (Phase C/D)" の Phase A〜B 本実装。Stein achievability (`InformationTheory/Shannon/Stein.lean` Phase A〜B、別セッション完了) の対偶半分:
...
- `InformationTheory/Shannon/Stein.lean` (+497 行 → 累計 ~1123 行) — Phase A: `klDiv_pi_eq_n_smul` (Pi 化 KL chain rule、98 行 induction) / Phase B: Bool 上 sum 形 + `mul_log_le_toReal_klDiv` 下界 + α-level + sign 解析 → `stein_converse_finite_n` (concrete inequality with `1/(1-ε)` correction)
- `InformationTheory/Shannon/DPI.lean` (1 行) — `klDiv_map_le` を `private` から public 化 (caller `mutualInfo_le_of_postprocess` のみ、破壊リスクゼロ)
- `lake env lean InformationTheory/Shannon/Stein.lean` / `InformationTheory/Shannon/DPI.lean` 共に silent
- `lake build InformationTheory.Shannon.Stein` 通過 (2758/2758)
...
- **Pi 化 KL chain rule** (`klDiv (Measure.pi _) (Measure.pi _) = n • klDiv P Q`) — 自前 98 行 induction (`klDiv_pi_eq_n_smul`)。`piFinSuccAbove` + 既存 `klDiv_map_measurableEquiv` (InformationTheory 内、本 Track 開始時点で既存) + `klDiv_prod_const_left` で組成。**上流 PR 候補**。
...
- **DPI on KL** (集合関数 KL に対する `klDiv (κ ∘ μ) (κ ∘ ν) ≤ klDiv μ ν`) — Mathlib 不在、InformationTheory 既存 `InformationTheory/Shannon/DPI.lean:52` `klDiv_map_le` (`private` だった、本 Track で public 化)。Jensen + condExp で 50〜100 行構築済 (別セッション)。
...
**教訓**: Pi 値 measure 系 (Pi 値 KL chain rule、Pi 値 abs continuity、Pi 値 isFiniteMeasure 等) は Mathlib に系統的に薄い。Sanov / AEP / Stein 系のいずれを実装する時も同種の補題不在に当たる前提で plan すべき。InformationTheory で蓄積した `InformationTheory/Shannon/Pi.lean` 系の補題群は Mathlib 上流化候補のクラスタ。
...
**症状**: Bool 上の KL を sum 形に展開する際、`InformationTheory/Shannon/Bridge.lean` の `private klDiv_discrete_toReal_eq_sum` が直接使えず、~50 行を `klDiv_bool_toReal_eq_sum` として Stein.lean 内に duplicate。

**抜け方**: duplicate (path of least resistance)。public 化リファクタは別 Track として保留。

**教訓**: `private` は file-scoped (CLAUDE.md 既載) のため、ファイルをまたぐ再利用には public 化必須。今回は Track 3 のスコープを小さく保つため duplicate を選んだが、3 番目の caller が現れた時点で `InformationTheory/Shannon/KLDivSum.lean` (仮) のような共通 helper module に lift すべき。Track 1 で `condEntropy_nonneg` を Pi.lean に lift した時と同じ判断軸。
...
- 採らなかった代替案: (i) Bernoulli KL 閉形式を自前で書いて使う — `mul_log_le_toReal_klDiv` 経由が短いため不採用、(ii) `fDiv` framework を InformationTheory 内に新規構築して KL を inst — Stein 単体には過剰、(iii) Phase C (Tendsto 統合) を本 Track で続行 — 行数 +200 で 700 行超、context 圧迫リスクで別 plan に分離。
