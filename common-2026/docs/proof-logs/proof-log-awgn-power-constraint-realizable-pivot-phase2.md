...
- `Common2026/Shannon/AWGNAchievabilityDischarge.lean` — 989 行（pivot 前 1563 行、−574 行）、0 errors / 1 warning（`isAwgnTypicalityHypothesis` body の sorry 1 件、Phase 3 で fill 予定）
- 新規 predicate 2 件:
...
`lake env lean Common2026/Shannon/AWGNAchievabilityDischarge.lean` 出力は `Common2026/Shannon/AWGNAchievabilityDischarge.lean:910:8: warning: declaration uses 'sorry'` のみ。
...
- **外部依存の壊れ**: `grep -rn` で Common2026/ 配下に `isAwgnTypicalityHypothesis` / `awgn_achievability_F1_via_staged_hyps` / `awgn_theorem_F4_discharged_F1_via_staged` / `IsAwgnPowerConstraintRealizable` の external dependent が無いことを 1 発で確認、横断的修正不要。
...
```bash
# body 範囲特定
grep -n "linarith \[h_awg, hε₁_le_ε\]\|^theorem awgn_achievability_F1_via_staged_hyps\|^theorem isAwgnTypicalityHypothesis\|^theorem awgn_theorem_F4" Common2026/Shannon/AWGNAchievabilityDischarge.lean

# anchor の uniqueness 検証
grep -cn "intro R hR_pos hR ε hε" Common2026/Shannon/AWGNAchievabilityDischarge.lean   # 2 (docstring + body)
grep -cn "linarith \[h_awg, hε₁_le_ε\]" Common2026/Shannon/AWGNAchievabilityDischarge.lean   # 1

# body 削除
sed -i '' '985,1552d' Common2026/Shannon/AWGNAchievabilityDischarge.lean

# 外部依存の確認
grep -rn "isAwgnTypicalityHypothesis\|awgn_achievability_F1_via_staged_hyps\|awgn_theorem_F4_discharged_F1_via_staged\|IsAwgnPowerConstraintRealizable" Common2026/ --include="*.lean" | grep -v "AWGNAchievabilityDischarge.lean"

# 検証
lake env lean Common2026/Shannon/AWGNAchievabilityDischarge.lean
```
