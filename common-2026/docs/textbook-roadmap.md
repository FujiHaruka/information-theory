...
| 9 | Gaussian Channel | 🟡 (① ✅ / ② frontier 縮小) | **① 単一文字情報容量 `awgn_capacity_closed_form_genuine` = (1/2)log(1+P/N) genuine ✅** (0/0、sorryAx 非依存、監査 OK) + hypothesis-free MI 後継 ✅ + ParallelGaussian 電力制約 lintegral pivot ✅ + F-1 / L-PG0 / L-WF1/WF2 discharge。**② 進展 (2026-05-29): subadditivity 壁 genuine ✅ + `wall:multivariate-mi` (achiever MI per-channel 加法性 = compProd-of-pi factorization + lintegral Fubini) genuine closure ✅ (監査 OK、sorryAx 非依存)** | **② Parallel Gaussian water-filling** `parallel_gaussian_capacity_formula_minimal` の残 frontier: (a) achiever_mi の per-coord MI 値 `h_perCoordMI` + finiteness `awgn_mutualInfoOfChannel_ne_top` [`@residual(plan:l-pg1-discharge)`、AWGN single-channel MI 解析、① の `.toReal` 経路からは closeable でない] (b) `max_ent` の channel↔RV decomp `h_decomp` + per-coord max-entropy `h_perCoord` (c) `bddAbove`。bottleneck = AWGN single-channel MI bridge family (`IsAwgnOutputGaussian`/`IsAwgnMIDecomp` load-bearing)。**③ operational 符号化定理は scope-out (judgment #11)**。Shannon-Hartley Nyquist も scope-out |
...
- **Ch.9 ② Parallel Gaussian water-filling**: ① 単一文字容量 genuine ✅。② frontier 縮小 (2026-05-29): subadditivity 壁 + `wall:multivariate-mi` (achiever MI per-channel 加法性) 両方 genuine closure ✅。残 frontier = (a) achiever_mi の per-coord MI 値 + finiteness `awgn_mutualInfoOfChannel_ne_top` (b) `max_ent` の channel↔RV decomp + per-coord max-entropy (c) `bddAbove`。bottleneck は AWGN single-channel MI bridge family (`IsAwgnOutputGaussian`/`IsAwgnMIDecomp` load-bearing) の closure — 次セッション target。③ operational 符号化定理は scope-out (judgment #11)
