...
- **Ch.5 Huffman 強形**: Hyp1 (SwapNormalization) 無条件 discharge 済 ✅ + 決定的 relabel cornerstone `huffmanLengthAux_relabel_det` genuine ✅ (2026-05-30)。残 frontier = collapse correspondence (`MergedHuffmanAuxIdentHypothesis`、`HuffmanColexDeterminism.lean:316` `collapseLabel_huffmanLengthAux` + `HuffmanWalls.lean:70` honest sorry)。**壁 = Huffman 語長の tie-order 独立 invariant** (確率 multiset が depth を決め tie-break colex 選択に非依存、~150-250 行 self-build、Mathlib 壁でも C3 tie-invariance でもない)。次セッション target は this invariant の機械化
