...
| 9 | Gaussian Channel | 🟡 (① ✅ / ② achiever genuine・converse のみ residual) | **① 単一文字情報容量 `awgn_capacity_closed_form_genuine` = (1/2)log(1+P/N) genuine ✅** (0/0、sorryAx 非依存、監査 OK) + hypothesis-free MI 後継 ✅ + ParallelGaussian 電力制約 lintegral pivot ✅ + F-1 / L-PG0 / L-WF1/WF2 discharge。**② achiever side 完全 genuine ✅ (2026-05-29 judgment #13): subadditivity 壁 + multivariate-mi 加法性 + AWGN single-channel MI finiteness (`awgn_mutualInfoOfChannel_ne_top`、false-statement edge を `N≠0` で正直に修正) + per-coord MI 値 (`mutualInfoOfChannel_gaussianInput_closed_form'` を in-body 適用、Q=0 退化も `klDiv_self` で genuine) すべて genuine closure・sorryAx 非依存。headline の load-bearing hyp 3 種 (`h_bdd_global`/`h_multivar_decomp`/`h_perCoordMI`) を honesty 監査で tier-5 検出 → drop + sorry/genuine 化 (旧 `@audit:ok` は誤付与、是正済)** | **② Parallel Gaussian water-filling** 残 residual = **converse 側 2 件のみ** (`ParallelGaussianPerCoordRegularity.lean:120` `bddAbove` / `:127` `max_ent`、`@residual(wall:multivariate-mi)`)。独立 wall 再検証で **self-buildable と判定** (真の Mathlib 壁ではない、~285-430 行、1-D template の `Fin n` lift)。closure plan = `docs/shannon/parallel-gaussian-converse-closure-plan.md` (7 Phase、core = `h_decomp` の `Fin n→ℝ` lift)。**③ operational 符号化定理は scope-out (judgment #11)**。Shannon-Hartley Nyquist も scope-out |
...
- **Ch.9 ② Parallel Gaussian water-filling**: ① 単一文字容量 genuine ✅。**② achiever side 完全 genuine ✅ (2026-05-29 judgment #13)** — finiteness (`N≠0` で false-statement edge 修正) + per-coord MI 値 (genuine closed form in-body) + multivariate-mi 加法性 + subadditivity すべて genuine、headline の load-bearing hyp 3 種を honesty 監査で tier-5 検出 → 是正。**残 residual = converse 2 件のみ** (`bddAbove`/`max_ent`、`@residual(wall:multivariate-mi)`)。独立 wall 再検証で self-buildable 判定 (~285-430 行、1-D template の `Fin n` lift)、真の Mathlib 壁ではない。次セッション target = converse closure (`docs/shannon/parallel-gaussian-converse-closure-plan.md`、7 Phase、core = `h_decomp` lift)。③ operational 符号化定理は scope-out (judgment #11)
