...
Snapshot of all `@audit:KIND(SLUG)` tags under `InformationTheory/` as of commit `8e3645f`.
...
|---|---|---|---|---|---|---|
| 1 | `InformationTheory/Shannon/BrunnMinkowski.lean:133` | `defect(circular)` | `defer(brunn-minkowski-from-epi-discharge)` + `staged(epi-n-dim)` | `def IsBrunnMinkowskiEntropyHypothesis` (line 134) | 4 files / 20 refs | **Mathlib-gap** (EPI n-dim 経由、`docs/shannon/brunn-minkowski-from-epi-discharge-plan.md`) — long-term staged |
| 2 | `InformationTheory/Shannon/BrunnMinkowski.lean:191` | `defect(circular)` | `defer(brunn-minkowski-from-epi-discharge)` | `theorem brunn_minkowski_entropy_inequality` (line 192) | 同上 predicate を hyp 取り | **Mathlib-gap** (#1 と同 plan、predicate 経由で間接) |
| 3 | `InformationTheory/Shannon/AWGNAchievability.lean:46` | `defect(circular)` | `defer(awgn-achievability-typicality)` + `staged(n-dim-gaussian-aep)` | `def IsAwgnTypicalityHypothesis` (line 47) | 9 files / 28 refs | **Mathlib-gap** (n-dim Gaussian AEP / continuous SMB、`docs/shannon/awgn-moonshot-plan.md` Phase B-0) |
| 4 | `InformationTheory/Shannon/AWGNAchievability.lean:85` | `defect(circular)` | `defer(awgn-achievability-typicality)` | `theorem awgn_achievability` (line 86) | 上記 predicate consumer | **Mathlib-gap** (#3 と同 root) |
| 5 | `InformationTheory/Shannon/AWGNAchievabilityDischarge.lean:730` | `defect(false-statement)` | `audit-history: prior staged(awgn-power-constraint-realizable)` | `def IsAwgnPowerConstraintRealizable` (line 731) | 1 file / 2 refs (history only) | **honesty-record (温存)** — AWGN pivot plan 判断ログ #7 (`docs/shannon/awgn-power-constraint-realizable-pivot-plan.md:230`) で「**削除せず orphan 化 / tag・body 完全不変で残置 (honesty record)**」と明示決定済。alias 化を試みたが honest predicate に degenerate instance を作ると依然 unsatisfiable で audit-tags rule に抵触するため放棄、現状の温存が正解。 |
...
| SLUG | 検出箇所 | 対応案 |
|---|---|---|
| `awgn-converse-aux-plan` | `InformationTheory/Shannon/AWGNConverse.lean:91` | 新規 plan stub (docstring で「Tier 3 未着手」と明示済、slug は確保意図) |
| `awgn-mi-bridge-plan` | `InformationTheory/Shannon/AWGN.lean:122` | 新規 plan stub (docstring に「`awgn-mi-bridge-plan.md`」と直接言及済) |
| `prekopa-leindler-induction-plan` | `InformationTheory/Shannon/BrunnMinkowskiFunctional.lean:209` | 新規 plan stub (docstring に「`prekopa-leindler-induction-plan.md` (未着手) で `n` 帰納 + 1-dim Hölder 経路」と明示済) |
...
| tag数 | file |
|---|---|
| 20 | `InformationTheory/Shannon/AWGNAchievabilityDischarge.lean` |
| 15 | `InformationTheory/Shannon/RelayInnerBodyDischarge.lean` |
| 15 | `InformationTheory/Shannon/HuffmanT1APPrimeBody.lean` |
| 15 | `InformationTheory/Shannon/EPIStamDischarge.lean` |
| 14 | `InformationTheory/Shannon/EPIStamToBridge.lean` |
| 14 | `InformationTheory/Shannon/EPIL3Integration.lean` |
| 11 | `InformationTheory/Shannon/BrunnMinkowskiFunctional.lean` |
| 11 | `InformationTheory/Shannon/BrunnMinkowskiConcavity.lean` |
| 9 | `InformationTheory/Shannon/RelayInnerBound.lean` |
| 9 | `InformationTheory/Shannon/EPIStamStep3Body.lean` |
| 8 | `InformationTheory/Shannon/MultipleAccessChannel.lean` |
| 7 | `InformationTheory/Shannon/ChernoffPerTiltSanov.lean` |
| 6 | `InformationTheory/Shannon/WynerZivConverseChain.lean` |
| 6 | `InformationTheory/Shannon/HoeffdingInteriorGradientBody.lean` |
| 6 | `InformationTheory/Shannon/EPIStamDeBruijnConclusion.lean` |
| 6 | `InformationTheory/Shannon/BroadcastChannel.lean` |
| 5 | `InformationTheory/Shannon/RelayCFBinningBody.lean` |
| 5 | `InformationTheory/Shannon/ParallelGaussianPerCoord.lean` |
| 5 | `InformationTheory/Shannon/LZ78ZivCombinatorics.lean` |
| 5 | `InformationTheory/Shannon/LZ78FinalGlue.lean` |
...
3. **honesty inline alert 2 件** (W1-D LZ78 系、CLAUDE.md「専用監査を待たない」inline 即フラグ規律該当):
   - `InformationTheory/Shannon/LZ78ZivTreeNode.lean:651, :712` — predicate `IsLZ78ZivCombinatorialCoreOverhead` が docstring 内で **「FALSE で vacuously conditioned」**と self-flag 済。現状 `@audit:suspect` だが、規約上は `@audit:defect(degenerate)` or `@audit:residual` 格上げ候補。
   - `InformationTheory/Shannon/LZ78SMBSandwich.lean:362` `IsLZ78ConverseChainHyp.ofSMBBridge` — body `:= h` の defeq 別名 (circular)、`lz78-residual-discharge-plan` 判断ログ #6 で defect として記録済だが **本 line には audit tag 未付与**。`@audit:defect(circular)` 付与必要。
