...
- 対象: `InformationTheory/Shannon/*.lean` 全 family。Hoeffding は closed (2026-05-25)。
...
ただし Phase 2 で signature 改変したら **必ず** `lake build InformationTheory.<改変 module>`
...
```bash
ln -sfn /Users/haruka/dev/lean-projects/.lake .lake
```

worktree root で実行。親の `.lake` (Mathlib 7-8 GB) を
...
`InformationTheory/Shannon/<family>*.lean` の legacy tag 在庫を verbatim 確認して
...
canonical grep one-liner:
    ...
    rg -l '@audit:suspect\(|@audit:staged\(|@audit:closed-by-successor\(|@residual\(plan:<family-slug>' InformationTheory/
...
1. **worktree .lake 共有**: `ln -sfn /Users/haruka/dev/lean-projects/.lake .lake`
2. **ブランチ規律**: 起動時にいる worktree branch に居続ける。`feat/...`
   作成禁止、`git checkout` 他禁止
3. **scope**: 1 family、`InformationTheory/Shannon/<family>*.lean` のみ。完了時
   `InformationTheory.lean` の import 行は **変更しない** (declaration 名は維持)
...
6. **olean refresh** (Pilot Pattern A): Phase 2 で signature 改変したら
   `lake build InformationTheory.<改変 module>` で olean refresh、その後 dependent
...
```bash
lake build InformationTheory.Shannon.<改変 module>
```

で olean refresh、その後

```bash
for f in $(rg -l '改変 module の名前' InformationTheory/); do
  lake env lean "$f"
done
```
...
#### Pattern D 発展形 — declaration-direct タグの bareword 誤計数 (Round 3 Wave 3-A)

同種の誤計数は `@audit:suspect` / `@audit:staged` / `@audit:closed-by-successor`
の bareword grep でも発生する。docstring sign-off note の **文字列リテラル参照**
(例: "...の旧 `@audit:suspect` 解消...") を false positive ヒットし、orchestrator
brief 計数 drift の根本原因になる。Round 3 Wave 3-A で 4 planner が独立に発見:
...
**回避策**: declaration-direct タグの per-family 件数集計には **必ずパーレン
付き pattern** を使う。canonical one-liner:

```bash
rg -c '@audit:suspect\(|@audit:staged\(|@audit:closed-by-successor\(' InformationTheory/<family pattern>
```

詳細は `docs/audit/audit-tags.md`「declaration-direct タグ検索の canonical
pattern」section が SoT。
...
| **S2 import 実依存** | `import InformationTheory.Shannon.<他 family>` あり + 当該 family の lemma / def を本体で使用 | sweep 単独実施 OK。下流 (consumer) として transitive sorry を引き継ぐが、Phase 2.x ripple で散文化対応可能 (Pattern C) | inventory 表で「S2: import + use of <decl>」と注記。Pattern C 散文 transitive 適用、predicate 自身の削除は不要 |
...
### Pattern I — `closed-by-successor` migration の Phase Z recipe (Chernoff pilot)

`@audit:closed-by-successor(<slug>)` の sweep は通常の P retreat とは
構造が異なる。tag が指す後継 declaration が **(a) honest discharge 済 / (b)
false predicate に依存 / (c) load-bearing hyp を thread している** の 3
sub-pattern で扱いが分岐するため、Round 3 Wave 3-B Chernoff bookkeeping
pilot で確立した decision tree を Phase Z recipe として登録する。LZ78 /
InfinitePiTiltedChangeOfMeasure 等の closed-by-successor 残存 family で
再利用可能。

| Recipe | 後継 declaration の状態 | 当該 wrapper の処置 |
|---|---|---|
| **Recipe A — CS-honest** | 後継が 0-sorry / `@audit:ok` で genuine 完成済 | `@audit:closed-by-successor(<slug>)` tag を削除 (本 declaration は単純 alias / forwarding。closure 責任は後継側で完結済、bookkeeping 不要) |
| **Recipe B — CS-false** | 後継が false-predicate ベース (`@audit:retract-candidate(false-hypothesis)` 等) | `@audit:closed-by-successor` を `@audit:retract-candidate(false-replaced-by-eps-relaxed)` 等の Reason に変換 + docstring に「ε-relaxed 後継版が canonical」と明記 |
| **Recipe C — CS-genuine-hyps** | 後継が genuine な sorry + `@residual(<class>:<slug>)` 持ち (load-bearing hyp は無いが Mathlib 壁を表現) | `@audit:closed-by-successor` を削除 + 本 declaration を P retreat (hyp 削除 + body sorry + `@residual(plan:<slug>)`)。後継と同 slug で集約 |

**判定方法**: 後継 declaration の signature + body + 既存タグを verbatim
Read。Recipe A/B/C は二択ではなく相互排他、後継状態で一意に決まる。

**Phase 構造への組込**: planner が closed-by-successor を含む family を
sweep するとき、Phase 1 (V/C cleanup) と Phase 2 (P retreat) の間に
**Phase Z (CS migration)** を挿入。Phase Z は Recipe A/B/C のいずれかで
1 declaration ずつ処置し、Phase 2 の P retreat とは独立に進める。
Chernoff pilot では Phase Z で 19 件の `@audit:closed-by-successor` を
全て削除/変換し、bookkeeping 削減 -24 件の最大寄与となった。

### Pattern J — L-MIG-4 expansion (inverted dependency、Round 3 Chernoff observation)

通常 P retreat は **upstream sorry 化 → downstream は transitive sorry を
引き継ぐ** という top-down フロー。だが downstream genuine proof が
upstream の load-bearing hyp を **discharge する形** (= downstream が hyp
を消費して証明を完結している) で書かれていると、upstream を sorry 化した
瞬間 downstream の証明が type error になる (consumer 側が hyp を実装に
組み込んでいる、inverted dependency)。

**Round 3 Chernoff 実観測**: `chernoffPerTiltDischarge_<...>` (詳細は
chernoff-sorry-migration-plan.md) で発覚。orchestrator brief は upstream
P retreat 1 件想定だったが、downstream の 2 declaration が hyp consumer
で type drift → minimal scope expansion で transitive sorry 2 件追加が
必要となった。

**回避策**:
- closed-by-successor migration plan の **Phase 0 step** に「downstream
  body が migrated wrapper の old hypothesis を thread しているか確認」
  を追加 (planner step で 1 度確認すれば事前に scope 拡張可能)
- 確認方法: 当該 wrapper の hyp の名前 / 型を `rg` で downstream caller
  body 内に grep、`exact <hyp>` / `apply <hyp>` / `<hyp>.field` で
  consumption が ≥ 1 ヶ所あれば inverted dependency 候補 → Phase 0 で
  transitive sorry 追加件数を planner brief に含める
- 発覚した turn では implementer が minimal scope expansion (upstream
  sorry 化 + downstream の 該当 step を sorry 化) で対応可能だが、plan
  に最初から書いておくと audit-2 の verdict が clean になる
...
合算 (Hoeffding は closed)。状態欄: ✅ = Round 1/2/3 で sweep 済、◯ = 部分着手、未印 = 残:

| family | files (代表) | suspect | staged | 🟢ʰ | defer/closed | 推定サイズ | shared wall? | 状態 |
|---|---|---:|---:|---:|---:|---|---|---|
...
| **Relay** | RelayInnerBoundDischarge, RelayInnerBound, RelayCFBinningBody, RelayDFBlockMarkovBody, RelayCutset | 36 | ~0 | 15 | ~0 | 中-大 | (調査要) | ✅ Round 2 |
| **MAC/BC** | MultipleAccessChannel, BroadcastChannel, BroadcastChannelExistenceBridgeBody, MACCornerPoint, MACBodyDischarge, MACFanoConverseBody | 25 | ~0 | 18 | ~0 | 中 | (調査要) | ✅ Round 2 |
| **BrunnMinkowski** | BrunnMinkowskiFunctional, BrunnMinkowskiConcavity, BrunnMinkowski, BrunnMinkowskiPLBody | 30 | 1 | 6 | 2 | 中-大 | `wall:brunn-minkowski-functional` 候補 | ✅ Round 2 + Round 3 residual (0 sorry / 2 retract `closure-plan-completed`) |
...
| **LZ78** | LZ78FinalGlue, LZ78ZivCombinatorics, LZ78ConverseDischarge, LZ78SMBSandwich, LZ78ZivTreeNode, LZ78AsEventualAchievability, LZ78AchievabilityLimsup | 26 | ~0 | ~0 | ~0 | 中 | なし? | ✅ Round 2 |
| **Huffman** | HuffmanT1APPrimeBody, HuffmanT1APPrimePartial, HuffmanSwapNormalizationBody, HuffmanSwapStepChainBody, HuffmanOptimality, HuffmanMergedIdentBody, HuffmanStrongForm | ~0 | 30 | ~0 | 2 | 中 | なし? (staged の migration recipe を pilot で確立する candidate) | ✅ Round 1 |
| **WynerZiv** | WynerZiv, WynerZivConverse, WynerZivAchievability, WynerZivConverseChain, WynerZivBinningCovering, WynerZivPackingBody, WynerZivCoveringBody, WynerZivBinningBody | 6 | 13 | 3 | ~0 | 中 | なし? | ✅ Round 1 + Phase 2.x |
| **Chernoff** | ChernoffPerTiltSanov, ChernoffPerTiltDischarge, ChernoffSanovDischarge, ChernoffConverse, ChernoffInformation | ~0 | ~0 | 8 | 19 | 中 | closed-by-successor 多め (bookkeeping 系の pilot candidate) | ✅ Round 3 Wave 3-B pilot (22 honest_residual + Phase Z recipe 確立) |
| **Cramer** | Cramer, CramerLC2PhaseC, CramerPhaseDGapWorkaround | 12 | ~0 | ~0 | ~0 | 小 | なし? | ◯ Round 1 部分着手 |
| **ParallelGaussianPerCoord** | ParallelGaussianPerCoord, ParallelGaussianPerCoordRegularity | ~0 | 1 | 5 | 5 | 小 | (EPI/Stam dependency 要確認) | ✅ Round 3 (PGPC 5 tag 削除のみ、`IsParallelGaussianPerCoordRegularity` 残置 = 後続 PGPC discharge plan で incidental migration) |
| **MultivariateDiffEntropy** | MultivariateDiffEntropy | 4 | ~0 | ~0 | ~0 | 小 | なし? | ✅ Round 3 small cluster |
| **RateDistortion** | RateDistortionAchievabilityPhaseEStrongFinal | 2 | ~0 | ~0 | ~0 | 最小 | なし? | ✅ Round 3 (4 sorry + 1 transitive) |
| **WhittakerShannonFull** | WhittakerShannonFull | 2 | ~0 | ~0 | ~0 | 最小 | なし? | ✅ Round 3 small cluster |
| **small cluster (Round 3)** | 8 file (MultivariateDiffEntropy + WhittakerShannonFull 含む + 6 file) | — | — | — | — | 小 | なし | ✅ Round 3 (12 honest_residual + 2 retract) |
