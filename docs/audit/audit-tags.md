...
3. **退化/`:True` check**: vacuous shape / `:True` slot / 退化定義悪用で意味を空にしていないか。**結論の形だけでなく仮説の充足可能性まで問う** — 結論が退化していなくても、**仮説群を同時に満たす入力が存在しなければ主張は空虚に真**で、これも退化 (`@audit:defect(degenerate)`) である。抽象定理を具体対象へ instantiate した宣言で起きやすい (instantiate で対象を狭めた結果、discharge しなかった側の仮説が充足不能になる)。判定 = **全仮説を同時に満たす witness を 1 つ in-tree で示せるか**。示せないなら、仮説から `False` が出ないことを確かめるまで通さない。
   - **機械検査の第 2 の抜け道 (公開表に載り続ける)**: 空虚に真な定理は `sorry` を使わず証明できるので、`#print axioms` にも `scripts/gen_readme_table.ts --check` の honesty invariant (0 sorry / 0 custom axiom) にも掛からず、README の成果一覧に載り続ける。CLAUDE.md「`sorryAx`-free は必要条件であって十分条件でない」の、**load-bearing hypothesis とは別の抜け道**である (あちらは explicit hyp が証明の核心を負う形、こちらは hyp を満たす入力がそもそも無い形)。**`@[entry_point]` 宣言について「仮説を満たす witness が in-tree にあるか」を問う仕組みは存在しない** ⟹ この check が唯一の関門であり、instantiate 形の headline を `@[entry_point]` / 公開表に載せる前に必ず通す。実例: Portfolio `growingMemory_logWealth_tendsto_condOptGrowthInfty_concrete` — 価格写像を座標そのものに固定したまま正値性を列空間の全点で要求し `hpos → False` (`@[entry_point]` 撤去 + 公開表から削除、`cause:degenerate-boundary`)。当時の gateway verify も両 gate も *discharge 対象の* 仮説しか見ておらず素通しした。
   - **結論の内側の `∃` は、witness に何が課されているかを別に見る**: 仮説側が regularity だけで、結論の形 (収束・不等式) も正しく、仮説も充足可能なのに、その結論が `∃ w, P w` の形で **`P` が `w` に主張の核を課していない**ことがある。仮説だけを読む検査 (check 2 と上の充足可能性) は通り、結論の**形**だけを見る検査も通るので、両方の網から落ちる。判定 = **結論の `∃` を、名前と docstring が名乗る性質をもたない自明な witness (定数・恒等写像・任意の 1 点) で満たせるか**。満たせるなら、その宣言が機械検証しているのは名前が名乗る主張ではない。直し方は **結論を強める**こと (witness に性質を課す) で、**仮説に足すのは不可** (足すと check 2 の load-bearing bundling になる)。実例: Portfolio `stationaryInfPast_logOptimal_growth_tendsto_condOptGrowthInfty` — 結論 `∃ bstar bstarInf, …` が二つの witness に最適性を一つも課しておらず、両方を同じ定数ポートフォリオにとれば Birkhoff だけで成り立つ (同じファイルの `exists_condLogOptimalSeq` / `exists_infPast_condLogOptimal` / `exists_condOptGrowth_tendsto_condOptGrowthInfty` は witness に条件付き優越性を課すので当たらない。→ `docs/shannon/portfolio-stationary-woo-plan.md`「残タスク」(b))。
4. **sufficiency check** (仮説 ⊢ 結論): 仮説群から結論が **semantic に follow** するか。最低でも反例構成を 1 つ試みて棄却できるか確認する。derivative-of-gap / 不等式系の結論では特に「**差分形 `g'` か、比 (log) `g'` か**」を Stam 等の出口形と照合する (出口形を取り違えると非循環・非バンドルでも偽の含意を主張しうる)。**反例 1 つは必要条件であって十分条件ではない** — 2 つの追加 guard で under-hyp の pass を塞ぐ:
   - **class であって instance でない**: 特定反例を殺す fix の後、**全仮説を満たす最も一般の対象を構成して再 test** する (2 つの構造的に異なる degenerate 境界を試す)。fix が instance を patch しても反例 *class* は開いたまま残ることが多い (consistency hyp 追加で constant/point-mass 反例を殺しても、entropy 保存 relabel が同じ仮説を満たしつつ結論を破る)。
   - **pin される invariant を名指す**: 仮説が実際に pin する invariant を明示し、結論がそれと **同じ粒度** を要求するか確認する。再発する false-as-framed 軸は「仮説が coarse な scalar 汎関数 (entropy 値) を pin するが結論は fine 構造 (full law / TV) を要する」= coarser-than-needed で false-as-framed (非循環・非バンドルでも)。target が named textbook object (Markov lemma 等) なら、その標準仮説の strength (strong vs weak typicality の別) を in-project 定義と diff する — build 前の 1 diff で reframe 時に検出できる。
...
| `stam-step2-density` | Stam 不等式 Step 2-3 の density-level 解析核: 条件付き Cauchy-Schwarz (`s_Z(z)² ≤ E[(λ s_X + (1-λ) s_Y)² \| X+Y=z]`) を `p_Z` に対し積分して凸 Fisher 上界 `J(Z) ≤ λ² J(X) + (1-λ)² J(Y)` を得る部分 (λ 最適化で `IsStamCauchySchwarzOptimal` 形 `J(Z) ≤ J(X)J(Y)/(J(X)+J(Y))`)。Mathlib に score-of-convolution の condExp 表現も Fisher info 畳み込みも無い (a)+(b) 混合壁。`stam` (inverse harmonic-mean 結論形 `IsStamInequalityResidual`) と semantic 区別: 本壁は λ ごとの凸上界 = 結論の上流。`StamInequality.stamCauchySchwarzOptimal_of_indepFun` で集約、旧 load-bearing hyp `IsStamCauchySchwarzOptimal` を `entropy_power_inequality_via_body` から除去した置換先。**[CLOSED 2026-06-04 (2026-05-31 owner-level pivot `epi-wall-reattack-plan`): genuine, no longer a wall]**。`IsStamInequalityHyp`/`IsStamInequalityResidual` を pointwise convolution 制約 + `IsBlachmanConvReady` bundle 同期 pivot した結果、`stamCauchySchwarzOptimal_of_indepFun` (`EPIStamInequalityBody.lean:349`) が `convex_fisher_bound_of_ready` (genuine `convex_fisher_bound`) + `stam_lambda_min` + linarith で genuine に閉じた。`#print axioms isStamInequalityHyp_of_indepFun` (= `isStamInequalityHyp_via_body (stamCauchySchwarzOptimal_of_indepFun ...)` の一行、`EPIStamStep3Body.lean:119`) = `[propext, Classical.choice, Quot.sound]` (sorryAx-free、本監査 2026-06-04 機械確認、chain `stamCauchySchwarzOptimal_of_indepFun`/`isStamInequalityHyp_via_body`/`convex_fisher_bound_of_ready` も全て sorryAx-free)。active `@residual(wall:stam-step2-density)` 0 件 (この class を持つ実 sorry tactic トークンは 0、散文中の言及のみ) | Ch.17 EPI (Step 2-3, `stamCauchySchwarzOptimal_of_indepFun`) |
...
| `nyquist-2w-dof` | 時間帯域幅 DOF-per-second カウント: `[-W,W]` 帯域制限 & `[0,T]` 本質的時間制限の信号は `≈2WT` 自由度 (時間帯域制限作用素の prolate-spheroidal / Landau-Pollak-Slepian 固有値集中)。Mathlib 不在 (loogle Found 0: `prolate`/`whiteNoise`/`Bandlimited`/`Slepian`、2026-07-14 確認)、self-build ~1000-2000 行のスペクトル理論。**Mathlib 側資産の精査 (leg 15、grep で機械確認)**: (a) trace-class / Schatten / Hilbert-Schmidt 作用素論は**不在** (`Analysis/InnerProductSpace/SingularValues.lean` / `Trace.lean` はいずれも `[FiniteDimensional]` 前提の有限次元専用)。(b) だが**無限次元のコンパクト自己共役スペクトル定理は存在する** — `Analysis/InnerProductSpace/Spectrum.lean:443` `orthogonalComplement_iSup_eigenspaces_eq_bot` (固有空間の iSup の直交補が `⊥` = 固有空間が総和的) + `:464` `finite_dimensional_eigenspace` (本 family が既に消費中)。⟹ 「無限次元スペクトル理論が Mathlib に無いから壁」は**誤った壁論拠**。厳密 trace 等式 `∑ λₙ = 2WT` は Bessel を **Parseval へ昇格**させる道 (完全固有基底 + 非負項ゆえ Tonelli で `∑∫ = ∫∑`) が開いており、行数見積の再算定が要る。**壁でない周辺 (すべて in-project 定義可能)**: 雑音測度 (帯域制限射影した白色雑音 = 有限分散 Gaussian 過程、Nyquist サンプルは iid Gaussian、`IsGaussianProcess`) / 連続時間 AEP / per-sample 符号化定理 (`awgn_achievability`/`awgn_converse` 所有)。**sampling theorem が operational を閉じない理由**: `whittaker_shannon_bandlimited` は全直線上の全単射 (両側無限サンプル ↔ 帯域制限 L² 信号) だが operational capacity は毎秒レート `limsup (log M(T))/T` — 全直線全単射は時間窓の次元カウントを与えない。**循環 caveat**: 連続時間 code をサンプルベクトルに制限して定義すると Landau-Pollak-Slepian converse DOF 限界を def に埋め込む循環、`2W` は DOF 証明側から emerge させる。標本化定理そのものは 2026-07-14 に proof done (`whittaker_shannon_hasSum`/`whittaker_shannon_bandlimited` sorryAx-free) で本壁から分離済。**consumer (2026-07-17 leg 15 更新): 2 — LIVE** (`ShannonHartleyOperational.lean:477` = `contAwgn_eq_shannonHartley` / `ShannonHartleyAchievability.lean:725` = `contAwgn_ge_shannonHartley`、いずれも `sorry -- @residual(wall:nyquist-2w-dof)`、`@audit:defect` 残存なし = 純粋な壁 residual)。~~consumer: 0 — DORMANT~~ は **Leg P 以前の状態**であり失効。経緯: 3 宣言は一度 `@audit:defect(false-statement)` に再分類された (`d2938749`) — `ContAwgnCode` の観測写像が点標本だったため (`cause:signature-drops-constraint`) 超振動符号語で `BddAbove` が破れた。**Leg P (2026-07-17、`4fd8a47c`) の def-fix が着地**し観測写像を `[0,T]` 台の正規直交テスト関数 (Karhunen-Loève) へ修理 ⟹ 3 宣言すべてが false-statement から離脱。うち `contAwgnMaxMessages_bddAbove` は Bessel 単独で **壁非依存に proof-done** (`fb18b681`、`@audit:ok`)、残る 2 本が本壁の live consumer に復帰。**gateway atom 結果 (leg 15、`69152fd9` + 監査 `7c43417a`)**: crude trace bound `c·#{λ>c} ≤ 2WT` (`sum_inner_timeBandLimitingOp_le` / `prolateCount_mul_le`、`TimeBandLimiting.lean`) が **Bessel + 再生核で壁非依存に closure、sorryAx-free + `@audit:ok`**。だが**本 slug が名指す tight LPS 集中は無傷** — Bessel は片方向ゆえ下界に届かず、Markov は `1/c` 倍の過大計上で `2WT + O(log WT)` の鋭さを与えない (`cause:weaker-relative` = textbook-object strength diff)。**consumer の要求 (コード側 SoT で確認、`ShannonHartleyOperational.lean:461-462` / `ShannonHartleyAchievability.lean:697-698`)**: `eq` は上下両半分を鋭く要し、`ge` は下半分のみを要す ⟹ **gateway atom はどちらの consumer も unblock しない。slug は retire しない。** **壁の残渣は第 2 モーメントに絞り込まれた (leg 15、E-trace、`9f1129e1` + 監査 `21981fc8`)**: 厳密 trace 等式 `∑' i, ⟪A bᵢ, bᵢ⟫ = 2WT` (任意の `HilbertBasis`、`tsum_inner_timeBandLimitingOp_eq`) が **sorryAx-free で closure** (Parseval + Tonelli、**スペクトル定理すら不要**)。⟹ **本 slug が今なお名指すのは `∑ λₙ(1-λₙ) = tr A − tr A² = O(log WT)` (Landau-Widom) ただ 1 点** — 第 1 モーメント (総質量 = 粗いスカラー) は cliff を跨ぐ分布 (細かい構造) を pin しない (監査が反例クラスを自作して確認: `∑λₙ=2WT` かつ全 `λₙ ≤ c` な平坦スペクトルは `#{λ>c}=0`)。**行数見積の再算定 (leg 15、実装者の paper 見積 = 未機械検証)**: ~630-1150 行、性格も変化 (Schatten 理論・特殊関数を要さない)。旧「~1000-2000 行のスペクトル理論」は論拠ごと失効。旧 consumer `IsTwoWDegreesOfFreedom` / `shannon_hartley_formula` は Phase 1-fix (2026-07-15) で除去済 (stale)。**🎉 第 2 モーメント = 本 slug の名指す残渣は CLOSED (2026-07-17、leg 16、Leg E-sharp、`552ac8de` + `00cb1c8b` + 監査 all OK)**: `tsum_inner_sub_norm_sq_timeBandLimitingOp_le : ∑' i, (⟪A bᵢ,bᵢ⟫.re − ‖A bᵢ‖²) ≤ 2 + Real.log (1 + 2*W*T)` (任意 `HilbertBasis`、仮説は regularity のみ、sorryAx-free) = `tr A − tr A² = O(log WT)` そのもの。機構: (a) `bandKernel_window_deficit_le` = 純 calculus (`k(u)=sin(2πWu)/(πu)` ゆえ `k² ≤ 1/(π²u²)`、`|sin|≤1` だけ + 既存 `bandKernelLp_norm_sq`)、(b) `tsum_norm_timeBandLimitingOp_sq_eq` = `tr A² = ∫₀ᵀ∫₀ᵀ|k(t−s)|²` (E-trace と同じ Parseval テンプレを polarize、`A↔B` bridge も三重 Fubini も不要 — `[0,T]` へ peel すれば `integral_tsum` + AM-GM domination で足りる)。**壁論拠の 3 度目の誤りが確定**: 旧論拠は Landau-Widom を**鋭い漸近等式**として枠付けしていたが、consumer が要するのは**緩い片側上界のみ** — `0 ≤ λ ≤ 1` + `tr A = 2WT` 厳密値 + 第 2 モーメント片側上界だけで Chebyshev 分割が `#{λ>c} = 2WT + O(log WT)` の**両半分**を与える (監査が独立再導出 + consumer docstring と照合)。⟹ **`cause:weaker-relative` の逆向き適用** = 壁が consumer の要求より**強い親戚**で枠付けされていた。leg 15 の E-atom (弱い親戚を閉じて過大主張) の鏡像であり、**同一の strength-diff 規律が E-atom を refute し E-sharp を是認した**。**⚠️ slug はまだ retire しない**: 解析核は消えたが consumer 2 本は未 unblock。残る obligation は **R1 = 固有基底 + 多重度/カウント bridge** (`plan:` 級、`tsum_prolateEigenvalues_eq` L2322 が部分追跡、資産は全て機械確認済: `Spectrum.lean:443/:463`、`l2Space.lean:528`) / **R2 = Chebyshev 分割** (初等) / **R3 = `prolate_eigenvalue_count` が宣言として存在しない** (plan 散文と 3 docstring が名前だけ参照 = 指示対象なき名前) / **R4 = operational bridge** (Gram + Cauchy interlacing + capacity 計算、**Leg P 以降未評価 = 最大の未計測債務**)。 **🎉 R4 の C1 (Cauchy interlacing の count domination 上半分) = CLOSED (2026-07-18、leg 23、`c5822fed` + 監査 `73ec6559`)**: `finrank_le_prolateCount_of_form_gt` (`TimeBandLimiting.lean:2617`) proof-done sorryAx-free @audit:ok。crux (Vᗮ で `⟪Av,v⟫.re ≤ c‖v‖²` = `inner_timeBandLimitingOp_le_of_mem_orthogonal`) は **Leg E で既に in-tree** ⟹ C1 は純線形代数、advisor の「self-build 公算」を反証 (`cause:loogle-blind` 再演)。⟹ **live `@residual(wall:nyquist-2w-dof)` = 0** (`contAwgn_eq_shannonHartley` の残 sorry を監査 CONFIRMED で `@residual(plan:shannon-hartley-phase2-spectral-plan)` へ再分類)。**slug は register に history として残す** — 残 converse (C0 headline / C2 Gauss 回転 = Mathlib 在 / C3 operational parallel-Gaussian converse = self-build / C4 water-filling+極限) は project-internal で**壁ではない**。C3 self-build が genuine Mathlib gap を露呈したらその時点で新 slug を建てる。 **🎉 converse 全段 CLOSED = メイン定理 proof-done (2026-07-18、leg 29、`71903f08` + 監査 `2d267dfc` all-OK)**: C2 Gauss 回転 (leg 27-28) / C3 per-coord / C4 water-filling + 二重極限 / C0 `contAwgn_le_shannonHartley` / assembly `le_antisymm` = 全て proof-done sorryAx-free (新規 `ShannonHartleyConverseFinal.lean`、`@audit:ok`)。`contAwgn_eq_shannonHartley` は Main→ConverseFinal へ移動し**残 sorry 削除 = 無条件証明** (`#print axioms` 独立再導出 = `[propext, Classical.choice, Quot.sound]`、署名走査で隠れ load-bearing hyp 無し確認)。**🎉 R1 exact-identity `tsum_prolateEigenvalues_eq` も CLOSED = 本 slug 近傍の live sorry = 0 (2026-07-18、`7832caf3`/`7a08b9c2`/監査 `3d602c4b` all-OK)**: R1 の厳密 trace 等式 `∑λ=2WT` を **完全固有 HilbertBasis** で proof-done sorryAx-free 化 — compact self-adjoint spectral theorem (`Spectrum.lean:443` `orthogonalComplement_iSup_eigenspaces_eq_bot`) を per-eigenspace ON 基底の糊付け (`orthogonalFamily_eigenspaces` → `exists_hilbertBasis` → `orthonormal_sigma_orthonormal` → `mkOfOrthogonalEqBot`) で完成 (P1、`exists_eigen_hilbertBasis`) → `tsum_inner_timeBandLimitingOp_eq` を eigenbasis へ特殊化 (P2) → layer-cake 多重度 bridge (`lintegral_eq_lintegral_meas_lt` + generalized-inverse duality、P3、`tsum_eigen_eq_tsum_prolateEigenvalues`)。**糊付けは real work だったが Mathlib gap は無かった** (inventory `docs/shannon/shannon-hartley-eigenbasis-inventory.md` 予告どおり: 既製 OrthogonalFamily→HilbertBasis collation は不在ゆえ自前糊付けだが原子補題は全て在庫)。⟹ **本 slug が過去に名指した全 obligation (解析核 E-sharp / R4 converse / R1 exact-identity) は CLOSED**、プロジェクト全体 `gen_readme_table.ts --check` = 0 sorry / 0 wall / 0 custom axiom。**slug は register に history としてのみ残す** (R1 exact-identity は Shannon-Hartley メイン定理には不要な bonus だった)。
...
| `lz78-aseventual-ziv` | LZ78 a.s.-eventual Ziv inequality `limsup (lz/n) ≤ entropyRate₂` (bit、achievability 上界、Cover-Thomas Lemma 13.5.5)。**[CLOSED 2026-06-21 commit `c22f2d5`、sorryAx-free + 独立監査 PASS]** — `ziv_aseventual_le_blockLogAvg₂` (`AsymptoticOptimality.lean:917`) + consumer `lz78Greedy_achievability_ae` (`:1005`) を genuine discharge、両 `@audit:ok` (`#print axioms = [propext, Classical.choice, Quot.sound]`)。**最終 closure = 各固定 k bound + inf over k (対角化不要)**: 最終 LHS `limsup(lz/n)` が k 非依存ゆえ、composition brick `c·log c ≤ negLogQk + o(n)` を各固定 k で AEP に乗せ `limsup(lz/n) ≤ conditionalEntropyTail k / log2` (Lemma 1 `ziv_aseventual_le_condEntropyTail_bits`) を示し、inf over k は RHS だけで `inf_k H_k = entropyRate` (Lemma 2 `ziv_aseventual_le_entropyRate₂`、`entropyRate_eq_lim_condEntropy`) を取れば足りた。当初「最大リスク」と評価された **k(n)→∞ 対角化 (k-limit/n-limsup 交換) は不要**、`Lmax = o(n)` a.s. known sub-task も不要 (overhead vanishing は `c/n → 0` + `x·log(1/x) → 0` boundary に帰着)。cause:single-route (対角化必須という single framing 過大評価)。**元の経緯 (defect → entropyRate₂ 化)**: 符号長 def-fix (genuine longest-prefix parse `lz78PhraseStrings`、`c·bitLength c |α|`、rate `O(1)`、commit `5d08566`) で当初のダミー1シンボル parse degenerate defect を解消。続く独立監査で **bit-vs-nat units mismatch** (second defect) を発覚: `lz78GreedyEncodingLength/n` は bit-rate だが旧 RHS は nat-単位の `entropyRate`、正entropy源 A≥2 で `limsup = log₂A > logA` = 偽 (prior audit `9b09790` はこれを見落とし overturn)。`99acb58` で `@audit:defect(false-statement)` 確定 → `55e1cd9` で RHS を `entropyRate₂` (= `entropyRate/Real.log 2`、bit) 化して TRUE-as-framed に修正、再監査 PASS。cause:false-statement (過小評価系、units defect、entropyRate₂ 化で解消) | Ch.13 LZ78 (achievability) |
| `lz78-converse-aseventual` | LZ78 a.s.-eventual converse 下界 `entropyRate₂ ≤ liminf (lz/n)` (bit、Cover-Thomas Thm 13.5.3 lower bound、harder SMB-lower + Kraft 方向)。**[CLOSED 2026-06-21 commit `bd28e0e`、sorryAx-free + FINAL completion 監査 PASS]**: 旧「research-level scope-out wall」判定を gateway-atom-first 在庫が overturn (cause:single-route + gateway-atom-untried)、M4 converse は既存 sorry-free ergodic machinery (G4 SMB-liminf / G3 Z-side Barron テンプレ + Markov/BC) の配線 + 単一の genuine 組合せ brick `lz78_block_kraft_poly` (G2、polynomial n-block Kraft `∑ 2^{−Lₙ} ≤ (n+1)²`) に分解され、G2 を parent-extension invariant (`lz78PhraseStrings_dropLast_earlier`) + injection cardinality bound で genuine closure (live sorry 0)。consumer chain `lz78_converse_bad_set_measure_le` / `blockLogAvg₂_minus_error_le_rate_ae` / `lz78Greedy_converse_ae` (LHS = `entropyRate₂ μ p`) は G2 を forward-reference 消費する sorry-free helper (bundling なし、signature は `(μ, p)` + regularity のみ、独立監査 2026-06-21 PASS)。G2 statement は TRUE-as-framed (polynomial 上界を α=Bool / Fin 8, n≤6 で数値裏取り、large slack、`n=0` 境界 `1 ≤ 1`)。headline `lz78_asymptotic_optimality_with_greedy` + converse `lz78Greedy_converse_ae` が共に sorryAx-free (`#print axioms = [propext, Classical.choice, Quot.sound]`)、`@audit:ok`。**経緯**: genuine longest-prefix def-fix (`5d08566`、rate `O(1)`) で旧ダミー parse 時代の rate 発散を解消後、独立監査で同じ **bit-vs-nat units mismatch** を発覚 (nat `entropyRate` を bit-rate と sandwich する LHS が誤 unit、prior audit `9b09790` overturn、`99acb58` で `@audit:defect(false-statement)` 確定) → `55e1cd9` で LHS を `entropyRate₂` (bit) 化、再監査 PASS。cause:false-statement (過小評価系、units defect、entropyRate₂ 化で解消) | Ch.13 LZ78 (converse) |
...
| Reason | 意味 | 典型例 |
|---|---|---|
| `circular-passthrough` | 仮説型 ≡ 結論型の循環で、honest な代替経路が他に存在 | EPIBridge:265 (`isStamToEPIBridgeHyp_of_forall`、仮説が `IsStamToEPIBridgeHyp` の unfolding そのもので body は `h`) / EPIBridge:429 (`epi_pipeline_idempotent`、`h_epi` が既に結論型を持ち body はそれに簡約) |
