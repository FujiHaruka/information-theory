...
P8 符号化定理 (Levin) → P9 Chaitin の Ω → P10 十分統計量。別 moonshot
[`kolmogorov-w2-moonshot-plan.md`](kolmogorov-w2-moonshot-plan.md) に隔離 (第 2 波 = 兄弟 moonshot)。
**当初「唯一の genuine Mathlib 壁」と見立てた prefix 塔は over-estimation で覆った** — Mathlib は
Kraft-McMillan 不等式 (有限 UD 符号版) を既に持ち、Ω/P_U 収束は解析壁でなく plumbing に縮む (再検証 =
w2 plan §settled facts / [`kolmogorov-w2-inventory.md`](kolmogorov-w2-inventory.md))。⟹ 単一
`@residual(wall:prefix-free-tower)` は撤回、第 2 波の途中 sorry は object 別 `plan:kolmogorov-w2-*` slug に分割。
...
- **含意**: これは Mathlib 壁ではない。plain C 背骨に genuine Mathlib 壁は無い。第 1 波の途中 sorry は全て
  `@residual(plan:…)`。**第 2 波 prefix 塔も当初の「唯一の genuine 壁」見立ては over-estimation で覆り
  (Kraft 既存)、単一 `wall:prefix-free-tower` は撤回済** — 詳細は
  [`kolmogorov-w2-moonshot-plan.md`](kolmogorov-w2-moonshot-plan.md) §settled facts。
...
1. **prefix 塔の隔離 (active)**: P7–P10 は第 2 波 = 別 moonshot
   ([`kolmogorov-w2-moonshot-plan.md`](kolmogorov-w2-moonshot-plan.md))、本計画に prefix 依存を持ち込まない。
   第 1 波背骨には壁 residual を作らない。**当初「唯一の genuine 壁 `wall:prefix-free-tower`」に一括隔離と
   書いたが、第 2 波 inventory の実測で Kraft 既存が判明 ⟹ 単一壁 slug は撤回、object 別 `plan:kolmogorov-w2-*`
   に分割** (child = w2 inventory/plan が SoT、兄弟整合)。
