...
- [~] Phase P4 — K↔H エントロピー率 (**flagship** `@[entry_point]`) — flagship 文確定 + 基礎 plumbing + **下界 proof-done**。
  ...
  `76a18d6d`、#7 `334b2e4e`)。**crux #4 `Partrec₂` は proof-done `20bdeaa3` だが decoder 設計 + `encodeBlock` が
  レート無効と判明** (assembly leg `334b2e4e`、子 plan §rate 欠陥)。残 = **decoder + `encodeBlock` の再設計**
  (子 plan §再アーキ R1-R4)。#5/#6 per-string 上界に局所化済 (`sorry`)。commits `357107ec`/`ce507e7c`/`77230dae`
...
上界の 7 helper のうち #1-#3 + #7 積分組立 banked。**crux #4 `Partrec₂` proof-done だが decoder + `encodeBlock`
再設計必要** (子 plan §rate 欠陥/§再アーキ R1-R4)。#5/#6 per-string 上界に局所化済 (`sorry`)。
下界・flagship squeeze 骨格・P1/P2/P3/P5 は proof-done。P6 SLLN は stretch (第 1 波成否に無関係)。

**次 leg 戦略 (control state)**: **#7 積分組立 banked**、#5/#6 per-string 上界に局所化済 (`334b2e4e`)。
**重要発見 (assembly leg)**: crux #4 の `Partrec₂` は proof-done だが **decoder 設計 + `encodeBlock` がレート無効**
(`Nat.pair` 2 倍化 / encoding 齟齬 / `encodeBlock` doubly-exp 長さ — 子 plan §rate 欠陥)。∴ 残 = **decoder +
`encodeBlock` の再設計** (子 plan §再アーキ R1-R4)。R1 `encodeBlock`→base-card computable / R2 decoder→length-additive
packing + `Partrec₂` 再証明 / R3 `natLen ≤ log2+1` utility / R4 #5/#6 closure ⟹ #7 経由で flagship 完全 proof-done。
これは assembly leg でなく **Phase-U 級 redesign** (子 plan が owner)。#7 は encoding 非依存で redesign に耐える。

## Sub-plan 一覧

| 子 plan | scope | 状態 | backlink |
|---|---|---|---|
| [`kolmogorov-p4-upper-decoder-plan.md`](kolmogorov-p4-upper-decoder-plan.md) | P4 上界 helper #4 `typeDecoder_partrec` (decoder 構成 + `Partrec₂` 認証、method-of-types crux) | ✅ CLOSED `20bdeaa3` | 親 §Phase P4 上界分解 |
...
| **4** | `typeDecoder_partrec` (`Partrec₂`) ★crux | `EntropyRateUpper.lean` (candidate B) | ⚠️ `Partrec₂` proof-done `20bdeaa3` **だが設計がレート無効 → 再設計** (子 plan §rate 欠陥/§再アーキ) |
| 5 | `condComplexity_block_typical_le` (典型 per-string 上界) | `EntropyRate.lean:103`、`sorry` | 🔄 局所化済、R1-R4 待ち |
| 6 | `condComplexity_block_uniform_le` (uniform O(n) 上界) | `EntropyRate.lean:116`、`sorry` | 🔄 局所化済、R1 後 literal 上界で自明 |
| 7 | 積分組立 (`kolmogorov_entropy_rate_upper` body) | `EntropyRate.lean:127`、#5/#6 の**文**を消費 | ✅ **banked** `334b2e4e` (encoding 非依存、unconditional body) |

**戦略 (control state)**: **#1-#4 banked 完了** (#1-#3 `475293d1`/`979a10a4`/`76a18d6d`、#4 crux
`20bdeaa3` — candidate B で make-or-break gateway PASSED、proof-done sorryAx-free)。**残 = #5→#6→#7 assembly**
(`kolmogorov_entropy_rate_upper` closure、in progress)。#5 は #4 (`typeDecoder`) を `invariance` に食わせ #1-#3 を
raw→exp bridge で消費、#6 (atypical 吸収) / #7 (積分組立、下界 plumbing 流用) が合流。**#5 の要石 = encoding
橋** (下記 crux 節末の caveat)。
