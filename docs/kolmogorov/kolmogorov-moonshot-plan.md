...
- [~] Phase P4 — K↔H エントロピー率 (**flagship** `@[entry_point]`) — flagship 文確定 + 基礎 plumbing + **下界 proof-done**。
  ...
  上界は 7 helper に分解済 (下 §Phase P4 上界分解)、#1-#3 + #7 積分組立 banked (#1-#3 `475293d1`/`979a10a4`/
  `76a18d6d`、#7 `334b2e4e`)。旧 crux #4 はレート無効判明後に **再アーキ R1-R3 landed** (base-card `encodeBlock`
  `94b8b1f6` / div-mod decoder `7a529ce8` / natLen utility `167a62f4`、全 proof-done sorryAx-free)。**残 = R4 =
  #5/#6 closure** (子 plan §R4 recipe)。#5/#6 per-string 上界に局所化済 (`sorry`)。commits `357107ec`/`ce507e7c`/`77230dae`
...
上界の 7 helper のうち #1-#3 + #7 積分組立 banked。再アーキ **R1-R3 landed** (base-card `encodeBlock` /
div-mod decoder + `Partrec₂` 再証明 / natLen utility、全 proof-done sorryAx-free)。**残 = R4 = #5/#6 closure**
(子 plan §R4 recipe)。#5/#6 per-string 上界に局所化済 (`sorry`)。
下界・flagship squeeze 骨格・P1/P2/P3/P5 は proof-done。P6 SLLN は stretch (第 1 波成否に無関係)。

**次 leg 戦略 (control state)**: 再アーキ R1-R3 完了。**残 = R4 (#5/#6 closure)** — 子 plan §R4 recipe が SoT。
まず `import EntropyRateUpper` を EntropyRate に追加 → #6 (literal echo で容易) → #5 (crux: decoder matching
補題 `encodeBlock n b ∈ typeDecoder m n` + 型クラス濃度 index 上界 + entropyByCount 会計) ⟹ #7 経由で flagship
完全 proof-done。R4 は 2 sorry (@residual) を消す = completion claim ⟹ honesty-auditor 必須。#7 は encoding 非依存。

## Sub-plan 一覧

| 子 plan | scope | 状態 | backlink |
|---|---|---|---|
| [`kolmogorov-p4-upper-decoder-plan.md`](kolmogorov-p4-upper-decoder-plan.md) | P4 上界 decoder 再アーキ (R1 base-card `encodeBlock` / R2 div-mod decoder + `Partrec₂` / R3 natLen utility / R4 #5/#6 closure、method-of-types crux) | 🔄 R1-R3 landed、残 R4 (§R4 recipe) | 親 §Phase P4 上界分解 |
...
**下界 proof-done により残るは上界 `kolmogorov_entropy_rate_upper` (`EntropyRate.lean:99`) 1 本のみ。** 上界 =
method of types。program = `⟨型記述子 c⟩ ++ ⟨index i⟩` を `invariance` に食わせ、長さ `≈ |α|·log n + n·(H+ε)` を
得て P4 上界を組む。7 helper に分解 (proof-pivot-advisor 診断、2026-07-20 settled):
...
| **4** | `typeDecoder_partrec` (`Partrec₂`) ★crux | `EntropyRateUpper.lean` (div-mod pack + base-card 出力) | ✅ **再アーキ済** `7a529ce8` (旧設計はレート無効 → R2 で div-mod packing に再設計、`Partrec₂` 再証明 proof-done sorryAx-free) |
| 5 | `condComplexity_block_typical_le` (典型 per-string 上界) | `EntropyRate.lean:131`、`sorry` | 🔄 局所化済、R4 (§R4 recipe) 待ち = crux |
| 6 | `condComplexity_block_uniform_le` (uniform O(n) 上界) | `EntropyRate.lean:144`、`sorry` | 🔄 局所化済、R4 で literal echo + `encodeBlock_lt` + `natLen_le_of_lt_two_pow` により容易 |
| 7 | 積分組立 (`kolmogorov_entropy_rate_upper` body) | `EntropyRate.lean:126`、#5/#6 の**文**を消費 | ✅ **banked** `334b2e4e` (encoding 非依存、unconditional body) |

**戦略 (control state)**: **#1-#4 + #7 banked 完了** (#1-#3 `475293d1`/`979a10a4`/`76a18d6d`、#4 は再アーキ
`7a529ce8` で div-mod pack に再設計・`Partrec₂` proof-done sorryAx-free、#7 積分組立 `334b2e4e`)。R1-R3 の下部構造
(base-card `encodeBlock` + `encodeBlock_eq_ofDigits` bridge + div-mod decoder + natLen utility) も landed。**残 = R4 =
#5/#6 closure** (子 plan §R4 recipe が SoT)。#5 は `typeDecoder` を `invariance` に食わせ #1 raw→exp bridge で消費
(matching 補題 `encodeBlock n b ∈ typeDecoder m n` が crux)、#6 は literal echo で容易。旧「encoding 橋」caveat は
R1/R2 (base-card 出力の一致) で解消済。
...
- #3 は既存 `conditionalKL_HXemp_le` (`ConditionalMethodOfTypes/Mass/Concentration.lean:786`、public) の直接
  wrapper (~15 行、当初「b/中 40-80 行」見積を既存資産 reuse で大幅短縮)。`EntropyRate.lean` は Concentration を
  import 済 (cycle なし)。

**crux #4 = decoder の computability 認証** (子 plan [`kolmogorov-p4-upper-decoder`](kolmogorov-p4-upper-decoder-plan.md)
に外部化)。`invariance_code` の加法定数 `b = encodeCode c + 2` は c にのみ依存 ⟹ decoder は **n を入力に取り n に
ついて uniform な単一 code** でなければならない (per-n 別 code は b 発散で o(n) 崩壊)。decoder `A(m,n)`: m を
`(型記述子 c, index i)` に unpair → n と c から `T_n(c)` を canonical 順で列挙 → i 番目を `encodeBlock`。重い核 =
**n 依存 dependent 型族 `Fin n→α` 上の decidable subset 列挙を n について uniform に `Partrec₂` 認証**。子 plan の
first-move framing = block を length-n 値 `List α` として扱い Mathlib `Primrec` list API に載せる (型 `Fin n→α` を
計算対象にしない)。

**settled fact (proof-pivot-advisor 診断、confidence = `loogle-neg`、下 §settled facts 参照)**: method-of-types
family 全体に `Partrec`/`Computable` 0 件、Mathlib に `Computable` × `List.filter` loogle Found 0 ⟹ **decoder は
既存資産で 1 行も軽くならない genuine real work**。**ただし全有限・decidable ゆえ原理 computable = 「選択 (big)」で
あって「壁 (hard)」ではない** — `@residual(wall:…)` は打たず `@residual(plan:kolmogorov-p4-upper-decoder)` を使う。
代替アーキテクチャは全却下 (子 plan §settled facts): naive 典型集合 index は μ 実数閾値で非 computable / 全ブロック
列挙は rate `log₂|α|` で H 未達 / 数え上げ逆用は下界のみ / n ごと別 code は b 発散。

**#4 の `Partrec₂` は proof-done だが設計がレート無効 (2026-07-24、assembly leg `334b2e4e` で判明)**: decoder
`typeDecoder` + `typeDecoder_partrec : Partrec₂` (`EntropyRateUpper.lean`、`20bdeaa3`) は計算可能性としては正しいが、
`Nat.pair` packing (index 2 倍化) + `encodeBlock` との encoding 齟齬 + `encodeBlock` doubly-exp 長さの 3 欠陥で
上界レート `H/log2` を達成できない (詳細 = 子 plan [`kolmogorov-p4-upper-decoder`] §rate 欠陥)。**教訓**: `Partrec₂`
認証は length-efficiency と直交 — decoder を usable asset と認める前に rate/length-budget チェックを要する。

**現状のコード側 residual**: `EntropyRate.lean` の per-string 上界 #5 `condComplexity_block_typical_le` (`:103`) と
#6 `condComplexity_block_uniform_le` (`:116`)、両 `sorry + @residual(plan:kolmogorov-p4-upper)`。#7
`kolmogorov_entropy_rate_upper` (`:127`) は #5/#6 の**文**を消費する body として banked (encoding 非依存)。closure は
子 plan §再アーキ R1-R4 (encodeBlock→base-card / decoder→length-additive / natLen utility / #5-#6 埋め) で達成。
