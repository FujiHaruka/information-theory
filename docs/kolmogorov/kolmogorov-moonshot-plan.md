...
- [~] Phase P4 — K↔H エントロピー率 (**flagship** `@[entry_point]`) — flagship 文確定 + 基礎 plumbing landed
  (`EntropyRate.lean`: `kolmogorov_entropy_rate` を上界/下界 2 半分 + squeeze で outright 証明、`encodeBlock`/可積分性/
  base bridge/`condIncompressible_count` は proof-done sorryAx-free)。残 = 上界 (`p4-upper`、method-of-types 難所) +
  下界 (`p4-lower`、数え上げ + 強典型性、tractable) の 2 honest sorry。両ゲート PASS (commit `357107ec`)
