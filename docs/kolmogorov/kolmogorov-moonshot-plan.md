...
- [~] Phase P4 — K↔H エントロピー率 (**flagship** `@[entry_point]`) — flagship 文確定 + 基礎 plumbing + **下界 proof-done**。
  ...
  上界は 7 helper に分解済 (下 §Phase P4 上界分解)、うち crux #4 decoder は子 plan
  [`kolmogorov-p4-upper-decoder-plan.md`](kolmogorov-p4-upper-decoder-plan.md) に外部化。
...
**次 leg 戦略 (control state)**: 上界 = method of types、7 helper に分解済 (下 §Phase P4 上界分解)。
**crux #4 decoder (`typeDecoderPartrec`、Phase-U 級) の gateway atom を 1 本 probe** (make-or-break、子 plan
[`kolmogorov-p4-upper-decoder`](kolmogorov-p4-upper-decoder-plan.md) §gateway atom) + **並行で #1-#3 を
proof-done banking** (crux #4 と独立、既存資産 + 下界組立 pattern 流用)。#4 が通れば #5-#7 直線的で上界
closure が視界に入る、詰まれば R-DEC で上界 sorry 据え置き確定 → flagship は「下界 proof-done + 上界 banked」の
honest 部分勝利 commit。
...
|---|---|---|---|
| [`kolmogorov-p4-upper-decoder-plan.md`](kolmogorov-p4-upper-decoder-plan.md) | P4 上界 helper #4 `typeDecoderPartrec` (decoder 構成 + `Partrec₂` 認証、method-of-types crux) | OPEN 📋 (未着手) | 親 §Phase P4 上界分解 |
...
| # | lemma 名案 | 主依存資産 `file:line` | 難度 |
|---|---|---|---|
| 1 | `typeClassByCount_card_le` (`\|T_c\| ≤ exp(n·entropyByCount)`) | `TypeClassLowerBound.lean:55` (`pow_div_prod_pow_eq_exp_n_entropyByCount`) + `Sanov/MultinomialLowerBound.lean:136` (`multinomial_le_typeClass_card`) + 多項定理 1 項 | b (~40-60 行) |
| 2 | `numTypes_le` (型数 `≤ (n+1)^\|α\|` ⟹ O(log n) bit) | `Sanov/MultinomialLowerBound.lean:298` (`piAntidiag_card_le`) | b (~20-40 行) |
| 3 | `entropyByCount_le_of_strongTypical` (強典型上で `entropyByCount ≤ H+ε`) | `StrongTypicality.lean:398/429` | b/中 (~40-80 行、entropy 連続性) |
| **4** | **`typeDecoderPartrec` (decoder A + `Partrec₂ A`) ★crux** | 既存資産なし、`Nat.Partrec`/`Primrec` 自前 | **c (Phase-U 級、150-400 行)** |
| 5 | `condComplexity_block_le` (#4 を `invariance` に食わせ per-string 上界) | `Invariance.lean:36/53`、#1-#4 | b (~40-80 行、#4 待ち) |
| 6 | atypical 吸収 (literal 上界 + P→0) | `Invariance.lean:24` (`complexity_le_natLen`)、`StrongTypicality.lean:212` (`stronglyTypicalSet_prob_tendsto_one`) | a/b (~40-80 行) |
| 7 | 積分組立 (typical/atypical 分割 + squeeze) | 下界 `kolmogorov_entropy_rate_lower` の plumbing pattern 流用 | b (~60-100 行) |

**戦略**: **#4 が唯一の crux (Phase-U 級)、#1-#3 は #4 と独立に proof-done 可** (既存 method-of-types 資産 +
下界組立 pattern だけで立つ ⟹ **並行 banking 推奨**)。#5 は #4 を消費、#6-#7 が合流。#4 が通れば #5-#7 は直線的。
...
**現状のコード側 residual**: `EntropyRate.lean:99` は上界全体 1 本 `@residual(plan:kolmogorov-p4-upper)` のまま
(decoder はまだ skeleton 化されておらず独立 decl が無い)。次 leg が skeleton を書く際に #4 を
`@residual(plan:kolmogorov-p4-upper-decoder)`、残りを `@residual(plan:kolmogorov-p4-upper)` に tag 分割する。
