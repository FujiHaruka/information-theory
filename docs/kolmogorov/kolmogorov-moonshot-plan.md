...
- [~] Phase P4 — K↔H エントロピー率 (**flagship** `@[entry_point]`) — flagship 文確定 + 基礎 plumbing + **下界 proof-done**。
  ...
  上界は 7 helper に分解済 (下 §Phase P4 上界分解)、**#1-#3 は proof-done banked**
  (commits `475293d1`/`979a10a4`/`76a18d6d`)、残 crux #4 decoder は子 plan
  [`kolmogorov-p4-upper-decoder-plan.md`](kolmogorov-p4-upper-decoder-plan.md) に外部化。
  flagship 完全 proof-done は #4 + #5-#7 closure 待ち。commits `357107ec`/`ce507e7c`/`77230dae`
...
上界の 7 helper のうち **#1-#3 は banked、残 crux = #4 decoder** (子 plan)。下界・flagship squeeze 骨格・
P1/P2/P3/P5 は proof-done。P6 SLLN は stretch (第 1 波成否に無関係)。

**次 leg 戦略 (control state)**: **#1-#3 banked 完了** (2026-07-20)。残 = **crux #4 decoder
(`typeDecoderPartrec`、Phase-U 級) の gateway probe が唯一の make-or-break** (子 plan
[`kolmogorov-p4-upper-decoder`](kolmogorov-p4-upper-decoder-plan.md) §gateway atom)。#4 が通れば
#5→#6→#7 は直線的 (#5 = #4 + #1-#3 を消費、raw→exp bridge で接続 — 下 §Phase P4 上界分解) で上界 closure が
視界に入る、詰まれば R-DEC で上界 sorry 据え置き確定 → flagship は「下界 proof-done + 上界 helper banked」の
honest 部分勝利 commit。
...
| **4** | **`typeDecoderPartrec` (decoder A + `Partrec₂ A`) ★crux** | 既存資産なし、`Nat.Partrec`/`Primrec` 自前 | **c (Phase-U 級、150-400 行) — 未着手** |
| 5 | `condComplexity_block_le` (#4 を `invariance` に食わせ per-string 上界) | `Invariance.lean:36/53`、#4 + #1-#3 | b (~40-80 行、#4 待ち) |
...
**戦略 (control state)**: **#1-#3 は banked 完了** (commits `475293d1` / `979a10a4`+`76a18d6d`)。
**残 = crux #4 (`typeDecoderPartrec`、Phase-U 級) の gateway probe が唯一の make-or-break** — 通れば
#5→#6→#7 は直線的で上界 closure が視界に入る。#5 は #4 と #1-#3 を消費、#6-#7 が合流。
