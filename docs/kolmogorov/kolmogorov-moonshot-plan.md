...
Phase P3 数え上げ        (P2 条件上界)
 非圧縮存在                 │
   ...
   │            │   下界 = P3 数え上げ + AEP                     │
   ▼            ▼                                              │
Phase P6 SLLN   Phase P5 非計算可能性 (halting_problem/rice)     │
 (stretch)      (独立 iconic、P1/P3 のみ依存 = 並走レーン)         │
```
...
- **依存**: Phase U。
- **成果物**: `invariance (A : ℕ → ℕ → Part ℕ) (hA : Partrec₂ A) : ∃ c, ∀ x, complexity x ≤ C_A x + c`。
  機構: A を `ℕ →. ℕ` として `exists_code` で `eval c_A` に落とし、interpret モードの固定 selector
  `1 ∷ selfDelim(c_A の idx)` を前置。加法翻訳補題 `l(translate_A p) ≤ l(p) + c_A`
  (`List.length_append` + 固定 selector 長) が核。
...
- **成果物** (`@[entry_point]`):
  ...
  theorem kolmogorov_entropy_rate
      ...
      Tendsto (fun n => (1/n) * ∫ ω, (condComplexity (encodeBlock (Xs · ω) n) n : ℝ) ∂μ)
...
- **撤退ライン**: 途中 sorry は `sorry + @residual(plan:kolmogorov-p4)`。P1 park 時は P4 park、ただし
  **下界のみ独立に取得** (P3 数え上げ + AEP だけで立つ部分勝利、`sorry + @residual(plan:kolmogorov-p4-upper)`
  を上界側に残す)。

### Phase P5 — 非計算可能性 (並走レーン) 📋

- **依存**: P1 / P3 のみ (P4 と独立に走れる = relay 並走レーン候補)。
- **成果物**: `complexity_not_computable : ¬ Computable complexity`。halting/rice は既製の弾
  (`ComputablePred.halting_problem` / `rice`)、Berry (「`C(x) ≥ k` の最小 x を計算 ⟹ その記述が短くなり矛盾」)
  または停止還元の**還元骨格のみ自作**。
- **見積行数**: 40–80 行。ファイル `Noncomputable.lean`。
- **proof-log**: no。
- **撤退ライン**: 還元が詰まる → `sorry + @residual(plan:kolmogorov-noncomputable)`。R2 の最小成果の 1 本。
...
6. **gateway atom 先行 (active)**: 背骨の残りを dispatch する前に、Phase U + P1 + P2 の 3 点セットを 1 本通して
   make-or-break を確認 → 通れば roadmap の Ch.14 scope-out (✖) を解除。
