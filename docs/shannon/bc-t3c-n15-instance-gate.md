...
> ⭐ **実行済 — 判定 = GO** (§4.1)。⚠⚠ **本書の §0 と §1 は着手前に書かれたものであり、事後に 1 文字も
> 書き換えていない** (親 plan §4.4-2 の義務。⚠ **見立ての当たり外れ / 反証条件の発火状況は §4.1 / §4.2
> が担う**)。**不変性は起票時の commit `4a8061f7` と現行の同区間 (`## 0.` 行から `## 2.` の直前まで、
> 起票時のみ在ったプレースホルダ 1 行を除く) の md5 一致で機械確認できる**。
> **§4.6 の敵対的独立監査 = [`bc-t3c-n15-audit.md`](bc-t3c-n15-audit.md) (commit `8502840a`)、
> 判定 = 訂正あり生存** (46 系統中 7 成立 / 訂正 7 件 / ⚠⚠ **主判定 GO を動かすもの 0 件**)。
> ⚠ **その訂正 7 件は §2 以降へ反映済である** (⚠ **反映先は §2 以降だけ**)。
...
| **Lemma 3** | `glnsum.txt:224-225` | "If TY Z\|X is either a less-noisy or **semi-deterministic** broadcast channel, then TY Z\|X ∈ P^{λ0,λ1,λ2} **for all** λ0 ≥ max{λ1 , λ2 } ≥ 0." ⚠ **監査 訂正 1** — `:224` は "… for all λ0 ≥" で行が切れており、続きは **`:225` の `max{λ1 , λ2 } ≥ 0`** である (起票側は続きを Definition 1 の順序条件から補って `λ0 ≥ λ1 ≥ λ2 ≥ 0` と狭く写していた)。⚠ **向きは保守側** — **実文の方が広い `λ` で成り立つ**ので、本 leg の結論は狭い側の条件でも立つ |
...
| **(B) [probc] の積 3 クラス** (Theorem 2 / Theorem 3 / Remark 4) | ○ | ⚠ **無条件の `○` は付かない** — ⚠ **監査 訂正 6**: `glnsum.txt:69` の厳密性の witness は **Claim 3 = 逆向き semi-deterministic 側のみ**であり (`probc.txt:471` / `:484-487` も strictness を **Claim 3 から**導いている)、⚠⚠ **more-capable 側の厳密性は [probc] のどこにも無い** (監査が全走査)。⟹ §6-8 の「現行 more-capable インスタンスで `UV ⊋ C` かは確かめていない」と整合する | ⚠ **[probc] の内側**。かつ §3.3 の連鎖で**和レート面は文献だけで閉じる**。⚠ **族 (B) はどちらに転んでも本 gate には渡らない** — `UV = C` の側なら段 1 によりむしろ強く死ぬ |
| **(C) [glnsum] の和クラス** (成分が degraded / less-noisy / more-capable / deterministic / semi-deterministic) | ○ (**Corollary 1 = `M(T)` の支持関数の閉形**。⚠ **監査 訂正 5** — Corollary 1 が閉形で与えるのは **`M(T)`** であり、**`C` の支持関数と書くには Theorem 4 (`M = C`) を経由する**) | ○ (明示 witness `(0,5/4,5/4)`)。⚠ **監査 訂正 2** — `glnsum.txt:73` が言うのは **`O_UVW ⊋ M`** であって `⊋ C` ではない ⟹ **Theorem 4 (`M = C`) を 1 段挟んで初めて `UV ⊋ C` が出る** (挟む Theorem 4 は §5.1 資産 2 が供給しているので結論は立つ) | ⭐ **[probc] の外** (`glnsum.txt:75-76` 逐語「積分解を持たないので [14] の外界は適用できない」)。⚠ **監査 訂正 3** — 同逐語は "**this** sum-broadcast channel" = **Fig. 3 の単数**についての言明であり、**「和クラス全体が [probc] の外」とは書けない**。⚠ **§5.1 が渡すのは Fig. 3 のインスタンス 1 本**なので、**渡す対象の水準では逐語どおりである** |
...
⟹ **`d < 2/15` ならば `h_UV(λ) > h_C(λ)`** (`G5`、厳密有理。`d = 2/15` で境界の差はちょうど 0)。
⚠⚠ **監査 訂正 4** — これは**十分条件だけ**であって**同値ではない**。使った 2 本はどちらも**片側**の評価
(`h_UV ≥ …` / `h_C ≤ …`) だからである (監査は同じ 2 入力と整合しつつ `d = 1/2 > 2/15` でも
`h_UV > h_C` となる模型を構成して「同値」を反証した)。⚠ **訂正の向きは GO を広げる側である** —
**真の錐はより広い可能性がある**。⚠ **これは §6-12 (「錐は下界であって最大ではない」) と整合が取れる**。
...
`glnsum.txt:75-76`)、**`C` の支持関数が全方向で閉形で書け** (Corollary 1 の `M(T)` を
**Theorem 4 (`M = C`) 経由**で渡す。⚠ **監査 訂正 5**)、**`UV ⊋ C` が
...
| **C** (Theorem 2 のクラスは [probc] の鏡像で `known-result` で死ぬ) | **締める** | ⚠ **結論は当たり / 経路は外れ** (⚠ **監査 訂正 7** — 同節が立てた 2 軸へ揃えた。⚠⚠ **`当たり (覆らなかった)` の単記に丸めない** — 表だけを読むと締める方向の的中率が実際より高く見える) | ⚠⚠ **ただし着手前に書いた 1 行反証は「逐語としては正しいが的を外していた」** — 「Theorem 2 と Theorem 3 のクラス仮説は交わらない」は `probc.txt:128-135` の逐語どおり**正しい**が、`known-result` の死は**クラス仮説の一致からではなく共有された逆定理 (Claim 4 = C) から来る** ⟹ **仮説が違っても死因は同じである**。⟹ **反証の対象が違っていた** |
...
| # | 資産 | 所在 |
|---|---|---|
| 1 | **`M(T)` の支持関数の閉形** (全方向 `λ0 ≥ λ1 ≥ λ2 ≥ 0`) ⟹ **資産 2 (Theorem 4 の `M = C`) を経由して `C` の支持関数になる**。⚠ **監査 訂正 5** — Corollary 1 が直接与えるのは `M(T)` であって `C` ではない | `glnsum.txt:481-493` Corollary 1 (+ 資産 2) |
| 2 | `C = M = Thm8` (**領域レベル**、全方向) | `glnsum.txt:682-698` Theorem 4 + `:224-225` Lemma 3 (⚠ **監査 訂正 1** の行番号) |
...
| 5 | ⭐ **生きた方向の錐** = `λ = (1,1,1−d)`, `d ∈ (0, 2/15)` (⚠ **`d` の上限は厳密有理**)。⚠⚠ **十分条件であって同値ではない** (監査 訂正 4 / §3.4) ⟹ **この錐は下界であって最大ではない** (§6-12) | 検証器 `G5` |
...
**反映済 (伝播 leg が実施)**: 親 plan §5.1 の **N15 着地ブロック** / 親 plan §5 の**進捗・消化数・
「次に来るもの」** / facts [`bc-facts.md`](bc-facts.md) の **`## N15 (T3c)` 節の新設** /
⭐ **facts `## M1 (T3b)` 行 4 の訂正 (→ §6-1) と同節 P4 行の隙間の数え** / **本書冒頭の注記** /
**監査の訂正 7 件** (§2.4 / §3.2 の (B)(C) 行 / §3.4 / §4.1 / §4.3 / §5.1)。
⚠ **§0 / §1 は 1 文字も動かしていない** (⚠ **消したのは §2 の直前に置かれていた
`⏳ 以下 §2–§6 は未記入である` のプレースホルダ 1 行だけ**で、それは起票時にのみ在ったものである)。
**親 plan §4.6 の敵対的独立監査は実施済**である ([`bc-t3c-n15-audit.md`](bc-t3c-n15-audit.md))。

**⚠ 未反映のまま残るもの (⚠ 落とさない)**:

- ⚠⚠ **N16 が何をするかは本書に無い** — **配分は orchestrator の仕事**であり、条文の SoT は
  親 plan §7 判断ログ 10-(4) である。⚠ **本書が渡すのは §5.1 の族 1 本だけ**であって、
  後続 leg の予定ではない。
- ⚠ **§6 の「確かめていない」16 項目は 1 つも消化されていない** — 監査が独立に潰せたのは
  **§6-7 の一部だけ**である (監査 §4 の併記 3)。⚠ **監査自身の未確認 9 項目は監査 §5 が SoT**。
