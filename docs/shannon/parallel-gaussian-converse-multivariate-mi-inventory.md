# ② parallel-gaussian converse — multivariate-mi 壁 独立再検証 + Mathlib/InformationTheory 在庫

> ✅ **SUPERSEDED (2026-05-29 closure)**: 本 inventory が研究した converse 壁 (旧 `@residual` の
> multivariate-mi 分類) は **genuine closure 済** — `parallelOutput_joint_logDensity_integrable`
> (`InformationTheory/Shannon/ParallelGaussian/Converse/MixtureDensity.lean:408`、多変量 mixture
> density lift、`@audit:ok`)。本 inventory の「self-buildable, 真の壁でない」予測は的中した
> (roadmap judgment #13)。**以下は当時の research snapshot** — ファイル参照は ParallelGaussian
> ディレクトリ再編前の flat 名を現構造に更新済 (struct fields は `PerCoord.lean`、headline/constructor は
> `PerCoordRegularity.lean`、converse 補題は `Converse/*.lean`)。
>
> 対象 (当時): `InformationTheory/Shannon/ParallelGaussian/PerCoord.lean:187` (`bddAbove` field) +
> `:200` (`max_ent` field)、両 `sorry` + 旧 multivariate-mi 壁 residual。
> achiever 側 (`achiever_mi`, `PerCoord.lean:191`) は genuine 化済。
> 親計画: `parallel-gaussian-chain-rule-plan.md`。
...
`ParallelGaussian/PerCoord.lean:182-208` (`IsParallelGaussianPerCoordRegularity` struct)、converse 側 2 field:
...
`InformationTheory/Shannon/ParallelGaussian/PerCoord.lean:293-328`。各引数 verbatim:
...
`InformationTheory/Shannon/AWGN/ContChannelMIDecomp.lean:276-289`、verbatim:
...
`InformationTheory/Shannon/DifferentialEntropy.lean:520-528`、verbatim:
...
`InformationTheory/Shannon/AWGN/CapacityConverseMaxent.lean:742-829` `awgn_per_input_mi_le_log`。
...
- **`multivariate-mi 壁` は誤分類気味**: Mathlib に continuous channel MI decomp が無い (loogle `ProbabilityTheory.mutualInfo` / `MeasureTheory.condEntropy` = unknown identifier、InformationTheory が全て自前) のは事実だが、**InformationTheory 側に 1-D genuine 完成形が既に揃っており**、converse は「1-D template の `Fin n` lift」で genuine 着地可能。"hard wall" でなく "big (規模)" 側。
...
### 真の wall 残量 (`@residual(multivariate-mi 壁)` 維持対象)

なし (full closure 可能の見込み)。loogle 確認:
- `MeasureTheory.condEntropy` → `unknown identifier` (Mathlib 不在、InformationTheory 自前)
- `ProbabilityTheory.mutualInfo` → `unknown identifier` (同上)
→ Mathlib 側 high-level MI/entropy API は不在だが、**InformationTheory が必要部品を全て genuine 保有**するため、self-build で wall を消せる。**shared sorry 補題化は不要** (新規 wall は生じない)。
...
`sorry` + `@residual(multivariate-mi 壁)` で staged 維持 (他 3 piece は genuine 着地)」。
...
- **`gaussianReal μ 0 = Measure.dirac μ`** (Mathlib `Distributions/Gaussian/Real.lean:207`
  ...
  は `hN : ∀ i, (N i : ℝ) ≠ 0` を要求 (`ParallelGaussian/PerCoordRegularity.lean:182`)、
...
`InformationTheory/Shannon/ParallelGaussian/Converse/` (実装先、当時は flat な新規 `ParallelGaussianConverse.lean` を想定):

```lean
import InformationTheory.Meta.EntryPoint
import InformationTheory.Shannon.ParallelGaussian
import InformationTheory.Shannon.ParallelGaussian.PerCoord
import InformationTheory.Shannon.AWGN.ContChannelMIDecomp     -- 1-D decomp 部品 (generic 核 reuse)
import InformationTheory.Shannon.MultivariateDiffEntropy -- jointDifferentialEntropyPi + subadditivity
import InformationTheory.Shannon.DifferentialEntropy           -- differentialEntropy_le_gaussian_of_variance_le
import InformationTheory.Shannon.AWGN.CapacityConverseMaxent  -- 1-D converse 手本
...
> 注: `@residual` class は実装 plan 確定後 `plan:<slug>` に。現状 `multivariate-mi 壁` は self-build
