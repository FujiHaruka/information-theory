...
> **STATUS 2026-07-12 (Leg 13) — Atom F CLOSED + Atom E statement reframe 済 (監査 SOUND)、core body build 中**:
> gateway (`7812cfcf`) が crux (strong typicality ⟹ `M(xb)` を `H(wsm)` に pin) を機械確認。**Atom E+F coupled
> (`d8954711`)**: Ecov を strong-at-ε_cov ∩ weak-at-ε に強化、`ε_cov=ε/(2(1+C))` の radius separation で
> same-ε の class-not-instance トラップを解消 (判断ログ #4)、chain (outer/inner/leaf) sorry-free reduction、
> `@audit:defect(false-statement)` 4 件除去、**独立 honesty-auditor PASS**。残 = **Atom E core body proof**
> (`wz_covering_jointBand_markov_core` の sorry、~150-250 行 conditional-AEP concentration) → G → H。
> headline `wyner_ziv_achievability` に仮説追加なし (#9 crux 維持)。converse (P2) は無影響 (FULLY CLOSED
> sorryAx-free)。詳細 → 下 §Approach + §判断ログ #3/#4 + `wz-facts.md`。
...
- [ ] **Atom E-kernel — conditional-AEP kernel** `wz_covering_uyBand_condSlice_le` (~L5797、新規 `sorry + @residual(plan:wz-binning-covering)`、`b489d51f`): from-scratch correlated conditional AEP。recipe → 下 §Atom E-kernel。最難所、~150-250 行、Mathlib gap なし 🚧
...
- [ ] isolated kernel を genuine に埋める。statement = good xb (strong-cover at ε_cov ∧ (x,y)-typ) 下で
