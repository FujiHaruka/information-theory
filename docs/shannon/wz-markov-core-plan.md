...
- **gateway-atom STAGED ✅ (`239be104`、主リスク retired)**: `wz_covering_strongTypical_indep_mass_ge` (~L5725、
  `jointStronglyTypicalSet_indep_prob_ge` の WZ-instance、Leg-A ambient-regularity lemmas で直接証明、
  **sorryAx-free**) が strong covering-success 下界を供給。∴ reopening 全体が `plan`-class plumbing (wall でない) と決定的に確認。stale docstring (S5a/gateway-2 機構) も訂正済。
- **残 = (1) reorder/hoist + (2) joint-derandomize (両方 confirmed plumbing、Leg 14 mainline)**:
  1. **reorder/hoist**: Markov-core chain (**L4838-6723**) + shared helpers (**~11 本、L2263-3518 散在**) が covering atom (L1154) の**後方** ⟹ Lean 後方参照不可。chain block を L1154 の前に hoist。**⚠ chain は現在 `wyner_ziv_achievability_codes` (L4799) の後方 = dead/unwired code** (sorryAx-free だが headline 未接続) — hoist がこれを wire する。`dep_consumers.sh` で移動対象の consumer 確認、段階的に type-check-done 維持。
  2. **joint-derandomize**: `rate_distortion_achievability` は distortion のみ露出。leaf の premise `SRC.real(wzCoveringSuccessStrongᶜ) ≤ tol/2` には**同一 code** の strong covering-failure decay が要る (staged gateway `wz_covering_strongTypical_indep_mass_ge` → codebook-averaged decay → `exists_codebook_low_avg`)、`rate_distortion_achievability_strong` を `codebookMeasure` averaging で reopening し distortion と結合。
- **素材**: 上記 gateway + leaf `wz_covering_chosenWord_sideInfo_typical` (acceptance-failure SRC bound、chain 末尾) + `exists_codebook_low_avg` (joint derandomize) + `rate_distortion_achievability`/`_strong`。
- **tractability**: med-large (機構は確定、量が主コスト = 大 hoist + derandomize build)。撤退 = `sorry + @residual(plan:wz-binning-covering)`。
