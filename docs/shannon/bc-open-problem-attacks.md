...
**現在**: leg 10 / 20 (**L9 = 棚卸し #2 完了** — 残り枠を **gate-first** ([探索 + gate] → [構築 + probe] の
2 leg 1 組) へ組み替え、`markovity-conjecture` の再開条件を訂正。**有効な経路は依然 0 本** (R1 は L8 で
**(4) を通り (5) で不合格**)。次は **L10 = 探索 + gate (軸 C `jog-nair-inequality`。LP gate
`shannon-cone-lp` を先に建てる)**、親 plan §6.2 / §7)。
