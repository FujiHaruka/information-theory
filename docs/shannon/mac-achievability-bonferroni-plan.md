...
- [ ] M5 — 組立 `mac_random_codebook_average_le`（sorry L274 解消）📋
- [ ] MV — verify（sorryAx-free + 推移的 `mac_achievability` proof done）📋
...
（`Achievability.lean:405`、唯一の consumer）が推移的に proof done → **MAC achievability
genuine closure**（Cover–Thomas Thm 15.3.1 corner-point form、converse は Phase A2 で既達）。

## 判断ログ

append-only。決着済 entry は削除（git が履歴）、active のみ残す。≤ 10 entry。

1. **gateway-atom-first = M3 E1 swap（active）**: 単一ユーザに無い唯一の新規発想は pair-channel
   の conditional-output fold-in（真 user-1 符号語のみ周辺化、出力軸が `(X₂,Y)` pair）。これを
   `mac_random_codebook_E1_swap` で最初に dispatch。通れば E2 鏡像 / E0・E3 は単一ユーザ同型で連鎖。
2. **promote vs 再導出（active、M0 で確定）**: `codebook_marginal_one/two` は型ジェネリック
   private。mainline は MAC-local 再導出（自己完結・proven 資産にリスク非波及）、代替は public 化
   流用（重複ゼロ・ripple ゼロ）。**どちらも既存 signature 不変**ゆえ親 consumer への波及なし。
3. **指数符号橋は検算済（settled、削除可）**: gateway 指数 core = `-(macInfoⱼ)`、
   `macAmbient_entropy_coord` (IIDAmbient L160) + `macInfoⱼ` 定義（L218/225/232）から一致確認。
