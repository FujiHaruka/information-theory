...
### route（次アクション）= **USER-DECISION（(b) 壁の正面突破は WSEB=TRUE+wall で失敗、投資判断が要る）**

leg 9 probe で WSEB gateway は決着（TRUE + genuine wall）。(b) の「安く BddAbove を閉じる」路は無い。次の戦略は
**ユーザー投資判断**（relay stop #2 で PAUSED）。3 択:

1. **prolate/LPS spectral 理論を Lean で build（~800–1500 行、moonshot 規模）** → WSEB closure → BddAbove →
   Phase 3 achievability closure。**副次的に Phase 4 converse `contAwgn_eq_shannonHartley` の tight ≈2WT count
   （Legs B/C/E）も同系理論を要すため一部再利用可**。高コスト・高価値（複数 sorry を閉じうる）。多 leg。
2. **capacity def を標準 achievable-rate 形（code sequence で達成可能な rate の sup）へ refactor** →
   achievability（≥）方向を converse の boundedness 義務から**分離**（`contAwgn_ge_shannonHartley` docstring
   `:496–498` が明記する代替）。BddAbove を回避して achievability を安く閉じる。**def 変更**（何を証明しているかの
   意味が変わらないか要吟味、converse 側は依然壁）。中コスト・設計判断。
3. **honest sorry のまま park し、他 family へ**（`contAwgnMaxMessages_bddAbove` は正しく分類済、コード honest）。
   低コスト・(b) を保留。

**Legs B/C/E は Phase 4 に defer**（BddAbove の make-or-break は Leg W = WSEB で、それが壁と確定）。
