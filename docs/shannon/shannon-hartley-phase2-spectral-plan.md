...
`contAwgn_eq_shannonHartley` の exact 定数に必須。proof-log: yes。**Phase-4 専用**。
...
| **R4** | operational bridge（Gram 行列 + Cauchy interlacing + capacity 計算） | 未評価 | **Leg P 以降誰も見ていない = 最大の未計測債務**。count の**下流**にあるので R1–R3 を閉じても consumer の sorry は落ちない。**family を「ほぼ完了」と scope する前に評価を dispatch せよ**（監査の勧告） |
