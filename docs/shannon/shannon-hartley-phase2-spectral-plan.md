...
- [~] **Leg B — コンパクト性** `timeBandLimitingOp_isCompact` 🔄 **4 leaf 中 3 genuine（backbone + Leaf 1/3/4、全 sorryAx-free、独立監査 PASS）。残 sorry は Leaf 2 のみ** = headline の `sorryAx` は Leaf 2 単独に由来（機械確認済）:
  - **Leaf 2** `bandLimitProj_apply_ae`（make-or-break 橋 = 抽象 `P_W` ↔ sinc 畳み込み）🔄 **唯一の残 sorry**（`@residual(plan:shannon-hartley-phase2-spectral-plan)`、slug は子 plan = 本ファイルが SoT）。署名は **`(W : ℝ) (hW : 0 ≤ W) (f : E)`** — 元の framing は `W<0` で **FALSE**（`sincN` 偶ゆえ提示 kernel は `|W|` low-pass の符号反転、一方 LHS は `bandLimitSubspace W = ⊥` で 0）。48e499e2 で修正・監査 PASS（98d4da6c）。**scope 不変**: 両 headline は `(T W : ℝ)` のまま（`W<0` 枝を genuine に閉じた）。
    ...
    - **(b) = 残 sorry の中身。旧記述「`integral_exp_boxcar_eq_sincN` 経由の Lp² 畳み込み定理」は route WRONG**（その multiplier 定理は Mathlib に**存在しない**）。真の blocker = **L¹∩L² 橋** `g ∈ L¹∩L² ⟹ (𝓕⁻¹_{L²} g : ℝ→ℂ) =ᵐ 𝓕⁻ g`。Mathlib の L² Fourier (`Lp.fourierTransformₗᵢ`) は Schwartz 拡張定義の**ブラックボックス**（loogle: 自身以外の consumer 0 / `VectorFourier.fourierIntegral, MeasureTheory.Lp` Found 0）、`𝓢'` 経由も同断（`TemperedDistribution` × `fourierIntegral` 言及 file = 0）。**`plan:` で正しく `wall:` ではない**（監査独立確認）。
    - **(b) の安い route（監査発見、~150-300 行。code docstring に記録済 = SoT）**: 一般 L¹∩L² 橋（同時 L¹∩L² Schwartz 稠密が必要 = 不在）は**不要**。コンパクト台の場合だけ要り、そこでは L¹ leg が Cauchy-Schwarz で**タダ**: ① `g = 𝟙_[-W,W]·𝓕f` は `[-W,W]` 台 → ② `SchwartzMap.denseRange_toLpCLM`(p=2) で `ψₙ → g` → ③ 固定 cutoff `χ`（`[-W,W]` で 1、`[-W-1,W+1]` 台）を掛けた `χψₙ` は Schwartz・単一コンパクト台 ⟹ `‖χψₙ−g‖₁ ≤ √|K'|·‖χψₙ−g‖₂ → 0`（mollifier 不要）→ ④ `SchwartzMap.toLp_fourierInv_eq` + `fourierInv_coe`（Schwartz 側 `rfl`）と `VectorFourier.norm_fourierIntegral_le_integral_norm`（L¹ ⟹ 一様収束）で極限、L² 極限の a.e. 収束部分列で閉じる。連鎖の全 asset は Mathlib 実在確認済。
