...
theorem wyner_ziv_achievability
    ...
    WynerZivAchievable P_XY d R D := by sorry  -- @residual(plan:wz-binning-covering)

-- 【提案・未実装】converse leg
theorem wyner_ziv_converse
    (P_XY : Measure (α × β)) (d : DistortionFn α γ) (R D : ℝ)
    (h_ach : WynerZivAchievable P_XY d R D) :
    wynerZivRateFactorizable U P_XY.pmf d D ≤ R := by sorry  -- @residual(plan:wz-auxiliary-singleletter)
```
...
| **pmf ↔ measure 橋** | `wynerZivRateFactorizable U (P_XY as pmf) d D` と `condMutualInfo μ X U - condMutualInfo μ Y U` の同一視 | ❌ **不在** | ⚠️**訂正 (2026-07-05, P1 実地)**: 想定転写元「RD `mutualInfoPmf ↔ mutualInfo` 既存橋」は**在庫に無い** (project-wide grep 0-hit)。`klDiv_joint_eq_mutualInfo` (`Converse.lean:108`) は存在するが **`private` (file-scoped) で cross-file 不可** かつ klDiv↔`mutualInfo` (pmf↔measure ではない)。→ 3変数 pmf↔measure 橋は **fresh 構築** (discrete-klDiv → negMulLog-sum、~100-200 行)。P1 で `wzMutualInfoXU/YU_eq_mutualInfo` を honest sorry+@residual(plan:) で stated (`Operational.lean:82/100`) |
...
- **`condMutualInfo` の型クラス** (`CondMutualInfo.lean:59`):
  ...
  - **【訂正 2026-07-05 (gateway probe 実測)】** `[Fintype + MeasurableSingletonClass]` から `StandardBorelSpace` は **`#synth` で自動導出される** (`[Countable]+[MSC] → [DiscreteMeasurableSpace] → [StandardBorelSpace]` chain が発火、tuple 型も同様)。**明示追加が要るのは `[Nonempty]` のみ**。旧記述「自動導出されない → `attribute [local instance]` で file 限定発火」は誤り。実証: `csiszar_sum_identity_hetero` (`WynerZiv/ConverseGateway.lean:48`) は α/β 両側に variable block (`[Fintype][MeasurableSpace][MeasurableSingletonClass][Nonempty]`) のみで `condMutualInfo` を効かせ 0 sorry で通っている。→ **self-build item #6 (local-instance) は MOOT** (§7)。
...
| RD 凸性 (converse Jensen) | `theorem wynerZivRateFactorizable_convex_in_D_of_obj_convex` / 無条件版 `wynerZivRateFactorizable_convex_in_D` (凸性で ∑R_WZ(Dᵢ) ≥ n·R_WZ((1/n)∑Dᵢ)) | `FactorizableRate.lean:548` / `ConditionalEntropyConvexity.lean:374` | ✅ **converse の Jensen 段は情報側完成品をそのまま呼べる** |
...
> **【更新 2026-07-05】** gap (i) heterogeneous Csiszár は **`csiszar_sum_identity_hetero` として proved sorryAx-free** (`WynerZiv/ConverseGateway.lean:48`、同型版証明を verbatim port、commit `b780f782`)。local-instance は不要 (§4 訂正)。残る real gap は (ii) auxiliary 同定橋のみ = converse は pure plumbing 公算大。closure 計画 → [`wyner-ziv-main-plan.md`](wyner-ziv-main-plan.md) P2。
...
| 4 | ~~**heterogeneous Csiszár sum identity**~~ **DONE** ✅ | `csiszar_sum_identity_hetero` (`ConverseGateway.lean:48`、sorryAx-free、commit `b780f782`) | 完了 | 同型版証明の verbatim port が通った (chain-rule stack が polymorphic)。`[StandardBorelSpace]` は α/β の variable block から自動 derive、**local instance は不要** (§4 訂正) |
| 5 | **pmf 形 ↔ measure 形 R_WZ の橋** | ⚠️**訂正 (P1 実地)**: `klDiv_joint_eq_mutualInfo` (RD `Converse.lean:108`) は **`private`** で流用不可、かつ「RD `mutualInfoPmf↔mutualInfo` 既存橋」は**在庫に無い** (grep 0-hit)。→ WZ 3変数 (X,Y,U) 橋は **fresh 構築**。`wzMutualInfoXU U q` = `mutualInfo μ X U`、`wzMutualInfoYU U q` = `mutualInfo μ Y U` (`.toReal` 形)。P1 で honest sorry stated (`Operational.lean:82/100`) | ~100–200 行 | pmf `q : α×β×U→ℝ` と empirical measure の対応。discrete-klDiv → negMulLog-sum 橋を自前で。結論形は P2 single-letterization 出力に合わせて確定 (Mathlib-shape-driven) |
| 6 | ~~**`condMutualInfo` の discrete StandardBorel local instance**~~ **MOOT** ⛔ | — | scope-out | **【訂正 2026-07-05】** `StandardBorelSpace` は `[Fintype+MSC]` から `#synth` で自動 derive、明示追加は `[Nonempty]` のみ (§4 訂正)。local-instance サブタスクは不要 = 本項は不成立。旧「instance 忘れが最頻事故」警告も無効 |
...
→ **真の「壁」は Mathlib 側ではなく in-project atom の未実装** (§7 の #2/#3)。**撤退 class = `plan` (wall ではない)**: WZ operational main の gap は Mathlib gap でなく in-project atom **合成**の未実装 (operational coding-theory は Mathlib 完全不在だが atom はすべて in-project 実在) ゆえ、`@residual(wall:…)` は audit-tags「壁 = Mathlib 未整備」の語彙に不一致 = 誤分類。achievability leg (#2) と converse leg (#3) をそれぞれ headline に `sorry + @residual(plan:wz-binning-covering)` / `@residual(plan:wz-auxiliary-singleletter)` (in-plan 退避なら `@residual(plan:wyner-ziv-main-plan)`) で立て、内部 atom を段階的に discharge する (SW/RD が採った witness-form 段階着地パターン)。**撤退 class の authoritative SoT は子計画 [`wyner-ziv-main-plan.md`](wyner-ziv-main-plan.md)「撤退ライン / honesty」** (child = SoT)。side-info conditional covering が in-project atom から組めない真の Mathlib gap と gateway-atom-first で判明した場合のみ、後続 PR で wall register へ昇格。

**【更新 2026-07-05】** heterogeneous Csiszár (§7 #4) は `csiszar_sum_identity_hetero` として **proved sorryAx-free** (`WynerZiv/ConverseGateway.lean:48`、同型版証明の verbatim port、commit `b780f782`) = **DONE、shared sorry 集約は不要**。
...
- **提案する新規 degenerate fallback 撤退ライン** (復活する場合):
  1. achievability / converse を各々 headline で立て、本体を `sorry + @residual(plan:wz-binning-covering)` / `@residual(plan:wz-auxiliary-singleletter)` で type-check done 着地 (hypothesis bundling 禁止、`:True` slot 禁止)。撤退 class は `plan` (wall ではない、§8)。
