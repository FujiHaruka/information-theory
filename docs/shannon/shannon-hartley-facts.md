...
| **修理後 `ContAwgnCode` は非退化に inhabited** ⟹ `bddAbove`/`ge` は空虚に真ではない | machine (leg 14 監査が sorry-free に構成) | 未コミット probe (`probeRichCode`, ~55 行、Leg D' が要れば再構築) | leg 14 | `k=1` / `testFn = 𝟙_{[0,T]}/√T`（`∫φ² = 1` を証明）/ `encoder = synthSignal T 1 (fun _ => √P)` が `encoder_power` を**等号 `T·P`** で満たし、`P > 0` で `encoder 0 ≠ 0`。要 `1 ≤ 2WT`。**副産物**: `ContAwgnCode T W P 0` は `IsEmpty`（`decoder` が `Fin 0` へ写れない）— 無害（`sSup` の集合が `0` を含まないだけ）。**ファイルに実数版 `synthSignal_memLp` が無く**（複素版のみ）監査は `Complex.reCLM` 経由を強いられた = Leg D' も同じ穴に当たる |
