...
## §BIINF-PHANTOM — conditionally-complete な `⨅ ε ∈ s` は s が点を除く開集合のとき false-statement を生む (leg 22)

`contAwgnOperationalCapacity` の当初 def は bounded binder `⨅ ε ∈ Set.Ioo 0 1, contAwgnRate` だった。
これは leg 22 で false-as-framed と判明し subtype infimum へ def-fix した。**gateway probe (Leg 0, §OBSERVATION-MAP) は
観測写像・achievable rate を検査したが、Lean の binder 意味論の縮退は直交した軸ゆえ捕まえられなかった。**

| claim | confidence | 再検証コマンド | last-verified | notes |
|---|---|---|---|---|
| 旧 `contAwgnOperationalCapacity`（bounded binder `⨅ ε ∈ Ioo 0 1`）は cap を無条件 `= 0` に潰し `contAwgn_ge` / `contAwgn_eq` を **P>0 で false-as-framed** にしていた | machine（監査が独立 compiler 再導出で縮退を CONFIRMED） | `example (g : ℝ → ℝ) (hg : ∀ x, 0 ≤ g x) : (⨅ ε ∈ Set.Ioo (0:ℝ) 1, g ε) ≤ 0` が通る（`ε ∉ (0,1)` の内 `⨅` が空 index ⟹ `sInf ∅ = 0`、`g ≥ 0` ゆえ全体 `≤ 0`） | leg 22（`ef401a5d` / `173adcb3`） | conditionally-complete な `ℝ` 上で `⨅ ε ∈ s, g ε` = `⨅ ε, ⨅ (_ : ε ∈ s), g ε`。`ε ∉ s` の各項は空 index の `⨅` = `sInf ∅ = 0`。`contAwgnRate ≥ 0` ゆえ cap が `0` に潰れる = **壁 gated でなく定義バグ**。修理 = subtype infimum `⨅ ε : ↥(Set.Ioo 0 1), contAwgnRate`（nonempty ゆえ genuine infimum = docstring intent）。監査 all OK（intent-preserving + converse 壁無傷 = 偽装でない判別子 PASS） |
| leg 22 def-fix 後 `contAwgn_ge_shannonHartley`（achievability `≥`）は **proof-done sorryAx-free** | machine | `#print axioms InformationTheory.Shannon.contAwgn_ge_shannonHartley` → `[propext, Classical.choice, Quot.sound]`（先に `lake build InformationTheory.Shannon.ShannonHartleyMain`） | leg 22（`ef401a5d` / 監査 `173adcb3`） | hyp 全 regularity（`0<W` / `0<N₀` / `0≤P`、@audit:ok）。この def-fix が achievability を「偽」から proof-done へ移した = 主張に実質がある。残 sorry は `contAwgn_eq`（converse 専用）1 本のみ |

**教訓 — gateway probe は「代用か定義か」を問うが binder 意味論の縮退は別軸**: Leg 0 gateway（§OBSERVATION-MAP）は
観測写像が Nyquist を密輸する代用でないかを検査し PASS したが、**capacity の集約子 `⨅` が空 index で `sInf ∅ = 0` を
拾う縮退は観測写像と直交する**。数値 probe も achievable rate の検査も binder の空 index case を踏まない
（`ε ∈ (0,1)` の範囲でしか評価しない）。⟹ **conditionally-complete order 上の bounded binder `⨅ x ∈ s` / `⨆ x ∈ s` は
`s` が空 index case を持つとき junk（`sInf ∅ = 0` / `sSup ∅ = 0`）を拾う** — 定義段で subtype infimum / nonempty 保証を
入れるか `s = univ` に正規化する。本 family は「ℕ-sSup-returns-0」（Cramer / ParallelGaussian）に続く同型の
junk-default 罠の 2 例目（今回は `ℝ` の `sInf ∅ = 0`）。**メタ所見**（§NUMERIC-TRUE-ARTIFACT / §OBSERVATION-MAP と同じ形）:
複数の gateway probe が「真か」を問うても、全員が同じ軸（観測写像 / rate）でしか問わなければ、直交する軸（binder 縮退）は
すり抜ける。
...
**この節に値を再キャッシュしない — 状態遷移のみ記録する。** タグは
leg 12 で `wall:nyquist-2w-dof` → `defect:false-statement`（3 宣言、`encoder_power` 窓のみ拘束）→
leg 14（Leg P def-fix）で `defect` 離脱 → `wall` 復帰 →
**leg 22 で achievability 半分が proof-done 化し `contAwgn_ge` から residual が消え、
live `@residual(wall:nyquist-2w-dof)` consumer が 2→1（`contAwgn_eq` 1 本、converse 専用）へ縮約**、と遷移してきた。
現状は毎回 `rg '@residual' InformationTheory/Shannon/` + `#print axioms` で再導出せよ（CLAUDE.md「re-derive > cache」）。
...
| **不等利得 operational converse の核は in-tree**（R4-CONV の C3、最大項）| machine | `#print axioms InformationTheory.Shannon.ParallelGaussian.parallel_per_input_mi_le_sum` → sorryAx-free | leg 16（path/namespace 訂正 leg 23） | `InformationTheory/Shannon/ParallelGaussian/Converse/MixtureDensity.lean:901`。**`N : Fin n → ℝ≥0` slot が利得構造そのもの**。⚠️ **これは MI/capacity 側の上界**（`mutualInfoOfChannel p W ≤ ∑½log(1+P'ᵢ/Nᵢ)`、`∑P'ᵢ≤P`）。operational（message 数 → MI）の Fano+DPI 連鎖は **self-build**（in-tree の `awgn_converse` が equal-gain で使う Fano/DPI を unequal-gain へ移植）= 壁ではない。併せて `shannon_converse_single_shot`（`Converse.lean:70`）/ `awgn_channel_coding_theorem`（`AWGN/Main.lean:55`、`@audit:ok`）も R4 で直接消費可 |
| **C1 count domination（converse interlacing 上半分）は proof-done** = `finrank_le_prolateCount_of_form_gt`（`S` 上 Rayleigh 商 `>c` ⟹ `finrank S ≤ prolateCount T W c`） | machine | `#print axioms InformationTheory.Shannon.TimeBandLimiting.finrank_le_prolateCount_of_form_gt` → `[propext, Classical.choice, Quot.sound]`（先に `lake build InformationTheory.Shannon.TimeBandLimiting`） | leg 23（`c5822fed` / 監査 `73ec6559`） | @audit:ok。**crux（Vᗮ で `⟪Av,v⟫.re ≤ c‖v‖²`）は Leg E で既に in-tree**（`inner_timeBandLimitingOp_le_of_mem_orthogonal` L2501 + matched pair `le_inner_timeBandLimitingOp_of_mem` L2537、両 @audit:ok）。C1 は純線形代数（`S⊓Vᗮ=⊥` → `orthogonalProjectionOnto` 単射 → `LinearMap.finrank_le_finrank_of_injective`）。**advisor の「interlacing 非自明 = self-build 公算」を反証**（`cause:loogle-blind` 再演: crux は in-project 2 補題隣に既在）。⟹ `wall:nyquist-2w-dof` の live residual 0、`contAwgn_eq` 残 sorry を `plan:` へ再分類 |
...
**教訓 6 — 未検証の probe による 0-hit は、壊れた probe の出力と区別できない（leg 17、実装者が自己発見 → 監査が確定）**:
定数グラフ walk の `ConstantInfo.value?` は **`allowOpaque := false` が既定**で、**theorem には `none` を返す**
...
- 確認済: leg 17 の「atom は `:443` を 0 回消費」は **survives**（監査が positive control
  `norm_timeBandLimitingOp_sq_le_of_mem_orthogonal` → atom（L3390 で literally 呼ぶ）で probe を検証し、
  negative control 3 本 + closure = env の 11% で識別能も確認）。plan R1 行 / 下記 教訓 5 は訂正不要。
- ❌ **実装者の副次主張は誤り**: 「`env.find?` は import された証明本体を見られない ⟹ probe は同一ファイル内で
  走らせる必要がある」は**偽**（監査の positive control は import された本体から CONSUMED を得た）。
  証明本体は olean に入っている。将来の probe を不必要に高く見せるので訂正しておく。

**教訓 5 — 「資産の実在確認」は「資産の必要性の確認」ではない（leg 17、監査の 気づき）**:
R1 が `plan:` 級（壁でない）とされた根拠は「必要な資産は全て機械確認済」だった —
`Spectrum.lean:443` / `:463` / `l2Space.lean:528`。**leg 17 の atom はこの 3 資産を 0 回消費して
gate の目的を達成した**（監査が定数グラフ 56,356 定数を走査し機械確認。`simp` が textual occurrence なしに
補題を引くため **grep では決定不能**）。⟹ 実在確認は「その資産が使える」の証拠であって
「**その資産が要る**」の証拠ではない。教訓 2 の鏡像 — **肯定的主張（この経路が要る）も、
コンパイラが消費するまで確認したことにならない**。実害: 「資産が有るから壁でない」という自信が、
**一度も試されていない経路**の上に載っていた。R1 はいま「未検証」ではなく **dead obligation の公算**
（gate の目的が固有基底なしで達成された = 反対方向の証拠が出た）。
⚠️ 同型の罠が**残っている**: `prolateCount_mul_le`（L2375）は「`V` 上の有限次元スペクトル定理」として
引かれかけたが、**export された結論は crude Markov 上界のみ**で、再利用したい固有基底は
**証明本体内の `set` で export されていない**（監査が dispatch 前に捕捉）。
**「名前が in-tree にある」と「その名前が必要な形を export している」は別の主張。**

**教訓 7 — 「dead」判定は consumer ごとにスコープする（leg 18、A1 gateway で顕在化）**:
「R1 = DEAD」（教訓 5）は *count* consumer に対する判定だった — count は固有基底なしで閉じた。だが
**achievability consumer は固有関数を要する**（`contAwgn_ge_shannonHartley` docstring: boxcar 等の壁フリー族は
adversarial に反証済、prolate 以外に到達しない）。緊張の解消: achievability が要るのは**有限 V-固有基底**
`exists_orthonormal_eigenbasis_prolateEigenspaceSup`（L3529、`@audit:ok`、sorryAx-free、**既に in-tree**）で、
R1（フル無限固有基底）ではない。⟹ **「R1 dead」を「固有基底は一切不要」と読むと achievability で詰まる**。
plan の「A2 は R1 前提」は stale（有限 V-固有基底前提が正）。教訓: **dead obligation の判定は
「どの consumer に対して」を必ず明記する** — 別 consumer が同じ資産を別スコープで要しうる。
- **A1 gateway = 壁でない・plumbing 確定（leg 18、`1540d943` + 監査 `a11f9c39`）**: ℂ/ℝ 境界の解析核
  `timeBandLimitingOp_star_comm`（`A(star f)=star(A f)`）+ `star_mem_prolateEigenspaceSup`（V 星不変）、
  sorryAx-free・`@audit:ok`。**予測訂正**（次 family も踏む税）: Mathlib の `Lp` は**素の `Star` のみ**
  （`StarAddMonoid`/`StarModule` 無し）→ `star_add_Lp`/`star_smul_Lp` を self-build。共役線形 `starₗE` は再利用資産。
  V 上作用素下界 `le_inner_timeBandLimitingOp_of_mem`（`c‖v‖²≤Re⟪Av,v⟫`、`@audit:ok`）も leg 18 で所有
  （下界は上界と違い Parseval 和が不可避 = ノルム近道なし、実装者が確認）。
- **(B) 実基底 `exists_real_orthonormalBasis_prolateEigenspaceSup` = CLOSED sorryAx-free（leg 18、`53723ec2` + 監査 `231d96ab` `@audit:ok`）**:
  V の星不変 ℂ-正規直交基底。次元橋 `finrank ℝ V_ℝ = finrank ℂ V` は standalone 不要（`finrank_span_eq_card` で直接）、
  ambient `InnerProductSpace ℝ E` は既存 instance（ℂ 自身が実内積空間ゆえ `Complex.coe_smul` が `rfl`）で diamond 回避。
  self-build: `inner_star_star` / `real_inner_eq_re_complex` / `inner_complex_eq_real_of_star_fixed` / `star_sub_Lp` / `realForm`。
  ⚠️ **A2 に効く 2 点**: (i) この `u` は V の **ONB だが A-固有基底ではない**（V = 固有値*集合*上の sup、監査確認）
  ⟹ A2 の `Q_T ψᵢ/√μᵢ` は per-vector 固有値を要すので、実**固有**基底の別構成 or `A≥c on V` 作用素下界での再定式化が要る。
  (ii) star-fixed `Lp ℂ` 要素どまり = honest `ℝ→ℝ`（`[0,T]` 台）は 1 歩手前。docstring は両方 overclaim せず明記。
