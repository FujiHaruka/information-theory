...
>
> **Status**: **CLOSED — L-MAC5 fully closed (2026-07-05)**。achievability half (M0–P4) +
> converse half (CV `mac_timesharing_converse`) + full-region antisymmetry (V
> `mac_timesharing_capacity_region`、`@[entry_point]`、intersection 形) ともに proof-done
> sorryAx-free。**time-sharing 全凸包形 (Cover–Thomas Thm 15.3.1) を標準B (proof done) で完全達成**。
> CV/V の詳細は子 [`mac-timesharing-converse-plan.md`](mac-timesharing-converse-plan.md) が SoT
> (commits 軸 `c37333dc` / Dispatch C `2d45273c` / Dispatch D `67283ec4`)。corner-point form
> (`mac_converse` + `mac_achievability`、両 genuine) は入力資産。
...
**achievability half (M0–P4) + CV + V = 全て proof-done, sorryAx-free, 独立監査 PASS** (L-MAC5 fully closed)。
...
-- achievability half (headline、mac_achievability_region:887 = IsProbabilityMeasure + full-support union):
--   closedConvexHull ℝ (⋃ p₁ p₂ (full-support), macPentagon p₁ p₂ W) ⊆ macCapacityRegion W
-- converse half + full region は子 mac-timesharing-converse-plan.md が SoT。advisor 再精査で 3 軸訂正:
--   CV : {p | MACAchievable W p.1 p.2 ∧ 0≤p.1 ∧ 0≤p.2}
--          ⊆ closedConvexHull ℝ (⋃ p₁ p₂ (IsProbabilityMeasure), macPentagon p₁ p₂ W)
--   V  : macCapacityRegion W ∩ {0≤p.1 ∧ 0≤p.2}
--          = closedConvexHull ℝ (⋃ p₁ p₂ (IsProbabilityMeasure), macPentagon p₁ p₂ W)   (hW threading)
```
...
converse half（CV）+ full-region antisymmetry（V）は子サブプランに分離済（撤退口
`@residual(plan:mac-timesharing-converse-plan)`）。**子が CV/V signature の SoT、両 proof-done sorryAx-free で CLOSED**:

**→ [`mac-timesharing-converse-plan.md`](mac-timesharing-converse-plan.md)**。

要点のみ（詳細は子、判断ログ #4）:

- **finite-n gap（Gap 0/A/B′/C）+ CV/V assemble（Dispatch B/A/軸/C/D）は全て proof-done sorryAx-free**（commits `3377eba5`/`9c86884d`/`5a0419b0`/`302dbe03` + `d920a15e`/`315be033`/`c37333dc`/`2d45273c`/`67283ec4`）。genuine Mathlib 壁は無く、全て plan correction / Mathlib-shape-driven realignment で貫通。
- **CV/V signature 3 軸訂正**（達成、子 SoT）: (a) CV に `0≤p.1∧0≤p.2` guard（`MACAchievable` は負 rate down-set）、(b) union を `IsProbabilityMeasure` 限定（非確率 `p` は `macInfo` に junk）、(c) V は **intersection 形** `macCapacityRegion ∩ Q = RHS`（`macCapacityRegion=closure{MACAchievable}` が負象限含む、redefine 回避で blast radius 最小）+ `hW` threading。
- V の antisymmetry = CV + 本 plan P4 `mac_achievability_region` の all-prob upgrade（Dispatch C）で `Set.Subset.antisymm`（clamp-equivalence 経由、Dispatch D）。
- **honesty trap（達成後も遵守）**: per-letter chain を load-bearing hyp で受けない（削除済 tier-5 `MACPerLetterChain₁₂` 踏襲禁止）。追加した `0≤R` / `IsProbabilityMeasure` / `hW` は honest precondition。
...
frozen slug: **L-MAC5** (time-sharing 全凸包) = 本計画が genuine closure **達成** (旧「scope-out
維持」→ genuine closure)。親 `mac-moonshot-plan.md` の L-MAC5 行を CLOSED に同期済 (親 co-stage)。
...
4. **converse half + V を子サブプランで CLOSED (settled、child-is-SoT)**: CV/V は
   [`mac-timesharing-converse-plan.md`](mac-timesharing-converse-plan.md) が SoT（撤退口
   `@residual(plan:mac-timesharing-converse-plan)`、CLOSED ゆえ code 側 active 参照 0）。CV
   `mac_timesharing_converse` + V `mac_timesharing_capacity_region`（`@[entry_point]`、intersection 形）
   ともに proof-done sorryAx-free（軸 `c37333dc` / Dispatch C `2d45273c` / Dispatch D `67283ec4`）。
   advisor 再精査の **CV/V signature 3 軸訂正**（CV `0≤R` guard / union `IsProbabilityMeasure` 限定 /
   V intersection 形 + `hW` threading）は達成形。per-letter chain を load-bearing hyp で受けず貫通
   (`MACPerLetterChain₁₂` = 削除済 bundle 踏襲せず)。
