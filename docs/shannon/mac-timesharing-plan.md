...
- [ ] P2 — `{MACAchievable}` が `Convex` (P1 から) + 容量領域 = closure は Closed/Convex 自動 📋
- [ ] P3 — `macPentagon ⊆ {MACAchievable}` 閉包 (退化/軸/境界 limiting) 📋
- [ ] P4 — achievability headline `closedConvexHull ⊆ macCapacityRegion` assemble 📋
- [ ] CV — converse half (`{MACAchievable} ⊆ closedConvexHull`、per-letter → 凸包) 📋
- [ ] V — verify + full-region antisymmetry headline 📋
...
## CV — converse half (`{MACAchievable} ⊆ closedConvexHull`)
**proof-log**: yes (per-letter → 凸包 の time-sharing 論証は新規・より難、再開根拠必須)

**ファイル**: `InformationTheory/Shannon/MultipleAccess/TimeSharingConverse.lean`
(TimeSharing.lean を import)

- [ ] `mac_timesharing_converse`:
  ```lean
  {p | MACAchievable W p.1 p.2} ⊆ closedConvexHull ℝ (⋃ (p₁) (p₂), macPentagon p₁ p₂ W)
  ```
  骨格: 達成 rate pair `(R₁,R₂)` に対し既存 `mac_converse` の per-letter **和形**
  `R₁ ≤ (1/n)Σᵢ I(X₁ᵢ;Yᵢ|X₂ᵢ)` 等を得て、各時刻 `i` の周辺入力 `p₁ᵢ ⊗ p₂ᵢ` を露出
  (per-letter product-input pentagon 点 `(I(X₁ᵢ;Yᵢ|X₂ᵢ), I(X₂ᵢ;Yᵢ|X₁ᵢ))`)、
  rate pair `≤` (per-letter 点の時間平均 = 凸結合) → convex hull ∈。**点 ≤ 平均 ∈ hull** +
  per-letter marginal 露出 + time-sharing 補助変数 (Q-augmentation) が核。RHS が closed
  ゆえ `closure {MACAchievable} = macCapacityRegion ⊆ closedConvexHull` も従う。
- **依存 in-project decl**: `mac_converse` (`Converse.lean`、per-letter 和形 genuine)、
  `condMutualInfo`/`mutualInfo` 系、`macInfo₁/₂/Both`; `segment_subset_convexHull` /
  `convexHull` の finite convex combination membership (`Finset.centerMass_mem_convexHull` 類)。
- **予想規模**: ~300-500 行 (achievability half より難)。
- **注記 (肥大時)**: CV が単独で肥大 (>300 行 / 判断ログ >5) したら別サブプラン
  `mac-timesharing-converse-plan.md` に分離し `@residual(plan:mac-timesharing-converse-plan)` で
  参照 (本 plan は achievability half + full-region assemble を保持)。現時点は本 plan 内に置く。
- **撤退**: per-letter marginal 露出 / Q-augmentation で詰まったら該当 sub-lemma のみ `sorry` +
  `@residual(plan:mac-timesharing-plan)`。load-bearing `*Hypothesis` 禁止 (per-letter chain を
  hyp で受けない、`mac-inventory.md:136` の削除済 `MACPerLetterChain₁₂` bundle を踏襲しない)。
...
- [ ] `mac_timesharing_capacity_region` (`@[entry_point]`、full region):
  ...
  macCapacityRegion W = closedConvexHull ℝ (⋃ (p₁) (p₂), macPentagon p₁ p₂ W)
  ```
  = `Set.Subset.antisymm` (CV 経由 `macCapacityRegion ⊆ closedConvexHull` + P4
  `closedConvexHull ⊆ macCapacityRegion`)。
- [ ] 各ファイル `lake env lean …` silent (sorry warning のみ許容 = type-check done)。
- [ ] proof done 判定: `#print axioms mac_timesharing_achievability` /
  `mac_timesharing_converse` / `mac_timesharing_capacity_region` = `[propext,
  Classical.choice, Quot.sound]` (sorryAx-free)。
- [ ] 新規 `sorry` + `@residual` 導入 commit があれば **独立 honesty 監査** (`honesty-auditor`) を
  session 内で起動 (orchestrator-mandatory)。
- [ ] `InformationTheory.lean` に `TimeSharing` → `TimeSharingConverse` の import 2 本を追加
  (orchestrator が最後にまとめて)。
...
- achievability half (M0+P0+P1+P2+P3+P4) ≈ **510-660 LOC** (P2 訂正で advisor 案より軽)。
- converse half (CV) ≈ **300-500 LOC**。full region ≈ **810-1160 LOC over ≥2 sessions**。
- **推奨 leg 数 = 4-5** (同時 1 体まで、CLAUDE.md「Max one parallel agent」):
  ...
  5. CV converse half + full-region antisymmetry + V。
...
4. **converse half は肥大時に分離可 (active)**: CV (~300-500 LOC) が単独で肥大したら
   `mac-timesharing-converse-plan.md` に分離し `@residual(plan:mac-timesharing-converse-plan)` で
   参照。現時点は本 plan 内。per-letter chain を load-bearing hyp で受けない
