...
- [ ] CV — converse half（guard/IsProbabilityMeasure 訂正形、詳細は子が SoT）🚧 → [`mac-timesharing-converse-plan.md`](mac-timesharing-converse-plan.md)（**4 gap 全 done**: Gap 0 `3377eba5` / Gap A finite-n rate extract `9c86884d` / Gap B′ per-letter 同定 `5a0419b0` / Gap C `302dbe03`、いずれも proof-done sorryAx-free; 残 = CV/V assemble 6 obligation を **Dispatch B→A→C→D**）**← 次 Dispatch B**
- [ ] V — full-region antisymmetry headline（intersection 形 `macCapacityRegion ∩ Q = RHS`）📋 → [`mac-timesharing-converse-plan.md`](mac-timesharing-converse-plan.md)
...
`@residual(plan:mac-timesharing-converse-plan)`）。**子が CV/V signature の SoT**:

**→ [`mac-timesharing-converse-plan.md`](mac-timesharing-converse-plan.md)**。

要点のみ（詳細は子、判断ログ #4）:

- **4 独立 gap（Gap 0 code→ambient / Gap A finite-n rate extract / Gap B′ per-letter 同定 / Gap C 平均 pentagon→凸包）は全て proof-done sorryAx-free**（commits `3377eba5` / `9c86884d` / `5a0419b0` / `302dbe03`）。
- 残 = CV/V assemble の **6 obligation**（advisor 再精査で判明）を **Dispatch B→A→C→D** で攻略。いずれも genuine Mathlib 壁でなく **plan correction / Mathlib-shape-driven realignment**。
- **CV/V signature 3 軸訂正**（子 SoT）: (a) CV に `0≤p.1∧0≤p.2` guard（`MACAchievable` は負 rate down-set）、(b) union を `IsProbabilityMeasure` 限定（非確率 `p` は `macInfo` に junk）、(c) V は **intersection 形** `macCapacityRegion ∩ Q = RHS`（`macCapacityRegion=closure{MACAchievable}` が負象限含む、redefine 回避で blast radius 最小）+ `hW` threading。
- V の antisymmetry は CV + 本 plan P4 `mac_achievability_region` で `Set.Subset.antisymm`。ただし achievability は現状 **full-support union のみ** → V ⊇ に **all-prob upgrade（Dispatch C）**が unavoidable。
- **honesty trap**: per-letter chain を load-bearing hyp で受けない（削除済 tier-5 `MACPerLetterChain₁₂` 踏襲禁止）。追加した `0≤R` / `IsProbabilityMeasure` / `hW` は honest precondition。
...
4. **converse half + V を子サブプランに分離済 (active、child-is-SoT)**: CV/V は
   [`mac-timesharing-converse-plan.md`](mac-timesharing-converse-plan.md) が SoT（撤退口
   `@residual(plan:mac-timesharing-converse-plan)`）。**4 gap（Gap 0/A/B′/C）は全 proof-done
   sorryAx-free**（`3377eba5`/`9c86884d`/`5a0419b0`/`302dbe03`）。残 = CV/V assemble 6 obligation を
   Dispatch B→A→C→D。advisor 再精査で **CV/V signature を 3 軸訂正**（CV に `0≤R` guard / union を
   `IsProbabilityMeasure` 限定 / V を intersection 形 + `hW` threading、詳細は子）。本 plan は
