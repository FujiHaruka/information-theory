...
2. **最適点での 4 項分解が新しい診断になる** — どの項が損失を担うかがチャネルで切り替わる。
   Blackwell は 2 つの `p` のどちらでも 4 項すべて `0`；平滑化した dense Blackwell (`ε=0.1`) は
   `I(U;X|Y) = 0.270625` / `I(V;X|Z) = 0.244929` / 残り 2 項 `0`；BSC(0.1) 2 本は
   `0.000004 / 0.468996 / 0 / 0` で和がちょうど `min{H(X|Y),H(X|Z)}`。
...
- **P1 `Φ_det := max_{f,g}[I(f(X);Y) + I(g(X);Z) − I(f(X);g(X))]`** (`f,g` は `X` の関数、全列挙):
  ...
  **`T = Φ_det` =「最大化子は決定論的に取れる」と同値**。**(N1)(N2) とも PASS** — しかも `|X|=2`
  では [GJNW13] Theorem 1 そのものになる。**それでも refuted** (`+1.046862e-01`)。⚠ **死因は
  機構** — 最良組の空セルへ質量を流すと目的値が `+p(x)·ε·log₂(1/ε)` で超線形に増え、傾きが
  `ε → 0` で発散する (`+5.372e+00`) = **決定論的点は局所最大ですらない**。生存するのは
  `|X|=2` と決定論的 BC の 2 つの退化ケースだけ。
