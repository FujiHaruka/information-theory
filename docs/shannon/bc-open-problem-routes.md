...
|---|---|---|---|---|---|
| 1 | `H(X) − [I(U;Y)+I(V;Z)−I(U;V)] = [I(U;X)−I(U;Y)] + [I(V;X)−I(V;Z)] + H(X\|U,V) + I(U;V\|X)` | なし (恒等式) | `LHS ≤ H(X)` (4 項が Markov 下で非負) | `new` (初等) | **係数相殺で証明済** = probe §1 の (I1)(I2)。DPI 項は `I(U;X)−I(U;Y) = I(U;X\|Y) − I(U;Y\|X)` で、Markov から第 2 項が 0 |
| 2 | `max{I(X;Y),I(X;Z)} = H(X) − min{H(X\|Y),H(X\|Z)}` ⟹ `gap ≤ min{H(X\|Y), H(X\|Z)}` | step 1 | 上界その 1 | `new` (初等) | probe §1 の (I4) |
...
| **C2** | 定数ではなく**チャネル依存の汎関数** `Φ(T, p)` を作る。witness から強制される必要条件は 2 本 — **(N1)** `\|X\| = 2` で `Φ = max{I(X;Y), I(X;Z)}` ちょうど、**(N2)** 格子 BC で `Φ ≥ H(X)` | 未着手。**下の 2 案は probe 済で死亡** |

**C2 で既に死んだ 2 案** (次の leg が同じ穴に落ちないよう記録する。probe §11):
...
- ⚠ **教訓**: 2 案はそれぞれ (N2) / (N1) の**片方だけ**を満たす。片側だけ確認すると 2 回とも
  通ってしまうので、**次の案は必ず両方に当ててから** probe すること。
