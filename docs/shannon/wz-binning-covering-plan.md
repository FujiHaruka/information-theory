...
**Status**: ACTIVE 🚧 — 署名は全軸 honest (第1-5軸 closure、独立監査 all-PASS)。**A3 `wz_exists_binning_E2_bound` = body genuine sorry-free + 独立監査 PASS** (r7 `d1f2445a`)、**Leg E-mass `wz_source_codeword_sideInfo_mass_le` = sorry-free + sorryAx-free + `@audit:ok`** (`66417846` closure / `259ecccc` 監査 PASS、`#print axioms` = `[propext, Classical.choice, Quot.sound]`) ゆえ **A3 sub-tree は sorryAx-free**。残 literal sorry **2 = Leg F の C2 covering-acceptance のみ** (両 `@residual(plan:wz-binning-covering)`): (i) leaf `wz_covering_chosenWord_sideInfo_typical` (Achievability.lean ~L1001/L1022、honest tier-2、独立監査 OK `5d3ecd81`/`d2e68b10`)、(ii) covering atom `wz_coveringFamily_of_testChannel` (leaf を consume 未配線ゆえ自身の sorry 残置)。**C2 = 相関-joint acceptance concentration = Markov Lemma、gateway-atom-first probe = STALL → ~100-300行 from-scratch self-build (multi-leg、`plan`-class、Mathlib wall でも 1-2 dispatch port でもない)**。旧「S5a/gateway-2 ~40行 Fubini」は mis-mechanization (判断ログ #8)。**次 (mainline) = leaf `wz_covering_chosenWord_sideInfo_typical` を build (相関-joint Markov Lemma concentration) → covering atom を consume 配線 (covering-success 分解 + joint derandomize `exists_codebook_low_avg`) + 当該 atom の stale docstring 訂正**。両 sorry closing で `#print axioms wyner_ziv_achievability` sorryAx-free。**A2 `wz_ideal_expectation_eq_covering` `@audit:ok`。Leg 0-E DONE**。
...
- [ ] Leg F — covering atom C2 = **Markov Lemma leg** (mainline、~100-300行、multi-leg self-build)。**leaf `wz_covering_chosenWord_sideInfo_typical` (Achievability.lean ~L1001/L1022) に isolate 済 + 独立監査 OK** (`5d3ecd81` isolate / `d2e68b10` 監査 PASS、`sorry`+`@residual(plan:wz-binning-covering)`、honest tier-2)。gateway-atom-first probe = **STALL** ゆえ tractable だが from-scratch (Mathlib wall でない: KL-concentration primitives + `conditionalStronglyTypicalSlice_mass_ge` (Mass.lean:1274、下界・独立-product ingredient) は在るが相関/条件-product Markov-lemma ASSEMBLY が未構築)。leaf を build → covering atom `wz_coveringFamily_of_testChannel` を consume 配線 (covering-success 分解 `{(w,y) atyp} ⊆ {cover fails} ∪ {w typ-x ∧ (w,y) atyp}` + joint derandomize `exists_codebook_low_avg`) + 当該 atom の stale docstring 訂正 📋
...
**ripple**: Leg C.6 と同じ file-contained 5-decl chain (D3:3172 / D `wz_perDelta_covering_binning_eventual` / S6 `wz_perDelta_covering_binning` / `wz_perDelta_codes_exist` / `wz_coveringFamily_of_testChannel:962`) + covering atom `wz_covering_lossyCode_exists:711`。cross-file consumer 0 (機械確認済 settled-facts)。**effort 再見積り (Leg E-mass/A3 DONE 後)**: 残 = covering atom C2 = Markov Lemma leg F ~100-300行/multi-leg (from-scratch self-build、gateway-atom-first probe = STALL) + covering atom への consume 配線 (covering-success 分解 + joint-derandomize) ~1-2 dispatch。C2 は `plan` atom (wall でない、in-project 未実装 ASSEMBLY)。
...
- **署名 rework (pinned-ε) DONE** ✅ (`cf7d57cd`/`42abbf21`、独立監査 all-PASS): `wz_coveringFamily_of_testChannel` + A3 の acceptance conjunct を pinned-ε 形に書換 (ε を `∀ ε>0` binder 化、mass を単一 ε に pin) + A3 に `hd'_link : ∀ x' g, dα' x' g = d x'.1 g` 追加、D3 で `ε := gap/6` (gap = R−(R₁−I(Y;U))) を選び `hε_conf`/`hd'_link` を linarith/rfl discharge、チェーン (covering atom / S6 / D / D3、codes_exist は inference 自動) を threading。type-check done、A2 `@audit:ok`。G2/A1/A2 は不変 (再消費)。**残 = body fill** (下 step 4-5)。
  - **body fill (r7 で再分類、判断ログ #8)**: **A3 DONE** (`d1f2445a`) = E2b/S5b と C2/hcov_accept premise を union bound、`hε_conf` が confusion 指数 `R₁−I(Y;U)+3ε−R < 0` を保証。E2b の per-codeword mass は Leg E-mass `wz_source_codeword_sideInfo_mass_le` (sorry-free `@audit:ok`) が供給 — **訂正**: この exponent bridge は `wzMutualInfoYU_eq_mutualInfo` を使わず direct pmf-level `mutualInfoPmf` 計算 (`wz_mutualInfoPmf_wzMarginalYU_eq`) を self-build (`q'` が固定 factored pmf で empirical-pmf 前提が成り立たないため、監査で逸脱 sound 確認)。**covering atom C2 = leg F (Markov Lemma、~40行 Fubini は誤り)** = leaf `wz_covering_chosenWord_sideInfo_typical` に isolate 済 → `conditionalStronglyTypicalSlice_mass_ge` 上に相関-joint concentration を self-build。
...
### Leg F — C2 covering-acceptance Markov Lemma (mainline、active)

**proof-log**: yes (from-scratch concentration、再開根拠に必須)

**現状**: A3/E2b 側は closed (A3 body genuine + Leg E-mass sorry-free `@audit:ok`)。残 sorry 2 = **C2 covering-acceptance のみ** (leaf `wz_covering_chosenWord_sideInfo_typical` ~L1001/L1022、独立監査 OK `5d3ecd81`/`d2e68b10`; covering atom `wz_coveringFamily_of_testChannel` は leaf 未配線ゆえ自身の sorry を残置)。両 `@residual(plan:wz-binning-covering)`。

**verdict = tractable multi-leg from-scratch self-build (NOT wall, NOT 1-2 dispatch port)**: gateway-atom-first probe = **STALL**。KL-concentration primitives + `conditionalStronglyTypicalSlice_mass_ge` (Mass.lean:1274、下界・独立-product ingredient) は在るが、相関/条件-product 上の Markov-lemma **ASSEMBLY** が in-project 未構築 ゆえ ~100-300行 の from-scratch concentration self-build。Mathlib wall ではない (operational typicality は全て in-project、`plan`-class)。

**3 mechanism 訂正 (r7 pivot-advisor + 独立監査)**:

1. **r7「Markov Lemma 要」は独立 proof-pivot 再確認を SURVIVED**: gateway-2 / S5a は C2 を bound しない (方向逆 + 独立-product-not-correlated-joint + fixed-not-selected)。union-bound-over-codewords の escape は `M₁≈exp(nR₁)` が covering 指数を相殺して発散 → selection-conditioned Markov 解析が不可避。
2. **advisor 提案 leaf shape は FALSE-as-framed で OVERRIDDEN (key durable lesson)**: 提案形は **固定 word `u` を独立-product Y-law `Measure.pi (μ.map (Ys 0))`** の下に置いた。だが C2 の ACTUAL measure (verbatim: Achievability.lean ~L1057-1060 + A3 consumer ~L3818-3822) は **相関-joint source `Measure.pi (pmfToMeasure (fun (x',y) ↦ P_XY.real {(x'.1,y)}))`** で、covering word `u = c.decoder (c.encoder x)` は **source x の関数**。`u` を独立に固定すると x–y (ゆえ u–y) 相関が壊れ、`I(U;Y)>0` で atypicality mass → 1 (退化 `I(U;Y)=0` でのみ生存)。正しい leaf = **coupled 相関-joint-source 形** (chosen-word-of-x vs y)、implementer が述べ auditor が退化境界 check で TRUE-as-framed 確認。**教訓**: residual leaf を isolate する前に、消費 site の ACTUAL measure/coupling を verbatim 確認せよ — 独立-product 簡約が leaf を silently 偽にしうる。
3. **BC テンプレは transfer しない**: `bc_conditional_slice_prob_le` (BroadcastChannel/Achievability.lean:568) は **CONFUSION (gateway-1) sibling** で acceptance ではない。「BC を acceptance に port」は不可。acceptance 方向は 条件 product `∏ P_{Y|X=xᵢ}` 上の from-scratch Markov-lemma concentration + variance/Chebyshev。

**2 flag (non-blocking、次に covering atom を touch する際に処理)**:

- **stale docstring**: covering atom `wz_coveringFamily_of_testChannel` CODE docstring (~L1006-1020、pre-r7) が旧「S5a/gateway-2 Fubini derandomize」機構を記述 = STALE。leaf consume 配線時に訂正 (code がタグの SoT、本 plan からは fix せず flag のみ)。
- **factor-2 tolerance mismatch**: A3 の `hcov_accept` は `≤ δ/2/(8·(distortionMax d+1))` を要求するが covering atom は `≤ δ/(8·(distortionMax d+1))` を export = harmless (両者 C2→0 由来、「任意固定正 tol に eventually ≤」leaf が discharge、D3 が同 ε を threading)。

**撤退**: `sorry + @residual(plan:wz-binning-covering)` (leaf / covering atom 各)。
...
8. **Leg F = C2 covering-acceptance Markov Lemma mainline (r7 pivot-advisor 2026-07-12 → r7 独立再確認 SURVIVED、active、本線)**: 署名 honest tier-2 化後の body fill で判明 = **A3/E2b 側と C2 側は非対称**。**A3/E2b 側 CLOSED**: A3 `wz_exists_binning_E2_bound` body genuine (`d1f2445a`) + Leg E-mass `wz_source_codeword_sideInfo_mass_le` sorry-free `@audit:ok` (`66417846`/`259ecccc`) ゆえ A3 sub-tree sorryAx-free (訂正: exponent bridge は `wzMutualInfoYU_eq_mutualInfo` を使わず direct pmf-level `mutualInfoPmf` を self-build)。**C2 側 = 真の blocker = Markov Lemma** (Mathlib+codebase 双方に不在、`conditionalStronglyTypicalSlice_mass_ge:1274` は下界・独立 ingredient 止まり)、旧「S5a/gateway-2 ~40行 Fubini」は mis-mechanization。leaf `wz_covering_chosenWord_sideInfo_typical` に isolate + 独立監査 OK (`5d3ecd81`/`d2e68b10`)。**gateway-atom-first probe = STALL → from-scratch ~100-300行 multi-leg self-build (wall でない、`plan`-class)**。**3 mechanism 訂正**: (a) r7 Markov 判定は独立 proof-pivot 再確認 SURVIVED (union-bound-over-codewords escape は `M₁≈exp(nR₁)` が covering 指数を相殺し発散)、(b) advisor 提案の固定-word 独立-product leaf `Measure.pi (μ.map (Ys 0))` は FALSE-as-framed → 正しくは coupled 相関-joint-source 形 (chosen-word-of-x vs y、`u=c.decoder(c.encoder x)` は source x の関数、独立固定で x–y 相関崩壊 ⟹ atypicality mass→1)、(c) BC `bc_conditional_slice_prob_le` は confusion-sibling で acceptance に transfer せず。**mainline (B+D)**: leaf を build (相関-joint Markov concentration) → covering atom `wz_coveringFamily_of_testChannel` を consume 配線 (covering-success 分解 + joint-derandomize `exists_codebook_low_avg`) + 当該 atom の stale docstring 訂正。両 sorry closing で headline sorryAx-free。**教訓 (durable、#6 family 第3実例、親 #12 が ledger)**: residual leaf を isolate する前に消費 site の ACTUAL measure/coupling を verbatim 確認 — 独立-product 簡約が leaf を silently 偽にしうる、event 名の一致を mechanism の一致と取り違えるな。
