...
**Status**: DONE ✅ (Leg 15) — **WZ achievability FULLY CLOSED**: headline `wyner_ziv_achievability` sorryAx-free (`#print axioms` = `[propext,Classical.choice,Quot.sound]`) + 独立監査 PASS。C2 covering-acceptance chain は weak typicality で FALSE-AS-FRAMED だったが (`1ddc2887`、label-swap 反例、判断ログ #8)、strong-Ecov build (radius separation `ε_cov=ε/(2(1+C))`) で全閉: Markov-core chain (kernel/core/outer/inner/leaf) が sorryAx-free + `@audit:ok` (孫 plan Atom E-F、Leg 13、`d8954711`/`b489d51f`/`e4490dbb`) → covering atom `wz_coveringFamily_of_testChannel` を joint-derandomize で閉 (孫 Atom G、r14 `3a31e08c`+`9f5c7afc`、署名不変) → Atom H docs/wiring closure (r15 `173ccf67`+`3248bbc3`)。A3/E2b 側も CLOSED (Leg 0-E DONE、A3 sub-tree sorryAx-free、A2/mass `@audit:ok`)。詳細 → 孫 plan [`wz-markov-core-plan.md`](wz-markov-core-plan.md) STATUS / §Leg F / `wz-facts.md`。
...
- [x] Leg F — C2 covering-acceptance CLOSED ✅: leaf `wz_covering_chosenWord_sideInfo_typical` を genuine outer decomposition (`9ecffb41`) + inner `wz_covering_markov_concentration` を strong-Ecov build で閉 (孫 plan Atom E-F、`d8954711`/`b489d51f`/`e4490dbb`、conditional-AEP kernel `wz_covering_uyBand_condSlice_le` genuine 閉、独立監査 PASS `@audit:ok`)。covering atom `wz_coveringFamily_of_testChannel` は joint-derandomize (`exists_codebook_low_avg` 系) で consume 配線 (孫 Atom G、`3a31e08c`)。**headline `wyner_ziv_achievability` sorryAx-free**
