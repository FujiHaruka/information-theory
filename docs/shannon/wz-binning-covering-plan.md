...
- **first move (rework、pinned-ε)**: 署名 ripple `b5b22385` は landed だが free-∃ε + dα'-d で DEFECT (判断ログ #7) → `wz_coveringFamily_of_testChannel` + A3 の acceptance conjunct を **pinned-ε 形**に書換 (半径を A3 decoder と同一 ε に固定、mass bound は gateway-2 供給) + A3 に **dα'-d reconciliation precondition** 追加 → skeleton 型チェック → hcov₁ threading (D3 で ε 一致 + dα'-link を discharge) → L1006 Fubini bridge fill → A3 fill (E2b/S5b + C2/hcov_accept を union)。G2/A1/A2 は不変 (再消費)。新 sorry/署名変更は独立 honesty-auditor 1 pass、A2+A3 の comprehensive 監査を Leg E closure 時に実施。
