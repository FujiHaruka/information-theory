...
**Status**: ACTIVE 🚧 — **Leg E 署名 rework DONE + 独立監査 all-PASS** (`cf7d57cd`/`42abbf21`): pinned-ε 形で **第4軸 (covering-acceptance C2) + 第5軸 (dα'-d scaling) を署名レベルで closure**、`@audit:defect(false-statement)` 除去 → honest tier-2 復帰。ε を rate gap から D3 で選ぶ共有 explicit param 化 (`hε_conf : R₁-I(Y;U)+3ε < R`、huge-ε regime を unsatisfiable で排除)、acceptance mass を単一 ε に pin (free-∃ε 撤廃)、`hd'_link : ∀ x' g, dα' x' g = d x'.1 g` を rfl discharge。退化 binder table 全 BLOCKED (ε 巨大→hε_conf 偽 / dα'≫d→hd'_link 偽 / M 膨張→hM_ub 偽)。**A2 `wz_ideal_expectation_eq_covering` `@audit:ok`** (sorryAx-free、本 pass で comprehensive 監査済)。**残 literal sorry 2 = 解析 body fill のみ** (署名は honest): (i) covering atom `wz_coveringFamily_of_testChannel` (L1024) の acceptance mass (S5a/gateway-2 Fubini derandomize)、(ii) A3 `wz_exists_binning_E2_bound` (L3138) の E2b confusion 指数→0 (D2/S5b)。両方 `@residual(plan:wz-binning-covering)`、新 wall でなく ~40行 plumbing。**次の本線 = body fill leg** (下記 Leg E Approach step 4-5)。**Leg C.6/C.5/D G2/A1/A2 + Leg 0/A/B/C DONE**。
...
- [ ] Leg E body fill (**active 本線**) — covering atom acceptance Fubini (S5a/gateway-2) + A3 E2b confusion 指数→0 (D2/S5b) の 2 residual sorry を closure 📋
...
- **署名 rework (pinned-ε) DONE** ✅ (`cf7d57cd`/`42abbf21`、独立監査 all-PASS): `wz_coveringFamily_of_testChannel` + A3 の acceptance conjunct を pinned-ε 形に書換 (ε を `∀ ε>0` binder 化、mass を単一 ε に pin) + A3 に `hd'_link : ∀ x' g, dα' x' g = d x'.1 g` 追加、D3 で `ε := gap/6` (gap = R−(R₁−I(Y;U))) を選び `hε_conf`/`hd'_link` を linarith/rfl discharge、チェーン (covering atom / S6 / D / D3、codes_exist は inference 自動) を threading。type-check done、A2 `@audit:ok`。G2/A1/A2 は不変 (再消費)。**残 = body fill** (下 step 4-5)。
  - **body fill (active、Task #4)**: (4) covering atom acceptance Fubini bridge (`SupportingBounds.lean:461-488` テンプレ、gateway-2 供給、~40行) + (5) A3 body で E2b/S5b (D2 `hI_YU`+`wzMutualInfoYU_eq_mutualInfo` bridge) と C2/hcov_accept を union bound。`hε_conf` が confusion 指数 `R₁−I(Y;U)+3ε−R < 0` を保証。
