...
> **Sub-plan**: [`wz-markov-core-plan.md`](wz-markov-core-plan.md) — conditional-AEP atom 分解。孫 plan が atom 状態 + 現在のフォークの SoT。
>
> **⚠ 2026-07-12 (Leg 12) — weak-typicality Approach INVALIDATED、user-decision フォークで PAUSED**: Atom C (mean-identity)/A (finite-Fubini)/B-engine (conditional-Chebyshev) は sorry-free (`ef34494a`/`95a07fa4`/`1b5be107`/`469ae6f2`) だが、**core `wz_covering_jointBand_markov_core` が weak typicality で FALSE-AS-FRAMED** と判明 (advisor + 独立 auditor が label-swap 反例で confirm、`1ddc2887` で core/outer/inner/leaf に `@audit:defect(false-statement)`)。前回「PASS tier-2 sufficiency」を overturn。**C2 covering chain 全体が weak-`Ecov` 依存ゆえ伝播**。修正 = **Proposal A** (`Ecov` のみ strong joint typicality に強化、covering-success 下界 reopening) = 戦略的 def 変更ゆえ**ユーザー判断待ち** (孫 plan §Proposal A / `wz-facts.md`)。
