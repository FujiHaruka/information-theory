...
**Status**: ACTIVE 🚧 — 署名は全軸 honest (第1-5軸 closure、独立監査 all-PASS `cf7d57cd`/`42abbf21`)。**残 literal sorry 2 の性質が r7 pivot-advisor で再分類** (第6軸、下記 判断ログ #8): 2 sorry は同一 blocker でなく非対称。**A3 `wz_exists_binning_E2_bound` = body genuine (sorry-free) + 独立監査 PASS** (r7 `d1f2445a`): C2 を `hcov_accept` premise で取り E2b を binning derandomize+S5b で bound、δ/4 squeeze。副産物 = S5b abstract-`jts` generalize (`@audit:ok` 再確認) + per-codeword mass を新 atom `wz_source_codeword_sideInfo_mass_le` (L3081、honest_residual、~120-180行 self-build = Leg E-mass) に isolate。**covering atom `wz_coveringFamily_of_testChannel` L1024 の C2 conjunct = 真の blocker、MARKOV LEMMA を要し Mathlib+codebase 双方に genuinely 不在** (Leg F)。旧計画の「S5a+gateway-2、~40行 Fubini、wall-risk LOW」は **mis-mechanization** (S5a/gateway-2 は covering/独立-product 側、C2 は相関-joint acceptance の concentration = 別 measure・別 functional、event-label 混同 #6 の第3実例)。C2 closure = `conditionalStronglyTypicalSlice_mass_ge` (下界 ingredient) 上に variance/Chebyshev で組む in-project Markov Lemma (~100-300行、別 leg)。**A2 `wz_ideal_expectation_eq_covering` `@audit:ok`**。両 sorry `@residual(plan:wz-binning-covering)`。**次 = A3 fill + C2 を named Markov sub-lemma に isolate**。**Leg C.6/C.5/D G2/A1/A2 + Leg 0/A/B/C DONE**。
...
- [ ] Leg E-mass — `wz_source_codeword_sideInfo_mass_le` (L3081、`sorry`+`@residual(plan:…)`、監査 honest_residual = 非laundering) を self-build (~120-180行): D2 `wz_covering_codeword_sideInfo_mass_le`(`@audit:ok`) を source→ambient に side-info-marginal agreement (n-fold Y-law) + entropy→pmf-MI bridge で transport。A3 が S5b の `hmass` に消費 📋
