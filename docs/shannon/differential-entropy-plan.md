...
> 実態整合 (2026-05-20): DONE-HONEST-HYPS — `InformationTheory/Shannon/DifferentialEntropy.lean` (0 sorry, 0 vacuous)。
...
### 既存 InformationTheory 資産
- `InformationTheory/Shannon/Entropy.lean` — discrete entropy。**`Real.negMulLog`-shape を採用済み**。
  Phase A の continuous 版を同 shape で書くと自然に並立。
- `InformationTheory/Shannon/Bridge.lean` — `entropy μ X := ∑ x, Real.negMulLog ((μ.map X).real {x})`。
  本 plan は **新規 file** `InformationTheory/Shannon/DifferentialEntropy.lean` に切り出し、Bridge.lean
  は touch せず。
- `InformationTheory/Shannon/MaxEntropy.lean` — discrete max-entropy (`entropy μ X ≤ log |α|`, B-6)。
  Phase D の continuous 版が直接対応 (`differentialEntropy μ ≤ (1/2) log (2πe σ²)`)。
- `InformationTheory/Shannon/CsiszarProjection.lean` — `klDivPmf` Pythagorean (E-6)。本 plan は
  `Measure ℝ` ambient なので **`klDivPmf` は使わず、`InformationTheory.klDiv` 直接**。

## Phase A — `differentialEntropy` 定義 + 基本可積分性 📋

新規 file: `InformationTheory/Shannon/DifferentialEntropy.lean`。
...
1. **`Real.negMulLog`-shape 採用** (定義 Phase A):
   ...
   既存 `InformationTheory/Shannon/Bridge.lean` の discrete `entropy` も同 shape (`∑ x, Real.negMulLog (...)`)、
