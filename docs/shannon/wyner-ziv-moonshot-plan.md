...
**Status**: ACTIVE 🚧 — **operational main attack in progress**。Wyner–Ziv main theorem (Cover–Thomas Thm 15.9.1 achievability + converse) の operational code ↔ R_WZ(D) closure を子サブ計画 [`wyner-ziv-main-plan.md`](wyner-ziv-main-plan.md) で攻める。**converse single-letterisation core CLOSED (leg 8) + endpoint own-body/L1/L2/L3 すべて CLOSED sorry-free (leg 9-10、独立監査 PASS、core 3 decl は `@audit:ok` 移行)**。残 converse own-sorry は **1 本 = kernel support-reduction core `wz_support_reduce` (`Converse.lean:2041`、slug `wz-auxiliary-cardinality-bound`、K=|α|+2)** — L1 `wynerZivRate_eq_factorizable_finK` は genuine `sInf` 等式で CLOSED、residual をこの新 core lemma に relocate + narrowing。**Goal の残り = `wz_support_reduce` closure (crux = entropy-mixture identity、geometry は 0-wall) + achievability P3 (binning+covering、未着手)**。次 leg 群は `wz_support_reduce` か P3 を選ぶ。真ルートは `Uᵢ=(J,Y_{\i})` + conditional-MI chain (Csiszár `csiszar_sum_identity_hetero` は本ルート orphaned、真 gateway = `iIndepFun → Markov Uᵢ−Xᵢ−Yᵢ`、子計画 判断ログ #2)。**情報側 R_WZ(D) は完成済 (`InformationTheory/Shannon/WynerZiv/` 5 file、0 sorry、下記保存 record)**。
