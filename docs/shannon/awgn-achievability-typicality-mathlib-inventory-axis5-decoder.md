...
> 5 軸全体は plan §「Phase 0 — Mathlib + InformationTheory API 在庫」を参照。
>
...
既存の `AwgnCode` 構造 (`InformationTheory/Shannon/AWGN.lean:97`) が
...
```lean
-- InformationTheory/Shannon/AWGN.lean:97-102
...
#### 既存 `InformationTheory/Shannon/ChannelCoding.lean:204` `errorProbAt` の sigature
...
- `InformationTheory/Shannon/AWGN.lean:73-79`:
  ```lean
...
`InformationTheory/Shannon/AWGNAchievabilityDischarge.lean` の Phase C 部 ~30 行 skeleton:
