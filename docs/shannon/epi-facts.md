...
| 無条件 dispatch headline (`entropyPowerExt_add_ge_dispatch_skeleton`) は **sorryAx-free (全 4 枝 proof-done)**。`wall:epi-finite-entropy-ac-classical` は**解消済** (case-1 `entropyPowerExt_add_ge_finite_ac` が有限分散=smoothing / 無限分散=route T で両枝 closure) | machine | `#print axioms entropyPowerExt_add_ge_dispatch_skeleton` = `[propext, Classical.choice, Quot.sound]`、`rg '@residual\(wall:epi-finite-entropy-ac-classical\)' InformationTheory/` = Found 0 | 9163283 | 2026-06-08 機械確認で旧「sorryAx 残」記述が STALE と判明し訂正。この旧 skeleton は 21 precondition (16 integrability for case2/2symm + 5 finite-entropy) を持つが、**Phase 5 endgame で完全無条件版 `entropyPowerExt_add_ge` (precondition 0、下行) が別建てされ supersede 済**。旧 skeleton は proof-done の consumer 0 leaf として残置 |
...
| route β' (W-Y2) gateway ⊤ 枝 `differentialEntropyExt_top_of_indep_add` (`h(W)=⊤ ⟹ h(W+V)=⊤`、a.c.+独立) が sorryAx-free **かつ (i-a) を非継承** | machine | `#print axioms differentialEntropyExt_top_of_indep_add` = `[propext, Classical.choice, Quot.sound]` (経由 `TruncationLimit/Limit.lean`) | 803e489 | truncation 近似が無条件版②を genuine 迂回。独立 honesty-auditor all-OK (under-hyp 反例試行 PASS、`_unconditional` 命名 NOT name-laundering)。ファイル全体 0 sorry/0 @residual。**2026-06-11 dead-cleanup**: 旧 route α-ii の (i-a) `differentialEntropyExt_indep_add_eq_add_klDiv` (+ `Monotone.lean` 4 decl / `CondEntropyExt.lean` 5 decl の consumer-0 dead 島) を物理削除済 — route β' が完全 supersede したため旧 sorryAx 対比は moot |
| **full 無条件 gateway** `entropyPowerExt_mono_add` (`W a.c. ∧ W⊥V ⟹ N(W+V)≥N(W)`) + `differentialEntropyExt_mono_add` (`h(W)≤h(W+V)`) が sorryAx-free **かつ (i-a) 非継承** | machine | `#print axioms entropyPowerExt_mono_add` / `differentialEntropyExt_mono_add` = `[propext, Classical.choice, Quot.sound]` (経由 `EPIUncondTruncationLimit.lean`) | 9163283 | 3 枝 assembly (⊥=bot_le / ⊤=route β' / 有限=un-truncated per-fibre Gibbs `differentialEntropyExt_mono_add_of_integrable`)。有限枝 bridge `differentialEntropyExt_integrable_of_finite` (有限 h+a.c.⟹integrable) 新規。3 補題とも独立 honesty-auditor all-OK (`@audit:ok`)。**S5 = 方針 Y gateway 完全 proof-done** |
| **完全無条件 EPI dispatch** `entropyPowerExt_add_ge` (`hX hY : Measurable` + `hXY : IndepFun X Y P` **のみ**、precondition 0) が sorryAx-free。**Phase 5 endgame 完了** = gateway で singular cases (case2/2symm) を 0 仮説化 + case-1 を h の ⊤/⊥/有限で split し有限枝 bridge で integrability 供給 | machine | `#print axioms entropyPowerExt_add_ge` = `[propext, Classical.choice, Quot.sound]` (経由 `EPIUncondDispatchFull.lean`) | f767105 | 新 file `EPIUncondDispatchFull.lean`、4 枝 dispatch (両 a.c.=case-1 split / 混合=gateway / 両特異=RHS=0)。5 declaration 独立 honesty-auditor all-OK (`@audit:ok`、headline name-laundering NOT + case-3 非退化 + 各枝 genuine delegation)。旧 21-precondition `entropyPowerExt_add_ge_dispatch_skeleton` (proof-done、consumer 0 leaf) を supersede。**真の無条件 headline は ℝ≥0∞ 版のみ** (実数版は `EReal.exp_coe` 変換に a.c.+有限を要し `entropy_power_inequality_of_ac` が honest 限界) |
