...
> `RateDistortionConverseNLetter.lean` で **genuine proof done** (`rate-distortion-converse-plan.md`
> E-4-C 側)、本 plan の binary 凸性を n-way Jensen に持ち上げて消費。
...
- [x] Phase C — n-letter 規定歪み形 converse 主定理 ✅ — `rate-distortion-converse-plan.md` E-4-C 側で
      genuine closure 済 (`rate_distortion_converse_n_letter_singleLetter`、本 plan の binary 凸性を
      n-way Jensen に持ち上げて消費)
