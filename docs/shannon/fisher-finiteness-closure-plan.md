# Shannon EPI: fisher-finiteness 壁 closure 計画
...
## 要点
- fisher-finiteness 壁は当時コード側 SoT の生きた slug (`FisherConvBound.lean` の `gaussianConv_fisher_le_inv_var` 集約点 + per-time line consumer) だったが、legacy Stam route 物理削除 (ホスト `FisherConvBound.lean` ごと消滅) で code から消滅 = CLOSED。
