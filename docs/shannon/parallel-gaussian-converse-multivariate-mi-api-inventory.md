...
> ✅ **SUPERSEDED (2026-05-29 closure)**: 本 inventory の対象 #5
> `parallelOutput_joint_logDensity_integrable` は **genuine closure 済**
> (`InformationTheory/Shannon/ParallelGaussian/Converse/MixtureDensity.lean:408`、多変量 mixture
> density lift、`@audit:ok`)。本 inventory の「multivariate-mi 壁 分類は overstated、自作可能」予測は
> 的中した (roadmap judgment #13)。**以下は当時の research snapshot** — ファイル参照は ParallelGaussian
> ディレクトリ再編前の flat 名を現構造に更新済 (本文の `sorry` 付き #5 コードは当時の未閉形)。
>
> Scope (当時): close the last residual `parallelOutput_joint_logDensity_integrable`
> (`InformationTheory/Shannon/ParallelGaussian/Converse/MixtureDensity.lean:408`、旧 multivariate-mi 壁 residual).
...
最重要発見: **`multivariate-mi 壁` 分類は overstated（tier-4 寄り defect 候補）**。
...
`InformationTheory/Shannon/ParallelGaussian/Converse/MixtureDensity.lean:408` (当時は flat `ParallelGaussianConverse.lean:1093`、現在は genuine closure 済で `sorry` 不在):

```lean
-- 当時の未閉形 (現在は genuine、下記は research snapshot):
...
（`ParallelGaussian/Basic.lean:94`）、入力 `p : Measure (Fin n → ℝ)` は **相関あり**（積測度でない）、
ノイズは座標独立 = 積 Gaussian。
...
|---|---|---|---|---|
| 座標ごと second-moment integrability + Bochner 和 | `parallelGaussianPowerConstraintSet_mem_iff_integrable` | `InformationTheory/Shannon/ParallelGaussian/Basic.lean:179` | ✅ 自作既存・sorryAx-free | 各座標 `∫(xᵢ)² ≤ P` → 座標箱 Chebyshev の入力 |
| constraint set 定義（lintegral 形） | `parallelGaussianPowerConstraintSet` | `ParallelGaussian/Basic.lean:169` | ✅ 自作既存 | `∑ᵢ ∫⁻ ofReal((xᵢ)²) ≤ ofReal P` |
| marginal の constraint 継承 | `parallelMarginal_mem_awgnPowerConstraintSet` | `ParallelGaussian/Converse/Regularity.lean:141` | ✅ 自作既存 | 座標箱 Chebyshev で各座標を 1-D に落とす場合に再利用可 |
| marginal = conv | `parallelOutput_marginal_eq_conv` | `ParallelGaussian/Converse/Regularity.lean:50` | ✅ 自作既存・sorryAx-free | joint conv 等式の建材 |

**完全 signature（建材）:**
```lean
-- ParallelGaussian/Basic.lean:179
...
- むしろ **逆方向の修正提案**: コード docstring（当時 `ParallelGaussianConverse.lean:1059-1091`、現 `ParallelGaussian/Converse/MixtureDensity.lean` 周辺）の
  `multivariate-mi 壁` 分類は **overstated**。「principled-impossible factorization」は
  ...
  **本 inventory は分類の再評価材料を提示するのみ**（docstring 書換は実装/監査 task の責務）。
  推奨: honesty-auditor に「`multivariate-mi 壁` vs `plan:parallel-gaussian-converse-closure`
  の再判定」を回す。
