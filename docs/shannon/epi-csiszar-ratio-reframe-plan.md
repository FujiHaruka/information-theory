...
- **load-bearing と regularity precondition の線引き**: core 不等式 `1/J_sum ≥ 1/J_X+1/J_Y` は producer 側 (`stamCauchySchwarzOptimal_of_indepFun`) が genuine に持つ。consumer は ∀-量化 producer Prop を apply 入力 (IndepFun / convolution 同定 / IsBlachmanConvReady = regularity) で受けるのは OK、core 不等式そのものを hyp 化する (案 C) のは禁止。
