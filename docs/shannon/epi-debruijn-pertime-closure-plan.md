...
## 要点 (≤5 行)
- slug `@residual(plan:epi-debruijn-pertime-closure)` は撤退口記録。共有壁 fisher-finiteness (convolution Fisher bound) は legacy Stam route 物理削除 (2026-06-11、ホスト `FisherConvBound.lean` ごと消滅) で code から消滅 = CLOSED。
