...
- **line 番号が一部 stale**。実 file:line で確定した正値:
  ...
  - `stamCauchySchwarzOptimal_of_indepFun` = **`EPIStamInequalityBody.lean:359`** (ブリーフは「:359」と書きつつ別所で Step12Body と混同していたが、実体は **InequalityBody** にある)
...
|---|---|---|---|---|
| `IsStamCauchySchwarzOptimal` | `EPIStamInequalityBody.lean:359` | `stamCauchySchwarzOptimal_of_indepFun` | **生成** (結論=述語) | body `sorry` (:364)。`@audit:defect(false-statement)`。 |
...
| 〃 | `EPIStamInequalityBody.lean:574` | `entropy_power_inequality_via_body` | **消費** (内部生成→消費) | `have h_cs_opt := stamCauchySchwarzOptimal_of_indepFun P X Y hX hY hXY; have h_stam := isStamInequalityHyp_via_body h_cs_opt; exact epi_via_stam_main P X Y X hX hY hXY h_stam h_bridge` (:581-583) — **wall を内部呼出**、load-bearing hyp は持たない。 |
...
| 〃 | `EPIStamStep12Body.lean:278` | `stamCauchySchwarzOptimal_of_scoreConvHyp_of_condExpCSHyp` | **生成** | `stamCauchySchwarzOptimal_of_condExpCSHyp h_cs` (:283) 丸渡し。 |
...
| 〃 | `EPIStamStep12Body.lean:278` | `stamCauchySchwarzOptimal_of_scoreConvHyp_of_condExpCSHyp` | **消費** (`h_cs`) | `stamCauchySchwarzOptimal_of_condExpCSHyp h_cs` (:283)。 |
| 〃 | `EPIStamStep12Body.lean:299` | `isStamInequalityHyp_of_scoreConvHyp_of_condExpCSHyp` | **消費** (`h_cs`) | `isStamInequalityHyp_via_body (stamCauchySchwarzOptimal_of_scoreConvHyp_of_condExpCSHyp h_conv h_cs)` (:304-305)。 |
| 〃 | `EPIStamStep12Body.lean:313` | `isStamCauchySchwarz_of_scoreConvHyp_of_condExpCSHyp` | **消費** (`h_cs`) | `isStamCauchySchwarz_of_condExpCSHyp h_cs` (:318)。 |
...
| `isStamInequalityHyp_of_indepFun` | ✅ | `EPIStamStep3Body.lean:121`。body `isStamInequalityHyp_via_body (stamCauchySchwarzOptimal_of_indepFun P X Y hX hY hXY)` (:125) — wall を内部呼出、`IsStamCauchySchwarzOptimal` を間接消費。 |
| `isStamInequalityHyp_of_primitives` | ✅ | `EPIStamDeBruijnConclusion.lean:169`。body `isStamInequalityHyp_of_indepFun P X Y hX hY hXY` (:174)。 |
...
4. **transitive consumer** (`EPIStamStep3Body.lean:121` `isStamInequalityHyp_of_indepFun`、`EPIStamDeBruijnConclusion.lean:169/203`): wall を内部呼出すだけなので、wall signature に `hconv` precondition が増えれば呼出側で供給。
...
- **wall の再分類**: `stamCauchySchwarzOptimal_of_indepFun` (:359) は現 `@audit:defect(false-statement)`。pivot 後 `@residual(wall:stam-blachman)` (genuine Mathlib wall) に。`stamToEPIBridge_holds` (:223, `@residual(plan:epi-stam-to-conclusion-plan)`) は別 wall で pivot 対象外。
