...
| **L3** | **軸 D の第一手** — multi-letter 表現の定義を確定させ `bcCapacityRegion` との一致の筋を散文で書く。同時に「単一文字特徴付けを持つ」の定義候補を 2〜3 個書き出す | T3 の問いが**曖昧語なしで述べられる**状態になる (Lean の型で書けることは確認するが証明は書かない) |
