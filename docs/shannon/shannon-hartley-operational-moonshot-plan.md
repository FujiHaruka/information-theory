...
**次アクション = 子 plan の Leg D'**（`contAwgnMaxMessages_bddAbove` を **Bessel 単独**で壁非依存 closure）。
**Leg 0（gateway）✅ CLOSED（leg 13）→ Leg P（def-fix）✅ CLOSED（leg 14）**。
**⚠️ `ge` は Leg D' では閉じない**（旧計画の「`le_csSup` で transitive に closure」は def-fix 前の線 = 失効）—
achievability は ≈2WT 次元の構成を要し **Leg E 待ち**。子 plan の「DAG 訂正」節が SoT。
...
- **leg 2（`bddAbove`）= 壁を共有しない** ✓ — Bessel 単独で閉じる（子 Leg D'、`plan:` タグ）。
