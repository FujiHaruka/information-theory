...
**根本原因**: `ContAwgnCode.encoder_power` が窓 `[0,T]` のエネルギーだけを拘束する
（`cause:signature-drops-constraint`。全直線拘束も時間制限も無い）⟹ 超振動符号語による 1 標本コードで任意の `M` が
達成でき `BddAbove` が破れる（古典的な超指向性/超振動パラドックス）。**def-fix を負うのは子 plan
...
**次アクション = 子 plan の Leg 0（gateway）**: 修正後 def で命題が TRUE になることを**誰も検査していない**。
実装が 1 行も着地する前に敵対的検査を通す。settled facts（WSEB=FALSE の解析証明 / 倍精度アーティファクト）→
[`shannon-hartley-facts.md`](shannon-hartley-facts.md)（**散文に再キャッシュしない**）。
...
> **子 plan が 2 つの負債を負う（2026-07-17 leg 12 で更新）**: (1) **def-fix**（`ContAwgnCode.encoder_power` を
> 全直線エネルギー予算へ = Wyner モデル。3 宣言の false-as-framed を救出、コードが子 plan を名指し）、
> (2) **壁核 self-build**（`wall:nyquist-2w-dof` = prolate/LPS の `≈2WT` DOF カウント、Leg E）。
> **build order = Leg 0（gateway、実装を block）→ Leg P（def-fix）→ Leg D'（BddAbove 壁非依存）→ Leg E（壁）**。
...
- **proof-done 条件（= 目標）**: `≥` を fully genuine 証明（synthesis bridge + Parseval 電力橋 + BddAbove
  ...
  **leg 2 + leg 3 = 現行 def では false-as-framed** ⟹ 子 Leg 0 → Leg P（def-fix）→ Leg D' が前提。
...
**子 plan の Leg 0（gateway）→ Leg P（def-fix、Proposal A = 全直線エネルギー予算 / FAIL 時 Proposal C =
Landau-Pollak）→ Leg D'（BddAbove 壁非依存、~150–250 行）**。leg 3 は `le_csSup` 経由で transitive に closure。
旧選択肢 (b)「容量 def を achievable-rate 形へ再設計」は **park**（def-fix が成れば不要。子 plan Proposal C と別軸）。
