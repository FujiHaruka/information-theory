...
- 🔑 **旧壁論拠「無限次元スペクトル理論が Mathlib に無い」は誤り**（grep で機械反証）:
  ...
  台帳 **§SPECTRAL-ASSETS** が SoT。**leg 15 で E-trace も着地** — 厳密 trace 等式は Parseval で closure し、**スペクトル定理すら不要**だった（二重の誤り）。⟹ **壁の残渣 = 第 2 モーメント `tr A − tr A² = O(log WT)`（Landau-Widom）ただ 1 点**。行数見積 ~630–1150（未機械検証）。
