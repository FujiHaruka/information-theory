...
- [~] Phase 2 — prolate-DOF スペクトル理論（`timeBandLimitingOp` + 固有値集中）🔄 **[user 指示で全理論自前構築（leg 9+）: Leg A✅ → Leg B コンパクト性 = 4 leaf 中 3 genuine（kernel L² 6104d26b / generic HS e619b06c、監査 PASS）、残 sorry は Leaf 2 sinc 畳込橋のみ → Leg C 列挙 → WSEB → Leg E LPS count]** → [shannon-hartley-phase2-spectral-plan.md](shannon-hartley-phase2-spectral-plan.md)
