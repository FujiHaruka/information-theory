...
**🎉🎉 leg 17 = 壁が名指していた命題そのものが CLOSED。leg 18 = R4-gate A1 も gateway PASS（壁でない・plumbing 確定）。次 = R4-ACH-B（実基底を閉じる）。**
