...
- [~] Phase 4 — converse（`contAwgn_le_shannonHartley`、Phase 2 消費）🔄 **[C1 interlacing = CLOSED（leg 23）。残 = C0 headline / C2 Gauss 回転 / C3 operational parallel-Gaussian converse（最大 self-build）/ C4 water-filling+極限、全 project-internal・壁なし]**
...
**次アクション = C3（operational parallel-Gaussian converse、最大の残 self-build）**: `parallel_per_input_mi_le_sum`（MI 側、
sorryAx-free）+ Fano/DPI 連鎖（`awgn_converse` の equal-gain を unequal-gain へ移植 = self-build）で `log M → MI`。
その後 C2（Gauss 回転、Mathlib 在）+ C4（water-filling+極限、初等）→ Phase 4 → Phase 5-full（`le_antisymm`）。詳細 → 子 plan「R4-CONV」節が SoT。
