...
**Status**: 🧨 **mainline は false-as-framed（2026-07-17、leg 12、commit `d2938749`/`67e1ff3f`）**。
`contAwgn_eq_shannonHartley` / `contAwgnMaxMessages_bddAbove` / `contAwgn_ge_shannonHartley` の 3 宣言は
**偽**で、コード側は `@audit:defect(false-statement)` + `@residual(defect:false-statement)` +
`@audit:closed-by-successor(shannon-hartley-phase2-spectral-plan)`。**旧 Status の「true-as-framed な honest
単一 wall-sorry に復帰」は撤回**（2026-07-15 の「leg 2/3 は WALL-GATED」判定も同時に撤回 — 壁 gated ではなく
命題が偽だった）。

**根本原因（2026-07-17 leg 13 の Leg 0 gateway で*入れ替わった*）**: `ContAwgnCode.encoder_power` の窓限定は
`BddAbove` を落とす欠陥ではあるが、**`contAwgn_eq_shannonHartley` の falsity の根本原因ではない**。真の原因は
**観測写像** `sampledSignal` + `ContAwgnCode.errorProbAt`（`cause:signature-drops-constraint` の帰属先も訂正）:
`sampleCount` が自由ゆえ code は Nyquist より粗く標本でき、`√(T/n)` は Nyquist 間隔でのみ等長 ⟹ SH を超過。
**軸ごと違った**（入力クラス vs 観測写像）。**def-fix を負うのは子 plan
[`shannon-hartley-phase2-spectral-plan.md`](shannon-hartley-phase2-spectral-plan.md)**（コードが名指し）。
証拠・機序・数値・教訓 → [`shannon-hartley-facts.md`](shannon-hartley-facts.md) **§OBSERVATION-MAP**。

**`wall:nyquist-2w-dof` は live consumer 0 = DORMANT だが retire しない** — 修正後 def の converse が依然要する。
**Legs A/B/C/C' の作用素資産（`TimeBandLimiting.lean`）は本 refutation の影響を受けない**（有効資産、破棄しない）。

**次アクション = 子 plan の Leg P（def-fix、Proposal O）**。**Leg 0（gateway）は ✅ CLOSED（leg 13）** —
実装ゼロで **Proposal A（`encoder_power` → 全直線）と Proposal C（Landau-Pollak）の両方を反証**し、
**Proposal O（点標本 → `[0,T]` 台の正規直交テスト関数 = Karhunen-Loève 展開）を PASS** させた。
gateway は仕事をした。settled facts → [`shannon-hartley-facts.md`](shannon-hartley-facts.md)
（§OBSERVATION-MAP / §WSEB / §NUMERIC-TRUE-ARTIFACT。**散文に再キャッシュしない**）。
...
- [~] Phase 3 — achievability closure（`contAwgn_ge_shannonHartley`）🧨 **leg 1（synthSignal band-limit/energy）= ✅ audit PASS（commit 89ede2a3/646605c7）**。**leg 2（BddAbove）+ leg 3（assembly）= false-as-framed（2026-07-17 leg 12。2026-07-15 の WALL-GATED 判定は撤回 — 壁でなく偽）**。→ 子 plan の **~~Leg 0（gateway）~~✅ CLOSED → Leg P（def-fix = Proposal O）→ Leg D'** で救出。**修正後は壁非依存で閉じる**（**Bessel 単独** + `awgn_converse`、子 plan Approach。~~`bandlimited_sup_bound` 経由~~ は Proposal O では不要）
- [~] Phase 2 — prolate-DOF スペクトル理論（`timeBandLimitingOp` + 固有値集中）🔄 **[Leg A✅ → Leg B コンパクト性 ✅ PROOF-DONE（d16a74e1）→ Leg C 固有値列挙 ✅ PROOF-DONE（de758f19、監査 PASS 77a5fdf2）→ Leg C' 非空虚性 ✅ CLOSED（a040a456、監査 PASS 569c48f0）→ 境界補題/退化統合 ✅（a7595371）** → **Leg 0（gateway）✅ CLOSED（leg 13、Proposal A/C を反証・Proposal O を PASS）** → 次 = **Leg P（def-fix = Proposal O、観測写像を Karhunen-Loève 展開へ）→ Leg D'（BddAbove を Bessel 単独で壁非依存）→ Leg E（LPS count = 壁が残る場所）**。~~次 = WSEB~~ ❌ **WSEB は命題が FALSE（leg 12）ゆえ leg ごと終了**。残債 = `∀ n, λ n ≠ 0`（infinite rank、壁ではない、未着手）]** → [shannon-hartley-phase2-spectral-plan.md](shannon-hartley-phase2-spectral-plan.md)
...
> **子 plan が 2 つの負債を負う（2026-07-17 leg 13 で更新）**: (1) **def-fix**（**観測写像**を Karhunen-Loève
> 展開へ = Proposal O。3 宣言の false-as-framed を救出、コードが子 plan を名指し。~~Proposal A = Wyner 全直線
> エネルギー単独~~ は leg 13 の gateway で**不十分と判明**）、
> (2) **壁核 self-build**（`wall:nyquist-2w-dof` = prolate/LPS の `≈2WT` DOF カウント、Leg E）。
> **build order = ~~Leg 0（gateway）✅ CLOSED~~ → Leg P（def-fix = Proposal O）→ Leg D'（BddAbove を
> **Bessel 単独**で壁非依存 closure）→ Leg E（壁）**。
...
- **proof-done 条件（= 目標）**: `≥` を fully genuine 証明（synthesis bridge + Parseval 電力橋 + BddAbove
  ...
  **leg 2 + leg 3 = 現行 def では false-as-framed** ⟹ 子 ~~Leg 0~~✅ → Leg P（def-fix = Proposal O）→ Leg D' が前提。
**retreat line（2026-07-17 更新）**: **Phase 3 achievability は `wall:nyquist-2w-dof` を共有しない**
（2026-07-15 の「共有する」は撤回 — 共有に見えたのは偽の命題を壁と読んだため）。**full closure の前提** =
**~~子 plan の Leg 0（gateway）~~ ✅ CLOSED → Leg P（def-fix、**Proposal O** = 観測写像を正規直交テスト関数へ）
→ Leg D'（BddAbove を **Bessel 単独**で壁非依存 closure）**。leg 3 は `le_csSup` 経由で transitive に closure。
