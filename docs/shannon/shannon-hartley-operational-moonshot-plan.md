...
achievability は ≈2WT 次元の構成を要し **`eq` ともども Leg E 待ち**。子 plan の「DAG 訂正」節が SoT。

**Leg E-atom（leg 15）の二面的結果 — 壁は残るが壁論拠が入れ替わった**（詳細 = 子 plan「gateway atom」節が SoT）:
- ✅ crude trace bound `c·#{λ>c} ≤ 2WT` が **Bessel + 既存再生核で壁非依存に closure**（sorryAx-free、`@audit:ok`×7）。
- ❌ **tight LPS 集中は無傷** = 壁は retire しない。実装者の「壁は genuine でない」verdict は
  **独立監査が refute**（閉じたのは**弱い親戚**、`cause:weaker-relative`）。**verdict は監査を採用**。
- 🔑 **旧壁論拠「無限次元スペクトル理論が Mathlib に無い」は誤り**（grep で機械反証）:
  trace-class/Schatten は確かに不在だが、**コンパクト自己共役の無限次元スペクトル定理は存在する**
  （`Spectrum.lean:443` `orthogonalComplement_iSup_eigenspaces_eq_bot`）⟹ 行数見積の再算定が要る。
  台帳 **§SPECTRAL-ASSETS** が SoT。次 atom = 厳密 trace 等式 `∑ₙ λₙ = 2WT`（Bessel → **Parseval** 昇格）。
...
- [~] Phase 2 — prolate-DOF スペクトル理論（`timeBandLimitingOp` + 固有値集中）🔄 **[Leg A✅ → Leg B コンパクト性 ✅ PROOF-DONE（d16a74e1）→ Leg C 固有値列挙 ✅ PROOF-DONE（de758f19、監査 PASS 77a5fdf2）→ Leg C' 非空虚性 ✅ CLOSED（a040a456、監査 PASS 569c48f0）→ 境界補題/退化統合 ✅（a7595371）** → **Leg 0（gateway）✅ CLOSED（leg 13、Proposal A/C を反証・Proposal O を PASS）→ Leg P（def-fix = Proposal O）✅ CLOSED（leg 14、`4fd8a47c`、監査 all OK、判別子 PASS = 壁が消えなかった）** → **Leg D' ✅ PROOF-DONE（leg 14、`fb18b681`、監査 `@audit:ok`）→ Leg E-atom ✅（leg 15、crude trace bound が壁非依存 closure、`69152fd9`/`7c43417a`）→ 次 = Leg E-trace（厳密 trace 等式 `∑λₙ=2WT`、Bessel→Parseval 昇格）→ Leg E-sharp（第 2 モーメント = 壁の真の所在の候補）。`ge` と `eq` の両方がここ待ち**。~~次 = WSEB~~ ❌ **WSEB は命題が FALSE（leg 12）ゆえ leg ごと終了**。残債 = `∀ n, λ n ≠ 0`（infinite rank、壁ではない、未着手）]** → [shannon-hartley-phase2-spectral-plan.md](shannon-hartley-phase2-spectral-plan.md)
