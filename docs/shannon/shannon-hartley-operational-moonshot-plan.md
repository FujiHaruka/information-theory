...
- [~] Phase 2 — prolate-DOF スペクトル理論（`timeBandLimitingOp` + 固有値集中）🔄 **[user 指示で全理論自前構築（leg 9+）: Leg A✅ → Leg B コンパクト性 ✅ PROOF-DONE（d16a74e1、sorryAx-free、監査 PASS）→ **Leg C 固有値列挙 ✅ framework PROOF-DONE（de758f19、243 行、監査 PASS 77a5fdf2、14 decls ok/defect 0）** → 次 = **Leg C' 非空虚性**（1 atom `timeBandLimitingOp T W ≠ 0`、壁ではない。Leg C の 4 無条件 headline は定数ゼロ列で充足されるため、この atom 無しでは spectral content ゼロ）→ WSEB → Leg E LPS count]** → [shannon-hartley-phase2-spectral-plan.md](shannon-hartley-phase2-spectral-plan.md)
