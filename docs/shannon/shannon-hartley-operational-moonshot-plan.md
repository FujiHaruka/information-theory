...
**🎉🎉 leg 17 = 壁が名指していた命題そのものが CLOSED。leg 18 = R4-gate A1 gateway PASS + R4-ACH-B（実基底）も CLOSED。leg 19 = R4-ACH-A2 = route(ii)（作用素下界、per-vector μᵢ 不要）確定 + operator ブリック 3 本 + Lp→pointwise keystone、全 sorryAx-free。次 = A2 wiring（testFn ONB 構成 → encoder/observation → A4 transport）。**
