...
- [~] Phase 2 — prolate-DOF スペクトル理論（`timeBandLimitingOp` + 固有値集中）🔄 **[user 指示で全理論自前構築（leg 9+）: Leg A✅ → **Leg B コンパクト性 ✅ PROOF-DONE（d16a74e1、`timeBandLimitingOp_isCompact` sorryAx-free、4 leaf 全 genuine + 監査 PASS）** → 次 = Leg C 列挙 → WSEB → Leg E LPS count]** → [shannon-hartley-phase2-spectral-plan.md](shannon-hartley-phase2-spectral-plan.md)
