...
**🎉🎉 leg 17 = 壁が名指していた命題そのものが CLOSED。次アクション = 子 plan の R4-gate（A1）。**
...
⟹ **`wall:nyquist-2w-dof` のタグ class は現在 misclassified の疑い**（named proposition は閉じ、
残るのは leg 16 が「新しい壁はゼロ = 全て plumbing」と実測した R4）。**次 leg で再分類を判断**（下記）。
