...
（`@residual(wall:nyquist-2w-dof)`）は now **converse 専用に縮約**（live consumer 2→1）。
...
全て closed。**残 sorry（`eq`）は count の下流 = converse bridge（C0–C4）**、唯一の未 gateway-test = **C1（interlacing）**。
C1 PASS で `wall:nyquist-2w-dof` → `plan:` 再分類 license。**壁論拠は 3 度誤っていた**（子 plan「完了 leg」節）。
...
- [~] **Phase 2 — prolate-DOF スペクトル理論** 🔄 **[解析核・カウント・achievability bridge = CLOSED（leg 16–22）。残 = converse C1 interlacing のみ]** → [shannon-hartley-phase2-spectral-plan.md](shannon-hartley-phase2-spectral-plan.md)
- [ ] Phase 4 — converse（`contAwgn_le_shannonHartley`、Phase 2 消費）📋 **[残作業の実体 = C0–C4、C1 = 唯一の未 gateway-test]**
...
（`@residual(wall:nyquist-2w-dof)`、**converse 半分専用**）。
...
**次アクション = 子 plan の C1 gateway-atom-first probe**（1 interlacing atom を dispatch し wall/plumbing 判定）。
PASS で `wall:nyquist-2w-dof` → `plan:` 再分類 license（コード編集 = subagent + 独立 honesty-auditor）→ C0/C2/C3/C4
（全 plumbing）→ Phase 4 → Phase 5-full。詳細 → 子 plan「R4-CONV」節が SoT。
...
**中間状態**: Phase 4 が sorry なら `contAwgn_eq` は `≤` 経由で `@residual(wall:nyquist-2w-dof)` を継承（`≥` は既に genuine）。
...
| Phase | residual | 位置づけ |
|---|---|---|
| Phase 2（prolate 固有値集中） | ~~`wall:nyquist-2w-dof`~~ **名指す命題は closed（leg 16–17）** | 固有値集中の解析核 + カウント両半分が sorryAx-free。**残渣は count の下流 = converse bridge の C1 interlacing**。詰まったら `@residual(plan:shannon-hartley-phase2-spectral-plan)` |
| Phase 3（achievability） | **なし ✅ CLOSED（leg 22）** | `contAwgn_ge_shannonHartley` = proof-done sorryAx-free + @audit:ok |
| Phase 4（converse） | `wall:nyquist-2w-dof`（C1 gateway 未 → PASS で `plan:` 再分類） | consumer 1 本（`contAwgn_eq`:64）が未 unblock。C1 = 唯一の未 gateway-test |
...
1. **achievability CLOSED、converse が唯一の未着手（active、leg 22）**: `contAwgn_ge_shannonHartley` = proof-done
   ...
   次 = 子 plan の C1 gateway-atom-first probe → PASS で `wall:nyquist-2w-dof` → `plan:` 再分類。子 plan が SoT。
