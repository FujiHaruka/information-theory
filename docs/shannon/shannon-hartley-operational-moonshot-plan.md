...
**🎉 次アクション = 子 plan の R1（固有基底 + 多重度/カウント bridge）。壁 `wall:nyquist-2w-dof` の
解析核は leg 16 で消滅した**（`tr A − tr A² ≤ 2 + log(1+2WT)` = `tsum_inner_sub_norm_sq_timeBandLimitingOp_le`、
`552ac8de` + `00cb1c8b`、監査 all OK、sorryAx-free）。
...
**⚠️ 「解析核が落ちた」≠「family がほぼ完了」**（監査の明示的警告）。残債は壁の**下流**に移った:
**R1** 固有基底 + カウント bridge（`plan:` 級、資産は全て機械確認済）/ **R2** Chebyshev 分割（初等）/
**R3** `prolate_eigenvalue_count` を宣言として書く（現在**名前だけ存在**し指示対象が無い）/
**R4 operational bridge（Gram + Cauchy interlacing + capacity 計算）= 最大の未計測債務、Leg P 以降未評価**。
