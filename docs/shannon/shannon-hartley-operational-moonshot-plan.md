...
**R1** 固有基底 + カウント bridge（`plan:` 級）/ **R2** Chebyshev 分割（初等）/
**R3** `prolate_eigenvalue_count` を宣言として書く（現在**名前だけ存在**）/
**R4 = 独立した 2 本の bridge**（~1500–2500 行。converse 側 C0–C4 / achievability 側 A1–A4。
**achievability は interlacing を全く要さない**）。**R1+R2+R3（~2–3 leg）は consumer の sorry を 1 本も落とさない。**
