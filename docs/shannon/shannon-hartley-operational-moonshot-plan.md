...
- [~] Phase 4 — converse（`contAwgn_le_shannonHartley`、Phase 2 消費）🔄 **[C1 interlacing = CLOSED（leg 23）。C3 = leaf 5/6 CLOSED（leg 24、新 file `ShannonHartleyConverse.lean`、`contAwgn_operational_converse` type-check-done + 監査 CONFIRMED、残 1 sorry = L5 MI-finiteness）。残 = C3 L5 / C2 Gauss 回転 / C4 water-filling+極限 / C0 headline / assembly、全 project-internal・壁なし]**
...
**C3 = leaf 5/6 CLOSED（leg 24）**: 新 file `ShannonHartleyConverse.lean`、`contAwgn_operational_converse`（`log M ≤ ∑½log(1+P'ᵢ/(N₀/2)) + Fano`）type-check-done + 監査 CONFIRMED、L0–L4 + Markov 全 sorryAx-free（`parallel_per_input_mi_le_sum` + `shannon_converse_single_shot` + `mutualInfo_le_of_markov` 消費）。
**次アクション = C3 の L5（MI-finiteness `contAwgn_mi_W_ne_top`、最高リスク、`awgnConverseJoint_mi_W_ne_top` の density chain 移植）→ C2（Gauss 回転、Mathlib 在）+ C4（water-filling+極限、初等）→ C0 headline → Phase 4 → Phase 5-full（`le_antisymm`）**。詳細 → 子 plan「R4-CONV」節が SoT。
