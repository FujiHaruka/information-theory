...
`BroadcastChannel/SuperpositionTimeShare.lean` + `Shannon/CondMutualInfoMixture.lean` の 7 ファイル
(いずれも 1500 行ガイド内)。
...
構成して返す `exists_bcInfo_ge_of_lessNoisy_of_isUVChannelLaw` まで到達) が proof done** で、
**次の一手は S7** (全支持への摂動、残り ~210 行)。
