...
**現在 進行中の子は [`bc-open-problem-plan.md`](bc-open-problem-plan.md) のみ** (未解決本体への攻略、
L0 文献 / L1 数値 probe 基盤 / L2 還元層の決着 / L3 軸 A / L4 棚卸し #1 / L5 軸 G / L6 軸 B /
L7 経路構築 #1 / L8 経路 #1 の probe + novelty gate / L9 棚卸し #2 まで完了 = **10/20 leg**。
**L7 が立てた R1 は L8 で (4) を通り (5) で落ちた** — 数学的には無傷だが一次文献 3 本を取得した結果
12 step すべてに逐語出典が付いた (死因 `known-result`) ⟹ **§3.1 の 5 条件を満たす経路は 0 本**で、
R1 は経路ではなく軸 E の地図として [`bc-open-problem-routes.md`](bc-open-problem-routes.md) に残る。
**L9 は残り枠を gate-first へ組み替えた** ([探索 + gate] → [構築 + probe] の 2 leg 1 組)。次は
**L10 = 探索 + gate (軸 C `jog-nair-inequality`。LP gate `shannon-cone-lp` を先に建てる)**)。
