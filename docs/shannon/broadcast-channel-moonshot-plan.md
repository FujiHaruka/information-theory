...
**後片付けは完了**: README 定理表への登録 (`b545cbd7`)、`Superposition/` サブディレクトリ昇格 +
重複解消 + `ℝ≥0∞` イディオムの refactor leg (`4ea35cc0`…`d0ac3aed`、5 ファイル / **−85 行**、
新しい数学 0 行、style PASS)、族としての proof-log (`3d680d3e`)。**現在の本線は more capable の
等号** (子 plan §推奨実行順 **#2**) — **M0 在庫が着地** (`6527722b`、
[`bc-morecapable-equality-inventory.md`](bc-morecapable-equality-inventory.md)、probe 7 本 935 行が
すべて exit 0、**壁 0 件**、目標は真、見積り **~925 行 / 帯 820–980**)。残りは (3) degraded との
接続 → (4) Phase 2 の P4–P7 で、(5) semi-deterministic は L-BCO7 で「外界側だけで止める」と判断済。
