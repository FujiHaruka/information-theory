...
攻略路は **MAC 側の完全な先例** (`mac_pentagon_subset_capacityRegion_allprob` +
汎用 `macMix` 一族) がそのまま効き、superposition 側 S7 の混合不等式ルートは模倣不要だった
(Marton 側は全アルファベットが `Fintype` ゆえ連続性ルートが通る)。**残るのは子 §後続作業 B–G のみで
いずれも前提ではない**。(5) semi-deterministic は L-BCO7 で「外界側だけで止める」と判断済
(**新規作業なし**)。
