...
(UV 外界の集合化 + 操作的包含 `bcCapacityRegion ⊆ bcOuterRegionUV`)**。M0 在庫
([`bc-uv-operational-inventory.md`](bc-uv-operational-inventory.md)) が実装を 8 step (S1–S8、≈ 1370 行、
Mathlib 側の穴ゼロ) に分解し、**符号から ambient 測度を構成する橋 + per-letter 同定 = S1–S6 が完了**
(`ChannelCoding/CodeToAmbient.lean` = MAC/BC 共有層 + `BroadcastChannel/OuterBoundUV/Bridge.lean`、
headline `bc_uv_converse_from_code`、per-letter 側は `bcUVJointDistribution` + 情報スロット
`uvInfo₁/₂/Sum₁/Sum₂` + 同定 4 本)。残りは S7 集合化 / S8 極限と組み立て。
...
| [`bc-general-region-plan.md`](bc-general-region-plan.md) | 一般 BC 容量領域フレーム (操作的容量領域の定義 → 協調外界 → UV outer bound → 一致クラス拡張) | 進行中 🚧 — Phase 1 ✅ (`BCAchievable` / `bcCapacityRegion` / `marton_region_subset_capacity`) / Phase 3 ✅ (`bcOuterRegionCoop` / `bc_capacity_subset_coop`、Wolfowitz strong converse の対偶で ambient 構成を迂回) / Phase 4a ✅ (`bc_uv_converse` = Nair–El Gamal 単一文字化 4 本の floating 形 headline、**degradedness 前提なし**、核は新規自作 `csiszar_sum_identity_cond`)。**本線 = Phase 4b 🚧** (集合化 + 操作的包含)。**M0 在庫 ✅** ([`bc-uv-operational-inventory.md`](bc-uv-operational-inventory.md)) で S1–S8 の 8 step (≈ 1370 行 = MAC 実績 2191 行の 6 割強、Mathlib 側の穴ゼロ = `@residual(wall:…)` 候補なし) に分解済、うち **S1–S6 ✅** = 符号→ambient 橋 (`ChannelCoding/CodeToAmbient.lean` 新設 = MAC/BC 共有層 / `BroadcastChannel/OuterBoundUV/Bridge.lean` / headline `bc_uv_converse_from_code` / `uvAuxPad` による補助変数の型統一) + per-letter 同定 (`bcUVJointDistribution` + 情報スロット `uvInfo₁/₂/Sum₁/Sum₂` + 同定 4 本、自作補助補題ゼロ)。**残 S7 集合化 → S8 極限と組み立て** (S8 = 退化レート被覆が最後の難所)。設計 2 件は決着 (単一文字還元 = 案 A 補助変数吸収 / 結合 memoryless = 構成側、`OuterBoundUV.lean` の署名は無改変 — S3 で機械確認)。**S7 への必須設計制約が 1 件追加** (子 判断ログ 8): `uvInfo*` はチャネルを引数に取らない汎関数なので、union の添字集合に「出力の条件法が `W` に一致」+「補助 → 入力 → 出力の Markov 性」を課さないと外界が平面全体に退化する (反例 class)。Phase 2 は park 可、Phase 5 のクラス定義は 4b と独立。撤退 L-BCO1 不発動 / L-BCO2/3 据置 / **L-BCO4 は前半 (橋) closure により発動条件が S7–S8 側に縮小** / **L-BCO5 不発動** / L-BCO6 は S8 待ち |
