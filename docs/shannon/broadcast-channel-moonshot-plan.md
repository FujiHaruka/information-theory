...
**後続 (未着手)**: 一般 BC の**外界は 0 本**で、集合としての容量領域も未定義 (degraded 用
`InBCCapacityRegion` は点ごとの不等式バンドル)。挟み込み `martonRegion ⊆ bcCapacityRegion ⊆ outer`
の足場を組む計画 → 子 [`bc-general-region-plan.md`](bc-general-region-plan.md)。一般 BC 容量領域の
特徴づけ自体は未解決問題なので、内外一致は計画の Phase 外。
...
| [`bc-general-region-plan.md`](bc-general-region-plan.md) | 一般 BC 容量領域フレーム (操作的容量領域の定義 → 協調外界 → UV outer bound → 一致クラス拡張) | 未着手 📋 — Phase 1 (`BCAchievable` / `bcCapacityRegion`、MAC の `MACAchievable` / `macCapacityRegion` を雛形に写経) が最優先。外界より先に「主語」を作る順序。撤退 L-BCO1/2/3 |
