...
L7 経路構築 #1 / L8 経路 #1 の probe + novelty gate まで完了 = **9/20 leg**。**L7 が立てた R1 は
L8 で (4) を通り (5) で落ちた** — 数学的には無傷だが一次文献 3 本を取得した結果 12 step すべてに
逐語出典が付いた (死因 `known-result`) ⟹ **§3.1 の 5 条件を満たす経路は 0 本**で、R1 は経路ではなく
軸 E の地図として [`bc-open-problem-routes.md`](bc-open-problem-routes.md) に残る。次は L9 = 棚卸し #2)。
