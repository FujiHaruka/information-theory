...
Mathlib 側の穴ゼロ) に分解し、**符号から ambient 測度を構成する橋 = S1–S5 が完了**
(`ChannelCoding/CodeToAmbient.lean` = MAC/BC 共有層 + `BroadcastChannel/OuterBoundUV/Bridge.lean`、
headline `bc_uv_converse_from_code`)。残りは S6 per-letter 同定 / S7 集合化 / S8 極限と組み立て。
