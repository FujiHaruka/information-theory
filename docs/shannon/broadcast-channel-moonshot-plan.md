...
| [`marton-inner-bound-plan.md`](marton-inner-bound-plan.md) | L-BC5 解除 = 一般 (non-degraded) BC の Marton inner bound (EGK Thm 8.3、private message のみ) | 🚧 進行中 (**Phase 0–6a' 完了** = 抽象 second-moment 核 + 共分散の鋭化 + region/Fourier–Motzkin + 5-tuple ambient + mutual covering の weak / strong 両版 + 受信機 1 の誤り分解/E1/E2、Marton 6 ファイル + `Shannon/ConditionalAEP.lean` はいずれも 0 sorry / 0 @residual・root 登録済。残り Phase 6b = 受信機 2 / 7 = 組み立て headline `marton_achievability` / 8 = ゲート+README。生きた撤退口は L-MT6 のみ) |
