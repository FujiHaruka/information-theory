...
S5 が方針 Y gateway の本体で **2026-06-08 全 proof-done**(`entropyPowerExt_mono_add_unconditional` ほか、`@audit:ok`)。
...
- [ ] **完全無条件版を新 file `EPIUncondDispatchFull.lean` に別建て**: `entropyPowerExt_add_ge_unconditional`
  (`hX hY hXY` のみ)。gateway #1 `entropyPowerExt_mono_add_unconditional` (regularity のみ、`@audit:ok`)
...
7. **2026-06-08 Phase 5 を dispatch endgame に再構築 (method-Y gateway proof-done を受けて)**: S5 (method-Y full gateway) が 2026-06-08 全 proof-done (`entropyPowerExt_mono_add_unconditional` / `differentialEntropyExt_mono_add_unconditional` + bridge `differentialEntropyExt_integrable_of_finite`、全 sorryAx-free + (i-a) 非継承、`epi-facts.md` 達成表) に到達したことで、Phase 5 の残務を「gateway を使った headline wire endgame」に確定し、子 sub-plan `epi-uncond-dispatch-endgame-plan` (S4) に分離。
   - **設計**: 現 dispatch `entropyPowerExt_add_ge_dispatch_skeleton` (`EPIUncondDispatch.lean:145`、proof-done だが **21 precondition** = case2/2symm 用 16 integrability + case-1/case2 用 5 finite-entropy、consumer 0 leaf) は**改変せず残し**、gateway 経由の完全無条件版 `entropyPowerExt_add_ge_unconditional` (`hX hY hXY` のみ) を新 file `EPIUncondDispatchFull.lean` に**別建て**。柱 A (singular rewire = gateway 単調性で case2/2symm の RHS 一項を 0 に潰す → integrability 16 除去) + 柱 B (case-1 split = `h(X+Y)`/`h(X)`/`h(Y)` の ⊤/⊥/有限 by_cases → 有限 sub-case で bridge → 既存 `entropyPowerExt_add_ge_finite_ac` に落とす) + 柱 C (4 枝 assembly) + 柱 D (命名)。新 file は `import EPIUncondDispatch + EPIUncondTruncationLimit` で cycle なし (両 leaf、相互 import なし、機械確認済)。
