...
> **✅ 達成 + 2026-06-11 後片付け**: 無条件 EPI は `entropyPowerExt_add_ge` (ℝ≥0∞、precondition 0、sorryAx-free) で達成済 (2026-06-08)。下記「目標」の主定理 `entropy_power_inequality` (Real + `h_stam` 形) と legacy Stam-bridge route (`stamToEPIBridge_holds` 系 sorry + 露出 28 decl) は **2026-06-11 物理削除済** — route-T 後継 (`_unconditional` / `_of_ac` / `_of_density`) が完全 supersede。子 plan `epi-stam-to-conclusion-plan` は OBSOLETE 化済。本文中の `entropy_power_inequality:289` / `stamToEPIBridge_holds` 参照は全て履歴 (削除済を指す)。
>
...
- **a.c. 判定の definitional 化 GO**: `Decidable (μ ≪ volume)` は不在だが、Mathlib `klDiv`
  (Mathlib の `MeasureTheory.klDiv` 定義) が `open Classical in` +
...
- **核心壁 Mathlib 完全不在 (loogle Found 0 × 5)**: entropy/KL の弱収束下半連続性、Gaussian 畳み込み弱収束。
  当時は新規 wall 2 本 (entropy LSC 弱収束 / Gaussian approx identity 弱収束) を見込んだが、いずれも route T/β'
  で迂回され code 側に wall slug は作られていない (本節は SUPERSEDED、判断ログ 6 が SoT)。
...
- [x] Phase 2 — downstream re-port ✅ **CLOSED as no-op (rename 不要 verdict、書換 0)**: 二層設計 (旧 Real `differentialEntropy`/`entropyPower` を workhorse として温存、新 ℝ≥0∞ 層は別名 `entropyPowerExt`/`differentialEntropyExt`) のため名前衝突なし。headline は `entropyPowerExt` 直接 state、旧 10 consumer は全て (a) Real 層を不変参照で書換ゼロ。naming cleanup は不要と確定 (発生する rename が無い) → [sub-plan: epi-downstream-report-plan]
...
- [x] Phase 5 — 主定理 assembly = **dispatch endgame** ✅ **完了 (2026-06-08)** → [sub-plan: epi-uncond-dispatch-endgame-plan]: 完全無条件 dispatch `entropyPowerExt_add_ge` (`hX hY : Measurable` + `hXY : IndepFun X Y P` **のみ**、precondition 21→0) を新 file `EPIUncondDispatchFull.lean` に別建てし **sorryAx-free 達成** (`#print axioms` = `[propext, Classical.choice, Quot.sound]` 機械確認、5 declaration 独立 honesty-auditor all-OK、commit f767105)。柱 A (singular rewire、gateway で 16 integrability 除去) + 柱 B (case-1 split = h の ⊤/⊥/有限 by_cases + bridge で integrability 供給) + 柱 C (4 枝 assembly) + 柱 D (命名)。**真の `_unconditional` headline は ℝ≥0∞ 版のみ** (実数版は `EReal.exp_coe` 変換に a.c.+有限を要し完全無条件にならず、`entropy_power_inequality_of_ac` が honest 限界)。旧 21-precondition skeleton は proof-done consumer 0 leaf として残置 (supersede)。→ 判断ログ 7
...
| — | (case 1 a.c. core closure) | 柱 3: **route T pivot で closure 済** (Phase 3 = `entropyPowerExt_add_ge_finite_ac` sorryAx-free)。当初流用予定だった既存 plan 群 (`epi-stam-to-conclusion-plan` Phase B / `epi-csiszar-ratio-reframe-plan` / `epi-richness-route-b-plan` / difference G3) は route T 迂回で**不要 = superseded**。 | route T (S6) | ✅ CLOSED (2026-06-10 整合: case1 は route T、Stam-bridge plan は legacy) |
| S5 | [`epi-uncond-deffix-monotone-plan`](epi-uncond-deffix-monotone-plan.md) (W-Y1、🪦 **SUPERSEDED+DEAD 2026-06-11 #23**: route α-ii を route β' が (i-a) 非継承で supersede、`Monotone.lean`/`CondEntropyExt.lean` 物理削除済) / [`epi-uncond-truncation-lsc-plan`](epi-uncond-truncation-lsc-plan.md) (W-Y2 = route β'、**live closure 路**) | **方針 Y クリティカルパス (2026-06-07 再評価、smoothing FALSE WALL)**: W-Y1 拡張単調性 + +∞ 伝播 (plan-closeable plumbing、gateway atom 着手済) / W-Y2 = **route β' (truncation+monotone-limit)** で無限エントロピー a.c. 入力の gateway ⊤ 枝を closure。smoothing ルートの「t>0 平滑側無前提 EPI + t→0⁺ 極限」は route T/β' が迂回し不要に。 | case1 (route T closure 済) + S1 | 🚧 W-Y1: 新 file `EPIUncondCondEntropyExt.lean` で ①fibre同定/def genuine + (i-a) sorry 消滅、crux を **② EReal chain rule 1 本に局所化**。**✅ 2026-06-08: ② finite-entropy 版 `differentialEntropyExt_eq_condEntExt_add_klDiv_of_finite` genuine 着地** (sorryAx-free、`@audit:ok`)。**finiteness-free 版②は per-fibre mass 相殺で証明不能と確定** (advisor 裏取り) → 無条件版は **W-Y2 = route β' で別途**。**✅ 2026-06-08: route β' plan 起草 + Phase 0 gate = GO** (`epi-uncond-truncation-lsc-plan.md`、判定 (B) path 可視 moonshot、ターゲット = gateway ⊤ 枝不等式)。Phase 0 で weak-conv 回避を機械裏取り (Fatou lift `A_W≤liminf A_{W_n}` が density a.e. 収束のみから fire、`klDiv_le_liminf_of_ae_tendsto` 同型) ⇒ entropy LSC 弱収束壁 (当時想定、Mathlib 不在) を回避確定。**✅ 2026-06-08: Phase 1 skeleton + Phase 2 proof-done。`differentialEntropyExt_mono_add_truncW` (per-n 単調性) sorryAx-free、独立 honesty-auditor all OK**。**ルート転換**: chain rule 等式 (finite ②) は `hκ_dens_meas` (joint 密度可測、Mathlib 真 gap、loogle Found 0) を必須仮説に持ち proof-done 不能 → **単調性は不等式で足る**との advisor 判定で chain rule lemma を捨て、場合分け + explicit translate per-fibre Gibbs (`differentialEntropy_le_cross_entropy` + 平行移動 fibre + Tonelli collapse) に転換。`hκ_dens_meas`/`hκ_KL`/`hκ_cross_int` を**全廃**。残単一 crux = single-component Jensen helper `negPart_negMulLog_conv_single_ne_top` も genuine。**✅ 2026-06-08: Phase 4 を route (d'') (測度 domination + 有限性不要 ℝ≥0∞ Gibbs) に確定し atom 1 (`map_truncW_add_le_smul_map_add`) + atom 2 (`ennreal_gibbs_rearranged`、A=⊤ 許容、負部 1-有界で `A=⊤⟹crossPos=⊤`) proof-done + 独立 honesty-auditor all-OK** (commits 53d052e/dd3b6d2/ed533ba)。**✅ 2026-06-08: route β' (W-Y2) 完了** — Phase 3 (Fatou helper + W-divergence `h(Q_n.map W)→⊤`、`differentialEntropyExt_truncW_tendsto_top`) + Phase 4 assembly `differentialEntropyExt_top_of_indep_add` が proof-done (sorryAx-free、**(i-a) `differentialEntropyExt_indep_add_eq_add_klDiv` 非継承を機械確認** = truncation ルートが無条件版②を genuine 迂回、独立 honesty-auditor all-OK、commits 5cd28ca/803e489)。**✅ 2026-06-08: method-Y full gateway 完了 (S5 = 方針 Y gateway 完全 proof-done)**: (a) **W-Y1 有限枝** closure — per-fibre translate Gibbs が truncation 非依存と判明、core を一般化補題 `differentialEntropyExt_mono_add_of_integrable` に抽出 (sorryAx-free, `@audit:ok`)。(b) **full assembly** — `differentialEntropyExt_mono_add` (3 枝: ⊥=bot_le / ⊤=route β' / 有限=(a)) + `entropyPowerExt_mono_add` を Truncation file 末尾に建立、bridge `differentialEntropyExt_integrable_of_finite` 新規。3 補題とも sorryAx-free + **(i-a) 非継承を機械確認**、独立 honesty-auditor all-OK。**残 = headline wire のみ (Phase 5/S4、gateway consumer は依然 0)**。SoT → `epi-uncond-truncation-lsc-plan.md` 完了サマリ + facts 台帳「達成」表 |
| S4 | [`epi-uncond-dispatch-endgame-plan`](epi-uncond-dispatch-endgame-plan.md) | **柱 2+3 合流 = headline wire endgame**: S5 (method-Y gateway、proof-done) を使い、既存 21-precondition dispatch (proof-done) の precondition を全除去し `hX hY hXY` のみの完全無条件 dispatch を新 file `EPIUncondDispatchFull.lean` に別建て。柱 A (singular rewire) + 柱 B (case-1 split = ⊤/⊥/有限 by_cases) + 柱 C (4 枝 assembly) + 柱 D (命名)。 | S1–S3 + S5 (gateway proof-done 済) | ✅ **完了 (2026-06-08)**: `entropyPowerExt_add_ge` proof-done (precondition 0、sorryAx-free、5 decl 独立監査 all-OK、commit f767105、`EPIUncondDispatchFull.lean`) |
| S6 | [`epi-infinite-variance-truncation-plan`](epi-infinite-variance-truncation-plan.md) | **無限分散 a.c. 古典 EPI 構築 (epi-infinite-variance-classical 壁の genuine close)** ✅ **CLOSED 2026-06-07**: route T (conditioning truncation、有限分散 EPI 黒箱再利用 + R→∞) で **sorryAx-free genuine closure 完了**。epi-infinite-variance-classical 壁は FALSE WALL と判明 (sharp Young/Brascamp-Lieb 不要)。集約先 `EPIInfiniteVarianceCapstone.lean` の `entropyPowerExt_add_ge_infinite_variance` (case split + P 版負部可積分)、無条件 dispatch `entropyPowerExt_add_ge_dispatch_skeleton` まで sorryAx-free、独立 honesty audit PASS (defect 0)。 | case1 有限分散版 EPI (closure 済) | ✅ CLOSED (route T、capstone) |

**依存 DAG (方針 Y、2026-06-08 再評価)**: `Phase 0 → S1 → S2 → {S3, case1(route T closure 済)} → S5(gateway proof-done) → S4(=Phase 5 dispatch endgame)`。
S5 が方針 Y gateway の本体で **2026-06-08 全 proof-done**(`entropyPowerExt_mono_add` ほか、`@audit:ok`)。
**smoothing ルート (「平滑側無前提 EPI + 半連続性極限」) は route T が迂回し不要**。
S5 は W-Y1 (拡張単調性 + +∞ 伝播、plan-closeable) と W-Y2 = route β' (有限エントロピーすら無い a.c. を
truncation 近似で救う層、`epi-uncond-truncation-lsc-plan` 完了) に分離し、両者 closure 済。
**残務 = S4 = headline wire endgame** (`epi-uncond-dispatch-endgame-plan`): gateway 経由で既存
21-precondition dispatch の precondition を全除去し `hX hY hXY` のみの完全無条件 dispatch を別建て。
L-Uncond-3-scope (方針 X 縮退) は **発動不要** (smoothing 壁解消 + gateway proof-done、判断ログ 6/7)。
...
## Phase 5 — 無条件 dispatch endgame (gateway 経由で 21 precondition 全除去) 📋 → `epi-uncond-dispatch-endgame-plan`

proof-log: yes (sub-plan 側)。

方針 Y の gateway (S5 = method-Y) が **2026-06-08 全 proof-done** に到達したことで、Phase 5 の残務は
「method-Y gateway を使い、既存 21-precondition dispatch を `hX hY hXY` のみの完全無条件 dispatch に
wire する endgame」に確定した。詳細は子 sub-plan [`epi-uncond-dispatch-endgame-plan`](epi-uncond-dispatch-endgame-plan.md)
(柱 A/B/C/D) が SoT。本傘 plan は要点のみ:

- [ ] **既存 21-precondition dispatch は改変せず残す**: `entropyPowerExt_add_ge_dispatch_skeleton`
  (`EPIUncondDispatch.lean:145`、proof-done、consumer 0 leaf、16 integrability + 5 finite-entropy)。
- [ ] **完全無条件版を新 file `EPIUncondDispatchFull.lean` に別建て**: `entropyPowerExt_add_ge`
  (`hX hY hXY` のみ)。gateway #1 `entropyPowerExt_mono_add` (regularity のみ、`@audit:ok`)
  + bridge #2 `differentialEntropyExt_integrable_of_finite` + 既存 case-1 #3 `entropyPowerExt_add_ge_finite_ac`
  + singular #4/#5 への delegation で構成。柱 A (singular rewire) + 柱 B (case-1 split ⊤/⊥/有限) +
  柱 C (4 枝 assembly)。
- [ ] **`#print axioms entropyPowerExt_add_ge`** = `[propext, Classical.choice,
  Quot.sound]` (sorryAx-free) を機械確認。`by_cases (≪ volume)` の `Classical.choice` は許容 (sorryAx
  でない)。
- [ ] **命名の論点 (柱 D)**: 真の `_unconditional` 命名は **ℝ≥0∞ 版でのみ可**。実数版
  (`entropyPower : Measure ℝ → ℝ`) は `entropyPowerExt μ = ENNReal.ofReal (exp (2h))` 変換
  (`EReal.exp_coe`) に a.c.+有限を要し、`h=±∞` を ℝ で表現できないため**完全無条件にならない**。
  実数版の honest 限界は既存 `entropy_power_inequality_of_ac` (`:249`、proof-done、`@audit:ok`)。
- [ ] 旧条件付き `entropy_power_inequality` (`:289`、Real 版 + h_stam) は取り消し線化せず corollary
  として残す (a.c. 特殊形、honest 後継は `entropy_power_inequality_of_ac`)。
- [ ] **独立 honesty-auditor 起動** (子 Phase 3、新規 declaration 群導入時、CLAUDE.md 必須)。

### Phase 5 撤退ライン (詳細は子 §各 Phase 撤退ライン L-Endgame-*)

- **L-Uncond-5-α**: `by_cases (P.map X ≪ volume)` の `Classical.choice` は propext/Quot.sound と同列で
  許容範囲 (sorryAx でない)、honest。既存 dispatch skeleton も同構造。問題なし。
- **L-Uncond-5-β**: 柱 B (case-1 split) の有限 sub-case が plumbing で park された場合 → **honest 部分
  達成として commit** (子 docstring に「case-1 有限 sub-case は `epi-uncond-dispatch-endgame-plan` で
  closure 予定、他 3 枝 genuine」明示、`@residual(plan:epi-uncond-dispatch-endgame-plan)`)。無条件構造
  (4 枝 dispatch + precondition ゼロ) は達成、完全 sorryAx-free は柱 B closure 後。
...
3. **(2026-06-05 後刻、独立壁再確認で case 1 difference G3 を walled でないと訂正)**: 判断ログ 2 の Phase 3
   ...
   再確認 3 件で 2 つの誤前提を訂正:
   - **(i) blachman 一般密度壁は FALSE WALL** (当時想定 slug、code 側未作成): 一般密度 producer
...
4. **2026-06-06 S1 retype の false-statement defect 発見 + def-fix** (orchestrator 機械検証 + 独立 honesty audit): Phase 1 (S1 retype, commit `3421948`) で "genuine 0 sorry" とした `entropyPowerExt` は、a.c. 枝 `ofReal(exp(2·differentialEntropy μ))` が Bochner `differentialEntropy` の非可積分時 garbage `0` 返しにより **infinite-entropy a.c. 入力で FALSE-as-stated** だった (機械検証 `/tmp/test_garbage.lean`: 密度 ∝ 1/(x log²x) 等 h=±∞ の a.c. を entropyPowerExt=1 に潰し、dispatch case-1 obligation を偽に)。退化トラップ除去 (特異→0) は正しかったが a.c. 枝の ±∞ 表現が欠けていた。
   ...
   - **dispatch**: case-2 に finite-entropy 前提追加 + `_of_ac_integrable` 化 (genuine 維持)、case-1 の false bare sorry を named wall `entropyPowerExt_add_ge_finite_ac` に置換。**headline TRUE-as-stated 化 + sorry を named wall に局所化**。独立 honesty audit 0 defect。(2026-06-07: 旧 bundled wall を正則 [Phase A 既閉] / 有限分散 [closure 済、commit 452ea1b] / 無限分散 [当時 epi-infinite-variance-classical 壁、後に route T で CLOSED] に 3 分解。dispatch は import cycle 回避で `EPIUncondDispatch.lean` に分離移動。)
...
6. **2026-06-07 方針 Y 再評価 — smoothing 壁 FALSE WALL 判明 + W-Y1 gateway atom 着手**: 判断ログ 5 末尾「完全無条件は別 moonshot として park」を **ユーザー指示で再評価**し、§方針 Y feasibility verdict (2026-06-05、SUPERSEDED 済) を覆した。
   ...
   - **W-Y1/W-Y2 分離** (`epi-uncond-monotone-inventory.md`): 方針 Y の残壁を 2 本に分離。
     ...
     - **W-Y2 = entropy power 弱収束半連続性** (病的両部発散 a.c. = 有限エントロピーすら無い入力): 当時は Gaussian approx identity 弱収束壁 (loogle Found 0) を **genuine wall 1 本残置** = 完全無条件の限界と見込んだ。**→ その後 route β' (truncation+monotone-limit) で迂回 closure 済** (S5 行 / `epi-uncond-truncation-lsc-plan` 完了)、code 側に wall slug は作られていない。当時の「W-Y1 まで closure で h=±∞ 含む a.c. + 退化 case 無条件化」見込みは、最終的に W-Y1+W-Y2 両 closure で完全無条件 dispatch に到達。
...
7. **2026-06-08 Phase 5 を dispatch endgame に再構築 (method-Y gateway proof-done を受けて)**: S5 (method-Y full gateway) が 2026-06-08 全 proof-done (`entropyPowerExt_mono_add` / `differentialEntropyExt_mono_add` + bridge `differentialEntropyExt_integrable_of_finite`、全 sorryAx-free + (i-a) 非継承、`epi-facts.md` 達成表) に到達したことで、Phase 5 の残務を「gateway を使った headline wire endgame」に確定し、子 sub-plan `epi-uncond-dispatch-endgame-plan` (S4) に分離。
   - **設計**: 現 dispatch `entropyPowerExt_add_ge_dispatch_skeleton` (`EPIUncondDispatch.lean:145`、proof-done だが **21 precondition** = case2/2symm 用 16 integrability + case-1/case2 用 5 finite-entropy、consumer 0 leaf) は**改変せず残し**、gateway 経由の完全無条件版 `entropyPowerExt_add_ge` (`hX hY hXY` のみ) を新 file `EPIUncondDispatchFull.lean` に**別建て**。柱 A (singular rewire = gateway 単調性で case2/2symm の RHS 一項を 0 に潰す → integrability 16 除去) + 柱 B (case-1 split = `h(X+Y)`/`h(X)`/`h(Y)` の ⊤/⊥/有限 by_cases → 有限 sub-case で bridge → 既存 `entropyPowerExt_add_ge_finite_ac` に落とす) + 柱 C (4 枝 assembly) + 柱 D (命名)。新 file は `import EPIUncondDispatch + EPIUncondTruncationLimit` で cycle なし (両 leaf、相互 import なし、機械確認済)。
   - **命名の論点 (子 Phase 4)**: 真の `_unconditional` 命名は **ℝ≥0∞ 版 (`entropyPowerExt`) でのみ可**。実数版 (`entropyPower : Measure ℝ → ℝ`) は `EReal.exp_coe` 変換に a.c.+有限を要し `h=±∞` を ℝ で表現できないため完全無条件にならない (実数版の honest 限界 = 既存 `entropy_power_inequality_of_ac`)。親方針の旧記述「実数版 `entropy_power_inequality_unconditional` 新名」は実数版を `_unconditional` と命名しない方向に子 SoT で訂正。
   - **在庫マーク (子 Phase 0 委任、予測値禁止)**: `differentialEntropyExt = ⊥ ⟹ entropyPowerExt = 0` の named lemma **不在** (`entropyPowerExt_singular` は `¬ a.c.` 専用、a.c. かつ `h=⊥` 非対応) → 柱 B helper を ~3 行自作と見積り。`EReal.exp_top`/`exp_bot`/`exp_coe`/`mul_top_of_pos`/`mul_bot_of_pos` の verbatim、有限 sub-case の `h(X+Y) ≠ ⊥` 導出ルート、両 a.c.⟹X+Y a.c. の Mathlib lemma を在庫委任。`differentialEntropyExt μ=⊤ ⟹ entropyPowerExt μ=⊤` (#6 `entropyPowerExt_eq_top_of_diffEntExt_top`、`:129`) は既存確認済 (自作不要)。
