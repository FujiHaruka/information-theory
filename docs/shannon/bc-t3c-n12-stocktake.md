...
1. **`C` の境界は面単位で棚卸しされ、[probc] という 1 インスタンスの上で `Thm7 ⊆ C` の側が 6 方向 + 2 つの錐 + `R0=0` スライスについて閉じた** (⚠ **6 方向のうち 5 枚は無料で、F-SUM 値の 1 枚は `η = 1e-13` の条件つきである** — §1-9) — 最後に残った幅 `ε` は N10 が**厳密に 0** にし、その射程は半空間 `e ≤ h(p)` の全体である。
...
4. **検証器 11 本は HEAD で全数再現する** (合計 **263/263**、EXIT=0。⚠ **直列・無負荷で測ると所要時間も台帳の記録と一致する** — 台帳より 25% を超えたのは記録の在る 10 本中 **0 本**)。
5. **6 leg とも敵対的独立監査を通り、判定は 6 本とも生存した** (訂正 **計 36 件**、⚠ **主判定を動かした訂正は N6 の 1 件のみ**で、それは GO / NO-GO ではなく「未着手の面の数え方」である)。
...
| 2 | 判定枠 6 leg が層 3 へ載せたのは `@[entry_point]` 定理 **5 本ちょうど** (N8 = 3 / N11 = 2) | N8 / N11 | facts N8-a / N11-a、plan §5.1 の N8・N11 着地ブロック | `git show 51d1ccb1 -- InformationTheory/ \| rg -c '^\+@\[entry_point\]'` / 同 `4da3a9ad` / 窓内の全 commit (`git log --oneline 20b456b9..HEAD -- InformationTheory/`) について同じカウント | N8 = **3**、N11 = **2**。窓内で `InformationTheory/` に触れた commit は 5 本 (`51d1ccb1` / `72aaef9a` / `d9fa6558` / `4da3a9ad` / `db25d3dd`) で、**`@[entry_point]` を足したのは前 2 本だけ** (残り 3 本は `+0`) ⟹ **5 本ちょうど**。⚠ **この数え方の構造的な盲点 (多属性形 / インライン形) は §3.1 に記録した**。⚠ **同ディレクトリ全体の `@[entry_point]` は 24 ファイル 84 本**であり、**残る 79 本は前 relay 以前の分**である (⟹ ディレクトリ一括カウントは本項の反証にならない) |
...
⚠ **裏取りで見つかった食い違いは 0 件** (pass 本数・所要時間とも台帳と一致。⚠ **本 leg が当初記録した「一律に長い」という所見は検証器を並行実行した負荷 artefact であり、N12 監査が直列・無負荷で再測して否定した**)。
...
| 2 | **一般 BC への持ち上げ** | 連鎖は BEC 固有の恒等式と二値の `δ` に全面的に依存する。さらに N6-j が「**NO-GO は比較であり比較の相手が要る / 一般 BC では `C` が未知**」と却下の load-bearing な前提を特定している | N6-j が (γ) を ill-posed と判定 / N10-e が「決着は [probc] の族に留まる」と明記 / N11 は `ℝ` 上の 1 変数不等式のみ | 障害は N6-j が既に対象として名指しし回避条件も書いている (→ §4.1) ⟹ **要るのはその的 (`Thm7` 側の上界だけの持ち上げ) が §0 の計算可能性側に何かを足すかの判定**である。⚠ **N6-j は「足さない」と書いている** |
...
| 5 | **N10 §2.1–§2.4 の恒等式群を層 3 へ載せる** (⚠ **「『`ε = 0` が機械検証された』とは言えない」は §0-3 / §3.5 が書く確定事項であって未確定ではない**) | **N10 §2.1–§2.4 の恒等式群は Lean に 1 本も載っていない** — 載ったのは N10 §2.5 の解析的入力 `δ ≥ 0` の各点版だけである | N10 が散文 + 検証器で閉じ、N11 が入力 1 本を層 3 へ | §2.1–§2.4 の恒等式群の層 3 化。⚠ **本項は N11-g の逐語であり、本 leg が新たに出した判定ではない** |
| 6 | **`σ_x := d*_t − ψ_t(x) ≥ 0` (N10 §2.4) の独立な再導出** (⚠ **「唯一の解析的入力」という枠付けが過大であることは N11 監査 訂正 6 で確定済である**) | N10 §2.4 が併用する `σ_x := d*_t − ψ_t(x) ≥ 0` は `d*_t` が**最大値として達成される**ことに依り、厳密には代数ではない | N11 監査 訂正 6 が指摘。⚠⚠ **N10 からの継承であって N11 が作った誤りではない** / ⚠ **監査もこの点は再導出していない** | `σ_x ≥ 0` の独立な再導出 (達成性の議論を含む)。⚠ **本 leg も再導出していない** (→ §6-6) |
...
| 9 | **`C` の肩が `β` 族で尽きるか (一般の基数で)** | N6-f の族が `C` の境界そのものかは当時 screen のみで、**N7 の挟み込み + N10 の `ε = 0` で厳密に確定したのは方向 `(0,1,t)` (とその鏡映) に限る** | N6-f (screen) → N7-i / N7 §5 収穫 2 で**この方向に限り** `ε` 以内 → **N10-a が `h_Thm7(0,1,t) = h_C(0,1,t) = 2C + d*_t` を出したので幅は 0 に潰れ、この方向については `ε` なしで確定している** | 他方向 / 一般基数での証明書。⚠ **N6-q (5) より掃いた基数は `≤ 4` まで** |
...
- ⚠ **この数え方は多属性形 `@[entry_point, …]` とインライン形 `@[entry_point] def foo` を取りこぼす** (リポジトリ全体にインライン形 2 件 / 多属性形 8 件が実在する)。⚠ **窓の 5 commit と `InformationTheory/Shannon/BroadcastChannel/` 配下ではどちらの形も 0 件なので、本数 5 / 84 は取りこぼしていない** (N12 監査 `T03` / `T07` / `T07b` が独立に確認)。
...
| 定理 | 仮説 | 判定 |
|---|---|---|
| `plainDirectionalCombination_le_plainDirectionalBound` | `[IsProbabilityMeasure μ]` / `Measurable y` / `Measurable w` / `Measurable u` / `0 ≤ t` | **すべて precondition**。measurability は regularity、`0 ≤ t` は方向の定義域 (`t < 0` では重み `t` の符号が反転する)。証明本体は連鎖律 + `min_le_right` + `linarith` の 3 行で、核を仮説から受け取っていない |
| `plainDirectionalBound_eq_condFree` | `[IsProbabilityMeasure μ]` / `Measurable x/z/w/u` / `IsMarkovChain μ (w,u) x z` | **すべて precondition**。**マルコフ性は結論と無関係な構造条件ではない** — in-repo の**無条件の**連鎖律 `mutualInfoReal_pair_eq_add` だけを使うと、`t ≠ 0` について**結論の等式は `I((W,U);Z\|X) = 0` と同値**であり、`hmarkov` はその条件独立版である (N12 監査が Lean probe で機械証明)。⟹ **本定理は 2 つの同値な言い方の間の橋であり、その橋の中身 (`condMutualInfo_eq_zero_of_markov` + 連鎖律) は仮定ではなく証明済の既存補題 2 本が担っている** ⟹ **循環でも load-bearing でもない**が、「仮説に結論もその特殊形も現れない」とは書かない。⚠ **N8-d は監査が `hmarkov` 落としの明示反例で必要性を確認したと記録する。⚠⚠ ただし N8-d 自身が「probe が機械で確かめるのは実数変数の代数の水準」「反例の実現可能性は probe の射程外」と明記している** ⟹ 測度空間まで込みで殺したわけではない |
| `plainDirectionalCombination_le_plainDirectionalBoundCondFree` | `[IsProbabilityMeasure μ]` / 上 2 本の和集合 | **すべて precondition**。証明本体は上 2 本の合成 (`rw` + `exact`) |
...
| `binEntropy_binConv_sub_binEntropy_le` | 上 4 本 + `e * Real.log 2 ≤ Real.binEntropy p` | **`0 ≤ x ≤ 1` は定義域の制限、`p` の 2 本は落とせる (N11-c)。追加の 1 本 `he` は定義域の制限ではない — `x = 2⁻¹` を代入すると `x ∗ p = 2⁻¹` かつ `binEntropy 2⁻¹ = log 2` なので、`he` は結論そのものの `x = 2⁻¹` 各点版である** (N12 監査が Lean probe で両向きを機械証明)。⟹ **本定理の中身は「`x = 2⁻¹` の 1 点から `x ∈ [0,1]` の全体へ広げること」であり、`he` は落とすと命題が偽になる (`p = 1/4, e = 1, x = 1/2`) 最も鋭い形の半空間仮説である。結論の核 (全 `x` への拡張) は仮説に入っていないので load-bearing ではない** |
...
|---|---|---|---|---|
| 1 | **一般 BC への持ち上げ** | **`障害の構造`** | `## N6 (T3c)` N6-j (障害の名指し + 回避条件) / `## N10 (T3c)` N10-e (射程の限定) / `## L2 (T3)` (payoff が無いことの根拠) | `restatement` (⚠ **公算が最も高い** — N6-j 自身が「§0 への payoff は無い」と併記している) |
| 2 | **他インスタンス** ([probc] 以外の具体インスタンスで同じ 3 段を回す) | **`[探索+gate]`** | `## M5 (T3b)` (⭐ **別インスタンス族と生きた問いを供給する唯一の 1 本**) / `## N6 (T3c)` (方法) / `## M14 (T3b)` (⚠ **同じ [probc] 上の別の対であって別インスタンスではない** — 供給するのは明示 witness の方法) / `## L10` (⚠ `human-judgment (primary)` の逐語読解のみ) | `probe-failed` (`C` の閉形が書けるインスタンスが無い) / `known-result` (選んだインスタンスが [probc] の鏡像で新情報ゼロ) |
| 3 | **(γ) `(R2)` / `(R3)` を層 3 へ載せる** | **`[層3へ載せる]`** | `## N7 (T3c)` N7-b (合成後の上界 `h_Thm7(0,1,t) ≤ 2C + Ω(t)`。⚠ **`(R1)`/`(R2)`/`(R3)` の逐語は [`bc-t3c-n7-shoulder-certificate.md`](bc-t3c-n7-shoulder-certificate.md) §2.1**) / `## N8 (T3c)` N8-b (載っていない 2 本の名指し) / `## N2 (T3c)` (形式化債務の員数と強度 diff) | `mathlib-wall` (`sorry` + `@residual(plan:bc-open-problem-t3c-plan)` で honest に退出) / `too-hard`。⚠ **領域 def / 計算可能性層に手を出したら判断ログ 5 のテスト可能な線で停止して子 plan へ回す** |
| 4 | **`e > h(p)` 側** (N10 §4.1 の gate 出力) | **`[探索+gate]`** ⚠ **`障害の構造` の要件 (a) は満たす** (N10 §4.2-1 が障害を対象化し N6-j / N7-g (4) と同一機構と同定) | `## N10 (T3c)` N10-e (半空間の射程) / `## N7 (T3c)` N7-p (`t` を含まない 3 変数の不等式) / `## N6 (T3c)` N6-c (more capable の向きと Theorem 3 の適用条件) / `## N11 (T3c)` N11-b | `numeric-counterexample` / `probe-failed`。⚠⚠ **反証条件 3 本の SoT は [`bc-t3c-n10-epsilon-zero.md`](bc-t3c-n10-epsilon-zero.md) §4.2 であり、本書は参照のみ — 複製も書き換えもしない** |

### 4.1 候補 1 の型 (⚠ 起票の反証条件 (2) は不発火)

**アーク型 3 つのうち `障害の構造` が当たる**:

- **`定義` ではない** — 新しい対象の def が提案されていない。
- **`言い換え` ではない** — 新語彙も、元の問いとの同値も提案されていない。
- ⭐ **`障害の構造` である** — §5.3 が要求する 2 要件は facts N6-j が**同じ行で**満たしている: **障害の名指し** = 「却下の load-bearing な前提は『上界に `C` が要る』ではなく『**NO-GO は比較であり比較の相手が要る**』であり、一般 BC では `C` が未知だから」(同じ機構が N7-b の notes / N10 §4.2-1 から**同一対象として**指されている) / **回避条件** = 同じ行の「⭐ **反証の残骸**: 「**`Thm7` 側の上界だけを一般 BC へ持ち上げる**」は (γ) とは別の的として **well-posed**」(⚠ **機械の裏つき** — [probc] でないランダム BC 40 本で残差 `2.887e-15` の**チャネル非依存の恒等式**)。
...
⟹ **候補 1 は `障害の構造` 型として候補表に載る** (反証条件 (2) は不発火)。⚠⚠ **ただし N6-j 自身が「§0 への payoff は無い」と併記している** — `Thm7(W)` の一様実効コンパクト性は `## L2 (T3)` 行 6 で既に無条件の定理ゆえ、この的は計算可能性側に何も足さない ⟹ **`restatement` で死ぬ公算が高い**。⚠ **推すかどうかは配分の仕事であり本 leg は決めない**。

### 4.2 候補 4 の型についての限定 (⚠ 起票の見立て「緩める」への回答)

起票の**緩める**方向の見立ては「**`e > h(p)` 側は A2 の投入先候補として生きている**」であった。
**型照合の結果は「判定枠型は付く。⚠ アーク型 (`障害の構造`) は要件 (a) を満たすが要件 (b) が候補 4 固有の言葉で書かれていない」** — すなわち:

- **候補表には載る** (反証条件 (2) は「アーク型にも判定枠の 3 段にも型が付かないもの」を外すと定めており、本候補は判定枠型を持つ)。
- ⚠ **A2 の型として宣言するには、`障害の構造` の要件 (b) (回避条件) を本候補の言葉で 1 行書く必要が在る** — 機構は N6-j と同一なのでその回避条件がそのまま当たるが、候補 4 固有の形では書かれていない ⟹ **現状は宣言できないが、1 行の追記で宣言可能な状態である**。
- ⟹ **投入先として型が合うのは N16 (未配分) か、次の判定枠の組である**。⚠ **どちらに配るかは本 leg では決めない** (配分は起票の仕事)。

⚠ **この結論は起票の見立てが予告したとおりである** (「この候補は判定枠型であってアーク型ではない公算があり」)。⚠ **ただし「アーク型ではない」の部分は成り立たない** — `障害の構造` の要件 (a) は N10 §4.2-1 が満たしている (訂正 5)。
...
| **計** | — | **191 系統 + 反証 5 本** | — | **36** | **1** | **14** |

**読み (⚠ 判定ではない)**: **訂正 36 件のうち主判定を動かしたのは 1 件 (2.8%)** であり、**上方修正が 14 件 (38.9%)** ある。
...
⚠⚠ **「主判定」の指す対象は leg ごとに違う** (N6 = 面の数え方 / N7 = `ε` と NO-GO / N8 = Lean の命題 / N9 = 錐 / N10 = `ε = 0` / N11 = 層 3 化) ⟹ **`1/36` は同質な比率ではない**。とくに N10 の訂正 4 は決着の射程を曲線から半空間へ広げ**親 plan の判断ログ 7 になっている**が、この列では `0` に数えられている。
...
| **N12 (本 leg)** | **3 本** (plan の起票ブロックに事前記載) | **(1) 不発火** (§1 の 17 行はすべて機械で裏が取れた。⚠ **pass 本数・所要時間とも台帳と一致**) / **(2) 不発火** (候補 1 = 一般 BC にも `障害の構造` の型が付いた、§4.1) / **(3) 不発火** (新しい GO / NO-GO は 1 本も出していない) |
...
- **締める方向 = 計 18 本** (N6 4 / N7 5 / N9 5 / N10 3 / N11 1)。**完全に覆った 10 本 (56%) / 一部覆った 1 本 / 覆らなかった 7 本**。
  ...
  ⚠⚠ **この 56% は分類 1 つで動く** — 「一部覆った」を N9 見立て 5 だけに与えているが、N7 見立て 8 (「覆った (この方向に限る)」) と N10 見立て 1 (「当たり、ただし表現に誤り 1 か所」) も同型であり、そう数えると **完全 8 本 (44%) / 一部 3 本**になる ⟹ **母集団だけでなく分類基準も先に固定しないと数字は動く**。
...
- ⟹ **報告としての結論は 2 行**: **(i) §4.4 が「非対称なのは締める方向が全件覆るという一方だけ」と書いた、その一方が判定枠では崩れた** (56%、⚠ **分類を揃えると 44%**)。⚠⚠ **ただしこの低下を「§4.4 の主張が偽だった」と読んではならない** — **§4.4-1 が締める側に課した「着手前に 1 行の反証を機械に掛ける」義務そのものが交絡である** (N10 / N11 の締める側は着手前反証を実行したうえで覆らなかった 4 本である) ⟹ **前 relay の母集団にはこの介入が無い** ⟹ **比べているのは同じ量ではない**。
...
4. **Lean 側で証明本体を読んだのは N8 の 3 本だけである** — `MoreCapableBinary.lean` の **410 行 / 35 宣言**は**読んでいない**。N11 の 2 本については `#print axioms` + 署名 + import 一覧までである (⚠ CLAUDE.md §4.2 の「署名でなく証明本体を読む」は N11 について未実施)。
5. **各監査の訂正件数を数え直していない** — §5 (a) の 36 件は**各監査 / 台帳の自己申告を合計したもの**である。「主判定を動かしたか」の分類も各監査の自己申告であり、本 leg が独立に判定したものではない。
6. **`σ_x := d*_t − ψ_t(x) ≥ 0` (N10 §2.4) を再導出していない** — §2-6 が名指しする限定は N11 監査の指摘をそのまま引き継いだものであり、**本 leg も監査も達成性の議論を再導出していない**。
7. **§4 の型付けは我々の判定であり、機械には掛かっていない** — とくに候補 1 の型 (`障害の構造`) は §5.3 の要件との照合であって、反例や恒等式による判定ではない。
8. **`e > h(p)` 側の反証条件 3 本 (N10 §4.2) の中身を評価していない** — 参照しただけで、成立可能性も難度も測っていない。
9. **候補表の「接続先」は節名の実在確認までである** — その節の**内容**が候補に実際に使えるかは見ていない (⚠ **本 leg は facts のタグの実在を機械で確かめてもいない**。N12 監査が独立に走査し **34 タグ中 34 実在**を確認した)。
10. **BC ディレクトリの `@[entry_point]` 84 本のうち、判定枠 6 leg 由来でない 79 本が何を述べているかを見ていない** — 「レート領域の包含を述べたものは 1 本も無い」という §1-7 の確定は**判定枠が載せた 5 本についてのみ**であり、79 本については何も言っていない。
11. **N8 に反証条件と見立てが無かったことの影響を評価していない** — §5 (b) は事実として記録したが、実害の有無は判定していない (⚠ **判定は次の leg へ回す** = 起票の反証条件 (3))。
12. **A1 (N3–N5) を棚卸しの対象にしていない** — 本 leg の対象は判定枠 6 leg (N6 … N11) であり、A1 の終端判定と閾値較正 (§5.3) は N5 が既に行っている。`kappa2` 系検証器 2 本を実行したのは 11 本の内数としてであって、A1 の棚卸しではない。
13. **層 3 の 5 本が「N10 / N7 の散文の連鎖に実際に接続しているか」を本 leg では確かめていない** — N8-b / N11-b が接続を記録し、N11 監査の `F1` / `F2` が独立に確認したと台帳が書くが、**本 leg はその確認を再走していない**。
14. **`η = 1e-13` (N1 の DC 分枝限定) を誰も再実行していない** — §1-9 の「条件つき」はこの値に依るが、本 leg も N6 監査も台帳の引用に留まる。
15. **`@[entry_point]` の数え方の盲点 (多属性形 / インライン形) を本 leg では走査していない** (N12 監査が独立に走査し、窓と BC 配下では空だと確認した)。
...
> 監査 = [`bc-t3c-n12-audit.md`](bc-t3c-n12-audit.md) (⚠ **本節は反映の記録であり、監査そのものを事後に書き換えてはいない**)。

- **総合判定 = 訂正あり生存**。破りに振った **34 系統** (A 14 / B 7 / C 7 / D 3 / E 3) のうち **成立 17 / 不成立 17** (⚠ **成立 17 の内訳は 完全成立 13 + 一部成立 4** = A2-a / C2-c / D2 / D3 ⟹ `13 + 4 + 17 = 34`)。
- **訂正 17 件**を本書へ反映した。うち **主判定を動かすもの 1 件** (**訂正 4** = 候補 1 の型が `型なし` → `障害の構造` に覆り、§5 (b) の反証条件 (2) が発火 → **不発火**へ反転) / **上方修正 3 件** (訂正 1・7・9 = 本 leg が自分または資産を過小に書いていた側)。
- **独立検証器** [`verifiers/n12_audit_stocktake.py`](verifiers/n12_audit_stocktake.py) = 監査の実行で **34/34 passed**、EXIT=0。⚠ **本節の反映後に再実行しても全数 PASS** (`--skip-slow` で **32/32 passed** = T20 / T21 を除いた全数。⚠ **本 leg は 8 分かかる全数版を再実行していない**)。
- **監査の Lean probe 2 本を資産化した** — [`probes/t3c-n12/he-is-conclusion-pointwise.lean`](probes/t3c-n12/he-is-conclusion-pointwise.lean) (A4-a + A4-b) / [`probes/t3c-n12/hmarkov-equiv-condmi-zero.lean`](probes/t3c-n12/hmarkov-equiv-condmi-zero.lean) (A4-c)。**両方 `lake env lean` EXIT=0** で再実行できる (訂正 11 / 12 の機械的な裏)。
- **起票の反証条件の最終状態**: **(1) 不発火** / **(2) 不発火** (⚠ **訂正 4 で発火から反転**) / **(3) 不発火**。
- ⚠⚠ **監査自身が確かめていない 12 点の SoT は [`bc-t3c-n12-audit.md`](bc-t3c-n12-audit.md) §3 である** — 本書はそこへ**複製しない** (§6 が記録するのは本 leg の穴であって監査の穴ではない)。
