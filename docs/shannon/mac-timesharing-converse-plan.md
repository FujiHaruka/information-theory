...
- [x] Gap 0 — code→ambient bridge ✅ **proof-done, sorryAx-free**（commits `026d9457`→`946130a4`→`3377eba5`、`TimeSharingConverse.lean` `section CodeToAmbient`）。ambient `macConverseAmbient c W = macConverseInput ⊗ₘ (per-letter product channel)` を構成し、`mac_converse` の全 hyp（3 uniform marginal / memoryless / markov / indep）を genuine discharge、bridge `mac_converse_from_code` が end-to-end sorryAx-free。汎用 helper `isMarkovChain_of_compProd_encoder` / `isMemorylessChannel_of_compProd_pi`（+ `lintegral_pi_reRandomize`）を切出し、hκ = product 形の構造 precondition は use site で `fun m ↦ rfl` discharge（load-bearing でない、**独立監査 PASS**、`@audit:ok` ×7）。
- [ ] Gap A — 弱 converse 極限抽出（ε'→0/n→∞、Fano/n→0、R₁=0 軸 casework）📋 **← 次**
...
| Gap | 内容 | 新規? | LOC | 攻略順 |
|---|---|---|---|---|
| **Gap 0 ambient** | bare `MACCode` から `μ = uniform(Fin M₁ × Fin M₂) ⊗ per-letter channel W` を構成、`mac_converse` の hyp（uniform / memoryless / indep / markov / hcard）を discharge。`IIDAmbient.lean:64 macAmbientMeasure` は **product-input i.i.d. ambient**（固定 `p₁⊗p₂` の n 重）= **流用不可**（converse 入力は任意 code、`c.encoder` 経由の任意 joint input）。**greenfield、真の起点** | ✅ done | 200-300 | ~~2~~ ✅ |
...
- **Gap 0 = code→ambient bridge は proof-done sorryAx-free**（confidence machine、再検証:
  `lake env lean InformationTheory/Shannon/MultipleAccess/TimeSharingConverse.lean` silent +
  `#print axioms mac_converse_from_code` = `[propext, Classical.choice, Quot.sound]`、commit `3377eba5`）。
  memoryless/markov は condDistrib 明示計算だが**壁でなく self-build**（予測どおり、single-user
  `per_letter_markov_of_memoryless` が template）。汎用 helper 2 本の hκ = product 形は use site
  `fun m ↦ rfl` で load-bearing でない（独立監査 PASS `@audit:ok`）。
