...
**Status**: Phase A done ✅ — 鞍点 + gateway atom genuine closure（proof done）。`klDiv_channel_le_capacity` +
gateway atom `mutualInfo_segment_hasDerivAt`（`StrongConverseAsymptotic.lean`）の body を genuine 化、
`@residual(plan:capacity-saddle-point)` 除去、`@audit:ok`（commit `3db9d443`、`bb21942a` で certify）。
本 plan は完遂、親 [`channel-coding-strong-converse-plan.md`](channel-coding-strong-converse-plan.md) の Wolfowitz 強逆 headline へ load-bearing core を供給済。
...
- [x] keystone — 恒等式 `I(p;W).toReal = ∑_x p(x)·klDivPmf (W x) (q_p)` 自作 ✅
- [x] (ii) gateway atom `mutualInfo_segment_hasDerivAt`（**片側** `HasDerivWithinAt (Set.Ici 0) 0`）✅（envelope cancellation genuine、commit `3db9d443`）
- [x] (iii) `IsMaxOn` ⟹ 片側微分 ≤ 0 ⟹ `klDivPmf (W a) q* − C ≤ 0` → `klDiv_channel_le_capacity` closure ✅
...
### keystone — 恒等式 `I(p;W).toReal = ∑_x p(x)·klDivPmf (W x の pmf) (q_p の pmf)` ✅
...
### (ii) gateway atom `mutualInfo_segment_hasDerivAt` ✅（最難、genuine closure）
...
- gateway-atom-first（壁公算を先に反証）: この atom を最初に建てて壁判定を覆した。genuine closure 済（commit `3db9d443`、動く reference の envelope cancellation が `HasDerivWithinAt` 形で genuine に出た = Mathlib 壁でなく in-project KKT 開発で割れた）。

### (iii) 一次最適条件 → `klDiv_channel_le_capacity` closure ✅

**proof-log: no**（template 転用）。

- `IsMaxOn` ⟹ 右側方向微分 `≤ 0`（`csiszar_first_order_condition` の `IsMinOn`⟹符号を **不等号反転** して転用）→ gateway atom の値 `klDivPmf (W a) q* − I(p;W).toReal ≤ 0` → `I(p;W) = capacity W`（`p` が達成点）で `klDivPmf (W a) q* ≤ capacity W`。
- code 側 `klDiv_channel_le_capacity` の `sorry` を除去、`@residual` 除去 → proof done（commit `3db9d443`、certify `bb21942a`）。
...
2. **R-CSP1 決着（行使せず closure）**: gateway atom が `HasDerivWithinAt` 形で genuine に出たため、退避口（body のみ `sorry` + `@residual(plan:capacity-saddle-point)`、鞍点を predicate に bundle するのは禁止）は **行使されず** Phase A 完遂（commit `3db9d443`）。Proposed wall `capacity-saddle-directional-derivative` の promote も不要。
