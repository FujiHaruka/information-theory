...
[root B 配線(Phase B)= 下流新モジュール移設 + transport reduction]
  ⚠ 訂正(orchestrator が import 方向を実測): headline は
    ...
    LC2Discharge → Cramer の transitive import で **Cramer.lean を import 済**。
    よって Cramer.lean 内 in-place で `exact headline` は cycle になり不可。
  b0. root B chain 3 decl(cramer_lower / cramer_lower_legendre / cramer_tendsto、
      外部 consumer ゼロの閉鎖 chain)を Cramer.lean から新下流モジュール
      InformationTheory/Shannon/CramerGeneralLower.lean へ移設(namespace
      InformationTheory.Shannon.Cramer 維持で consumer 参照は名前解決)。
      import = CramerCltBoundaryClosure(headline)+ Shannon.Cramer.Cramer。
  b1-b6. iid joint law = 共通周辺の infinitePi(iIndepFun_iff_map_fun_eq_infinitePi_map,
      Mathlib)→ IdentDistrib.map_eq で各周辺統一 → event 移送(Measure.map_apply)
      → cgf 同定(cgf_eval_eq_cgf_base を直接利用、self-build 不要)。
      transport は **Ω₀:=Ω, μ₀:=μ, Y:=X 0** で取る(`Y:=id` は id が global に非有界で
      headline の h_bdd を満たせないため不可)。root B / canonical IID copy 両 joint law が
      共通の `infinitePi (μ.map (X 0))` を経由して event mass 一致。
  + root B signature に hVar 追加 → headline へ `exact` で discharge。
```
...
- [ ] **検証点**:`lake build InformationTheory.Shannon.CramerBoundaryUpstream` で新 module が clean、
  `lake env lean InformationTheory/Shannon/Cramer/InfinitePiTiltedChangeOfMeasure.lean` で旧 file が
...
- [ ] **検証点**:`lake build InformationTheory` で全 module 連鎖が clean(cycle error なし)。
  cycle が消えたことを `lake env lean InformationTheory/Shannon/Cramer/LC2PhaseC.lean` の
...
## Phase B — root B chain 下流移設 + transport reduction + `hVar` 追加で discharge ✅

proof-log: yes(cgf transport と joint-law 移送の型クラス整合を記録)

> **実装訂正(2026-06-11、達成)**: 当初「Cramer.lean 内 in-place で transport + `exact headline`」
> としていたが、headline が transitive import 上 Cramer.lean を import 済(orchestrator 実測)で
> in-place は cycle になり **不可**。実装は root B chain 3 decl を **下流新モジュール
> `InformationTheory/Shannon/CramerGeneralLower.lean`(namespace `Cramer` 維持)へ移設** してから
> transport で discharge。transport は `Ω₀:=Ω, μ₀:=μ, Y:=X 0`(在庫の `Y:=id` 案は id が global に
> 非有界で headline `h_bdd` を満たせず却下)。cgf 同定は `cgf_eval_eq_cgf_base` を直接利用(self-build 不要)。
> 3 decl すべて `#print axioms` = `[propext, Classical.choice, Quot.sound]`(sorryAx-free)。
...
- [ ] `lake env lean InformationTheory/Shannon/Cramer/Cramer.lean` clean(0 sorry)。
...
InformationTheory/Shannon/
  CramerBoundaryUpstream.lean            ← Phase A で新規(8 decl hoist 先、新規証明ゼロ)
  CramerCltBoundaryClosure.lean          ← 既存(headline、変更なし)
  CramerGeneralLower.lean                ← Phase B で新規(root B chain 3 decl 移設先、
                                            headline 下流。transport + hVar 追加 + discharge)
InformationTheory/Draft/Shannon/
  ...
  Cramer.lean                            ← root B chain を CramerGeneralLower へ移設(削除)。
                                            cramer_upper / cramer_upper_legendre 等は据置
...
InformationTheory.lean                          ← 新 module 2 本(Upstream / GeneralLower)の import 行
docs/shannon/
...
2. **2026-06-11 Phase B 達成(root B closure、full closure 到達)**:root B chain を **下流新モジュール
   `CramerGeneralLower.lean` へ移設** + transport で discharge。**plan 訂正点**:
   - **in-place 不可**(import 方向 orchestrator 実測):headline は transitive import
     `CramerCltBoundaryClosure → CramerBoundaryUpstream → LC2DischargeExt → LC2Discharge → Cramer` で
     Cramer.lean を import 済。よって当初 plan の「Cramer.lean 内 in-place transport + `exact headline`」は
     cycle になり不可 → root B chain 3 decl(外部 consumer ゼロの閉鎖 chain、`dep_consumers.sh` 実測)を
     headline 下流の新モジュールへ移設してから discharge する方針に変更。
   - **`Y:=id` 案却下**:在庫の transport 案は `Ω₀:=ℝ, Y:=id` だが、id は global に非有界で headline の
     `h_bdd : ∃ M, ∀ ω, |Y ω| ≤ M` を満たせない。`Ω₀:=Ω, μ₀:=μ, Y:=X 0` に変更(`h_bdd` は root B の
     bounded 仮定の i=0 instance で充足)。root B joint law と canonical IID copy(`X 0 ∘ eval`)joint law が
     共通の `infinitePi (μ.map (X 0))` を経由して event mass 一致。
   - **cgf self-build 不要**:`Y:=X 0` 採用により cgf 同定は in-project `cgf_eval_eq_cgf_base` を直接利用
     (在庫予想の ~10 行 self-build は不要)。
   - **結果**:`cramer_lower` / `cramer_lower_legendre` / `cramer_tendsto`(entry_point)すべて
     `#print axioms` = `[propext, Classical.choice, Quot.sound]`(sorryAx-free)。project 全体 sorry 16→15。
     `lake build InformationTheory` green(cycle なし)。両 root closure 達成 = L-WIRE3 部分達成でなく full closure。
