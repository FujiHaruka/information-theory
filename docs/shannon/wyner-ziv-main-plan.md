...
- [ ] P3 — achievability body (binning + covering ハイブリッド)。gateway atoms (leg 12) + 3 leaf atoms (leg 14) CLOSED sorry-free。**leg 15: S1/S2/C/E 4 sub-lemma + top-level reduction (E∘BD) を sorry-free closure**。**leg 16: BD steps 1-2 seam を sorry-free closure (`wz_coveringDistortion_reconcile` + `wz_coveringFamily_of_testChannel`、`@audit:ok`、独立監査 ALL-OK `4b7f6441`)**。残 = 唯一の sorry = BD `wz_perDelta_codes_exist` の **steps 3-7** (~L1043 body、単一 sorry L1084) — 次 leg 本線 = steps 3-7 (WZ code 構成 + 3 error exponents、cap 延長要) 🚧
