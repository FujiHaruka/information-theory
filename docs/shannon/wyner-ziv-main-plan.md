...
**Status**: ACTIVE 🚧 — **converse (P2) は FULLY CLOSED sorry-free + 独立監査 PASS (leg 11)**。最後の converse own-sorry `wz_support_reduce` を route C (K=`|α|+3`、bare ambient Carathéodory) で genuine closure → **`wyner_ziv_converse` は sorryAx-free (`#print axioms` = `[propext, Classical.choice, Quot.sound]`、`@audit:ok`)**。converse own-sorry は **ゼロ**。**achievability P3: gateway atoms (leg 12) + 3 leaf atoms (leg 14、`wz_nonempty_of_factorizable`/`wz_fullKernelSupport_perturbation`/`wz_tendsto_exp_mul_codebookSize_inv`) closed sorry-free + 監査 `@audit:ok`。残 = 唯一の sorry `wz_goodCode_exists_of_testChannel:514` (covering+binning core) — 本 leg で S1/S2/C/BD/E の 5 sub-lemma skeleton に分解。leg 14 finding (b): heavy core は covering source を subtype `{x // 0<P_X x}` に restrict し globally-full-support (X,U) joint を RD covering に供給する必要 (settled-facts)**。情報側 R_WZ(D) は完成済 (`WynerZiv/` 5 file、0 sorry)。
...
- [ ] P3 — achievability body (binning + covering ハイブリッド)。gateway atoms CLOSED (leg 12) + codes lemma 分解 (leg 13) + **3 leaf atoms sorry-free (leg 14、`@audit:ok`)**。残 = 唯一の sorry `wz_goodCode_exists_of_testChannel:514` (covering+binning core) を **S1/S2/C/BD/E の 5 sub-lemma skeleton に分解 (leg 14 本 leg)**。finding (b): heavy core は covering source を subtype `{x // 0<P_X x}` に restrict する必要 (settled-facts)。次 leg 本線 = (S1) 制限被覆 joint full-support (leaf 先埋め) 🚧
...
- leg 14 (`d5194840`) — covering+binning core が消費する **3 leaf atoms を sorry-free で closure** (`@audit:ok`)、new residual なし (残 sorry は `wz_goodCode_exists_of_testChannel:514` 唯一): `wz_nonempty_of_factorizable:281` (`Nonempty (Fin k)` from row-sum) / `wz_fullKernelSupport_perturbation:311` (feasible `qf` + slack δ → full-support **kernel** `0<κ'` + factorizable + obj<R + dist≤D+δ) / `wz_tendsto_exp_mul_codebookSize_inv:432` (`exp(nc)·codebookSize⁻¹→0`、SW private の public re-proof)。
...
### 構築本体 skeleton (skeleton-driven、leaf 先埋め)

`wz_goodCode_exists_of_testChannel:504` の monolithic sorry を ordered sub-lemma に分解。**全 sub-lemma の撤退口 = `sorry + @residual(plan:wyner-ziv-main-plan)` のみ** (class は `plan` 固定、in-project atom ゆえ `wall` 不可)。各 signature は regularity/feasibility hyp のみ、mathematical core は body に置き `*Hypothesis`/`*Reduction`/`IsXxxClaim` に **bundle しない**。leg 15 は下記を sorried sub-lemma として並べ (skeleton)、leaf から埋める。

- [ ] **(S1) 制限被覆 joint の global full support [LEAF、leg 15 推奨第一]**。`κ'` (leaf `:311` 供給) と `P_X x = ∑_y P_XY.real{(x,y)}` から、subtype `α'` 上の (X,U) joint を global full-support pmf として構成。**consume**: `wz_fullKernelSupport_perturbation:311`、`wzMarginalXU` (`Basic.lean:106`)、`stdSimplex`/`Subtype.fintype`。**proposed 署名**:
  ```lean
  -- α' := {x : α // 0 < ∑ y, P_XY.real {(x, y)}}
  private lemma wz_restrictedCoveringJoint_pos
      ...
      Nonempty {x : α // 0 < ∑ y, P_XY.real {(x, y)}}
        ...
        ∧ (fun p : {x : α // 0 < ∑ y, P_XY.real {(x, y)}} × Fin k ↦
            ...
            ∈ stdSimplex ℝ ({x : α // 0 < ∑ y, P_XY.real {(x, y)}} × Fin k)
  ```
  leaf: subtype instances (`Fintype`/`DecidableEq`/`MeasurableSingletonClass` は `Subtype` から derive、`Nonempty` は `∑_x P_X x = 1 > 0` から) + `κ'·P_X > 0` (α' 上で両因子 >0) + row-sum → simplex。**trap**: なし (出力は純 regularity)。~40–80 行。

- [ ] **(S2) block-distortion source-support WLOG reconciliation [HEAVY-tractable]**。full-support source sequence 上で一致する 2 code は expected distortion が等しい (`Measure.pi P_XY` が P_X ゼロ atom 到達 sequence にゼロ mass) — これが α'-code → α-code 延長を可能にする null-set transport。**consume**: `Measure.pi_map_pi` (Mathlib)、`rdAmbient_map_iidXs:165`、`WynerZivCode.expectedBlockDistortion` (`Basic.lean:73`)。**proposed 署名**:
  ```lean
  private lemma wz_expectedBlockDistortion_source_agree
      ...
      c₁.expectedBlockDistortion P_XY d = c₂.expectedBlockDistortion P_XY d
  ```
  heavy-tractable: 有限積 null set (`{x^n | ∃i, P_X(x i)=0}` は `Measure.pi P_XY`-null) 上の a.e. 一致 → 積分等式。**trap**: `hagree` は genuine agreement precondition (bundling でない)、結論は measure 等式のみ = covering 下界を **含めない**。~60–120 行。(S2) は (C) の LossyCode→WynerZivCode bridge と合成する。

- [ ] **(C) covering layer [HEAVY]**。α'-restricted global-full-support (X,U) joint に `rate_distortion_achievability` を適用 → `LossyCode` → (B3 adapter で) covering encoder + codeword map。**consume**: `rate_distortion_achievability:184` (full quintet + `qZ_min` §B1)、`rdAmbient` + marginals §B4、`lossyCodeOfCodebook`/`jointTypicalLossyEncoder` §B2、bridge `Measure.pi_map_pi` §B3。**proposed 署名** (RD の slack quintet `ε_X ε_join ε_dist δ_kl δ_typ`/`qZ_min` は body 内で construct = 露出しない):
  ```lean
  private lemma wz_covering_lossyCode_exists
      ...
      ∃ N, ∀ n, N ≤ n → ∃ M, Nat.ceil (Real.exp ((n:ℝ)*R₁)) ≤ M ∧
        ∃ c : LossyCode M n α' (Fin k),
          c.expectedBlockDistortion ((rdAmbient qStar).map (iidXs 0)) d' ≤ D + ε'
  ```
  heavy: quintet consistency 選択 (inventory「consistent choices exist for any full-support qStar with `mutualInfoPmf<R`」) + `rdAmbient` bundle。**trap**: RD の slack quintet を本 sub-lemma の **仮説に露出しない** (露出すると bundling 様に読める) — body で construct。「covering succeeds」を predicate に bundle しない。露出可なのは `hpos` (full-support regularity) + `hI` (rate feasibility) + `hfeas` (distortion feasibility) のみ。**subtle**: 被覆歪は X↔U proxy `d'`、WZ 実歪は X↔γ (per-letter `qf.2 (u,y)`) + side-info — この distortion-shape 整合は body / (BD) に残す load-bearing subtlety (bundle 禁止)。

- [ ] **(BD) binning + conditional-slice decoder assembly [HEAVY]**。covering codebook (C) + full-support (U,Y) marginal から per-n `WynerZivCode (codebookSize R n) n α β γ` を組み、E2/E3 を gateway 指数で bound。**consume**: `binningMeasure:62`/`binning_collision_prob:106`、`conditionalTypicalSlice:51`+`_card_le:140`、gateway `wz_sideInfo_decoder_confusion_expectation_le:105` (E2) / `wz_covering_sideInfo_mass_ge:143` (E3)、`rdAmbient` on `wzMarginalYU:110` (§B4 gap #2)、`codebookSize_neZero`。**two-ambient subtlety** (§B4/gap #2): covering ambient = `rdAmbient(wzMarginalXU qf'.1)`、side-info ambient = `rdAmbient(wzMarginalYU qf'.1)` — 同一 3-var `qf'` の 2 marginal で rate split `R=I(X;U)−I(Y;U)` が一貫。**trap**: gateway atom の i.i.d. bundle (`iIndepFun`/`IdentDistrib`/`hpos*`/`hmarg_*`) は `rdAmbient_*` lemma から body で discharge (regularity)、「decoder が codeword を復元する」を predicate に bundle しない。

- [ ] **(E) error→0 + pigeonhole + squeeze + diagonalization [HEAVY]**。per-n 歪超過 `≤ distortionMax·P(error)` + 3 誤り指数減衰 → `∀ε>0 ∀ᶠn, expectedBlockDistortion ≤ D+ε`、τ→0 / η_n→0 diagonalization で全 ε 到達。**consume**: `source_avg_distortion_le_simpler:203`、`exists_codebook_low_avg:138` (pigeonhole)、`ceil_exp_mul_exp_neg_tendsto_atTop:40` (E1/E3)、`wz_tendsto_exp_mul_codebookSize_inv:432` (E2、leg-14 leaf)、`exp_neg_tendsto_zero_of_tendsto_atTop:78`、`codebookSize_log_div_tendsto:241`。**trap**: 「P(error)→0」を仮説に bundle しない — 指数 atom から導出。**diagonalization**: perturbation slack δ (leaf `:311`) と typicality ε_n を同時に 0 へ送る (τ→0/η_n→0) 順序が honesty-critical (中間 slack を仮説化しない)。

**最終組立**: (S1)→(C)(α' 上)→(S2)(α 延長)→(BD)→(E) を合成して `wz_goodCode_exists_of_testChannel:514` の sorry を埋める。
...
- **予想規模**: ~800-1500 行 (本計画の重心、唯一の重い合成 gap + source-support restriction 新 subtask)。
...
9. **P3 construction 分解 + finding (b) + full-support 不追加 crux (leg 14、active、本線)**: `wz_goodCode_exists_of_testChannel:514` の唯一の sorry を **S1/S2/C/BD/E の 5 sub-lemma skeleton** に分解 (P3 §、次 leg 本線 = leaf 先埋め)。leg 14 で 3 leaf atoms を sorry-free closure (`wz_nonempty_of_factorizable`/`wz_fullKernelSupport_perturbation`/`wz_tendsto_exp_mul_codebookSize_inv`、`d5194840`、no new residual)。**finding (b) source-support** (settled-facts): perturbation は full-support **kernel** `0<κ'` しか与えず (X,U) joint は P_X ゼロ atom で消える → RD covering は GLOBAL full-support 要求ゆえ heavy core は covering source を subtype `α':={x // 0<P_X x}` に restrict する必要 (block distortion は `Measure.pi P_XY` 下で WLOG)。**crux (leg 13 由来、凍結)**: `wz_goodCode_exists_of_testChannel` の signature に **full-support 仮説を追加しない** — support-restriction は proof-internal (subtype α' 経由) であって missing hypothesis ではない (RD unconditional 版に対応)、追加すると load-bearing precondition が caller に移り headline honesty を壊す。両退化反証 FAIL (D<0→hqf 空、R≤0→hobj 空)、statement は TRUE-as-framed (独立監査 leg 13 ALL-OK)。
