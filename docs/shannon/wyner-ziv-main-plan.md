...
- [ ] P3 — achievability body (binning + covering ハイブリッド)。gateway atoms (leg 12) + 3 leaf atoms (leg 14) CLOSED sorry-free。**leg 15: S1/S2/C/E 4 sub-lemma + top-level reduction (E∘BD) を sorry-free closure (`@audit:ok`、独立監査 ALL-OK `0bb1bf38`)**。残 = 唯一の sorry (BD) `wz_perDelta_codes_exist` (covering+binning giant、~L873) — 次 leg 本線 = BD を sub-lemma に skeleton 分解して fill 🚧
