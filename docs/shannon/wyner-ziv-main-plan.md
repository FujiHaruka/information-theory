...
**Status**: P3 Proposal A — **Markov-core chain 全体 CLOSED (2026-07-12 Leg 13、machine-verified sorryAx-free)、残 sorry = Atom G covering atom `wz_coveringFamily_of_testChannel` のみ** 🚧。strong-Ecov build (`d8954711`/`b489d51f`/`e4490dbb`): Ecov を strong-at-ε_cov∩weak-at-ε に強化 (radius separation `ε_cov=ε/(2(1+C))`、監査 SOUND) → core wrapper + conditional-AEP kernel `wz_covering_uyBand_condSlice_le` genuine 閉 → kernel/wrapper/outer/inner/leaf が独立 `#print axioms` で sorryAx-free 確認。次 = Atom G (covering atom を strong-at-ε_cov∩weak-at-ε covering-success 供給に reopening) → Atom H (PV+closure)。詳細 → 子 [`wz-binning-covering-plan.md`](wz-binning-covering-plan.md) / 孫 [`wz-markov-core-plan.md`](wz-markov-core-plan.md) §Atom G/H。以下は build 履歴: **achievability C2 covering chain は weak typicality で FALSE-AS-FRAMED だった** (core `wz_covering_jointBand_markov_core`→outer→inner→leaf に `@audit:defect(false-statement)` `1ddc2887`、label-swap 反例) が、**gateway `wz_wsm_negLog_mean_pin_of_stronglyTypical` (`7812cfcf`、sorry-free + sorryAx-free) が strong typicality で crux を機械確認** (strong typicality → empirical type を TV で pin → `M(xb)` が `H(wsm)` に pin ⟹ label-swap 死)。∴ **Proposal A (`Ecov` のみ strong joint typicality に強化) GO 確定**、covering-success 下界 reopening を伴う strong-Ecov build (孫 plan Atom E-H、E+F coupled が本線) が進行中。詳細 → 子 [`wz-binning-covering-plan.md`](wz-binning-covering-plan.md) §Leg F / 孫 [`wz-markov-core-plan.md`](wz-markov-core-plan.md) §Approach / `wz-facts.md`。headline 署名不変 (#9 crux、strength は定義由来)。**converse (P2) は無影響 (FULLY CLOSED sorryAx-free `@audit:ok` 維持)**。以下は build 前の P3 記述:
> **converse (P2) は FULLY CLOSED sorry-free + 独立監査 PASS (leg 11)**。最後の converse own-sorry `wz_support_reduce` を route C (K=`|α|+3`、bare ambient Carathéodory) で genuine closure → **`wyner_ziv_converse` は sorryAx-free (`#print axioms` = `[propext, Classical.choice, Quot.sound]`、`@audit:ok`)**。converse own-sorry は **ゼロ**。**achievability P3 (leg 15): S1/S2/C/E の 4 sub-lemma + top-level reduction を sorry-free closure (`@audit:ok`、独立監査 ALL-OK)。top-level `wz_goodCode_exists_of_testChannel` は E∘BD reduction (`wz_diagonalize_slack (wz_perDelta_codes_exist …)`、signature 不変 = full-support 仮説不追加 crux 維持)。**leg 16: BD の steps 1-2 seam を sorry-free closure** (`wz_coveringDistortion_reconcile` + `wz_coveringFamily_of_testChannel`、両 `@audit:ok`、独立監査 ALL-OK `4b7f6441`)。**leg 17: BD steps 3-7 を S3-S7 sub-lemma へ skeleton 分解** (`400f9308`/`f6f31b83`、独立監査 2 pass): S3 `wzCodeOfCoveringBinning` / S4 `wzBinTypicalDecoder`(+uniqueness) / S7 `wzLiftSupportCode` の def を sorry-free closure + BD body を capstone S6 への own-sorry ゼロ reduction 化。S5a body も sorry-free closure (`35941c54`、`@audit:ok`)。**leg 18: S5b crux `wz_codebook_confusion_expectation_le` を sorry-free closure** (`356c8f25`、`@audit:ok` 独立監査 PASS `eebd88f5`): outer-sum-first (biUnion over `m':Fin M₁` → per-`m'` Tonelli swap → `hcollision`+`hmass`)、regularity 前提 `hYs`/`htrueIdx` を Tonelli 用に追加 (load-bearing でない、S6 が discharge)、codebook-restricted union は body 残置 = bundling 回避。**leg 19: S6→(D)+(B) 分解** — D1 rate-identity CLOSED / D2 `wz_covering_codeword_sideInfo_mass_le` CLOSED sorry-free / (B) collision `wzIndexBinningMeasure_collision` CLOSED sorry-free、S6 body は (D) `wz_perDelta_covering_binning_eventual` への sorry-free glue 化。**leg 20+: D3 `wz_perN_covering_binning_code` の under-hyp を子 plan [`wz-binning-covering-plan.md`](wz-binning-covering-plan.md) で段階的に honest 化 — 子 Leg 0 (budget δ-split) / C.5 (reconciliation `hd'_eq`/`hqf`) / C.6 (M-axis `M ≤ exp(nR₁)+1` pin) の 3 軸解消済 + Leg D の G2/A1 (E2-only decomp + lift identity) `@audit:ok` + A2 `wz_ideal_expectation_eq_covering` sorry-free (`84393413`)。D3 body は sorry-free reduction (M-axis defect 除去 honest tier-2 復帰)**。**子 Leg E 署名 rework DONE + 独立監査 all-PASS** (`cf7d57cd`/`42abbf21`): 第4軸 (covering-acceptance C2) + 第5軸 (dα'-d scaling) を pinned-ε 形で署名レベル closure、A3 `@audit:defect` 除去 honest tier-2 復帰、A2 `@audit:ok`。**A3 `wz_exists_binning_E2_bound` = body genuine sorry-free (`d1f2445a`) + Leg E-mass `wz_source_codeword_sideInfo_mass_le` = sorry-free + sorryAx-free + `@audit:ok`** (`66417846` closure / `259ecccc` 独立監査 PASS) ゆえ **A3 sub-tree sorryAx-free**。**残 literal sorry 2 = Leg F の C2 covering-acceptance のみ** (両 `@residual(plan:wz-binning-covering)`): inner `wz_covering_markov_concentration` (Markov-concentration kernel、r9 新規 isolate) + covering atom `wz_coveringFamily_of_testChannel` (leaf 未配線ゆえ自身の sorry 残置)。**leaf `wz_covering_chosenWord_sideInfo_typical` は r9 で genuine outer decomposition (sorry-free、`9ecffb41`) 化 + false-as-framed fix 後 再監査 ALL-OK (`867972b2`)**: 旧 free-param 形 (qStar/κ' unconstrained) は constant-word 反例で偽 (独立 auditor CONFIRMED `3965ba5e`、旧 `d2e68b10` PASS overturn) → 3 regularity (`hκ'_pos`/`hκ'_sum`/`hqStar`) を leaf+inner に threading (precondition-exposure、covering atom が verbatim export で discharge、headline-safe)。**inner の build = L0-L5 (子 Leg F、L4 Band-Joint conditional concentration が唯一の難所、`plan` not wall、advisor: few-sessions closable)**。**次 (mainline) = inner を L0-L5 で build → leaf を covering atom に consume 配線 (covering-success 分解 + joint-derandomize) + stale docstring 訂正**。finding (b): covering source を subtype `{x // 0<P_X x}` に restrict (子計画 settled-facts)。情報側 R_WZ(D) は完成済 (`WynerZiv/` 5 file、0 sorry)。
...
- [ ] P3 — achievability body (binning + covering ハイブリッド)。gateway atoms (leg 12) + 3 leaf atoms (leg 14) CLOSED sorry-free。**leg 15: S1/S2/C/E 4 sub-lemma + top-level reduction (E∘BD) を sorry-free closure**。**leg 16: BD steps 1-2 seam を sorry-free closure (`wz_coveringDistortion_reconcile` + `wz_coveringFamily_of_testChannel`、`@audit:ok`、独立監査 ALL-OK `4b7f6441`)**。**leg 17: BD steps 3-7 を S3-S7 sub-lemma へ skeleton 分解** — S3/S4/S4-uniq/S7 def を sorry-free closure + BD body を capstone S6 への own-sorry ゼロ reduction 化 (`400f9308`)、S5a/S5b の under-hyp DEFECT を独立監査で検出→mass+collision 仮説追加で honest 化 (`f6f31b83`、再監査 OK)。S5a body も sorry-free closure (`35941c54`、`@audit:ok`)。**leg 18: S5b crux `wz_codebook_confusion_expectation_le` を sorry-free closure** (`356c8f25` + 監査 `eebd88f5`、`@audit:ok`)。**leg 19: S6→(D)+(B) 分解** (D1 CLOSED / D2 `wz_covering_codeword_sideInfo_mass_le` + (B) `wzIndexBinningMeasure_collision` CLOSED sorry-free)。**leg 20: D3 `wz_perN_covering_binning_code` が false-as-framed 判明** → 子 plan [`wz-binning-covering-plan.md`](wz-binning-covering-plan.md) へ移譲。**子 Leg 0/A/B/C/C.5/C.6 DONE** (`a59e37cb`/`dfdf3e42`/`02ea97d7`/`25629b0a`/`22b64afa`/`fe3d9482`+`90996ed1`、全 sorry-free + 独立監査 PASS): under-hyp 3 軸 (budget δ-split + reconciliation `hd'_eq`/`hqf` + M-axis `M ≤ exp(nR₁)+1` pin) を honest 化、D3 の `@audit:defect` 除去 honest tier-2 復帰。子 Leg D で **G2/A1 (E2-only decomp + lift identity) `@audit:ok` + A2 `wz_ideal_expectation_eq_covering` sorry-free `@audit:ok` (`84393413`、finite-sum marginalization)**。**子 Leg E 署名 rework DONE + 独立監査 all-PASS** (`cf7d57cd`/`42abbf21`): 第4軸 (C2) + 第5軸 (dα'-d scaling) を pinned-ε で署名レベル closure、A3 `@audit:defect` 除去 honest tier-2、A2 `@audit:ok`。**子 Leg E-A3 DONE (body genuine `d1f2445a`) + Leg E-mass `wz_source_codeword_sideInfo_mass_le` sorry-free + sorryAx-free + `@audit:ok`** (`66417846`/`259ecccc`) → A3 sub-tree sorryAx-free。**残 literal sorry 2 = Leg F の C2 のみ** (両 `@residual(plan:wz-binning-covering)`): inner `wz_covering_markov_concentration` (r9 新規 isolate) + covering atom `wz_coveringFamily_of_testChannel`。leaf `wz_covering_chosenWord_sideInfo_typical` は r9 で genuine outer decomposition (sorry-free `9ecffb41`) 化、旧 free-param 形は false-as-framed (独立 auditor CONFIRMED `3965ba5e`、旧 `d2e68b10` PASS overturn) → 3 hyp threading で fix + 再監査 ALL-OK (`867972b2`)。**Leg 13 UPDATE**: Markov-core chain を strong-Ecov build で全閉 (`d8954711`/`b489d51f`/`e4490dbb`) — inner/leaf を含む core→outer→inner→leaf + kernel `wz_covering_uyBand_condSlice_le` が machine-verified sorryAx-free。**残 literal sorry 1 = Atom G covering atom `wz_coveringFamily_of_testChannel` のみ**。次 (mainline) = Atom G = covering atom を `wzCoveringSuccessStrong` (strong-at-ε_cov∩weak-at-ε) covering-success 供給に reopening + strong-Ecov leaf consume 配線 + stale docstring 訂正 → Atom H (PV+closure) 🚧
...
12. **D3 under-hyp を子 plan で honest 化、A3/E2b 側 DONE、残 literal sorry 2 = Leg F の C2 covering-acceptance (leg 20 発見 → 子 Leg 0-F、active、本線)**: D3 `wz_perN_covering_binning_code` の exact `≤ D+δ` は当初 under-hypothesized で false-as-framed → 子 plan [`wz-binning-covering-plan.md`](wz-binning-covering-plan.md) で under-hyp 4 軸を honest 化 (budget δ-split / reconciliation / M-axis pin / covering-acceptance C2、いずれも caller discharge の非 bundling precondition-exposure)。**A3/E2b 側 CLOSED**: A3 `wz_exists_binning_E2_bound` body genuine (`d1f2445a`) + Leg E-mass `wz_source_codeword_sideInfo_mass_le` sorry-free `@audit:ok` (`66417846`/`259ecccc`) → A3 sub-tree sorryAx-free。**C2 側 = 真の blocker (r9)**: leaf `wz_covering_chosenWord_sideInfo_typical` の旧 free-param 形 (qStar/κ' unconstrained) は constant-word 反例で false-as-framed (独立 auditor CONFIRMED `3965ba5e`、旧 `d2e68b10` PASS overturn) → 3 regularity (`hκ'_pos`/`hκ'_sum`/`hqStar`) を leaf+inner に threading (covering atom が verbatim export で discharge、headline-safe) で fix + 再監査 ALL-OK (`867972b2`)、leaf body は genuine outer decomposition (sorry-free `9ecffb41`) 化・残核を isolated inner `wz_covering_markov_concentration` に factor。**残 literal sorry 2 = inner + covering atom `wz_coveringFamily_of_testChannel`** (両 `@residual(plan:wz-binning-covering)`)。**inner build = L0-L5 scaffold (子 Leg F が SoT、L4 Band-Joint conditional concentration が唯一の難所、`plan` not wall、advisor: few-sessions closable)**。**教訓 (durable)**: (i) 2 sub-lemma が opposite monotone 方向に押す共有 param は片方内で量化不可 — 最近共通祖先で構造 gap から選ぶ explicit shared input 化が唯一 honest (子 #7)。(ii) residual leaf を isolate する前に消費 site の ACTUAL measure/coupling を verbatim 確認 — 独立-product 簡約 も free-param 化 も leaf を silently 偽にしうる、event 名の一致 ≠ mechanism の一致 (子 #8)。
