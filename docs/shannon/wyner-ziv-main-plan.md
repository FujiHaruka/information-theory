...
1. **converse を先に閉じる (pure plumbing 公算大)**。決定的 gateway = heterogeneous Csiszár `csiszar_sum_identity_hetero` は **既に sorryAx-free で proved** (同型版証明を verbatim port、chain-rule stack が polymorphic だったため)。骨格 `rate_distortion_converse_n_letter_singleLetter` (`ConverseNLetter.lean:659`、antitone+Jensen+superadd+DPI 連鎖) を流用、auxiliary 展開は `bc_input_singleletterize` (`Converse.lean:244`、`Uᵢ:=(W₂,Y-prefix)` 構造が WZ に酷似・X/Y 別型対応済) をクローン。凸性・単調性 (`wynerZivRateFactorizable_convex_in_D` / `_antitone`) は情報側完成品を直呼び。残る唯一の real gap = auxiliary 同定を `wzMutualInfoXU/YU` (pmf 形) へ落とす橋。**MAC converse の教訓 (gateway 通過後は骨格クローンで genuine closure) が WZ にも効く公算大**。
...
| converse auxiliary | `bc_input_singleletterize` (`BroadcastChannel/Converse.lean:244`) | `Uᵢ:=(J,Y^{i-1})` 展開の第一クローン元 (X/Y 別型対応済) |
...
## P2 — converse body (single-letterization、confidence-builder)

**ファイル**: `InformationTheory/Shannon/WynerZiv/Converse.lean` (`ConverseGateway` + `Operational` に依存)
**proof-log**: yes (single-letterization は converse の実体、再開根拠に必須)

証明 chain (inventory §2 の 6→10):

- [ ] **step 6**: `nR ≥ H(J) ≥ I(J;X^n) − I(J;Y^n)` (encoder 決定的 + DPI)。`rate_distortion_converse_n_letter_singleLetter` の block MI bound 段をクローン。
- [ ] **step 7**: chain rule + auxiliary `Uᵢ:=(J,Y^{i-1})` を identify。`bc_input_singleletterize` (`Converse.lean:244`) 骨格を写経 (X/Y 別型・auxiliary prefix 構造は既に持つので **軽い ~150 行**)。
- [ ] **step 8**: cross 項を **`csiszar_sum_identity_hetero`** (proved) で相殺。← gateway 通過済ゆえここは直呼び。
- [ ] **step 9**: `∑[I(Xᵢ;Uᵢ)−I(Yᵢ;Uᵢ)] ≥ ∑ R_WZ(Dᵢ) ≥ n·R_WZ((1/n)∑Dᵢ)`。per-letter feasibility + `wynerZivRateFactorizable_convex_in_D` (Jensen)。
- [ ] **step 10**: antitone `wynerZivRateFactorizable_antitone` + Jensen で `n·R_WZ(D)` 到達。
- [ ] **headline `wyner_ziv_converse`**: 上記を `WynerZivAchievable → wynerZivRateFactorizable ≤ R` に詰める。auxiliary 同定を **P1 の pmf↔measure 橋 (#5)** で `wzMutualInfoXU/YU` へ落とす (= 残る唯一の real gap、§7 #3、~400-700 行)。

- **依存 in-project decl**: `ConverseNLetter.lean:659` (骨格)、`:422` (`condEntropy_pi_le_sum_condEntropy_per_letter`)、`:532` (`mutualInfo_superadditive_of_indep`); `BroadcastChannel/Converse.lean:244`; `WynerZiv/ConverseGateway.lean:48` (hetero Csiszár); `FactorizableRate.lean` (`_convex_in_D` / `_antitone`); `ConditionalEntropyConvexity.lean:374` (`_unconditional`)。
- **gateway atom**: heterogeneous Csiszár = **既に proved sorryAx-free**。残 gateway = auxiliary 同定橋 (step 7 + #5)。
- **撤退条件 (honest、hypothesis bundling 禁止)**: auxiliary 同定橋が stall した場合、`wyner_ziv_converse` headline body を **`sorry + @residual(plan:wyner-ziv-main-plan)`** で type-check done 着地 (本計画内 closure)。同定橋が単独で重く独自 closure plan を要すると判明した場合のみ、split-out plan へ **`@residual(plan:wz-auxiliary-singleletter)`** (kebab-case、新 plan filename stem) にエスカレート。auxiliary `Uᵢ:=(J,Y^{i-1})` の同定を `*Hypothesis`/`*Reduction` predicate に **bundle しない** (tier-5 禁止)。regularity hyp (`IsProbabilityMeasure` / 可測性 / `iIndepFun` / memoryless-Markov / `Nonempty`) は precondition で OK。
...
InformationTheory/Shannon/WynerZiv/
  ...
  FactorizableRate.lean         -- 完成 (wynerZivRateFactorizable / _antitone / _convex_in_D)
...
**deferred (別 plan)**: 補助 alphabet cardinality bound `|U| ≤ |α|+1` (Carathéodory reduction、§7 #7) → `wz-auxiliary-cardinality-bound`。main は `U` を Fintype 引数で受けて (`∀ U`) 済ませ、bound は後付け。本計画にサブタスクは置かない。
