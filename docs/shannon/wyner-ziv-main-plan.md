...
- [ ] P3 — achievability body (binning + covering ハイブリッド)。**gateway atoms CLOSED sorry-free + 監査 PASS (leg 12)**: `wz_sideInfo_decoder_confusion_expectation_le` (= `swError_EX_expectation_le` instantiation) + `wz_covering_sideInfo_mass_ge` (= `conditionalStronglyTypicalSlice_mass_ge` instantiation)、`@audit:ok`。残 = plumbing body (headline `wyner_ziv_achievability` = `sorry + @residual`、次 leg 本線) 🚧
