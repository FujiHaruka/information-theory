...
**Status**: ACTIVE 🚧 — **converse (P2) は FULLY CLOSED sorry-free + 独立監査 PASS (leg 11)**。最後の converse own-sorry `wz_support_reduce` を route C (K=`|α|+3`、bare ambient Carathéodory) で genuine closure → **`wyner_ziv_converse` は sorryAx-free (`#print axioms` = `[propext, Classical.choice, Quot.sound]`、`@audit:ok`)**。converse own-sorry は **ゼロ**。**Goal の唯一の残チャンク = achievability P3 (binning+covering、未着手)**。情報側 R_WZ(D) は完成済 (`WynerZiv/` 5 file、0 sorry)。
...
- [ ] P3 — achievability body (binning + covering ハイブリッド、**Goal の唯一の残チャンク、未着手 = 次 leg 本線**) 📋
...
6. **次 leg = achievability P3 (active、本線)**: converse (P2) が leg 11 で FULLY CLOSED sorry-free + 独立監査 PASS になり、Goal の残チャンクは **achievability P3 (binning+covering、未着手、~800-1500 行) のみ**。次 leg は P3 gateway atom = side-info conditional covering (`conditionalStronglyTypicalSlice_mass_ge` + `conditionalTypicalSlice_card_le` の合成) を gateway-atom-first で dispatch する (P3 §)。
