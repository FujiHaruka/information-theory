...
**Status**: P3 PAUSED 🛑 (2026-07-12 Leg 12、user-decision フォーク) — **achievability C2 covering chain が weak typicality で FALSE-AS-FRAMED と判明** (core `wz_covering_jointBand_markov_core`→outer→inner→leaf に `@audit:defect(false-statement)` `1ddc2887`、label-swap 反例が 3 弱 entropy + 3 consistency hyps を満たしつつ core を破る、前回 sufficiency PASS を overturn)。修正 = **Proposal A** (`Ecov` のみ strong joint typicality に強化、covering-success 下界 reopening) = 戦略的 def 変更ゆえユーザー判断待ち。詳細 → 子 [`wz-binning-covering-plan.md`](wz-binning-covering-plan.md) §Leg F / 孫 [`wz-markov-core-plan.md`](wz-markov-core-plan.md) §Proposal A / `wz-facts.md`。**converse (P2) は無影響 (FULLY CLOSED sorryAx-free `@audit:ok` 維持)**。以下は overturn 前の P3 記述:
