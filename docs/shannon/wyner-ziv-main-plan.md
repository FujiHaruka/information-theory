...
12. **D3 が false-as-framed → split-out 子 plan へ移譲 (leg 20、独立監査 OVERTURN、active、本線)**: D3 `wz_perN_covering_binning_code:2054` の exact `≤ D+δ` 結論は budget 使い切りで偽 — `hfeas : expectedDistortionPmf d' qStar ≤ D+δ` が good-event proxy に δ 全額を使い finite-`n` の正の error term (`distortionMax d·P[error]`) の予約ゼロ。反例 `distortionMax d = D+δ+η` (η>0) で全 hyp 成立下 `∀n` fail (姉妹定理 `rate_distortion_achievability:184` は `≤ D+ε'` 止まり + 明示 slack `h_slack` L118/L202 で ε'/2 予約)。**FIX = δ-split** (`hfeas`/`hcov₁` target を `D+δ/2` に締め `δ/2` を誤差予約、precondition 締めであって bundling でない)。D3 に `@audit:defect(false-statement)` + `@audit:closed-by-successor(wz-binning-covering)`。**δ-split (Leg 0) + genuine closure (Leg A-D) は子 plan [`wz-binning-covering-plan.md`](wz-binning-covering-plan.md) が SoT**。ripple は Achievability.lean 内 5-decl chain (機械確認: cross-file consumer 0)。これは #11 under-hyp トラップの family 再発 (free-budget exact 結論版)。cause:false-statement。
