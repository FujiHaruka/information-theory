...
- [ ] **(BD) `wz_perDelta_codes_exist` — per-δ covering+binning giant [唯一の残 sorry、~L873、`@residual(plan:wyner-ziv-main-plan)`]**。結論 = per-δ achievability `∀δ>0 ∃c ∀ᶠn dist≤D+δ`、hyps `hqf`+`hobj` のみ (honest、core は body、独立監査 confirmed)。**body 内で S1/S2/C + gateway atoms を消費**: (S1) 制限被覆 joint → (C) covering LossyCode → (S2) α'→α null-set 延長、+ binning + conditional-slice decoder assembly + E1/E2/E3 指数 + pigeonhole + distortion squeeze。**次 leg 本線 = BD を skeleton-driven に sub-lemma 分解して fill** — ~800-1500 行、BD 自身が own decomposition を要する見込み (construction inventory §434-475 の 7-step wiring sketch: covering encoder → binning → conditional-slice decoder → E1/E2/E3 exponents → pigeonhole `exists_codebook_low_avg` → distortion squeeze `source_avg_distortion_le_simpler` + per-δ)。各 sub-lemma の撤退口 = `sorry + @residual(plan:wyner-ziv-main-plan)`。**consume**: `binningMeasure:62`/`binning_collision_prob:106`、`conditionalTypicalSlice:51`+`_card_le:140`、gateway `wz_sideInfo_decoder_confusion_expectation_le:105` (E2) / `wz_covering_sideInfo_mass_ge:143` (E3)、`rdAmbient` on `wzMarginalYU:110` (§B4 gap #2)、`codebookSize_neZero`、E 系指数 atom (`ceil_exp_mul_exp_neg_tendsto_atTop:40`/`exp_neg_tendsto_zero_of_tendsto_atTop:78`/`wz_tendsto_exp_mul_codebookSize_inv:432`)、pigeonhole `exists_codebook_low_avg:138` + squeeze `source_avg_distortion_le_simpler:203`。**two-ambient subtlety** (§B4/gap #2): covering ambient = `rdAmbient(wzMarginalXU qf'.1)`、side-info ambient = `rdAmbient(wzMarginalYU qf'.1)` — 同一 3-var `qf'` の 2 marginal で rate split `R=I(X;U)−I(Y;U)` が一貫。**trap**: gateway atom の i.i.d. bundle (`iIndepFun`/`IdentDistrib`/`hpos*`/`hmarg_*`) は `rdAmbient_*` lemma から body で discharge (regularity)、「decoder が codeword を復元する」「P(error)→0」を predicate に bundle しない。distortion-shape 整合 (被覆歪 X↔U proxy vs WZ 実歪 X↔γ + side-info) は body に残す load-bearing subtlety (bundle 禁止)。
...
10. **P3 E2 gateway-1 不十分 finding (leg 16、verbatim-confirmed、本線に影響)**: BD steps 3-7 の設計調査で判明 — gateway 1 `wz_sideInfo_decoder_confusion_expectation_le` は `swError_EX` (`SlepianWolf/FullRateRegion/Core.lean:49`、alias `x'` が **全 `Fin n → U`** を走る) 由来で confusion 指数が **`H(U|Y)`** (SW スタイル、全系列 binning)。`R > H(U|Y)` でないと消えないが、WZ headline は `R > I(X;U)−I(Y;U) = H(U|Y)−H(U|X)` で、noisy test channel (`H(U|X)>0`、WZ 一般ケース) では `H(U|Y) > I(X;U)−I(Y;U)` 厳密。→ **gateway 1 は WZ rate で E2 を消せない**。必要なのは **codebook 限定 confusion 補題** (WZ を SW と分ける真の核): `exp(nR₁)` 個の i.i.d. covering codeword (Y と独立) の union、各々 Y^n と条件付き typical 確率 `≈ exp(−nI(U;Y))` (`conditionalTypicalSlice_card_le` + codeword marginal)、bin collision `1/M` (`binning_collision_prob`) → `exp(n(I(X;U)−I(Y;U)))/M → 0`。既存 atom から組めるが新規 (非自明)。**statement の真理性は不変** (achievability は WZ rate で真、`hqf`/`hobj` のみ)、崩れたのは「gateways = 難所済 / plumbing」の**工数見積り**。inventory §453-460/§484-488/§536 訂正済。撤退口 = stall 時 `sorry + @residual(plan:wyner-ziv-main-plan)`。
11. **P3 skeleton sub-lemma の under-hypothesization トラップ (leg 17、独立監査で捕捉、family 再発トラップ)**: BD steps 3-7 の error-exponent sub-lemma を skeleton 化する際、exponent を **free real パラメータ** (`I`/`δ`/`I_YU`) で置き対応する mass 束縛仮説を signature に threading し忘れると、極端値 (`I→−∞`、`I_YU→+∞` 等で RHS→0) で LHS(確率∈[0,1]) を下回り **stated bound が偽 = false-statement DEFECT** になる (S5a/S5b 初版が該当、独立監査が退化境界 `p_typ≡0`/`M=1` で refute)。**正しい形**: exponent の値を pin する **derived-precondition** を明示仮説にする — S5a は mass 下界 `∀x, exp(−n(I+δ)) ≤ p_typ x` (covering-acceptance AEP、`wz_covering_sideInfo_mass_ge` が供給)、S5b は per-codeword mass 上界 `≤ exp(−n·I_YU)` (codeword×Y^n AEP) + binning collision `binMeas{f m'=f m}=M⁻¹` (`binning_collision_prob` mirror)。これらは reusable atomic precondition (SW `swError_EX_expectation_le` が同型入力を消費) であって **load-bearing でない** — codebook-restriction の union-over-M₁ count は結論 set-builder + body に残る。`wynerZivRate_nonneg` docstring が既に同型 under-hyp を対比参照しており **family 再発トラップ**として記録 (free-exponent sub-lemma は必ず mass-bound 仮説を同伴させる)。
