...
- [ ] P3 — achievability body (binning + covering ハイブリッド)。**gateway atoms CLOSED (leg 12)** + **codes lemma 分解 (leg 13)**。leg 12: `wz_sideInfo_decoder_confusion_expectation_le` + `wz_covering_sideInfo_mass_ge` (`@audit:ok`)。leg 13: `wyner_ziv_achievability_codes` を honest 分解 → **witness 抽出 `wz_testChannel_of_rate_lt` CLOSED sorry-free `@audit:ok`** (Step 0)、構築本体は単一 sorried sub-lemma **`wz_goodCode_exists_of_testChannel`** (feasible `Fin k` test channel + objective<R → code 列) に集約。独立監査 ALL-OK: signature TRUE-as-framed / under-hypothesis でない (full-support は construction 内部で perturbation+diagonalization で吸収、RD `rate_distortion_achievability` の unconditional 版に対応)。残 = 唯一の sorry `wz_goodCode_exists_of_testChannel` 本体 (covering+binning 構築、次 leg 本線) 🚧
...
- [x] **gateway atom = side-info conditional covering — CLOSED sorry-free + 監査 PASS (leg 12)** (§10): gateway-atom-first PASS で **2 つの独立 atom に分割**して割れた (転置双対は非問題): (a) `wz_sideInfo_decoder_confusion_expectation_le` = decoder confusion 指数 (weak slice card + binning、既存汎用 `ChannelCoding.swError_EX_expectation_le` の直接 instantiation)、(b) `wz_covering_sideInfo_mass_ge` = covering acceptance 質量下界 (`conditionalStronglyTypicalSlice_mass_ge` instantiation)。両者 sorryAx-free + `@audit:ok` (`108ba475`/`72133a7e`)。3 誤り指数すべて既存 in-project atom (E1 covering X→U = `encoder_failure_prob_le_exp_neg_M_avg` (α:=X,β:=U))。→ 残は plumbing のみ、**新 atom なし**。
...
8. **codes lemma honest 分解 + construction は TRUE-as-framed (leg 13、active、本線 = construction body)**: `wyner_ziv_achievability_codes` を (i) witness 抽出 `wz_testChannel_of_rate_lt` (`h_ne`+`h_rate` → feasible `Fin k` test channel + objective<R、`exists_lt_of_csInf_lt`+`mem_wzRateValueSet_iff`、**CLOSED sorry-free `@audit:ok`**) と (ii) 構築 `wz_goodCode_exists_of_testChannel` (test channel witness → code 列、sorried `@residual(plan:wyner-ziv-main-plan)`) に分解、codes lemma body は sorry-free reduction。**独立監査 ALL-OK**: (ii) の signature は hyps = `hqf∈WynerZivFactorizableConstraint` + `hobj:objective<R` の **feasibility precondition のみ** (RD `rate_distortion_achievability` の `qStar∈RDConstraint`+`mutualInfoPmf<R` と同型)、covering+binning core は sorry body に残置 (load-bearing bundling でない)。**under-hypothesis でない (crux 確定)**: `WynerZivFactorizableConstraint` は full-support を強制しない (κ は非負+row-stochastic のみ、`FactorizableRate.lean:76`) が、strict margin `objective<R` + fully-quantified 結論 (`∃c,∀ε>0,∀ᶠn` = limsup≤D) により construction が perturbation `qf→qf_η` (full-support 化、`continuous_wzObjective`) + `η_n→0` diagonalization を**内部で**吸収 → full-support/qZ_min/slack は proof-internal で missing hypothesis ではない (RD unconditional 版に対応、plan 148-149 と整合)。両退化反証 FAIL (D<0→hqf 空、R≤0→hobj 空、いずれも vacuous)。→ **将来 leg は full-support 仮定を追加しない**。leg 12 defect との対比: あの headline は `h_rate` のみで `sInf∅=0` 退化したが、codes lemma は `hqf`=実 feasible witness (h_ne より strictly stronger) で退化排除済。
