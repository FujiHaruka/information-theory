...
- [~] P2 — converse **reshape 後 3-sorry decomposition、type-check done** (`4532bd48`、reshape honesty-audit PASS)。両 headline (`wyner_ziv_converse` + 中間 `wyner_ziv_converse_n_letter_singleLetter`) は `wynerZivRate` に retarget 済 (`hU_card` 撤去、Carathéodory は critical path 外)。`Converse.lean` の残 sorry は 3 本、すべて `@residual(plan:wyner-ziv-main-plan)`: (1) `wzObjective_nonneg_of_factorizable` (data-processing 非負、gateway atom、in-project buildable)、(2) `h_sl` (single-letterization core、`wyner_ziv_converse_n_letter_singleLetter` 内)、(3) headline `wyner_ziv_converse`。step 6 block bound (`mutualInfo_diff_le_log_card`) + `(1/n)`-scaling + `wzRateValueSet_bddBelow_of_pmf` は sorry-free。**残の重心 = h_sl (Csiszár exact identity + per-letter feasible 着地 + n-ary Jensen + bridge)** 🚧
- [ ] P3 — achievability body (binning + covering ハイブリッド、本計画の重心) 📋
- [ ] PV — verify (`#print axioms` sorryAx-free + 独立 honesty 監査 + root 配線) 📋
- [ ] (Deferred, cosmetic — critical path 外) 補助 alphabet cardinality equivalence `wynerZivRate = wynerZivRateFactorizable (Fin (|α|+1))` (Carathéodory reduction)。reshape 後 main converse はこれを **要しない** ため slug `wz-auxiliary-cardinality-bound` は critical path から外れ、現時点で file 不要
