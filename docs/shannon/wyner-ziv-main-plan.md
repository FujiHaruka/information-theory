...
**Status**: DONE ✅ (Leg 15) — **WZ operational main theorem FULLY CLOSED (achievability + converse、両 headline sorryAx-free + 独立監査 PASS)**。詳細は下の P2/P3 + 子 [`wz-binning-covering-plan.md`](wz-binning-covering-plan.md) / 孫 [`wz-markov-core-plan.md`](wz-markov-core-plan.md)。
> **converse (P2)**: `wyner_ziv_converse` sorryAx-free (`#print axioms` = `[propext, Classical.choice, Quot.sound]`、`@audit:ok`、leg 11、own-sorry ゼロ)。**achievability (P3)**: `wyner_ziv_achievability` sorryAx-free。E∘BD reduction (top-level `wz_goodCode_exists_of_testChannel`、署名不変 = full-support 仮説不追加 #9 crux 維持) → S3-S7 + D/(B) 分解 (leg 15-19) → D3 を子 plan で honest 化 (子 Leg 0-E) → C2 covering-acceptance を strong-Ecov build で全閉 (孫 Atom E-F、radius separation `ε_cov=ε/(2(1+C))`、Markov-core chain 5 decl `@audit:ok`、Leg 13) → covering atom `wz_coveringFamily_of_testChannel` を joint-derandomize で閉 (孫 Atom G、r14 `3a31e08c`+`9f5c7afc`、署名不変) → Atom H docs/wiring closure (r15、root import 登録 + README Ch.15)。C2 chain は当初 weak typicality で FALSE-AS-FRAMED (`1ddc2887`、label-swap 反例) だったが strong-Ecov で TRUE-as-framed 化 (子 plan 判断ログ)。情報側 R_WZ(D) は完成済 (`WynerZiv/` 全 file、0 sorry)。
...
- [x] P3 — achievability body **DONE sorryAx-free + 独立監査 PASS** (`wyner_ziv_achievability`)。E∘BD reduction (top-level `wz_goodCode_exists_of_testChannel`、署名不変) → S3-S7 + D/(B) 分解 (leg 15-19) → D3 を子 plan で honest 化 (子 Leg 0-E、under-hyp 5 軸解消、A3 sub-tree sorryAx-free、A2/mass `@audit:ok`) → C2 covering-acceptance を strong-Ecov build で全閉 (孫 Atom E-F、radius separation `ε_cov=ε/(2(1+C))`、Markov-core chain kernel/core/outer/inner/leaf の 5 decl が sorryAx-free + `@audit:ok`、`d8954711`/`b489d51f`/`e4490dbb`、Leg 13) → covering atom `wz_coveringFamily_of_testChannel` を joint-derandomize で閉 (孫 Atom G、r14 `3a31e08c`+`9f5c7afc`)。C2 chain は当初 weak typicality で FALSE-AS-FRAMED (`1ddc2887`、label-swap 反例) → strong-Ecov で TRUE-as-framed 化 (子 plan 判断ログ #8)。✅
- [x] PV — verify **DONE** (r15 `173ccf67`+`3248bbc3`): `#print axioms wyner_ziv_achievability` = `[propext, Classical.choice, Quot.sound]` 機械確認 + Markov-core chain 5 decl 独立 honesty 監査 PASS → `@audit:ok` + root import 登録 (`InformationTheory.lean`) + stale `@residual` 一掃 + README Ch.15。✅
