...
12. **D3 の under-hyp 2 軸を子 plan で honest 化済、残 = body closure (leg 20 発見 → 子 Leg 0/C.5、active)**: D3 `wz_perN_covering_binning_code` の exact `≤ D+δ` は当初 under-hypothesized で false-as-framed だった — **軸1 (budget)**: `hfeas` が δ 全額を good-event proxy に使い finite-`n` error term (`distortionMax d·P[error]`) 予約ゼロ (leg 20 独立監査 OVERTURN、反例 `distortionMax d = D+δ+η`)。**軸2 (reconciliation)**: `d'`(proxy)/`qf` が無関係 opaque param で `d'=𝔼_{Y|X}[d∘qf.2]` 未 encode (子 Leg C finding、`d':=0` 反例、leg-20 監査が見落とし)。**両軸とも子 plan [`wz-binning-covering-plan.md`](wz-binning-covering-plan.md) で honest 化済**: 軸1 = δ-split (`hfeas`/`hcov₁` を `D+δ/2` に締め `δ/2` 予約、Leg 0 `a59e37cb`)、軸2 = `hd'_eq`+`hqf` precondition threading (Leg C.5 `22b64afa`)、いずれも caller discharge の非 bundling。**だが Leg C.5 監査の「第3軸なし PASS」は誤りだった** — 子 Leg D で **第3軸 (M-axis: `hcov₁` が covering size M に下界 `⌈exp(nR₁)⌉≤M` のみ、confusion は M 線形比例 → M 上界なしで反例) を発見・独立監査 confirm**。M は `hcov₁` 内の ∃-witness ゆえ「明示 param 網羅」の盲点だった。→ D3 は再び `@audit:defect(false-statement)`、子 Leg C.6 (hcov₁ M-pin) で解消予定。教訓強化 = under-hyp 軸は明示 param + **∃-witness** の両方を網羅せよ (#11 family トラップの 3 度目)。cause:false-statement (子 Leg C.6 で解消予定)。
