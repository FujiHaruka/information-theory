...
12. **D3 の under-hyp を子 plan で段階的に honest 化、A3/Leg E-mass DONE、残 literal sorry 2 = Leg F の C2 covering-acceptance (leg 20 発見 → 子 Leg 0-F、active、本線)**: D3 `wz_perN_covering_binning_code` の exact `≤ D+δ` は当初 under-hypothesized で false-as-framed。子 plan [`wz-binning-covering-plan.md`](wz-binning-covering-plan.md) で under-hyp 3 軸を honest 化済 — 軸1 budget (δ-split、`hfeas`/`hcov₁` を `D+δ/2` に締め `δ/2` 予約、Leg 0 `a59e37cb`)、軸2 reconciliation (`hd'_eq`+`hqf` threading、Leg C.5 `22b64afa`)、軸3 M-axis (`hcov₁` に `M ≤ exp(nR₁)+1` pin、Leg C.6 `fe3d9482`/`90996ed1`、独立監査 all-PASS)、いずれも caller discharge の非 bundling precondition-exposure。**D3 body は sorry-free reduction に復帰 (M-axis defect 除去 honest tier-2)、A2 `wz_ideal_expectation_eq_covering` も sorry-free (`84393413`)**。**だが子 Leg D で第4軸 = covering-acceptance C2 を発見・独立監査 CONFIRMED**: E2 = E2b(confusion)∪C2(acceptance) (回復に acceptance 要求 `wzBinTypicalDecoder_eq_of_unique` ゆえ C2 ⊂ E2) で A3 `wz_exists_binning_E2_bound` (Achievability.lean:3022) が C2 を bound する仮説を欠き false-as-framed (反例 all-atypical-image codebook)。子 Leg E (covering-acceptance 再設計 Proposal A) で closure。**Leg E first move (`b5b22385`) は DEFECT** (`d71b9dfb`: free-∃ε vacuous + 第5軸 dα'-d scaling) → **署名 rework で RESOLVED** (`cf7d57cd`/`42abbf21`、独立監査 all-PASS): ε を rate gap から D3 で選ぶ共有 explicit param 化 + acceptance mass を単一 ε に pin + `hd'_link` を rfl discharge、退化 binder table 全 BLOCKED、A3 `@audit:defect` 除去 honest tier-2、A2 `@audit:ok`。**A3 DONE (body genuine `d1f2445a`) + Leg E-mass `wz_source_codeword_sideInfo_mass_le` sorry-free + sorryAx-free + `@audit:ok`** (`66417846`/`259ecccc`、独立監査 PASS) → A3 sub-tree sorryAx-free。**残 literal sorry 2 = Leg F の C2 covering-acceptance のみ** (両 `@residual(plan:wz-binning-covering)`): leaf `wz_covering_chosenWord_sideInfo_typical` (isolate + 独立監査 OK `5d3ecd81`/`d2e68b10`) + covering atom `wz_coveringFamily_of_testChannel` (leaf 未配線)。C2 = **Markov Lemma、gateway-atom-first probe = STALL → from-scratch ~100-300行 multi-leg self-build** (Mathlib wall でも 1-2 dispatch port でもない、`plan`-class)。**key mechanism 訂正**: 旧「S5a/gateway-2 ~40行 Fubini」は mis-mechanization (covering/独立-side を相関-joint acceptance と混同)、**かつ advisor 提案の固定-word 独立-product leaf `Measure.pi (μ.map (Ys 0))` も FALSE-as-framed** (`u=c.decoder(c.encoder x)` は source x の関数、独立固定で x–y 相関崩壊 ⟹ atypicality mass→1) → 正しくは **coupled 相関-joint-source 形** (chosen-word-of-x vs y、退化境界で TRUE 確認)。BC `bc_conditional_slice_prob_le` は confusion-sibling で acceptance に transfer せず。**教訓 (durable)**: (i) 2 sub-lemma が opposite monotone 方向に押す共有 param は片方内で量化不可 — 最近共通祖先で構造 gap から選ぶ explicit shared input 化が唯一 honest (子 #7)。(ii) residual の named closure mechanism が別 measure (product/独立 vs 相関 joint) or 別 functional (cover-x vs test-y) の lemma を再利用しているときは event 名の一致を mechanism の一致と取り違えるな (子 #8)。(iii) residual leaf を isolate する前に消費 site の ACTUAL measure/coupling を verbatim 確認 — 独立-product 簡約が leaf を silently 偽にしうる。
