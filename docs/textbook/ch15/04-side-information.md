...
::: proof
$x \in \mathcal X$ と $y \in \mathcal Y$ について $s(x) := \sum_{k=1}^{m} b^\circ(k)\,x(k)$，$r_y(x) := \sum_{j=1}^{m} b(j \mid y)\,x(j)$ とおく．定義 15.1.1 と 定義 15.1.2 よりどちらも正である．
...
$S := \{\,(x,y) \mid p(x,y) > 0\,\}$ とおく．$S$ の外の項は $p(x,y) = 0$ で消えるから，上の和は $S$ 上の和である．同時分布の定め方から $\sum_{x} p(x,y) = p(y) \sum_{x} p(x \mid y) = p(y)$ なので $p(y)$ は $Y$ の周辺分布であり，$p(x)$ は 定義 15.4.2 のとおり $X$ の周辺分布である．$(x,y) \in S$ では $p(y) \ge p(x,y) > 0$ かつ $p(x) \ge p(x,y) > 0$ かつ $p(x \mid y) > 0$ であり，$p(x,y)/\big(p(x)p(y)\big) = p(x \mid y)/p(x)$ だから，定義 1.3.1 より
$$
I(X;Y) \;=\; \sum_{S} p(x,y) \log \frac{p(x \mid y)}{p(x)}
$$
である．二つを引き算すると
$$
W(b \mid Y) - W(b^\circ, p) - I(X;Y)
  \;=\; \sum_{S} p(x,y) \log t(x,y),
\qquad
t(x,y) \;:=\; \frac{p(x)\, r_y(x)}{p(x \mid y)\, s(x)}
$$
となる．$S$ の上では $t(x,y) > 0$ である．

補題 1.1.6 を，凹関数として $\log$（第1章 1.1 節が認めた狭義凹性による），重みとして $S$ 上の $p(x,y)$，点として $t(x,y)$ ととって当てる．重みは非負で，総和は $\sum_{S} p(x,y) = \sum_{x,y} p(x,y) = \sum_{y} p(y) \sum_{x} p(x \mid y) = 1$ だから，
$$
\sum_{S} p(x,y) \log t(x,y) \;\le\; \log\Big( \sum_{S} p(x,y)\, t(x,y) \Big)
$$
を得る．

内側の和を抑える．$S$ の上では $p(x,y) = p(y)\,p(x \mid y)$ と $p(x \mid y) > 0$ より
$$
p(x,y)\, t(x,y) \;=\; p(y)\, p(x)\, \frac{r_y(x)}{s(x)}
$$
である．この形の項はどの $(x,y)$ でも非負だから，$S$ の外の項を足しても和は減らず
$$
\sum_{S} p(x,y)\, t(x,y)
...
いっぽう $\sum_{S} p(x,y) = 1$ だから $S$ は空ではなく，$S$ 上の各項 $p(x,y)\,t(x,y)$ は正だから，この和は正である．したがって和は $0$ より大きく $1$ 以下であり，補題 8.2.5 よりその対数は $\log 1 = 0$ 以下である．以上を合わせて $W(b \mid Y) - W(b^\circ, p) - I(X;Y) \le 0$ を得る．
:::
...
::: proof
周辺分布は $p(x_\uparrow) = \frac12 \cdot 1 + \frac12 \cdot 0 = \frac12$，同様に $p(x_\downarrow) = \frac12$ で，例 15.1.5 の $p$ に等しい．例 15.2.6 よりこの市場の対数最適ポートフォリオは $b^{*}(1) = b^{*}(2) = 1/2$ であり，例 15.1.5 よりその倍加率は $\frac12\log\frac98$ だから，定義 15.4.2 より $W^{*}(X) = \frac12\log\frac98$ である．
