...
第 $1$ 項の $A$ が，型の個数を数えた 命題 11.1.3 の指数をそのまま切り上げた形になっているのが，Kraft の不等式が閉じる仕組みである．型ごとに $2^{-A}$ という同じ大きさの場所を割り当て，その中を型類の要素で分け合う．型の個数が $2^A$ 以下だから，全部足しても $1$ を超えない．なお，節のはじめに述べた二段の手続きそのものが語頭符号を与えることは直接確かめられるが，本書はそれを主張として述べない．命題 12.2.2 が与えるのは，この長さをもつ二元語頭符号が存在することであり，符号の存在は 定理 4.2.2 に任せている．二段の説明は長さの由来を述べたものである．
...
型類の要素数を本章の底で読み直す．第11章は $\log$ の底を自然対数にとっているので，定理 11.1.8 の上界は，そこで測ったエントロピーを $\mathrm e$ の肩に乗せた形をしている．自然対数で測ったエントロピーは，本章の底で測った $H(P)$ の $\log_{\mathrm e}2$ 倍だから
$$
\mathrm e^{\,n\,H(P)\log_{\mathrm e}2} \;=\; 2^{\,n H(P)}
$$
であり，定理 11.1.8 の上界は本章の底では $\lvert\mathcal T_n(P)\rvert \le 2^{\,n H(P)}$ と読める（$H$ は本章の底で測ったエントロピーである）．両辺の $\log_2$ をとると $\log_2\lvert\mathcal T_n(P)\rvert \le n\,H(P)$ であり，$P := \hat P_x$ ととって上の評価に入れると
$$
...
::: corollary 12.2.5
定義 12.1.1 の設定とする．どの $n \ge 1$ についても
$$
\Delta^*_n \;\le\; \frac{\lvert\mathcal X\rvert\log_2(n+1) + 2}{n}
$$
である（$\Delta^*_n$ は 定義 12.1.5 のミニマックス冗長度）．右辺は $\Theta$ にも $P_\theta$ にも依らず，$n \to \infty$ で $0$ に収束する．さらに，各 $n \ge 1$ に $\ell^{\mathrm T}_n$（定義 12.2.1）を対応させた列は，この族に対する万能符号である（定義 12.1.6）．
:::
...
最後の主張に移る．いま見たとおり $\max_{\theta}\Delta_n(\ell^{\mathrm T}_n,\theta)$ は右辺以下であり，命題 12.1.3 より $0$ 以上である．右辺は $0$ に収束するから，はさみうちにより $\max_{\theta}\Delta_n(\ell^{\mathrm T}_n,\theta) \to 0$ であり，定義 12.1.6 の条件が満たされる．
:::

万能符号があること（定義 12.1.6）は，三つの主張が合わさって出ている．命題 12.2.2 が，$\ell^{\mathrm T}_n$ が長さ $n$ の符号長の組であること，すなわちこの長さをもつ二元語頭符号があることを与える．定理 12.2.4 が，その冗長度を族のどの $\theta$ についても同じ値で抑える．そして 系 12.2.5 が，その値が $n \to \infty$ で $0$ に収束することを述べ，三つを合わせて 定義 12.1.6 の条件を確かめている．族をどう与えても $\ell^{\mathrm T}_n$ という一つの符号長の組でこれが言え，しかもその上界は族の中身を見ずに書けている．分布を知っている場合の $1/n$ 未満（例 12.1.4）に比べれば $\log_2 n$ の因子だけ大きいが，$1$ 文字あたりで見れば，どちらも $n \to \infty$ で消える量である．知らないことの代償として払っているのは，例 12.1.7 の決めつけのような定数ではなく，$\log_2 n$ の因子である．

上界が $0$ に向かうのは $n$ が大きいところでの話で，短い系列では逆になる．両端を見ておく．

::: example 12.2.6 短い系列に当てる
$\mathcal X = \{0, 1\}$，$n = 4$，$x = 0110$ とすると $\ell^{\mathrm T}_4(x) = 8$ である（$\ell^{\mathrm T}_4$ は 定義 12.2.1 の符号長）．
:::

::: proof
第 $1$ 項は $\lceil 2\log_2 5\rceil$ である．$2^4 = 16 < 25 = 5^2 < 32 = 2^5$ だから $4 < 2\log_2 5 < 5$ であり，切り上げは $5$ である．第 $2$ 項に移る．定義 11.1.1 より $x$ の型は $\hat P_x(0) = \hat P_x(1) = 1/2$ であり，その型類は $0$ と $1$ がそれぞれ $2$ 回現れる長さ $4$ の二値系列の全体だから，要素数は $1$ の位置の選び方の数 $6$ である（$0011$，$0101$，$0110$，$1001$，$1010$，$1100$ の六つ）．$2^2 = 4 < 6 \le 8 = 2^3$ より $\lceil\log_2 6\rceil = 3$ である．合わせて $5 + 3 = 8$ を得る．
:::
...
同じ $x$ をそのまま書き写せば $4$ ビットで済む（例 12.1.7 の $\ell'_n$）．$n = 4$ では，型を指すための $5$ ビットが系列そのものより長い．型による二段符号が得をするのは，この固定費 $\lceil\lvert\mathcal X\rvert\log_2(n+1)\rceil$ が $n$ に対して小さくなってからで，定理 12.2.4 の右辺が $n$ で割った形をしているのはそのためである．反対の端を見る．

::: example 12.2.7 二つの偏ったコイン（型による符号）
例 12.1.7 の族で $n = 1000$ とすると，系 12.2.5 の右辺は
$$
...
::: formalization-note
例 12.2.7 は形式化されていない．例 12.2.7 に付した計算が，この主張の保証のすべてである．
:::

数を並べると差がはっきりする．同じ族で，$\theta = 1$ と決めつけたときの冗長度は $2.535\ldots$ ビット以上であり（例 12.1.7），系列をそのまま書き写したときは $0.531\ldots$ ビットだった．型による二段符号は，$n = 1000$ でそのどちらより小さい $0.0219\ldots$ ビット以下に収まる．どの $\theta$ が真かを知らないまま，知っている場合との差を $0.022$ ビットを下回るところまで詰めたことになる．
