...
道具は一つ借りる．$1$ 次式による **置換積分**，すなわち $c \ne 0$ のときの $y = (x - b)/c$ という置き換えである．$dx = \lvert c\rvert\,dy$ という関係と，積分域の対応（区間 $[u,v]$ の上での積分は $[(u-b)/c,\,(v-b)/c]$ の上での積分に移り，区間でない集合 $A$ の上での積分は，$A$ の各点を $y = (x-b)/c$ で送った集合の上での積分に移る）を，微積分の計算規則として既知とする．当てる対象は，本節と 7.4 節と 7.5 節に現れる密度と，それに対数を掛けた関数である．依存するのは補題 7.3.1・命題 7.3.2・命題 7.3.3 の証明，7.4 節の例 7.4.4 の証明，および 7.5 節の命題 7.5.3 の証明である．
...
::: formalization-note
命題 7.3.2 の形式化は，本文の「$h(X)$ が定まる」にあたる仮定をもたない．積分できない場合に形式化が $h$ の値を $0$ と定める約束のもとでは，平行移動の前後でどちらも $0$ になって等式が成り立つからである．
:::
...
::: formalization-note
命題 7.3.3 の形式化は，本文の「$h(X)$ が定まる」にあたる積分可能性を仮定にもつ．結論は等式だけで，本文の主張のうち「$h(cX)$ も定まる」にあたる部分は含まれていない．
:::
...
::: formalization-note
系 7.3.4 の形式化も同じで，積分可能性を仮定にもち，結論は等式だけである．
:::
