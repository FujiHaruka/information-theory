...
証明はすでに置換積分で済んでいるので，ここでは $\log\lvert c\rvert$ という差を定理 7.1.5 の言葉で読み直しておく．$X$ が定理 7.1.5 の仮定を満たす，すなわちある区間 $[a,b]$ の上で密度が連続でその外で $0$ であるとき，補題 7.3.1 より $cX$ の密度も対応する区間の上で連続でその外で $0$ だから，$cX$ にも定理 7.1.5 が当たる．$cX$ を幅 $\delta$ の区間で読み取るのは $X$ を幅 $\delta/\lvert c\rvert$ の区間で読み取るのと同じことなので，読み取った値のエントロピーは両者で等しい．定理 7.1.5 は微分エントロピーを $H + \log(\text{幅})$ の極限として与えているから，幅が $\lvert c\rvert$ 分の $1$ になったぶんの $\log\lvert c\rvert$ が，そのまま差として残る．離散のエントロピーが値の付け替えで動かなかったのは，値の付け替えが「どの区間に入ったか」を変えないからである．引き伸ばしは，同じ幅の区間との関係を変えるので事情が違う．
