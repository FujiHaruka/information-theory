...
失う量を先に三つに割り振っておく．$H(p)$ から引かれるのは，典型集合に置く幅，記述が短いほうへ数え落とす確率に $H(p)$ を掛けたもの，記述の長さの下限にとる整数の端数の三つで，そのそれぞれを $\varepsilon/3$ 以下に抑える．二つめはさらに，$X^n$ が典型集合を外れる分と，典型集合に入ったうえで記述が短い分とに割れるので，どちらも $\varepsilon/6$ 以下にする．$m$ を選ぶのはその後者のためで，$2^{-m}$ がその確率の上界になる．
...
$\mathcal X = \{0, 1\}$，$p(0) = p(1) = 1/2$ とし，$b$ を 命題 13.3.3 の定数とする．このとき $H(p) = 1$ ビットであり，$n = 1000$ に対する 補題 13.3.4 の右辺が $H(p)$ を超える分は，第12章 例 12.2.7 が同じ $\lvert\mathcal X\rvert$ と $n$ について与えた $0.02193\ldots$ ビットより，ちょうど $b/1000$ ビットだけ大きい．
:::

::: proof
定義 1.1.1 より $H(p) = -2 \times \tfrac12\log_2\tfrac12 = 1$ である．$\lvert\mathcal X\rvert = 2$ だから，補題 13.3.4 の右辺が $H(p)$ を超える分は $\big(2\log_2 1001 + 2 + b\big)/1000$ である．第12章 例 12.2.7 が与えたのは $\big(2\log_2 1001 + 2\big)/1000$ で，差は $b/1000$ である．
:::

この差が，機械を一つ固定して一本の系列を測るようにした代償である．$b$ は 命題 13.3.3 の証明のとおり 定理 13.1.5 の定数から来ており，13.1 節で見たとおりその値は部分計算可能関数の番号づけの取り方に依る．第12章 例 12.2.7 の $0.02193\ldots$ ビットは，番号づけを持ち出さずに書けた数だった．いっぽう 補題 13.3.5 と 定理 13.3.6 には $b$ が現れない．下からの評価は機械を作らずに数え上げだけで出ており，極限のほうは $b$ を $n$ で割って消してしまうからである．
...
本節が結んだものを並べておく．平均については 定理 13.3.6 で，分布から決まる $H(p)$ と，分布を見ない機械が一本の系列に払う長さの平均が，$1$ 文字あたりで一致した．個々のブロックについては 定理 13.3.10 で，圧縮できないブロックの $1$ の頻度が $1/2$ に近いことが分かった．後者は一方向だけの主張である．頻度が $1/2$ に近いことは圧縮できないことの帰結であって，逆は成り立たない．

::: example 13.3.13 交互に並ぶブロック
偶数 $n \ge 2$ に対し，$x^{(n)} \in \{0,1\}^n$ を，第 $i$ 文字が $i$ を $2$ で割った余りであるもの，すなわち $0101\cdots01$ とする．このとき $N\big(1 \,\big\vert\, x^{(n)}\big)/n = 1/2$ である（$N(1\mid\cdot)$ は 定義 2.4.1 の，系列に含まれる $1$ の個数）．また定数 $w \in \mathbb N$ があって，すべての偶数 $n \ge 2$ について $K_{\mathcal U}\big(\langle x^{(n)}\rangle \,\big\vert\, n\big) \le w$ である．$w$ は $n$ に依らないから，$n > w$ を満たす偶数 $n$ については $x^{(n)}$ は 定義 13.3.8 の条件を満たさない．
:::

::: proof
$n = 2k$ と書く．$x^{(n)}$ の第 $i$ 文字が $1$ であるのは $i$ が奇数のときで，$0$ 以上 $n$ 未満の奇数は $1, 3, \dots, 2k-1$ の $k$ 個だから $N(1\mid x^{(n)}) = k = n/2$ である．

上界に移る．$\mathcal M(z, y)$ を，$y$ が $2$ 以上の偶数のとき $\langle x^{(y)}\rangle$ とし，そのほかでは値を持たないと定める（第 $1$ 引数は使わない）．$\langle x^{(y)}\rangle$ を求めるには 定義 13.3.1 の和を $y$ 項だけ足せばよいので，この対応は有限個の場合分けと有限回の繰り返しで書き下せており，Church–Turing のテーゼより部分計算可能で，定義 13.1.1 の意味で機械である．定理 13.1.5 をこの $\mathcal M$ に当てて定数 $w \in \mathbb N$ をとる．偶数 $n \ge 2$ について，空列 $s$ をとると $\mathcal M(s, n) = \langle x^{(n)}\rangle$ であり，$\lvert s\rvert = 0$ だから $K_{\mathcal U}(\langle x^{(n)}\rangle \mid n) \le w$ である．

最後の主張は，$w < n$ のとき 定義 13.3.8 の $K_{\mathcal U}(\langle x^{(n)}\rangle \mid n) \ge n$ が成り立たないことによる．
:::

例 13.3.13 の列は，$1$ の頻度がちょうど $1/2$ でありながら，長さによらない定数ビットの指示で書き出せる．長さ $n$ が条件として渡されているので，指示のほうは「交互に並べろ」とだけ言えばよく，$n$ を書き添える必要がないからである．定理 13.3.10 が言っているのは，圧縮できないブロックが偏った頻度を持ちえないというところまでで，頻度が $1/2$ に近いブロックの中には，このように短い記述を持つものがある．
