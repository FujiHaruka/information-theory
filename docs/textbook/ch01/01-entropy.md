...
**定義 1.1.1（エントロピー）.** 有限アルファベット $\mathcal X$ 上に分布する
...
**例 1.1.2（ベルヌーイ分布）.** $X$ が確率 $p$ で $1$、確率 $1-p$ で $0$ を
とるとき、
$$
...
**例 1.1.3（一様分布）.** $X$ が $\mathcal X$（要素数 $|\mathcal X| = M$）上で
...
**命題 1.1.4.** 任意の離散確率変数 $X$ について $H(X) \ge 0$。等号は $X$ が
...
**定理 1.1.5.** $|\mathcal X| = M$ のとき $H(X) \le \log M$。等号は $X$ が
...
**補題 1.1.6（有限 Jensen の不等式・帰納法版）.** $\varphi$ を区間 $I$ 上の凹関数と
