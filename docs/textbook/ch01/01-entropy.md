...
s(pq) \;=\; s(p) + s(q).
$$
「掛け算を足し算に変える」というこの性質を持つ（連続な）関数は、定数倍を除いて対数
しかない。これは Cauchy の関数方程式として知られる古典的な事実であり、本書では証明せずに
借りる。借りるのはこの動機づけの中だけで、次に置く定義から先の議論はこの一意性に依存
しない。よって $s(p) = -\log p$ と書ける（$0 < p \le 1$ で $-\log p \ge 0$ となるよう
符号を選んだ。定数倍の自由度は対数の底の選び方に吸収される）。

$s(p) = -\log p$ は「その値が出たときの驚き」であって、まだ観測前の不確かさではない。
観測前にわかっているのは分布だけなので、実際に出る値ごとの驚きを、その値が出る確率で
**平均**する。これが次のエントロピーである。Shannon は、いま並べた要請を満たす尺度が
本質的にこの一通りに定まることを示した。
...
の記号に長さ $-\log p(x)$ の符号語を割り当てると、平均符号長がちょうど $H(X)$ になる。
...
::: formalization-note
本ライブラリでは $-p\log p$ を一つの関数 `negMulLog p` として
...
::: proposition 1.1.4
任意の離散確率変数 $X$ について $H(X) \ge 0$。等号は $X$ が
...
::: formalized
`entropy_nonneg` (`InformationTheory/Shannon/Bridge.lean`)
:::
