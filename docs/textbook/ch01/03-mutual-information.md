...
**きもち.** 対数の中身は $\dfrac{p(x,y)}{p(x)p(y)} = \dfrac{p(x \mid y)}{p(x)}$ とも書ける。
これは「$y$ を見たことで、$x$ が起きるという見込みが何倍になったか」である。1 倍
...
**きもち.** 非負性は「観測は平均的には損にならない」ということである。個々の観測は
...
**きもち.** 二つの集合の面積として描くと見通しがよい。$H(X)$ と $H(Y)$ を二つの円の
