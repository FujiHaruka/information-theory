...
重要なのは、**増分の和は受け取る順序を変えても総和が同じ**（左辺は順序に依らない）
という点である。個々の項は順序で変わるが、合計は変わらない。
...
$(X_0, Y_0), \dots, (X_{n-1}, Y_{n-1})$ である。このとき定理 1.5.2 の各項で、$X_i$ と
$Y_i$ の組は先頭 $i$ 個の組とまったく無関係なので、条件を落として
