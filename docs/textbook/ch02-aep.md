# 第2章 漸近等分配性 (Asymptotic Equipartition Property)

> **このファイルについて**
>
> 本章は Cover & Thomas *Elements of Information Theory* (2nd ed.) の
> 漸近等分配性 (Asymptotic Equipartition Property, AEP) を題材に、**検証済み Lean 定理を
...
> 第1章 (`InformationTheory/Shannon/Bridge.lean`) と同じ measure-theoretic 定義
...
典型ブロック `x \in T_\varepsilon^{(n)}` の確率は
両側から指数的に挟まれる:
\[
...
## 2.3 データ圧縮への帰結 (Data Compression / Source Coding)

典型集合に約 `2^{nH}` 個の要素しか無いという事実から、
...
## 2.4 強典型性 (Strong Typicality)

C&T では強典型性はより後の章で導入されるが、本ライブラリは AEP の自然な拡張として
...
Cover & Thomas の対応章のうち、本章に対応する独立した proof-done 定理を
確認できなかった項目を正直に記す。
...
- **連続情報源の AEP / 微分エントロピー版**: 本章スコープは
  離散・有限アルファベットに限る。連続版は future work（ロードマップ上、
  連続版 AEP は「量の壁」として scope-out 済み）。
- **一般定常エルゴード過程の AEP (Shannon–McMillan–Breiman)**:
  本章は i.i.d. ソースのみ。SMB 定理は第3章で扱う。
...
1. **ロードマップの代表名 `stronglyTypicalSet_card_le` が実在しない**:
   roadmap の該当行は代表として `typicalSet, stronglyTypicalSet` を挙げるが、
