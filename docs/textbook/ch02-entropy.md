...
*証明.* 定理 2.3.4 の entropy 表現を対 $(Z,X)$ に適用すると
$$
I(Z,X;Y) \;=\; H(Z,X) - H(Z,X\mid Y).
$$
右辺の各項にエントロピーのチェイン則（定理 2.2.3）とその条件付き版を当てる。無条件版は
$H(Z,X) = H(Z) + H(X\mid Z)$。条件付き版は、定理 2.2.3 の証明をすべて「$Y$ を知った
うえで」繰り返せば
$$
...
$$
第 1 のかっこは定理 2.3.4 そのもので $I(Z;Y)$。第 2 のかっこは条件付き相互情報量の
entropy 表現（定理 2.5.4）で $I(X;Y\mid Z)$。合わせて主張を得る。$\qquad\blacksquare$
...
*証明.* 定義 2.5.1 の各 $y$ ごとの相対エントロピーで、被加数の対数比を
...
$$
第 1 項は定義 2.2.2 によりちょうど $-H(X\mid Y,Z)$。第 2 項は $z$ について先に和をとると
...
（$\varphi = -t\log t$ の凹性、2.1.5 の信頼の底の符号反転）に対する有限 Jensen——補題
2.1.6 を $-\psi$ に適用して符号を返したもの——は、重み $\lambda_i$・点 $t_i$ に対し
$$
...
\Big(\sum_i a_i\Big)\log\frac{\sum_i a_i}{\sum_i b_i}
  \;\le\; \sum_i a_i\log\frac{a_i}{b_i}.
$$
等号は補題 2.1.6 の等号条件（$\psi$ は狭義凸）より、正の重みをもつ $t_i = a_i/b_i$ が
...
*証明.* 写像 $g(x,y) := (x, f(y))$ を考え、定理 2.8.1 を $\mu \leftarrow p_{X,Y}$
...
*証明.* 対 $(X,Z)$ が $Y$ について持つ情報 $I(X,Z;Y)$ を、チェイン則（定理 2.4.1）で
二通りに展開する：
$$
...
$$
条件付き相互情報量の非負性（命題 2.5.2）より $I(Z;Y\mid X) \ge 0$ だから、
...
チェイン則（定理 2.2.3 を $Y$ のもとで適用）で二通りに展開する：
$$
...
- $H(E\mid Y) \le H(E)$：条件付けは平均エントロピーを増やさない（定理 2.2.4 の基本形）。
...
- $H(X\mid E, Y)$ を $E$ の二値で分ける：
  ...
  最大エントロピー上界（定理 2.1.5）より $H(X\mid Y, E{=}1) \le \log(|\mathcal X|-1)$。
