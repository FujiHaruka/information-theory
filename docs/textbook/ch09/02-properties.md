...
::: proof
筋は，対 $(x,\hat x)$ ごとに対数和不等式を当てて，それを足し上げるだけである．以下の場合分けはどれも，確率 $0$ の項を落としてよいことを確かめるためのものである．
...
なぜ弦より下になると期待できるのかは，時間で分けてみると見当がつく．符号を定義するのは 9.5 節なので符号の話としてはまだ言えないが，長さ $n$ のブロックのうち $\lambda$ の割合を歪み $D_1$ 用の符号で，残りを歪み $D_2$ 用の符号で処理したとする．歪みは成分についての平均だから全体で $\lambda D_1 + (1-\lambda)D_2$ になり，$1$ 文字あたりのレートも同じ重みの平均 $\lambda R(D_1) + (1-\lambda)R(D_2)$ になる．中間の歪みをこれだけのレートで実現する手立てが一つある以上，中間の点で弦より上には行きようがない，というわけである．
...
::: formalization-note
`rateDistortionFunction_convexOn` が述べているのも，$R(D)$ を測度の言葉で書いた宣言 `rateDistortionFunction` (`InformationTheory/Shannon/RateDistortion/Converse.lean`) についての凸性で，定義 9.1.5 に紐付けた宣言とは別のものである．二つを結ぶ宣言は無い．またこの宣言は，情報源の分布を第 $1$ 周辺にもつどの同時分布についても歪みが可積分である，という前提を受け取る．命題 9.2.3 が置いた有限のアルファベットのもとでは，$d$ が有限個の値しかとらないのでこの前提は満たされるが，アルファベットを有限に固定した形の宣言は形式化には無い．
:::

## 両端

残るは両端である．左の端では歪みをいっさい許さず，右の端では $R$ が $0$ になる．左の端で $R$ がエントロピーに戻るのは，情報源アルファベットが再現アルファベットに含まれ，かつ歪み尺度が「$0$ になるのは一致するときだけ」を満たすときで，この二つはどちらも主張の仮定に要る．
...
歪み尺度の条件は落とせない．たとえば $d$ が恒等的に $0$ なら，どの条件付き分布も $\mathcal Q(0)$ に属するので，$\hat{\mathcal X}$ の一点に集中する条件付き分布をとれば，情報源と再現は独立になって相互情報量は $0$ になる．相互情報量は非負（命題 1.3.2）だから，このとき $R(0) = 0$ である．$H(X) > 0$ である情報源では，これは命題 9.2.4 の結論と違う値である．
...
::: formalization-note
命題 9.2.4 にあたる宣言は無い．レート歪み関数とエントロピーを同じ主張の中で結ぶ宣言が形式化には無いためである．命題 9.2.4 に付した証明が，この主張の保証のすべてである．
:::
...
命題 9.2.5 により，$R(D) = 0$ となる $D$ のうち最小のものが $D_{\max}$ である．最小のものなのに $D_{\max}$ という最大を思わせる名前が付くのは，これが歪みを許す意味のある範囲の右端だからである．ここから先はいくら歪みを許しても値が $0$ のままで，見るべきものが残っていない．$q_0$ のとり方が読み方を与える．情報源を見ずに，あらかじめ決めた一文字 $\hat x_0$ をいつも出す，というのが「何も送らない」再現であり，その中でいちばん歪みの小さいものが払う歪みが $D_{\max}$ である．それより多くの歪みを許してよいなら，情報源について何も知らなくて済む．

::: formalization-note
命題 9.2.5 にあたる宣言は無い．$D_{\max}$ を定める宣言も，レート歪み関数が $0$ になることを述べる宣言も，形式化には無い．最悪の歪み $\max_{x,\hat x}d(x,\hat x)$ を定める `distortionMax` (`InformationTheory/Shannon/RateDistortion/AchievabilityAsymptoticFailureDecay.lean`) はあるが，これは命題 9.2.5 の $D_{\max}$ とは別の量である．命題 9.2.5 に付した証明が，この主張の保証のすべてである．
:::

**両端で確かめる.** 二値の情報源と Hamming 歪み（例 9.1.3）で，二つの端を見ておく．$\mathcal X = \hat{\mathcal X} = \{0,1\}$ とし，$\Pr[X = 1] = \pi$ とする．$\mathcal X = \hat{\mathcal X}$ であり，$d_H(x,\hat x) = 0$ は $\hat x = x$ と同値だから，命題 9.2.4 の二つの仮定はどちらも満たされ，$R(0) = H(X) = H_b(\pi)$ である（$H_b$ は例 1.1.2 の二値エントロピー）．右の端は，$\hat x = 0$ に対して $\sum_x p(x)d_H(x,0) = \pi$，$\hat x = 1$ に対して $1 - \pi$ だから，命題 9.2.5 より $D_{\max} = \min(\pi, 1-\pi)$ である．公平なコイン（$\pi = 1/2$）なら $R(0) = 1$ ビット，$D_{\max} = 1/2$ になる．後者は，情報源を見ずにいつも $0$ を出せば文字の半分が食い違う，という勘定と合っている．
