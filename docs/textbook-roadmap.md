...
**章単位で元から外す**: Ch.13 LZ 詳細 (LZ78 漸近最適性は in)。(Ch.16 Portfolio は 2026-07-19 に復帰・完遂 — 下表参照。Ch.6 stock-market 部も log-optimal portfolio として Ch.16 で回収。)

**当初 scope-out だが 2026-07-20 に復帰着手 (Ch.14 Kolmogorov)**: plain 複雑性 C の背骨に着手。**gateway atom (長さ加法万能機械 U + 不変性 P1 + 上界 P2) = proof-done sorryAx-free + `@audit:ok`** (`InformationTheory/Shannon/Kolmogorov/{UniversalMachine,Invariance}.lean`、U を Mathlib `eval` 委譲で 108 行に、P1 は退化 `sInf ∅` を避ける pointwise 形)。残り背骨 (P3 数え上げ/非圧縮 → P4 **K↔H flagship** `@[entry_point]` → P5 非計算可能性 → P6 SLLN) は in-progress。**prefix 複雑性の塔 (P7 普遍確率 / P8 符号化定理 Levin / P9 Chaitin Ω) は prefix-free 機械インフラ未整備ゆえ第 2 波 = 別 moonshot に隔離** (`wall:prefix-free-tower`)。子 plan SoT → `kolmogorov/kolmogorov-moonshot-plan.md`。
...
- **Ch.11 Chernoff converse / Cramér 下界** — `chernoff_converse` sorryAx-free (achievability `chernoff_lemma_achievability` の片割れ)、`cramer_lower_boundary` sorryAx-free。
- **Ch.13 LZ78** — headline `lz78_asymptotic_optimality_with_greedy` proof done (M3 achievability + M4 converse 両壁 closed)。
- **Ch.15 Network IT** — MAC 容量領域 full closure (`mac_converse` + `mac_achievability` + reconciliation `mac_capacity_region_reconciliation`)、degraded BC (`bc_degraded_converse` + `bc_achievability`)、relay cut-set outer bound (`relay_cutset_outer_bound`)、**Wyner-Ziv operational main (`wyner_ziv_achievability` + `wyner_ziv_converse`)** すべて `@audit:ok`・sorryAx-free。MAC time-sharing 全凸包形 (L-MAC5) も `mac_timesharing_capacity_region` (`@[entry_point]`、intersection 形) で proof done・sorryAx-free (2026-07-05 CLOSED、子 `mac-timesharing-plan.md` が SoT)。
- **Ch.17 EPI 一般版** — 完全無条件 `entropyPowerExt_add_ge` + a.c. 版 `entropy_power_inequality_of_ac` sorryAx-free。CT 17.9 Minkowski determinant `minkowskiDeterminantInequality` も Gaussian additivity から導出済。

**当初 scope-out だが 2026-07-18 (leg 29) に genuine closure して復帰済 (Ch.9)**:

- **Ch.9 Shannon-Hartley operational capacity (prolate DOF-per-second)**: Whittaker-Shannon 標本化定理は 2026-07-14 に proof done (`whittaker_shannon_hasSum` + `whittaker_shannon_bandlimited`、sorryAx-free)。**Phase 1-fix DONE 2026-07-15 (独立 honesty audit PASS、commit 7c3afc86)**: def 再設計 (L²-FT spectral-support `IsBandlimited` + `ContAwgnCode` の `encoder_memLp`/`encoder_continuous` regularity field) で 2 defect root (degenerate L¹-`𝓕` junk-0 / pointwise-vs-a.e. encoder gap) を dissolve、mainline `contAwgn_eq_shannonHartley` は **true-as-framed に復帰** (両 `@audit:defect` 除去済)、**2026-07-18 (leg 29) に全段 CLOSED = メイン定理 `contAwgn_eq_shannonHartley` PROOF-DONE sorryAx-free + @audit:ok**。両半 = achievability `contAwgn_ge_shannonHartley` (leg 22) + converse `contAwgn_le_shannonHartley` (legs 23-29) を `le_antisymm`。旧 `wall:nyquist-2w-dof` は C1 (leg 23) で `plan:` 再分類→converse 全段が project-internal で壁なく着地 (Mathlib gap ゼロ)、`#print axioms`=`[propext,Classical.choice,Quot.sound]`、独立監査で隠れ load-bearing hyp 無し。残 open work 進捗: (1) shared bridge `l2Fourier_eq_fourierIntegral` (+inverse) + `bandlimited_sup_bound` = ✅ **proof-done・sorryAx-free・独立 audit PASS** (commit 9d8608a8/40c2e449/30b59a15)。(2) Phase 3 achievability: **leg 1 (synthSignal band-limit/energy 3 sorry) = ✅ proof-done・audit PASS** (commit 89ede2a3/646605c7)、**achievability 全段 CLOSED (leg 22)**: `contAwgn_ge_shannonHartley` proof-done sorryAx-free + @audit:ok。旧 leg 2/3 の WALL-GATED verdict は C1 (leg 23) の count-domination closure + Nyquist 2W-DOF の Fourier 級数ルート実装で解消 (旧「壁」は project-internal 配線に縮約)。(3) converse (Phase 2/4) + Phase 5-full も leg 23-29 で全段 CLOSED。子 plan が SoT → `shannon/shannon-hartley-operational-moonshot-plan.md`。
...
| 7 | Channel Capacity | ✅ | `shannon_noisy_channel_coding_theorem_general`, `channel_coding_feedback_converse`, `shannon_converse_single_shot` (Verdú-Han), `channelCoding_strong_converse_asymptotic` (Wolfowitz 強逆) |
...
| 9 | Gaussian Channel | 🟢 (①②③) | ① `awgn_capacity_closed_form_genuine` = (1/2)log(1+P/N)。② parallel Gaussian water-filling proof done (`parallel_gaussian_capacity_formula_minimal` sorryAx-free, achiever+converse+per-coord+L-WF1+`KKT.isWaterFillingOptimal_of_kkt`)。③ operational `awgn_achievability` + `awgn_converse`。Whittaker-Shannon 標本化定理は proof done (`whittaker_shannon_hasSum` + `whittaker_shannon_bandlimited`、2 headline sorryAx-free)、operational 容量 mainline `contAwgn_eq_shannonHartley` = **2026-07-18 (leg 29) PROOF-DONE sorryAx-free + @audit:ok** (achievability leg 22 + converse legs 23-29 を `le_antisymm`、旧 `wall:nyquist-2w-dof` は C1 で `plan:` 再分類後 converse 全段が壁なく着地)、子 plan SoT |
...
| 11 | Statistics | 🟢 | `stein_*`, `sanov_ldp_*`, `tvNorm_le_sqrt_klDiv` (+`_div_two` sharp Pinsker), `conditionalStronglyTypicalSlice_mass_ge`, `chernoff_lemma_achievability` + `chernoff_converse`, `hoeffding_tradeoff_exp`, `cramer_lower_boundary`。Hoeffding interior 述語 island (`IsHoeffdingInteriorGradient/Minimizer` 系 9 decl) は dead 確定 (production `hoeffding_tradeoff_exp` sorryAx-free で bypass) → 物理削除済 (2026-07-13) |
...
| 14 | Kolmogorov | 🟡 | plain C 背骨着手: gateway `universalEval` (長さ加法 U) + `invariance`/`invariance_code` (P1, pointwise) + `complexity_le_natLen` (P2) proof-done sorryAx-free `@audit:ok`。P3-P6 (flagship `kolmogorov_entropy_rate` K↔H) in-progress。prefix 塔 P7-P9 は第2波隔離 |
| 15 | Network IT (DSC mini-chapter) | 🟢 | Slepian-Wolf 完全 (corner + full rate region `slepian_wolf_full_rate_region_achievability` + 4 converse), Wyner-Ziv convexity body + operational main (`wyner_ziv_achievability` + `wyner_ziv_converse`), MAC 容量領域 full (`mac_converse` + `mac_achievability` + `mac_capacity_region_reconciliation`), degraded BC (`bc_degraded_converse` + `bc_achievability`), relay `relay_cutset_outer_bound` |
| 16 | Portfolio | ✅ | log-optimal portfolio proof-done sorryAx-free + @audit:ok: static 4 定理 `growthRate_concaveOn` (CT 16.2.2 凹性), `logOptimal_of_kuhnTucker` / `kuhnTucker_of_logOptimal` (CT 16.2.1 KT 特徴付け 双方向), `competitive_optimality` (CT 16.6.1); operational/universal 群 `seqLogWealth_div_tendsto_growthRate` (AO-iid CT 16.3), `sideInfo_growthRate_increment_le_mutualInfo` (side-info ΔW≤I(X;Y) CT 16.4), `seqLogWealth_div_tendsto_stationary` (定常 fixed-b CT 16.5), `universal_portfolio_regret_tendsto_zero` (Cover universal portfolio CT 16.7; 当初 wall 想定を shrink 論法で not-a-wall closure)。gambling (Ch.6 §6.3) の非対角 mirror。**W_∞ AEP 完全形 (CT 16.5.1) proof-done sorryAx-free + @audit:ok**: growing-memory headline `growingMemory_logWealth_tendsto_condOptGrowthInfty_concrete` (Algoet–Cover sandwich、抽象 sandwich を concrete pastFiltration/shiftZ/coord0 で instantiate + shift/past coherence 具体 discharge、`Portfolio/StationaryWinftyConcrete.lean`)。子 SoT → `docs/shannon/portfolio-moonshot-plan.md` |
| 17 | Inequalities | 🟢 | `han_inequality`, `shearer_inequality`, `loomis_whitney`, `brascamp_lieb_finset`, `jointEntropySubset_submodular` (polymatroid), `tvNorm_le_sqrt_klDiv`, `edgeBoundary_entropy_sharp`, 一般 EPI `entropyPowerExt_add_ge` + a.c. 版, `entropyPower_gaussian_additivity`, `minkowskiDeterminantInequality` (CT 17.9), Stam `stam_inequality_smoothed_density` + de Bruijn `debruijn_identity_*`。詳細 SoT → `docs/shannon/ch17-inequalities-status.md` |
...
- **(c) 数学的深さ** (旧「真の壁」= 2026-07-18 解消済): Ch.9 operational 容量の prolate DOF-per-second カウント (converse 側)。**leg 29 に CLOSED = メイン定理 `contAwgn_eq_shannonHartley` PROOF-DONE sorryAx-free + @audit:ok** — 旧 `wall:nyquist-2w-dof` は C1 (leg 23) で `plan:` 再分類後、converse 全段 (achievability leg 22 + converse legs 23-29) が project-internal で壁なく着地 (Mathlib gap ゼロ) = 真の壁ではなかった。標本化定理は 2026-07-14 overturn (proof done)。
...
- **Ch.2 パイロット済** (`docs/textbook/ch01-entropy.md`): 検証済 33 declaration を §2.1-2.10 の prose に紐付け。判明した課題 = (i) 値の型不一致 (`ℝ≥0∞` vs `ℝ`、橋渡し `.toReal`)、(ii) Markov 定義の表層差 (compProd 分解形)、(iii) n 変数 chain rule の右辺長大、(iv) 章↔file が 1:1 でない。代表名照合済 (2026-06-22、章対応表の backtick 識別子を実 declaration と機械照合)。
...
- **2026-06-08**: 一般 EPI 無条件化 `entropyPowerExt_add_ge` を 3-case dispatch + route T (truncation+Gibbs+DCT、「無限分散 a.c. 壁」を FALSE WALL 判定) で sorryAx-free 化。legacy Stam-bridge route は物理削除。facts → `epi-facts.md`。
- **2026-06-10〜11**: scope-out 低リスク 4 家系を独立再判定。Ch.7 converse / Ch.10 RD ach / Ch.11 Hoeffding interior は dead or false-as-stated と確定 → retract-tag + 物理削除。Ch.10 RD 凸性 `rateDistortionFunction_convexOn` + converse `rate_distortion_converse_n_letter_singleLetter` を genuine closure (converse は under-hyp 修正 + MI superadditivity 壁を self-build)。Cramér: false-statement 2 root を def-fix (`h_deriv` 追加) → CLT-boundary closure moonshot `cramer_lower_boundary` (gateway `gaussianReal_Ici_eq_half` で「genuine 壁公算」予測を反証) → root 配線で sorryAx-free 化。facts → `cramer-facts.md`。
...
- **2026-06-28**: Ch.15 MAC 容量領域 full closure (`mac_converse` + `mac_achievability` + reconciliation)、degraded BC converse `bc_degraded_converse` (Route B entropy-difference)、relay cut-set outer bound `relay_cutset_outer_bound` (BC-cut telescoping を gateway-atom-first で tractable 化)。AWGN ① load-bearing wrapper 群を Tier-3 `@audit:superseded-by` 一掃 (genuine `awgn_capacity_closed_form_genuine` 併存ゆえ)。`@audit:closed-by-successor` project-wide 0 化。
