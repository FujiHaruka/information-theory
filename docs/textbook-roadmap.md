...
- **Ch.9 Shannon-Hartley operational capacity (prolate DOF-per-second)**: Whittaker-Shannon 標本化定理は 2026-07-14 に proof done (`whittaker_shannon_hasSum` + `whittaker_shannon_bandlimited`、sorryAx-free)。**Phase 1-fix DONE 2026-07-15 (独立 honesty audit PASS、commit 7c3afc86)**: def 再設計 (L²-FT spectral-support `IsBandlimited` + `ContAwgnCode` の `encoder_memLp`/`encoder_continuous` regularity field) で 2 defect root (degenerate L¹-`𝓕` junk-0 / pointwise-vs-a.e. encoder gap) を dissolve、mainline `contAwgn_eq_shannonHartley` は **true-as-framed に復帰** (両 `@audit:defect` 除去済)、honest 単一 wall-sorry `@residual(wall:nyquist-2w-dof)` (converse 側 prolate DOF、Mathlib 不在) が genuine documented wall として復帰 (genuine なのは statement が true になった後)。**まだ proof-done ではない = true-as-framed な honest wall-sorry**。残 open work 進捗: (1) shared bridge `l2Fourier_eq_fourierIntegral` (+inverse) + `bandlimited_sup_bound` = ✅ **proof-done・sorryAx-free・独立 audit PASS** (commit 9d8608a8/40c2e449/30b59a15)。(2) Phase 3 achievability: **leg 1 (synthSignal band-limit/energy 3 sorry) = ✅ proof-done・audit PASS** (commit 89ede2a3/646605c7)、**leg 2 (BddAbove) + leg 3 (assembly) = 2026-07-15 WALL-GATED** (proof-pivot-advisor verdict 2): BddAbove は標本↔窓エネルギーの時間帯域集中 = mainline と同一 `wall:nyquist-2w-dof` を要し旧「crude・壁非依存」は under-estimation、leg 3 は `le_csSup` で BddAbove を消費し transitively 同壁。コード側は leg 2 = `@residual(wall:nyquist-2w-dof)`・leg 3 = `@residual(plan:…)` (壁を prerequisite として le_csSup 経由で消費、独立監査で単一壁の二重計上を回避)、Phase 3 achievability closure は壁ブロック (full closure は壁 self-build か 容量 def 再設計が前提)。(3) prolate 壁 (Phase 2/4) / Phase 5-full は不変。子 plan が SoT → `shannon/shannon-hartley-operational-moonshot-plan.md`。
...
| 9 | Gaussian Channel | 🟢 (①②③) | ① `awgn_capacity_closed_form_genuine` = (1/2)log(1+P/N)。② parallel Gaussian water-filling proof done (`parallel_gaussian_capacity_formula_minimal` sorryAx-free, achiever+converse+per-coord+L-WF1+`KKT.isWaterFillingOptimal_of_kkt`)。③ operational `awgn_achievability` + `awgn_converse`。Whittaker-Shannon 標本化定理は proof done (`whittaker_shannon_hasSum` + `whittaker_shannon_bandlimited`、2 headline sorryAx-free)、operational 容量 mainline は **Phase 1-fix DONE 2026-07-15 (audit PASS、commit 7c3afc86)** = def 再設計 (L²-FT spectral-support `IsBandlimited` + `encoder_memLp`/`encoder_continuous`) で `contAwgn_eq_shannonHartley` を true-as-framed に復帰、honest 単一 wall-sorry `@residual(wall:nyquist-2w-dof)` (prolate DOF、converse 側) が genuine wall として復帰 (wall-sorry-blocked で proof-done でない)、子 plan SoT |
...
- **(c) 数学的深さ** (高、真の壁): Ch.9 operational 容量の prolate DOF-per-second カウント (converse 側限定、`wall:nyquist-2w-dof`)。**Phase 1-fix DONE 2026-07-15 (audit PASS、commit 7c3afc86)** — def 再設計 (L²-FT spectral-support `IsBandlimited` + `encoder_memLp`/`encoder_continuous`) で `contAwgn_eq_shannonHartley` は true-as-framed に復帰 (`@audit:defect` 除去済)、`nyquist-2w-dof` が genuine documented wall として復帰 (mainline は honest wall-sorry で proof-done でない)。標本化定理は 2026-07-14 overturn (proof done)。
...
- **Ch.9 operational 容量 (Phase 1-fix DONE 2026-07-15、audit PASS、commit 7c3afc86)**: def 再設計 (L²-FT spectral-support `IsBandlimited` + `ContAwgnCode` の `encoder_memLp`/`encoder_continuous` regularity field) で 2 defect root (degenerate L¹-`𝓕` / pointwise-vs-a.e. encoder gap) を dissolve → mainline `contAwgn_eq_shannonHartley` は **true-as-framed に復帰** (両 `@audit:defect` 除去済、ただし honest 単一 wall-sorry で proof-done ではない)。時間帯域幅 DOF-per-second カウント (prolate/Landau-Pollak-Slepian、Mathlib 不在、converse 側) `@residual(wall:nyquist-2w-dof)` が genuine documented wall として復帰。残 open work 進捗: bridge `l2Fourier_eq_fourierIntegral` (+inv) + `bandlimited_sup_bound` = ✅ proof-done・audit PASS (commit 9d8608a8/40c2e449/30b59a15)、Phase 3 achievability leg 1 (synthSignal band-limit/energy) = ✅ proof-done・audit PASS (commit 89ede2a3/646605c7)、**Phase 3 leg 2 (BddAbove) + leg 3 (assembly) = 2026-07-15 WALL-GATED** (proof-pivot-advisor verdict 2、mainline と同一 `wall:nyquist-2w-dof`、旧「crude・壁非依存」は under-estimation、leg 2 = wall・leg 3 = plan で le_csSup 経由の壁 prerequisite を明記)。→ Phase 3 achievability closure は壁ブロック (prolate 壁 self-build か 容量 def 再設計が前提)。標本化定理は 2026-07-14 proof done。子 plan SoT → `shannon/shannon-hartley-operational-moonshot-plan.md`。
