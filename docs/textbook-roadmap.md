...
- **Ch.9 operational 容量 (Phase 1-fix DONE 2026-07-15、audit PASS、commit 7c3afc86)**: def 再設計 (L²-FT spectral-support `IsBandlimited` + `ContAwgnCode` の `encoder_memLp`/`encoder_continuous` regularity field) で 2 defect root (degenerate L¹-`𝓕` / pointwise-vs-a.e. encoder gap) を dissolve → mainline `contAwgn_eq_shannonHartley` は **true-as-framed に復帰** (両 `@audit:defect` 除去済、ただし honest 単一 wall-sorry で proof-done ではない)。時間帯域幅 DOF-per-second カウント (prolate/Landau-Pollak-Slepian、Mathlib 不在、converse 側) `@residual(wall:nyquist-2w-dof)` が genuine documented wall として復帰。残 open work: bridge `l2Fourier_eq_fourierIntegral` fill (`bandlimited_sup_bound` genuine 化、~150–250 行、壁でない) → Phase 3 achievability closure (un-gated) → prolate 壁 (Phase 2/4)。標本化定理は 2026-07-14 proof done。子 plan SoT → `shannon/shannon-hartley-operational-moonshot-plan.md`。
