...
| **ほぼ全 docstring を `**ラベル**:` で太字始まり** | 太字は **named theorem に限る**。通常の topic ラベルは太字にしない（地の文で書く） | ✅ 一括移行を 2 度実施（2026-06-22 / 2026-07-26。後者は 275→90 箇所・111→69 ファイル）。規則は不変で、2026-07-26 実測の残り 90 は全て item 5 が明示的に許す named theorem（`**Fano's inequality**` / `**Cramér's theorem**` / `**Rate-distortion theorem**` 等）＝ topic ラベルの残件は無い。ただし**再発する軸**で、2026-06-22 の後は規約を知らない新規宣言が topic ラベルを書き戻して 275 まで膨らんだ — 同じ機構は今も生きている。再計測: `rg -U -o '/--\s*\n?\s*\*\*' InformationTheory -g '*.lean' \| wc -l`。**このコマンドが返すのは太字始まりの総数であって違反数ではない**（named theorem か topic ラベルかは機械判定できない）。compliant な床が 90 なので、床を超えた分のラベルだけを目視で仕分ける |
...
| **docstring / module doc にプロセス語彙**（Phase/Wall/判断/Retraction/撤退、監査プロセス実況含む） | 永続記録は数学のみ。プロセスは plan / handoff へ | ✅ 一括除去済（2026-07-26、[`../docstring-tidyup-plan.md`](../docstring-tidyup-plan.md) Phase 7）。残るのは honesty タグ内の slug のみ。再計測: `rg -o '202[0-9]-[0-9]{2}-[0-9]{2}'`／`rg -o '[Aa]udited [0-9]\|independent (honesty )?audit\|audit PASS\|auditor'`／`rg -o '\bPhase [0-9A-Z]\|\bLeg [A-Z][0-9]?\b\|\bleg [0-9]'`／`rg -o 'wall:[a-z0-9-]+'`（各 `InformationTheory --glob '*.lean' \| wc -l`）で期待値 `1/0/0/0`（日付の唯一の残りは `@[deprecated (since := ...)]` という Lean 必須構文で対象外）。太字始まりと同じ機構で**再発しうる** — 規約を知らない新規宣言がプロセス語彙を書き戻す可能性があるため、完了状態を散文にキャッシュせず上記コマンドで毎回引くこと |
...
継続行字下げの一括除去は完了（トップレベル宣言 docstring）。太字始まり・末尾ピリオド無しの乖離も 2026-06-22 に一括移行した（[`../docstring-tidyup-plan.md`](../docstring-tidyup-plan.md) Phase 4）が、**太字始まりはその後に再発し、2026-07-26 に再スイープしている**（上表参照）。一括移行は一度きりでは効かず、新規宣言が規約を再導入する — 完了状態を散文にキャッシュせず、上表の再計測コマンドで毎回引き、床を超えた増分だけをラベル単位で仕分けること。
