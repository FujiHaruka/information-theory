...
- **未完（別パス）**: ② 真の単独フラットファイルはフラット維持（1 ファイルのサブ化は無価値）。③ docstring/プランの旧モジュールパス prose 参照の sweep（build 非依存）— `InformationTheory/**.lean` の docstring/comment は完了（旧フラットファイル名参照・存在しない namespace 参照ともに 0 件）。**残作業**: `docs/` 配下の歴史的 plan / inventory / proof-log prose は未実施（stale な `.lean` 参照が数百ファイル規模で残る）。着地した参照規約: ファイルへの言及は `Shannon/` からの相対パスをバッククォートで書き、行番号は付けない — 行番号は再配置のたびに無効化されるキャッシュであり、宣言名こそが安定したハンドルなので、行番号無しでも `rg` で引き当てられる。
